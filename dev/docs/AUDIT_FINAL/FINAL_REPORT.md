...
### TESTRUNNER nretest + LOT O Secret Bazaar (commits `15597f78`/`a0cbb623`)
- **« SDK .NET » sans SDK** : tous les canaux de téléchargement MS/NuGet/release-assets récents sont bloqués par DPI (seuls github.com/codeload/api passent ; release-assets.githubusercontent.com et objects GET refusés). Solution : **testrunner in-process** `PMDO_NRETEST=1` — compile `dev/engine/tests/*.cs` avec le Roslyn du bundle + référence NewEra.Engine, exécute `NewEra.Tests.Suite.Run()`, écrit `/tmp/nretest.jsonl`, exit. Équivalent fonctionnel de `dotnet test` pour notre moteur, en ~5 s.
- **Valeur immédiate prouvée** : les tests unitaires (lois ROM chiffrées : dims impaires, déterminisme seed, caps MH 18/45, formule items 6-7, géométrie île 6×6) ont ATTRAPÉ un vrai bug de `RoomGenMaze` (remplissage interne au lieu des marches depuis les bords extérieurs pret l.3482+ ; labyrinthe 7 % ouvert, 16 composantes) → réécrit pret-exact (marches depuis start-1/end aux positions impaires) : **connexe (1 composante), ratio 0.32, NRETEST_ALL_PASS 11/11** (`nretest_proof.jsonl`).
- **LOT O Secret Bazaar** : contre-épreuve pmdsky-debug (hidden_stairs_type 1=BAZAAR/2=ROOM/255=50-50 ; behaviors PNJ 16-20 ; GetRandomBazaarItem EU 0x22E8568, table non publiée). Implémentation native : étages PAIRS du segment secret = bazar (ShopStep **Swalot** sticky_hold, sécurité voleur native, grab bag approx = items de la zone), impairs = salle trésor — **223 étages / 32 zones**. Preuve runtime : alternance exacte `swalot+shop_security` / trésor (`secret_bazaar_proof.jsonl`). Services de dialogue (Mime Jr. soin, Lickilicky décollage, Kirlia sortie) : PORTED_APPROXIMATED — les textes ROM ne sont pas extraits ; le flux (entrer→profiter→sortir vers l étage suivant) est complet via le DestState du segment.
