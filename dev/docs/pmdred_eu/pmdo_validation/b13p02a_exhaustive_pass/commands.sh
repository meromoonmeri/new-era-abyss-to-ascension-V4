...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02a')
assert e == {'canonical_debug_id': 85, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00085', 'debug_string_address': '0x0827B03C', 'ground_place_id': 11, 'map_file_id': 86, 'map_id': 85, 'stable_ground_id': 'b13p02a', 'weather_id': -1}
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_CONSTRUCTION' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B13P02A_IDENTITY_PASS map_id=85 map_file_id=86 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_CONSTRUCTION')
PYID
...
print('B13P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
