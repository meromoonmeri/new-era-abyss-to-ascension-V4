...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h14p01')
assert e == {'canonical_debug_id': 147, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00147', 'debug_string_address': '0x0827A91C', 'ground_place_id': 9, 'map_file_id': 148, 'map_id': 142, 'stable_ground_id': 'h14p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_DECREPIT_LAB' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H14P01_IDENTITY_PASS map_id=142 map_file_id=148 symbol=MAP_FRIEND_AREA_DECREPIT_LAB')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 181
...
print('H14P01_EXHAUSTIVE_PASS samples=53 unique_rgba=13 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
