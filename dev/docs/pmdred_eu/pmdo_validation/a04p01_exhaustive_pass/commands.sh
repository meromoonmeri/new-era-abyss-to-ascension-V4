...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p01')
assert e == {'canonical_debug_id': 180, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00180', 'debug_string_address': '0x0827A55C', 'ground_place_id': 21, 'map_file_id': 181, 'map_id': 172, 'stable_ground_id': 'a04p01', 'weather_id': -1}
assert 'MAP_SKY_TOWER' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A04P01_IDENTITY_PASS map_id=172 map_file_id=181 symbol=MAP_SKY_TOWER')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a04p01 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a04p01.rsground" | cut -d' ' -f1)" = c6b2b6938f37f2c5f4d10f0686c7b7734aab500144ed036ecc5eeee53c79d6af
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 427
...
print('A04P01_EXHAUSTIVE_PASS samples=136 unique_rgba=62 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a04p01 --plan "$PLAN" \
  --ids a04p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
