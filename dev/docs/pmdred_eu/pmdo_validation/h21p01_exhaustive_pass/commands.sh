...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h21p01')
assert e == {'canonical_debug_id': 156, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00156', 'debug_string_address': '0x0827A83C', 'ground_place_id': 9, 'map_file_id': 157, 'map_id': 149, 'stable_ground_id': 'h21p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_AGED_CHAMBER_O_EXCLAIM' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H21P01_IDENTITY_PASS map_id=149 map_file_id=157 symbol=MAP_FRIEND_AREA_AGED_CHAMBER_O_EXCLAIM')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 205
...
print('H21P01_EXHAUSTIVE_PASS samples=61 unique_rgba=14 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
