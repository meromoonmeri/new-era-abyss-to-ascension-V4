...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p06')
assert e == {'canonical_debug_id': 126, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00126', 'debug_string_address': '0x0827AB9C', 'ground_place_id': 9, 'map_file_id': 127, 'map_id': 122, 'stable_ground_id': 'h06p06', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_MIST_RISE_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H06P06_IDENTITY_PASS map_id=122 map_file_id=127 symbol=MAP_FRIEND_AREA_MIST_RISE_FOREST')
PYID
...
print('H06P06_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
