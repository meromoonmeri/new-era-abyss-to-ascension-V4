...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p07')
assert e == {'canonical_debug_id': 134, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00134', 'debug_string_address': '0x0827AABC', 'ground_place_id': 9, 'map_file_id': 135, 'map_id': 129, 'stable_ground_id': 'h07p07', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_SECRETIVE_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H07P07_IDENTITY_PASS map_id=129 map_file_id=135 symbol=MAP_FRIEND_AREA_SECRETIVE_FOREST')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 409
...
print('H07P07_EXHAUSTIVE_PASS samples=129 unique_rgba=32 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
