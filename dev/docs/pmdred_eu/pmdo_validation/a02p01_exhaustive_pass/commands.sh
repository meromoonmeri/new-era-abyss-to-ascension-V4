...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p01')
assert e == {'canonical_debug_id': 173, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00173', 'debug_string_address': '0x0827A63C', 'ground_place_id': 35, 'map_file_id': 174, 'map_id': 165, 'stable_ground_id': 'a02p01', 'weather_id': -1}
assert 'MAP_FUGITIVES_FIRE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A02P01_IDENTITY_PASS map_id=165 map_file_id=174 symbol=MAP_FUGITIVES_FIRE')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a02p01 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a02p01.rsground" | cut -d' ' -f1)" = 016551d87ddf6b5556a4f9181ec8061a5af2df1bda97e3f43e376f3dc64dc3b2
...
print('A02P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a02p01 --plan "$PLAN" \
  --ids a02p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
