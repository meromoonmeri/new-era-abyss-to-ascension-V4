...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p01b')
assert e == {'canonical_debug_id': 77, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00077', 'debug_string_address': '0x0827B13C', 'ground_place_id': 10, 'map_file_id': 78, 'map_id': 77, 'stable_ground_id': 'b12p01b', 'weather_id': 9}
assert 'MAP_TEAM_BASE_CUBONE_FINAL' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B12P01B_IDENTITY_PASS map_id=77 map_file_id=78 symbol=MAP_TEAM_BASE_CUBONE_FINAL')
PYID
...
print('B12P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
