...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p02-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p02')
assert e == {'canonical_debug_id': 174, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00174', 'debug_string_address': '0x0827A61C', 'ground_place_id': 35, 'map_file_id': 175, 'map_id': 166, 'stable_ground_id': 'a02p02', 'weather_id': 13}
assert 'MAP_FUGITIVES_LAPIS_CAVE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A02P02_IDENTITY_PASS map_id=166 map_file_id=175 symbol=MAP_FUGITIVES_LAPIS_CAVE')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a02p02 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a02p02.rsground" | cut -d' ' -f1)" = f9aac6971906cbb93eab368bcd91bb4a9723180bd854d17b19811474c3484a3c
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 454
...
print('A02P02_EXHAUSTIVE_PASS samples=145 unique_rgba=30 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a02p02 --plan "$PLAN" \
  --ids a02p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
