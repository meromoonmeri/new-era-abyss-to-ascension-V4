...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p05')
assert e == {'canonical_debug_id': 125, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00125', 'debug_string_address': '0x0827ABBC', 'ground_place_id': 9, 'map_file_id': 126, 'map_id': 121, 'stable_ground_id': 'h06p05', 'weather_id': 13}
assert 'MAP_FRIEND_AREA_SACRED_FIELD' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H06P05_IDENTITY_PASS map_id=121 map_file_id=126 symbol=MAP_FRIEND_AREA_SACRED_FIELD')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 193
...
print('H06P05_EXHAUSTIVE_PASS samples=57 unique_rgba=36 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
