...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p04')
assert e == {'canonical_debug_id': 124, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00124', 'debug_string_address': '0x0827ABDC', 'ground_place_id': 9, 'map_file_id': 125, 'map_id': 120, 'stable_ground_id': 'h06p04', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_SCORCHED_PLAINS' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H06P04_IDENTITY_PASS map_id=120 map_file_id=125 symbol=MAP_FRIEND_AREA_SCORCHED_PLAINS')
PYID
...
print('H06P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
