...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h15p01')
assert e == {'canonical_debug_id': 148, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00148', 'debug_string_address': '0x0827A8FC', 'ground_place_id': 9, 'map_file_id': 149, 'map_id': 143, 'stable_ground_id': 'h15p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_MT_DISCIPLINE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H15P01_IDENTITY_PASS map_id=143 map_file_id=149 symbol=MAP_FRIEND_AREA_MT_DISCIPLINE')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 649
...
print('H15P01_EXHAUSTIVE_PASS samples=209 unique_rgba=175 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
