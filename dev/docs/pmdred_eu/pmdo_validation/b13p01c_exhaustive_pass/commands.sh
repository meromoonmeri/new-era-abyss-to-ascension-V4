...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p01c')
assert e == {'canonical_debug_id': 84, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00084', 'debug_string_address': '0x0827B05C', 'ground_place_id': 10, 'map_file_id': 85, 'map_id': 84, 'stable_ground_id': 'b13p01c', 'weather_id': 9}
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_BASIC' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B13P01C_IDENTITY_PASS map_id=84 map_file_id=85 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_BASIC')
PYID
...
print('B13P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
