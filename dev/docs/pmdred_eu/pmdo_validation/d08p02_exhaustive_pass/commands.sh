...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d08p02')
assert e == {'canonical_debug_id': 203, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00203', 'debug_string_address': '0x0827A27C', 'ground_place_id': 23, 'map_file_id': 208, 'map_id': 199, 'stable_ground_id': 'd08p02', 'weather_id': 9}
assert 'MAP_FROSTY_FOREST_MID' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D08P02_IDENTITY_PASS map_id=199 map_file_id=208 symbol=MAP_FROSTY_FOREST_MID')
PYID
...
print('D08P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
