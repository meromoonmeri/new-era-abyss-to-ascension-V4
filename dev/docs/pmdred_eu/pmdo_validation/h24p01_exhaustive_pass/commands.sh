...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h24p01')
assert e == {'canonical_debug_id': 160, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00160', 'debug_string_address': '0x0827A7BC', 'ground_place_id': 9, 'map_file_id': 161, 'map_id': 153, 'stable_ground_id': 'h24p01', 'weather_id': 10}
assert 'MAP_FRIEND_AREA_ICE_FLOE_BEACH' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H24P01_IDENTITY_PASS map_id=153 map_file_id=161 symbol=MAP_FRIEND_AREA_ICE_FLOE_BEACH')
PYID
...
print('H24P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
