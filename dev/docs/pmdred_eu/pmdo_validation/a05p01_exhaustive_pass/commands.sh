...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p01')
assert e == {'canonical_debug_id': 184, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00184', 'debug_string_address': '0x0827A4DC', 'ground_place_id': 35, 'map_file_id': 185, 'map_id': 176, 'stable_ground_id': 'a05p01', 'weather_id': -1}
assert 'MAP_NIGHT_SKY_2' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A05P01_IDENTITY_PASS map_id=176 map_file_id=185 symbol=MAP_NIGHT_SKY_2')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 27382
...
print('A05P01_EXHAUSTIVE_PASS samples=9121 unique_rgba=228 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
