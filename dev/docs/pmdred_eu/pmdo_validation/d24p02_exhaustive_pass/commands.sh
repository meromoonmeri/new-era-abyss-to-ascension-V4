...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d24p02')
assert e == {'canonical_debug_id': 230, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00230', 'debug_string_address': '0x08279F1C', 'ground_place_id': 12, 'map_file_id': 235, 'map_id': 226, 'stable_ground_id': 'd24p02', 'weather_id': -1}
assert 'MAP_LOGO_POKEMON_COMPANY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D24P02_IDENTITY_PASS map_id=226 map_file_id=235 symbol=MAP_LOGO_POKEMON_COMPANY')
PYID
...
print('D24P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
