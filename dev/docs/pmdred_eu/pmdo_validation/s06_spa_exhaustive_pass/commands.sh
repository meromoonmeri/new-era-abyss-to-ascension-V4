...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s06_spa')
assert e == {'canonical_debug_id': 240, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00240', 'debug_string_address': '0x08279E3C', 'ground_place_id': 37, 'map_file_id': 254, 'map_id': 245, 'stable_ground_id': 's06_spa', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text(); assert 'MAP_COUNT' in header and 245 >= 229
print('S06_SPA_IDENTITY_PASS map_id=245 map_file_id=254 identity=ROM_EU_POST_ENUM_MAP_ID_245')
PYID
...
print('S06_SPA_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
