#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the t01p07 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-t01p07-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-t01p07-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-t01p07-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-t01p07-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/t01p07.rsground" | cut -d' ' -f1)" = cc7ce085938d10aa4d564e6037e0ea0671ab1736f08448b38c6ad9c517a35546
test "$(sha256sum "$CANONICAL/tiles/t01p07_Base.tile" | cut -d' ' -f1)" = 456d26b0fd97241ec47db70822d18604a291bd65f99032f4a5e84c0c5273327f
test "$(sha256sum RESERVE/red_grounds/t01p07.rsground | cut -d' ' -f1)" = 27ee868a6cbb37d43957d7fd1b683d070d3dbb8b3cd48dac2f19830f019b8fd9
test "$(sha256sum RESERVE/red_tiles/t01p07_Base.tile | cut -d' ' -f1)" = f5ffa239eb9389e34644b9d2a72dbcc17f3d5c679bfa2bd9317f7bd78e75e50c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='t01p07')
assert entry == {
 'canonical_debug_id':8, 'conversion_type':2,
 'debug_string':'__ground_amd_conversion_00008',
 'debug_string_address':'0x0827B9BC', 'ground_place_id':8,
 'map_file_id':8, 'map_id':8, 'stable_ground_id':'t01p07', 'weather_id':0}
header=Path('.runtime-cache/pmd-red-ground-map.h').read_text()
assert 'MAP_LUMINOUS_CAVE' in header
print('T01P07_IDENTITY_PASS map_file_id=8 symbol=MAP_LUMINOUS_CAVE')
PYID
...
test "$(wc -l < "$FIX/events.jsonl")" -eq 313
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 97
...
assert r['grounds']==['t01p07'] and r['sample_count']==97 and len(primary)==96
assert r['exact_sample_count']==97 and r['fully_opaque_sample_count']==97
...
assert validations[('t01p07','primary')]['movement_probe']=='PASS'
assert validations[('t01p07','primary')]['blocked_probe']=='PASS'
assert validations[('t01p07','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('T01P07_EXHAUSTIVE_PASS samples=97 unique_rgba=19')
PY
...
test "$(sha256sum Data/Ground/t01p07.rsground | cut -d' ' -f1)" = cc7ce085938d10aa4d564e6037e0ea0671ab1736f08448b38c6ad9c517a35546
test "$(sha256sum Content/Tile/t01p07_Base.tile | cut -d' ' -f1)" = 456d26b0fd97241ec47db70822d18604a291bd65f99032f4a5e84c0c5273327f
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/t01p07.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/t01p07_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids t01p07 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 97 comparative PNGs,
