...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s03')
assert e == {'canonical_debug_id': 235, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00235', 'debug_string_address': '0x08279E9C', 'ground_place_id': 37, 'map_file_id': 243, 'map_id': 234, 'stable_ground_id': 's03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text(); assert 'MAP_COUNT' in header and 234 >= 229
print('S03_IDENTITY_PASS map_id=234 map_file_id=243 identity=ROM_EU_POST_ENUM_MAP_ID_234')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 790
...
print('S03_EXHAUSTIVE_PASS samples=257 unique_rgba=23 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
