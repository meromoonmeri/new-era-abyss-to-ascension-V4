...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p04')
assert e == {'canonical_debug_id': 183, 'conversion_type': 6, 'debug_string': '__ground_amd_conversion_00183', 'debug_string_address': '0x0827A4FC', 'ground_place_id': 35, 'map_file_id': 184, 'map_id': 175, 'stable_ground_id': 'a04p04', 'weather_id': -1}
assert 'MAP_NIGHT_SKY_1' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A04P04_IDENTITY_PASS map_id=175 map_file_id=184 symbol=MAP_NIGHT_SKY_1')
PYID
...
print('A04P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
