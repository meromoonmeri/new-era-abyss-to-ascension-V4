...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p02')
assert e == {'canonical_debug_id': 128, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00128', 'debug_string_address': '0x0827AB5C', 'ground_place_id': 9, 'map_file_id': 129, 'map_id': 124, 'stable_ground_id': 'h07p02', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_OVERGROWN_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H07P02_IDENTITY_PASS map_id=124 map_file_id=129 symbol=MAP_FRIEND_AREA_OVERGROWN_FOREST')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 121
...
print('H07P02_EXHAUSTIVE_PASS samples=33 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
