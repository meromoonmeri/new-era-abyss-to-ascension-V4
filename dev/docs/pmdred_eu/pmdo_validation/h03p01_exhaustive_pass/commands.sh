...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h03p01')
assert e == {'canonical_debug_id': 116, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00116', 'debug_string_address': '0x0827ACDC', 'ground_place_id': 9, 'map_file_id': 117, 'map_id': 112, 'stable_ground_id': 'h03p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_MT_DEEPGREEN' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H03P01_IDENTITY_PASS map_id=112 map_file_id=117 symbol=MAP_FRIEND_AREA_MT_DEEPGREEN')
PYID
...
print('H03P01_EXHAUSTIVE_PASS samples=257 unique_rgba=109 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
