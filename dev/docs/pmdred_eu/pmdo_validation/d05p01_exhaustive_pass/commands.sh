...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d05p01')
assert e == {'canonical_debug_id': 195, 'conversion_type': 6, 'debug_string': '__ground_amd_conversion_00195', 'debug_string_address': '0x0827A37C', 'ground_place_id': 17, 'map_file_id': 200, 'map_id': 191, 'stable_ground_id': 'd05p01', 'weather_id': -1}
assert 'MAP_GREAT_CANYON_ENTRY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D05P01_IDENTITY_PASS map_id=191 map_file_id=200 symbol=MAP_GREAT_CANYON_ENTRY')
PYID
...
print('D05P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
