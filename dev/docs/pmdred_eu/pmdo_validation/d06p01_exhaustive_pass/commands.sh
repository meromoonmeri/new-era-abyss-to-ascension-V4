...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d06p01')
assert e == {'canonical_debug_id': 197, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00197', 'debug_string_address': '0x0827A33C', 'ground_place_id': 18, 'map_file_id': 202, 'map_id': 193, 'stable_ground_id': 'd06p01', 'weather_id': -1}
assert 'MAP_LAPIS_CAVE_ENTRY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D06P01_IDENTITY_PASS map_id=193 map_file_id=202 symbol=MAP_LAPIS_CAVE_ENTRY')
PYID
...
print('D06P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
