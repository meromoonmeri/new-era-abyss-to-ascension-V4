...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h16p01')
assert e == {'canonical_debug_id': 149, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00149', 'debug_string_address': '0x0827A8DC', 'ground_place_id': 9, 'map_file_id': 150, 'map_id': 144, 'stable_ground_id': 'h16p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_THUNDER_MEADOW' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H16P01_IDENTITY_PASS map_id=144 map_file_id=150 symbol=MAP_FRIEND_AREA_THUNDER_MEADOW')
PYID
...
print('H16P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
