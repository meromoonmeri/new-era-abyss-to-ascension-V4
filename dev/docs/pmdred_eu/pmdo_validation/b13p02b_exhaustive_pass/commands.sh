...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02b')
assert e == {'canonical_debug_id': 86, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00086', 'debug_string_address': '0x0827B01C', 'ground_place_id': 11, 'map_file_id': 87, 'map_id': 86, 'stable_ground_id': 'b13p02b', 'weather_id': -1}
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_FINAL' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B13P02B_IDENTITY_PASS map_id=86 map_file_id=87 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_FINAL')
PYID
...
print('B13P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
