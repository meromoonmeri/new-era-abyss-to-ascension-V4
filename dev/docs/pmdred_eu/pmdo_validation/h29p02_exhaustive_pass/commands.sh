...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p02')
assert e == {'canonical_debug_id': 167, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00167', 'debug_string_address': '0x0827A6DC', 'ground_place_id': 9, 'map_file_id': 168, 'map_id': 160, 'stable_ground_id': 'h29p02', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_ENCLOSED_ISLAND' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H29P02_IDENTITY_PASS map_id=160 map_file_id=168 symbol=MAP_FRIEND_AREA_ENCLOSED_ISLAND')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 9853
...
print('H29P02_EXHAUSTIVE_PASS samples=3277 unique_rgba=1186 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
