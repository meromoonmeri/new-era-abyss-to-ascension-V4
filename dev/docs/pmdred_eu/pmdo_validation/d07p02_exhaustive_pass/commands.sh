...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d07p02')
assert e == {'canonical_debug_id': 201, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00201', 'debug_string_address': '0x0827A2BC', 'ground_place_id': 21, 'map_file_id': 206, 'map_id': 197, 'stable_ground_id': 'd07p02', 'weather_id': -1}
assert 'MAP_MT_BLAZE_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D07P02_IDENTITY_PASS map_id=197 map_file_id=206 symbol=MAP_MT_BLAZE_END')
PYID
...
print('D07P02_EXHAUSTIVE_PASS samples=136 unique_rgba=62 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
