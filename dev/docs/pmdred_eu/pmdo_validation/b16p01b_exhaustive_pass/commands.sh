...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p01b')
assert e == {'canonical_debug_id': 101, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00101', 'debug_string_address': '0x0827AE3C', 'ground_place_id': 10, 'map_file_id': 102, 'map_id': 101, 'stable_ground_id': 'b16p01b', 'weather_id': 9}
assert 'MAP_TEAM_BASE_TREECKO_FINAL' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B16P01B_IDENTITY_PASS map_id=101 map_file_id=102 symbol=MAP_TEAM_BASE_TREECKO_FINAL')
PYID
...
print('B16P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
