...
function M.Enter(map)
  ...
  pcall(function()
    NNVEcology.Load('nnv_rm39_summer')
    NNVEcology.Setup()
  end)
  LivingWorld.SyncStory()
  M.Context = LivingWorld.Context('no_name_village', 'nnv_rm39_summer')
  ...
  if M.SourceSeason ~= 'ete' then
    error('NNV canonical season mismatch for nnv_rm39_summer: '..tostring(M.SourceSeason))
  end
...
function M.Exit_North_Touch(obj, activator)
  ...
  GAME:EnterGroundMap('nnv_rm38', 'Entry_South')
end
function M.Exit_North_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm49', 'Entry_South')
end
