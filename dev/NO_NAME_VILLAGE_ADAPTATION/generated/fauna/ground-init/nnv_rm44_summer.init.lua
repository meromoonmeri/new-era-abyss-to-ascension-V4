...
function M.Enter(map)
  ...
  pcall(function()
    NNVEcology.Load('nnv_rm44_summer')
    NNVEcology.Setup()
  end)
  LivingWorld.SyncStory()
  M.Context = LivingWorld.Context('no_name_village', 'nnv_rm44_summer')
  ...
  if M.SourceSeason ~= 'ete' then
    error('NNV canonical season mismatch for nnv_rm44_summer: '..tostring(M.SourceSeason))
  end
...
function M.Exit_West_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm34', 'Entry_East')
end
function M.Exit_South_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm45', 'Entry_North')
end
