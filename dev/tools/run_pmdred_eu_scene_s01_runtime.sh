#!/usr/bin/env bash
# Reproduce the isolated real-PMDO s01 prelude gate without touching Grounds.
set -euo pipefail
IFS=$'\n\t'
...
FIX="$ROOT/.runtime-cache/pmdred-eu-narrative-s01-fixture"
EVENTS=/tmp/pmdred_eu_narrative_s01.jsonl
...
"$PYTHON" tools/build_pmdred_eu_narrative_fixture.py --output "$FIX"
...
env "${PMDO_ENV[@]}" PMDO_NARRATIVE_VALIDATOR=pmdred_eu_s01 \
...
"$PYTHON" tools/validate_pmdred_eu_scene_s01_runtime.py --fixture "$FIX" --check-only
cmp "$FIX/events.jsonl" docs/pmdred_eu/narrative_migration/scenes/s01/runtime/events.jsonl
"$PYTHON" - "$FIX" <<'PY'
import hashlib, json, sys
from pathlib import Path
fixture = Path(sys.argv[1])
expected = json.loads(Path('docs/pmdred_eu/narrative_migration/scenes/s01/runtime/validation.json').read_text())['captures']
actual = sorted((fixture / 'appdata/SCREENSHOT').glob('*.png'), key=lambda path: (path.stat().st_mtime_ns, path.name))
assert len(expected) == len(actual) == 7
assert all(row['sha256'] == hashlib.sha256(path.read_bytes()).hexdigest() for row, path in zip(expected, actual, strict=True))
print('S01_CAPTURE_HASH_REPRODUCTION_PASS')
PY
python3 tools/verify_pmdred_eu_narrative_migration_guard.py

echo PMDRED_EU_S01_REPRODUCTION_PASS_PARTIAL_DEPENDENCY_BLOCKED
