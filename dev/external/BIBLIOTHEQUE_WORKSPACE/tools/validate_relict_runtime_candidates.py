#!/usr/bin/env python3
"""Validate a Relict candidate in exact PMDO 0.8.12."""
...
WORK=Path(__file__).resolve().parents[1];REPO=WORK.parents[1];CAND=WORK/"games/relict/conversion/pmdo_candidates";MAP_ID=12
def config(mid):
 global MAP_ID,ASSET,EVENTS,FIXTURE,STAGED,OUTPUT
 MAP_ID=mid;ASSET=f"relict_map_{mid:03d}";EVENTS=Path(f"/tmp/bibliotheque_relict_map{mid:03d}_validator.jsonl");FIXTURE=REPO/f".runtime-cache/bibliotheque-relict-map{mid:03d}-fixture";STAGED=REPO/f".runtime-cache/bibliotheque-relict-map{mid:03d}-candidate";OUTPUT=CAND/f"runtime/map_{mid:03d}"
config(74)
def stage(cand,dest):
 shutil.rmtree(dest,ignore_errors=True);gp=dest/f"Data/Ground/{ASSET}.rsground";gp.parent.mkdir(parents=True)
 with zipfile.ZipFile(cand/"relict_grounds.zip") as z:gp.write_bytes(z.read(f"{ASSET}.rsground"))
...
def validate(cand,out,timeout=180):
 sheet,gp,tp=stage(cand,STAGED);builder.ASSET=ASSET;builder.SHEET=sheet;builder.QUEST_NAME="bibliotheque_relict_fixture";builder.EVENTS=str(EVENTS);manifest=builder.build(REPO,STAGED,FIXTURE)
 ...
 report={"schema_version":"1.0.0","result":"RELICT_PMDO_RUNTIME_PASS","validation_status":"PMDO_VALIDATED","map_id":MAP_ID,"asset":ASSET,"viewport_px":[320,240],"tex_size":2,"ground_sha256":sha256_file(gp),"tile_sheet":sheet,"tile_sha256":sha256_file(tp),"pmdo_sha256":sha256_file(PMDO),"headless_sdl_sha256":sha256_file(next(DOTNET_HEADLESS.rglob("libSDL2-2.0.so.0"))),"runtime_destination_written":False,"runtime_probes":{"ground_load":"PASS","successful_movement":"PASS","blocked_movement":"PASS","terminal_event":"PASS"},"actor_exclusion_rect":{"x":ex[0],"y":ex[1],"width":ex[2]-ex[0],"height":ex[3]-ex[1]},"visual_validation":visual,"forbidden_content_check":{"fixture_actor_pixels_exported":False,"source_actor_pixels_exported":False,"dialogue_exported":False,"script_body_exported":False,"audio_binary_exported":False}};write_json(out/"runtime_report.json",report);return report
def main():
 p=argparse.ArgumentParser();p.add_argument("--candidates",type=Path,default=CAND);p.add_argument("--map-id",type=int,default=12);p.add_argument("--output",type=Path);p.add_argument("--timeout",type=int,default=180);a=p.parse_args();config(a.map_id);r=validate(a.candidates.resolve(),a.output.resolve() if a.output else OUTPUT.resolve(),a.timeout);print(json.dumps({"result":r["result"],"map_id":r["map_id"],"viewport_px":r["viewport_px"],"environment_different_pixel_count":r["visual_validation"]["environment_different_pixel_count"]},sort_keys=True));return 0
