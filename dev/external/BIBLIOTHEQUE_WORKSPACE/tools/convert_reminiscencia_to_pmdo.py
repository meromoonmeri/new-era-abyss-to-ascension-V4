#!/usr/bin/env python3
"""Convert all 552 Reminiscencia RMXP Maps to staged PMDO Ground candidates."""
...
PMDO_VIEWPORT=(320,240); SOURCE_VIEWPORT=(640,480); AUTOTILE_FRAME_LENGTH=20; STATIC_FRAME_LENGTH=60
...
def build(source,archive,output):
 ...
 for ts in sorted(used):
  tileobj=tilesets[ts];desc=tileset_descriptor(ts,tileobj);deps=visual_dependencies(tileobj,source,graphics,"ARCHIVE_VERIFIED");renderer=TileRenderer(source,deps,desc);sheet=f"reminiscencia_tileset_{ts:03d}_Base"
 ...
 exclusions=DEFAULT_EXCLUDED_MAPS["reminiscencia"];results=[];status_counts=defaultdict(int)
 for mid,(obj,tab,ts) in sorted(maps.items()):
  ...
  status="UNSUPPORTED" if excluded else "ADAPTATION_REQUIRED" if missing or unresolved or partial or atlas["fog_name"] or envadapt else "PMDO_CONVERTED";status_counts[status]+=1;asset=f"reminiscencia_map_{mid:03d}";layers=[]
