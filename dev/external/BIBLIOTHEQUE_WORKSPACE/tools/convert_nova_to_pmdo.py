#!/usr/bin/env python3
"""Convert all Nova RMXP Maps to staged PMDO Ground candidates."""
...
PMDO_VIEWPORT=(320,240); SOURCE_VIEWPORT=(960,540); AUTOTILE_FRAME_LENGTH=15; NOVA_VIEWPORT_ADAPTATION=True; STATIC_FRAME_LENGTH=60
...
def frame_length(p):
 if not p["step_animation"]:return 60
 speed=p["move_speed"]
 return 3 if speed>=6 else 6 if speed==5 else max(1,round(60/(2**speed)))
...
def build(source,archive,output):
 ...
 for ts in sorted(used):
  tileobj=tilesets[ts];desc=tileset_descriptor(ts,tileobj);deps=visual_dependencies(tileobj,source,graphics,"ARCHIVE_VERIFIED");renderer=TileRenderer(source,deps,desc);sheet=f"nova_tileset_{ts:03d}_Base"
 ...
 exclusions=DEFAULT_EXCLUDED_MAPS["nova"];results=[];status_counts=defaultdict(int)
 for mid,(obj,tab,ts) in sorted(maps.items()):
  ...
  status="UNSUPPORTED" if excluded else "ADAPTATION_REQUIRED" if missing or unresolved or partial or atlas["fog_name"] or envadapt or NOVA_VIEWPORT_ADAPTATION else "PMDO_CONVERTED";status_counts[status]+=1;asset=f"nova_map_{mid:03d}";layers=[]
