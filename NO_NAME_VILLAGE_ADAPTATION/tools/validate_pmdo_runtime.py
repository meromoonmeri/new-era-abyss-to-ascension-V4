...
def main():
 ...
 for asset in assets:
  print('VALIDATING',asset,flush=True);report=g.validate_runtime(ROOT,ROOT/'Data/Ground'/f'{asset}.rsground',None,ROOT/'.runtime-cache'/f'{asset}-runtime',ROOT/'NO_NAME_VILLAGE_ADAPTATION/reports/runtime'/asset,[0,15],300);report['fixture_entity_isolation']='Ignored fixture only; actors/triggers removed to isolate BMA movement. Candidate hash is unchanged.';(ROOT/'NO_NAME_VILLAGE_ADAPTATION/reports/runtime'/asset/'runtime_report.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');dynamic=len({x['sha256'] for x in report['captures']})==2;rows.append({'asset':asset,'ground_sha256':report['candidate']['ground_sha256'],'result':report['result'],'load':report['runtime_probes']['load'],'movement':report['runtime_probes']['movement'],'blocked':report['runtime_probes']['blocked'],'seasonal_animation_captures_distinct':dynamic if asset.startswith('no_name_village_') else None,'report_sha256':sha(ROOT/'NO_NAME_VILLAGE_ADAPTATION/reports/runtime'/asset/'runtime_report.json')})
 assert all(x['seasonal_animation_captures_distinct'] for x in rows if x['asset'].startswith('no_name_village_'))
 phases=[]
