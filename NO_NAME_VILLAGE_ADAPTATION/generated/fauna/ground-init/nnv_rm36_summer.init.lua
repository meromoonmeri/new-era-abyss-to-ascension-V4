...
function M.Enter(map)
  ...
  pcall(function()
    NNVEcology.Load('nnv_rm36_summer')
    NNVEcology.Setup()
  end)
  LivingWorld.SyncStory()
  M.Context = LivingWorld.Context('no_name_village', 'nnv_rm36_summer')
  ...
  if M.SourceSeason ~= 'ete' then
    error('NNV canonical season mismatch for nnv_rm36_summer: '..tostring(M.SourceSeason))
  end
...
function M.Exit_North_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm46', 'Entry_South')
end
function M.Exit_West_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm26', 'Entry_East')
end
function M.Exit_South_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm37', 'Entry_North')
end
