...
function M.Enter(map)
  ...
  pcall(function()
    NNVEcology.Load('nnv_rm85_summer')
    NNVEcology.Setup()
  end)
  LivingWorld.SyncStory()
  M.Context = LivingWorld.Context('no_name_village', 'nnv_rm85_summer')
  ...
  if M.SourceSeason ~= 'ete' then
    error('NNV canonical season mismatch for nnv_rm85_summer: '..tostring(M.SourceSeason))
  end
...
function M.Exit_North_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm95', 'Entry_South')
end
function M.Exit_West_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm75', 'Entry_East')
end
function M.Exit_South_Touch(obj, activator)
  GAME:FadeOut(false, 20)
  GAME:EnterGroundMap('nnv_rm86', 'Entry_North')
end
