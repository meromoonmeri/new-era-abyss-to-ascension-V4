...
def install_probe(quest: Path) -> None:
    ...
    probe_path.write_text(
        ...
        f'''  local floor_tex=0;local wall_tex=0;local secondary_tex=0;local missing_neighbor=0;local resolved_neighbor=0\n'''
        ...
        f'''      local tex=map:GetTile(loc).Data.TileTex\n'''
        f'''      local set=tostring(tex.AutoTileset)\n'''
        f'''      if set=='sinister_woods_floor' then floor_tex=floor_tex+1 elseif set=='sinister_woods_wall' then wall_tex=wall_tex+1 elseif set=='sinister_woods_secondary' then secondary_tex=secondary_tex+1 end\n'''
        f'''      if tex.NeighborCode==-1 then missing_neighbor=missing_neighbor+1 else resolved_neighbor=resolved_neighbor+1 end\n'''
        f'''    end\n'''
        f'''  end\n'''
