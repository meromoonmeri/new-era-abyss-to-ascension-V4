#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b50.
...
TOOL_VERSION = "2.0.0-b50-staging"
SHEET = "PMDRedB50"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
CATEGORY_FILES = {
    "floor": "pmdred_b50_floor.json",
    "wall": "pmdred_b50_wall.json",
    "secondary": "pmdred_b50_secondary.json",
}
EXPECTED = {
    "b50fon": (12672, "9f23a1318d0d0422105a13dbbdbb0fea49e052f5bba677a8e7c6ff089f06f542"),
    "b50cel": (4500, "a7470f258e8f36329ee1804751d67e254240cae5735753410dbc35a45e08db7f"),
    "b50cex": (2352, "d3e948ef4b4e860eeac9368ff602ef08ba8d392411f9263f753e5a2ae0ed3666"),
    "b50pal": (768, "6e476a701078ae227ec864a9a48dde10f820256c84a13170c83d5cf03ab6645a"),
}
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b50 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b50 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b50 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    for name in ("b50fon", "b50cel", "b50cex"):
        data, meta = archive.decompress(name, expected_size=EXPECTED[name][0])
        source[name] = data
        source_meta[name] = meta
    data, meta = archive.raw("b50pal", EXPECTED["b50pal"][0])
    source["b50pal"] = data
    source_meta["b50pal"] = meta
    ...
    records, canm_meta = parse_canm(archive, "b50canm")
    require(canm_meta["active_record_count"] == 16, "b50canm must have 16 active records")
    require([r.index for r in records if r.active] == list(range(16)), "b50canm active indices drift")
    require(len(source["b50fon"]) % 32 == 0, "b50fon is not whole 4bpp tiles")
    require(len(source["b50cel"]) == GROUND_CHUNK_COUNT * 18, "b50cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b50pal"])
    animated_by_chunk = {
        chunk_id: animated_indices(source["b50fon"], source["b50cel"], chunk_id)
    ...
    for chunk_id in range(GROUND_CHUNK_COUNT):
        ...
        atlas_entries[(chunk_id % ATLAS_COLUMNS, chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
            source["b50fon"], source["b50cel"], chunk_id, base_palette, mode
        )
        for record_index in range(16):
            ...
            for state, color in enumerate(record.colors):
                ...
                atlas_entries[(chunk_id % ATLAS_COLUMNS, ((1 + record_index) * 16 + state) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b50fon"], source["b50cel"], chunk_id, palette, "record", record_index
                )
            if startup_adapter:
                atlas_entries[(chunk_id % ATLAS_COLUMNS, (17 * 16 + record_index) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b50fon"], source["b50cel"], chunk_id, base_palette, "record", record_index
                )
    ...
    for category in ("floor", "wall", "secondary"):
        payload, evidence = auto_tile_payload(
            category, source["b50cex"], animated_by_chunk, records,
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b50-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b50fon", "b50cel", "b50cex", "b50pal", "b50canm"],
        ...
        "source_geometry": {
            "font_tile_count": len(source["b50fon"]) // 32,
            "cel_chunk_count": len(source["b50cel"]) // 18,
            "cex_size": len(source["b50cex"]),
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB50.tile",
                "Data/AutoTile/pmdred_b50_floor.json",
                "Data/AutoTile/pmdred_b50_wall.json",
                "Data/AutoTile/pmdred_b50_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b50-candidate"))
