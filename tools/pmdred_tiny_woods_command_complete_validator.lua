-- Exact-PMDO validator for the command-complete Tiny Woods production overlay.
...
function V:initialize()
  ...
  self.sceneStarts={};self.sceneCompletes={};self.rewards={};self.progression=false
end
...
PMDRED_TINY_WOODS_SCENE_TRACE=function(kind,scene,sequence,pointer,speaker,text,detail)
  ...
  if kind=='opening_automation_checkpoint' then v.pendingFinish=true end
...
function V:finish()
  ...
  local expectedRoute=self.mode=='opening_naming' and {'d01p01'} or {'d01p02','d01p01','b01p00a','s02_fre'}
  local passed=commandCount==975 and arrayCount==27 and equalRoute(self.route,expectedRoute)
  if self.mode=='opening_naming' then
  ...
  emit(string.format('{"event":"command_complete_verdict","mode":"%s","verdict":"%s","commands":%d,"arrays":%d,"dialogues":%d,"menus_closed":%d,"hero_naming":%d,"hero_naming_after_dialogues":%d,"team_naming":%d,"cues":%d,"route":"%s","tiny_woods_unlock":"%s","scenario_main":%s,"scenario_level":%s,"event_division":"%s"}',quote(self.mode),passed and 'PASS' or 'FAIL',commandCount,arrayCount,self.dialogues,self.menus,self.namingHero,self.namingHeroAfter,self.namingTeam,cueCount,quote(table.concat(self.route,'>')),quote(_DATA.Save:GetDungeonUnlock('tiny_woods')),jsonScalar((SV.Scenario or {}).Main),jsonScalar((SV.Scenario or {}).MainLevel),quote((SV.PmdRedTinyWoodsProgress or {}).EventDivision)))
  self.finishing=true
...
function V:OnInit()
  ...
  if self.mode~='opening_naming' and self.mode~='post_route' then error('unsupported command-complete mode') end
