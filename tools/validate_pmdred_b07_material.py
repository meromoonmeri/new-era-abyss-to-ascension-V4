#!/usr/bin/env python3
"""Independently validate the staged b07 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    fon, _ = archive.decompress("b07fon", expected_size=11392)
    cel, _ = archive.decompress("b07cel", expected_size=4500)
    cex, _ = archive.decompress("b07cex", expected_size=2352)
    pal, _ = archive.raw("b07pal", 768)
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB07", "autotile_files": dict(FILES)})
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b07-material-validation.v1",
        ...
        "source": {"b07fon_sha256": sha256(fon), "b07cel_sha256": sha256(cel), "b07cex_sha256": sha256(cex), "b07pal_sha256": sha256(pal), "b07canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b07-candidate"))
