#!/usr/bin/env python3
"""Independently validate the staged b00 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    fon, _ = archive.decompress("b00fon", expected_size=10848)
    cel, _ = archive.decompress("b00cel", expected_size=4500)
    cex, _ = archive.decompress("b00cex", expected_size=2352)
    pal, _ = archive.raw("b00pal", 768)
    records, canm_meta = parse_canm(archive, "b00canm")
    require(canm_meta["active_record_count"] == 32, "CANM active count mismatch")
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB00", "autotile_files": dict(FILES)})
    ...
    expected_atlas_coordinates = GROUND_CHUNK_COUNT * (
        1 + sum(len(record.colors) for record in records[:32])
        + (32 if startup_adapter else 0)
    )
        ...
        for code in CODES:
            ...
            for variant, layers in enumerate(variants):
                ...
                for layer_pos, record_index in enumerate(used, 1):
                    ...
                    if startup_adapter:
                        raw_expected = {"X": chunk_id % 16, "Y": (33 * 16 + record_index) * 16 + chunk_id // 16}
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b00-material-validation.v1",
        ...
        "source": {"b00fon_sha256": sha256(fon), "b00cel_sha256": sha256(cel), "b00cex_sha256": sha256(cex), "b00pal_sha256": sha256(pal), "b00canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b00-candidate"))
