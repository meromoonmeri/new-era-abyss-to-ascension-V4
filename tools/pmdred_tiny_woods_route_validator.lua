...
function V:validate_launcher(group)
  ...
  local expectedDirection = group=='g3' and Direction.Up or Direction.Down
  expect(hero.Direction==expectedDirection,'d01p01 hero direction differs: actual='..tostring(hero.Direction)..' expected='..tostring(expectedDirection))
...
function V:validate_ending(group, cleanup)
  ...
  expect(hero.Direction==Direction.Up,'d01p02 hero direction differs: actual='..tostring(hero.Direction)..' expected='..tostring(Direction.Up))
  ...
  if cleanup then
    ...
    expect(partner.Direction==Direction.Up and caterpie.Direction==Direction.Up,'ending actor direction differs: partner='..tostring(partner.Direction)..' caterpie='..tostring(caterpie.Direction)..' expected='..tostring(Direction.Up))
    expect(SOUND:GetCurrentSong()=='In the Depths of the Pit.ogg','ending BGM differs')
  end
  emit(string.format('{"event":"ending_ground_probe","visit":%d,"group":"%s","cleanup":%s,"width":%d,"height":%d,"solid_cells":%d,"hero_x":%d,"hero_y":%d,"hero_direction":"%s","partner":%s,"partner_direction":"%s","caterpie":%s,"caterpie_direction":"%s","music":"%s","verdict":"PASS"}',self.groundVisits.d01p02,quote(group),cleanup and 'true' or 'false',map.Width,map.Height,solid,hero.Position.X,hero.Position.Y,quote(hero.Direction),partner~=nil and 'true' or 'false',quote(partner and partner.Direction or 'nil'),caterpie~=nil and 'true' or 'false',quote(caterpie and caterpie.Direction or 'nil'),quote(SOUND:GetCurrentSong())))
end
