...
class Tests(unittest.TestCase):
 def test_visual_batches_are_hash_bound_and_non_promoting(self):
  manifests=sorted(BASE.glob('*/manifest.json'));self.assertEqual(len(manifests),46)
 ...
 def test_material_contact_sheets_are_inspectable(self):
  for family in ('b00','b06','b07','b38','b49','b50','b59'):
 ...
 def test_marvelous_b00_b49_pixel_startup_runtime(self):
  base=BASE/'marvelous_sea';d=json.loads((base/'manifest.json').read_text());self.assertEqual(d['status'],'STAGED_ROM_RECONSTRUCTED_PIXEL_STARTUP_RUNTIME_PASS')
