...
class Tests(unittest.TestCase):
 def test_visual_batches_are_hash_bound_and_non_promoting(self):
  manifests=sorted(BASE.glob('*/manifest.json'));self.assertEqual(len(manifests),36)
  for path in manifests:
   base=path.parent;d=json.loads(path.read_text());self.assertEqual(d['status'],'STAGED_SOURCE_BOUND');self.assertFalse(d['gates']['promotion']);files=d['canonical_tilesets']['files'];self.assertGreater(len(files),0)
   for r in files:
