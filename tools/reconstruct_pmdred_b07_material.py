#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b07.
...
TOOL_VERSION = "2.0.0-b07-staging"
SHEET = "PMDRedB07"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
CATEGORY_FILES = {
    "floor": "pmdred_b07_floor.json",
    "wall": "pmdred_b07_wall.json",
    "secondary": "pmdred_b07_secondary.json",
}
EXPECTED = {
    "b07fon": (11392, "c09416021c45f6b1e45d7b764276ed4ba1289262f9693f07af57cea177fa0b1d"),
    "b07cel": (4500, "421e037cebe1ec2013e59db77f7140136ca06d8ce0f4534d965923caaa0995df"),
    "b07cex": (2352, "a9d6873370af4498fc60093f0aada929f059ea818750e985b1b7ad746ef9a543"),
    "b07pal": (768, "8da96ae4661780f779065aa1ff2ae7676c0b6b4606f12b171705d3ed7d95d0c3"),
}
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b07 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b07 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b07 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    for name in ("b07fon", "b07cel", "b07cex"):
        data, meta = archive.decompress(name, expected_size=EXPECTED[name][0])
        source[name] = data
        source_meta[name] = meta
    data, meta = archive.raw("b07pal", EXPECTED["b07pal"][0])
    source["b07pal"] = data
    source_meta["b07pal"] = meta
    ...
    records, canm_meta = parse_canm(archive, "b07canm")
    require(canm_meta["active_record_count"] == 16, "b07canm must have 16 active records")
    require([r.index for r in records if r.active] == list(range(16)), "b07canm active indices drift")
    require(len(source["b07fon"]) % 32 == 0, "b07fon is not whole 4bpp tiles")
    require(len(source["b07cel"]) == GROUND_CHUNK_COUNT * 18, "b07cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b07pal"])
    animated_by_chunk = {
        chunk_id: animated_indices(source["b07fon"], source["b07cel"], chunk_id)
    ...
    for chunk_id in range(GROUND_CHUNK_COUNT):
        ...
        atlas_entries[(chunk_id % ATLAS_COLUMNS, chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
            source["b07fon"], source["b07cel"], chunk_id, base_palette, mode
        )
        for record_index in range(16):
            ...
            for state, color in enumerate(record.colors):
                ...
                atlas_entries[(chunk_id % ATLAS_COLUMNS, ((1 + record_index) * 16 + state) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b07fon"], source["b07cel"], chunk_id, palette, "record", record_index
                )
            if startup_adapter:
                atlas_entries[(chunk_id % ATLAS_COLUMNS, (17 * 16 + record_index) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b07fon"], source["b07cel"], chunk_id, base_palette, "record", record_index
                )
    ...
    for category in ("floor", "wall", "secondary"):
        payload, evidence = auto_tile_payload(
            category, source["b07cex"], animated_by_chunk, records,
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b07-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b07fon", "b07cel", "b07cex", "b07pal", "b07canm"],
        ...
        "source_geometry": {
            "font_tile_count": len(source["b07fon"]) // 32,
            "cel_chunk_count": len(source["b07cel"]) // 18,
            "cex_size": len(source["b07cex"]),
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB07.tile",
                "Data/AutoTile/pmdred_b07_floor.json",
                "Data/AutoTile/pmdred_b07_wall.json",
                "Data/AutoTile/pmdred_b07_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b07-candidate"))
