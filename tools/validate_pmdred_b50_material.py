#!/usr/bin/env python3
"""Independently validate the staged b50 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    fon, _ = archive.decompress("b50fon", expected_size=12672)
    cel, _ = archive.decompress("b50cel", expected_size=4500)
    cex, _ = archive.decompress("b50cex", expected_size=2352)
    pal, _ = archive.raw("b50pal", 768)
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB50", "autotile_files": dict(FILES)})
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b50-material-validation.v1",
        ...
        "source": {"b50fon_sha256": sha256(fon), "b50cel_sha256": sha256(cel), "b50cex_sha256": sha256(cex), "b50pal_sha256": sha256(pal), "b50canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b50-candidate"))
