#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b00.
...
TOOL_VERSION = "2.0.0-b00-staging"
SHEET = "PMDRedB00"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
CATEGORY_FILES = {
    "floor": "pmdred_b00_floor.json",
    "wall": "pmdred_b00_wall.json",
    "secondary": "pmdred_b00_secondary.json",
}
EXPECTED = {
    ...
    "b00pal": (768, "8b6d2c911bdccbf5cc72165316ab48173fe23ab72109a9aae07f11cf688cd11a"),
}
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b00 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b00 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b00 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    data, meta = archive.raw("b00pal", EXPECTED["b00pal"][0])
    source["b00pal"] = data
    source_meta["b00pal"] = meta
    ...
    records, canm_meta = parse_canm(archive, "b00canm")
    require(canm_meta["active_record_count"] == 32, "b00canm must have 32 active records")
    require([r.index for r in records if r.active] == list(range(32)), "b00canm active indices drift")
    require(len(source["b00fon"]) % 32 == 0, "b00fon is not whole 4bpp tiles")
    require(len(source["b00cel"]) == GROUND_CHUNK_COUNT * 18, "b00cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b00pal"])
    animated_by_chunk = {
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b00-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b00fon", "b00cel", "b00cex", "b00pal", "b00canm"],
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB00.tile",
                "Data/AutoTile/pmdred_b00_floor.json",
                "Data/AutoTile/pmdred_b00_wall.json",
                "Data/AutoTile/pmdred_b00_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b00-candidate"))
