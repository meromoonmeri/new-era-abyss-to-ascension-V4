#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b38.
...
TOOL_VERSION = "2.0.0-b38-staging"
SHEET = "PMDRedB38"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
CATEGORY_FILES = {
    "floor": "pmdred_b38_floor.json",
    "wall": "pmdred_b38_wall.json",
    "secondary": "pmdred_b38_secondary.json",
}
EXPECTED = {
    "b06fon": (16160, "cf511843c9335432a1ed2d3ef13042d9ab857c4480361093f5547a13dad68c81"),
    "b06cel": (4500, "4ce06fa63985466e1cd2ae7e5350bea57715cd7607cfb30daf43047a79e5a7e1"),
    "b06cex": (2352, "bf68cd436650f86ea0102425a81e479a5503f4a1a5df6cc3961aed373708458f"),
    "b38pal": (768, "9ccb40163bab25eb0bddb0f0e05445207477271b701c5b539fcf10d624c9bec9"),
}
        ...
        for y in range(8):
            for pair in range(4):
                ...
                for side, color_index in enumerate((value & 0x0F, value >> 4)):
                    ...
                    if mode == "base" and 160 <= global_index < 176:
                        continue
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b38 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b38 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b38 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    for name in ("b06fon", "b06cel", "b06cex"):
        data, meta = archive.decompress(name, expected_size=EXPECTED[name][0])
        source[name] = data
        source_meta[name] = meta
    data, meta = archive.raw("b38pal", EXPECTED["b38pal"][0])
    source["b38pal"] = data
    source_meta["b38pal"] = meta
    ...
    records, canm_meta = parse_canm(archive, "b38canm")
    require(canm_meta["active_record_count"] == 16, "b38canm must have 16 active records")
    require([r.index for r in records if r.active] == list(range(16)), "b38canm active indices drift")
    require(len(source["b06fon"]) % 32 == 0, "b38fon is not whole 4bpp tiles")
    require(len(source["b06cel"]) == GROUND_CHUNK_COUNT * 18, "b38cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b38pal"])
    animated_by_chunk = {
        chunk_id: animated_indices(source["b06fon"], source["b06cel"], chunk_id)
    ...
    for chunk_id in range(GROUND_CHUNK_COUNT):
        ...
        atlas_entries[(chunk_id % ATLAS_COLUMNS, chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
            source["b06fon"], source["b06cel"], chunk_id, base_palette, mode
        )
        for record_index in range(16):
            ...
            for state, color in enumerate(record.colors):
                ...
                atlas_entries[(chunk_id % ATLAS_COLUMNS, ((1 + record_index) * 16 + state) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b06fon"], source["b06cel"], chunk_id, palette, "record", record_index
                )
            if startup_adapter:
                atlas_entries[(chunk_id % ATLAS_COLUMNS, (17 * 16 + record_index) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b06fon"], source["b06cel"], chunk_id, base_palette, "record", record_index
                )
    ...
    for category in ("floor", "wall", "secondary"):
        payload, evidence = auto_tile_payload(
            category, source["b06cex"], animated_by_chunk, records,
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b38-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b06fon", "b06cel", "b06cex", "b38pal", "b38canm"],
        ...
        "source_geometry": {
            "font_tile_count": len(source["b06fon"]) // 32,
            "cel_chunk_count": len(source["b06cel"]) // 18,
            "cex_size": len(source["b06cex"]),
            ...
            "animated_palette_indices": list(range(16)),
        },
        "animation_adapter": {
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB38.tile",
                "Data/AutoTile/pmdred_b38_floor.json",
                "Data/AutoTile/pmdred_b38_wall.json",
                "Data/AutoTile/pmdred_b38_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b38-candidate"))
