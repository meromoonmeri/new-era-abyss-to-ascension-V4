#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b59.
...
TOOL_VERSION = "2.0.0-b59-staging"
SHEET = "PMDRedB59"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
CATEGORY_FILES = {
    "floor": "pmdred_b59_floor.json",
    "wall": "pmdred_b59_wall.json",
    "secondary": "pmdred_b59_secondary.json",
}
EXPECTED = {
    "b59fon": (10016, "78ba75a32bc95462fabe56dc3d0bd253b06c55b86c38cd19342697738dbc8002"),
    "b59cel": (4500, "86264df4d8420489655611de384b3c8c95b45b24b39b8ea703c66fcd9c4e56a6"),
    "b59cex": (2352, "c138c96899fb26b120c9e5c1da11ebb98635b226b2ba23eebab22d8ccf53a0b5"),
    "b59pal": (768, "0e238e6ad6e2246b6da9f48678885dfec9b43a32bc267b9662c82d1c1a5d9c9b"),
}
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b59 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b59 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b59 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    for name in ("b59fon", "b59cel", "b59cex"):
        data, meta = archive.decompress(name, expected_size=EXPECTED[name][0])
        source[name] = data
        source_meta[name] = meta
    data, meta = archive.raw("b59pal", EXPECTED["b59pal"][0])
    source["b59pal"] = data
    source_meta["b59pal"] = meta
    ...
    records, canm_meta = parse_canm(archive, "b59canm")
    require(canm_meta["active_record_count"] == 32, "b59canm must have 32 active records")
    require([r.index for r in records if r.active] == list(range(32)), "b59canm active indices drift")
    require(len(source["b59fon"]) % 32 == 0, "b59fon is not whole 4bpp tiles")
    require(len(source["b59cel"]) == GROUND_CHUNK_COUNT * 18, "b59cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b59pal"])
    animated_by_chunk = {
        chunk_id: animated_indices(source["b59fon"], source["b59cel"], chunk_id)
    ...
    for chunk_id in range(GROUND_CHUNK_COUNT):
        ...
        atlas_entries[(chunk_id % ATLAS_COLUMNS, chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
            source["b59fon"], source["b59cel"], chunk_id, base_palette, mode
        )
        for record_index in range(32):
            ...
            for state, color in enumerate(record.colors):
                ...
                atlas_entries[(chunk_id % ATLAS_COLUMNS, ((1 + record_index) * 16 + state) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b59fon"], source["b59cel"], chunk_id, palette, "record", record_index
                )
            if startup_adapter:
                atlas_entries[(chunk_id % ATLAS_COLUMNS, (33 * 16 + record_index) * 16 + chunk_id // ATLAS_COLUMNS)] = render_filtered_chunk(
                    source["b59fon"], source["b59cel"], chunk_id, base_palette, "record", record_index
                )
    ...
    for category in ("floor", "wall", "secondary"):
        payload, evidence = auto_tile_payload(
            category, source["b59cex"], animated_by_chunk, records,
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b59-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b59fon", "b59cel", "b59cex", "b59pal", "b59canm"],
        ...
        "source_geometry": {
            "font_tile_count": len(source["b59fon"]) // 32,
            "cel_chunk_count": len(source["b59cel"]) // 18,
            "cex_size": len(source["b59cex"]),
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB59.tile",
                "Data/AutoTile/pmdred_b59_floor.json",
                "Data/AutoTile/pmdred_b59_wall.json",
                "Data/AutoTile/pmdred_b59_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b59-candidate"))
