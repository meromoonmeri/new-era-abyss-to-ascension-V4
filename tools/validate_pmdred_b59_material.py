#!/usr/bin/env python3
"""Independently validate the staged b59 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    fon, _ = archive.decompress("b59fon", expected_size=10016)
    cel, _ = archive.decompress("b59cel", expected_size=4500)
    cex, _ = archive.decompress("b59cex", expected_size=2352)
    pal, _ = archive.raw("b59pal", 768)
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB59", "autotile_files": dict(FILES)})
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b59-material-validation.v1",
        ...
        "source": {"b59fon_sha256": sha256(fon), "b59cel_sha256": sha256(cel), "b59cex_sha256": sha256(cex), "b59pal_sha256": sha256(pal), "b59canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b59-candidate"))
