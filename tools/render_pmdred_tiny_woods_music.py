#!/usr/bin/env python3
"""Render Tiny Woods from the authenticated PMD Red EU ROM into PMDO Ogg.

The renderer is intentionally source-pinned.  It compiles the repository's C
frontend against exact local checkouts of M4plAy, libogg, and libvorbis, then
validates the resulting Vorbis metadata, PCM length, and deterministic hash.
"""
...
SONG_INDEX = 125
SAMPLE_RATE = 48_000
TOTAL_SAMPLES = 4_264_800
LOOP_START = 2_216_800
LOOP_LENGTH = 2_048_000
...
def parse_ogg(path: Path) -> tuple[int, dict[str, str]]:
    """Return final PCM granule and Vorbis comments from a single-stream Ogg."""
...
""",
    ...
    executable = build / "pmdred_m4a_to_ogg"
    command = [
        compiler, "-O2", "-w", "-include", "stdlib.h",
        "-I", str(build / "include"),
        "-I", str(ogg / "include"),
        "-I", str(vorbis / "include"),
        "-I", str(vorbis / "lib"),
        "-I", str(m4play),
        "-I", str(m4play / "src"),
        str(source),
        str(ogg / "src" / "bitwise.c"),
        str(ogg / "src" / "framing.c"),
...
def main() -> int:
    ...
    parser.add_argument("--m4play-source", type=Path,
                        default=root / ".runtime-cache/M4plAy-source")
    parser.add_argument("--ogg-source", type=Path,
                        default=root / ".runtime-cache/libogg-source")
    parser.add_argument("--vorbis-source", type=Path,
                        default=root / ".runtime-cache/libvorbis-source")
    parser.add_argument("--output", type=Path,
                        default=root / "Content/Music/Tiny Woods.ogg")
    ...
    with tempfile.TemporaryDirectory(prefix="pmdred-m4a-") as temporary:
        build = Path(temporary)
        executable = build_renderer(
            compiler, root / "tools/pmdred_m4a_to_ogg.c", m4play, ogg, vorbis, build
        )
        candidate = build / "Tiny Woods.ogg"
        completed = subprocess.run(
            [executable, rom, str(SONG_INDEX), candidate, EXPECTED_ROM_SHA256],
        ...
        expected_comments = {
            ...
            "M4A_SONG_TABLE_INDEX": str(SONG_INDEX),
            "LOOPSTART": str(LOOP_START),
            "LOOPLENGTH": str(LOOP_LENGTH),
        }
        if granule != TOTAL_SAMPLES:
            raise RuntimeError(f"PCM length mismatch: expected {TOTAL_SAMPLES}, got {granule}")
    ...
    result = {
        ...
        "renderer": completed.stdout.strip(),
        "dependencies": {
