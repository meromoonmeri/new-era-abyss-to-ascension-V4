#!/usr/bin/env python3
"""Independently validate the staged b49 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB49", "autotile_files": dict(FILES)})
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b49-material-validation.v1",
        ...
        "source": {"b49fon_sha256": sha256(fon), "b49cel_sha256": sha256(cel), "b49cex_sha256": sha256(cex), "b49pal_sha256": sha256(pal), "b49canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b49-candidate"))
