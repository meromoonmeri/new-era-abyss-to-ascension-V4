#!/usr/bin/env python3
"""Reconstruct a staging-only PMDO material bundle from PMD Red b06.
...
TOOL_VERSION = "2.0.0-b06-staging"
SHEET = "PMDRedB06"
TILE_SIZE = 24
ATLAS_COLUMNS = 16
...
EXPECTED = {
    ...
    "b06pal": (768, "e0383614a0c97a36f05a89fa5db156d548cf75fa4b9dfcca4b8747faa1d57f75"),
}
    ...
    *,
    ...
    payload = {
        ...
        "Object": {
            ...
            "Name": {
                "DefaultText": f"PMD Red b06 {category.title()}",
                "LocalTexts": {"fr": f"PMD Red b06 — {category}"},
            },
            "Comment": (
                "STAGING ONLY. Generated from PMD Red EU b06 CEX/CEL/FON/PAL/CANM; "
...
def build(rom_path: Path, output: Path, *, startup_adapter: bool = False) -> dict[str, Any]:
    ...
    data, meta = archive.raw("b06pal", EXPECTED["b06pal"][0])
    source["b06pal"] = data
    source_meta["b06pal"] = meta
    ...
    require(canm_meta["active_record_count"] == 16, "b06canm must have 16 active records")
    require([r.index for r in records if r.active] == list(range(16)), "b06canm active indices drift")
    require(len(source["b06fon"]) % 32 == 0, "b06fon is not whole 4bpp tiles")
    require(len(source["b06cel"]) == GROUND_CHUNK_COUNT * 18, "b06cel is not 250 3x3 chunks")
    base_palette = parse_ground_palette(source["b06pal"])
    animated_by_chunk = {
    ...
    manifest = {
        "schema": "new-era.pmdred-eu.pmdred-b06-material-candidate.v2",
        ...
        "authority": {
            ...
            "procedural_resources": ["b06fon", "b06cel", "b06cex", "b06pal", "b06canm"],
        ...
        "production_route": {
            ...
            "resources_to_replace_only": [
                "PMDRedB06.tile",
                "Data/AutoTile/pmdred_b06_floor.json",
                "Data/AutoTile/pmdred_b06_wall.json",
                "Data/AutoTile/pmdred_b06_secondary.json",
            ],
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path(".runtime-cache/pmdred-b06-candidate"))
