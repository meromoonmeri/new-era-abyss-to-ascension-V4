#!/usr/bin/env python3
"""Independently validate the staged b38 PMDO material candidate.
...
def validate(rom_path: Path, candidate: Path) -> dict[str, Any]:
    ...
    namespace = candidate_manifest.get("namespace", {"sheet": "PMDRedB38", "autotile_files": dict(FILES)})
    ...
    validation = {
        "schema": "new-era.pmdred-eu.sinister-woods-b38-material-validation.v1",
        ...
        "source": {"b38fon_sha256": sha256(fon), "b38cel_sha256": sha256(cel), "b38cex_sha256": sha256(cex), "b38pal_sha256": sha256(pal), "b38canm_root_sha256": canm_meta["root_sha256"]},
...
def main() -> int:
    ...
    parser.add_argument("--candidate", type=Path, default=Path(".runtime-cache/pmdred-b38-candidate"))
