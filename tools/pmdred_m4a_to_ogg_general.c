 ...
 * Vorbis comments in PCM samples.  Unlike the frozen Tiny Woods renderer,
 * this focused companion accepts a title and independently supplied loop gate;
 * pass 0/0 only for the first disposable loop-discovery run.
 */
#include <errno.h>
#include <stdint.h>
#include <stdio.h>
...
struct TrackSequenceState {
    uint8_t wait, patternLevel, repeatCount, runningStatus;
    uintptr_t cmdPtr;
    uintptr_t patternStack[3];
};

struct SequenceState {
    struct TrackSequenceState tracks[MAX_MUSICPLAYER_TRACKS];
};

static void capture_sequence(struct SequenceState *out, int track_count) {
    memset(out, 0, sizeof(*out));
    for (int i = 0; i < track_count; ++i) {
        struct MusicPlayerTrack *src = &gMPlayInfo_BGM.tracks[i];
        struct TrackSequenceState *dst = &out->tracks[i];
        dst->wait = src->wait;
        dst->patternLevel = src->patternLevel;
        dst->repeatCount = src->repeatCount;
        dst->runningStatus = src->runningStatus;
        dst->cmdPtr = (uintptr_t)src->cmdPtr;
        for (int j = 0; j < 3; ++j)
            dst->patternStack[j] = (uintptr_t)src->patternStack[j];
    }
}

static uint64_t hash_sequence(const struct SequenceState *state) {
...
int main(int argc, char **argv) {
    if (argc != 8) {
        fprintf(stderr, "usage: %s ROM SONG_INDEX OUTPUT_OGG ROM_SHA256 TITLE EXPECTED_LOOPSTART EXPECTED_LOOPLENGTH\n", argv[0]);
        return 2;
    }
    ...
    long long expected_loop_start = strtoll(argv[6], &end, 10);
    if (!end || *end || expected_loop_start < 0)
        fail("invalid expected loop start");
    end = NULL;
    long long expected_loop_length = strtoll(argv[7], &end, 10);
    if (!end || *end || expected_loop_length < 0)
        fail("invalid expected loop length");
    ...
    vorbis_comment_add_tag(&comment, "TITLE", title);
    ...
    char song_tag[32], loop_start_tag[32], loop_length_tag[32];
    snprintf(song_tag, sizeof(song_tag), "%ld", song_index);
    snprintf(loop_start_tag, sizeof(loop_start_tag), "%lld", expected_loop_start);
    snprintf(loop_length_tag, sizeof(loop_length_tag), "%lld", expected_loop_length);
    vorbis_comment_add_tag(&comment, "M4A_SONG_TABLE_INDEX", song_tag);
    vorbis_comment_add_tag(&comment, "LOOPSTART", loop_start_tag);
    vorbis_comment_add_tag(&comment, "LOOPLENGTH", loop_length_tag);
    ...
    struct SequenceState *states = calloc(MAX_RENDER_FRAMES, sizeof(*states));
    uint64_t *state_hashes = calloc(MAX_RENDER_FRAMES, sizeof(*state_hashes));
    int64_t *state_samples = calloc(MAX_RENDER_FRAMES, sizeof(*state_samples));
    if (!states || !state_hashes || !state_samples)
        fail("could not allocate sequence-loop history");
    int state_count = 0;

    for (int frame = 0; frame < MAX_RENDER_FRAMES; frame++) {
        uint8_t *master_before = gMPlayInfo_BGM.tracks[0].cmdPtr;
        RunMixerFrame(interleaved, SAMPLES_PER_FRAME);
        int repeated = -1;
        if (gMPlayInfo_BGM.tracks[0].cmdPtr < master_before) {
            struct SequenceState current;
            capture_sequence(&current, track_count);
            uint64_t current_hash = hash_sequence(&current);
            for (int i = 0; i < state_count; ++i) {
                if (state_hashes[i] == current_hash &&
                    memcmp(&states[i], &current, sizeof(current)) == 0) {
                    repeated = i;
                    break;
                }
            }
            if (repeated < 0) {
                states[state_count] = current;
                state_hashes[state_count] = current_hash;
                state_samples[state_count] = rendered_samples;
                state_count++;
            }
        }
        if (repeated >= 0) {
            loop_start = state_samples[repeated];
            loop_length = rendered_samples - loop_start;
            break; /* this buffer starts the next complete arrangement cycle */
        }
    ...
    if (loop_start < 0 || loop_length <= 0)
        fail("did not find a repeated complete M4A sequence state");

    /* A nonzero gate is supplied only after an independent disposable run of
       this executable discovers the synchronized boundary in a fresh process. */
    if ((expected_loop_start != 0 || expected_loop_length != 0) &&
        (loop_start != expected_loop_start || loop_length != expected_loop_length))
        fail("detected loop points differ from the independent discovery run");
