...
local V=Class('MtSteelNativeValidator',BaseService)
...
local function emit(s)
 PrintInfo('[MT_STEEL_NATIVE] '..s)
 local f=io.open('/tmp/pmdred_mt_steel_native.jsonl','a');if f then f:write(s..'\n');f:flush();f:close() end
end
...
function V:initialize()
 ...
 self.enabled=os.getenv('PMDO_GROUND_VALIDATOR')=='mt_steel_native_fixture'
 self.floor=8
 self.seed=tonumber(os.getenv('MT_STEEL_SEED') or '424242') or 424242
...
function V:probe()
 ...
 local mob_parts={};local mob_count=0;local skarmory_ok=false
 for ti=0,map.MapTeams.Count-1 do
  ...
  for pi=0,team.Players.Count-1 do
   ...
   if tostring(mob.CurrentForm.Species)=='skarmory' and mob.Level==10 and mob.CharLoc.X==4 and mob.CharLoc.Y==8 and d~=nil then skarmory_ok=true end
 ...
 local ally_parts={};local ally_count=0;local diglett_ok=false
 for ti=0,map.AllyTeams.Count-1 do
  local team=map.AllyTeams[ti]
  for pi=0,team.Players.Count-1 do
   local mob=team.Players[pi];ally_count=ally_count+1
   local protected=safe(function()return mob.StatusEffects:ContainsKey('all_protect')end,false)
   local tactic=safe(function()return tostring(mob.Tactic.ID)end,'')
   if tostring(mob.CurrentForm.Species)=='diglett' and mob.Level==5 and mob.CharLoc.X==4 and mob.CharLoc.Y==3 and protected and tactic=='wait_only' then diglett_ok=true end
   ally_parts[#ally_parts+1]=string.format('{"species":"%s","level":%d,"x":%d,"y":%d,"tactic":"%s","all_protect":%s}',quote(mob.CurrentForm.Species),mob.Level,mob.CharLoc.X,mob.CharLoc.Y,quote(tactic),protected and 'true' or 'false')
  end
 end
 ...
 local entry_ok=#entry_parts==1 and sx==4 and sy==9
 local terrain_ok=terrain.floor==77 and terrain.unbreakable==59 and terrain.water==17
 local verdict=(w==9 and h==17 and tostring(map.Music)=='PMD Red EU - Mt. Steel.ogg' and entry_ok and terrain_ok and walkable==77 and reachable==48 and #stairs==0 and #traps==0 and map.Items.Count==0 and mob_count==1 and ally_count==1 and skarmory_ok and diglett_ok) and 'PASS' or 'FAIL'
 emit(string.format('{"event":"floor_probe","bootstrap_seed":%d,"adventure_seed":%s,"map_seed":%s,"requested_floor":%d,"zone":"%s","segment":%d,"map":%d,"width":%d,"height":%d,"music":"%s","start":{"x":%d,"y":%d},"entries":[%s],"terrain":{%s},"effects":{%s},"walkable":%d,"reachable":%d,"canonical_isolated_walkable":%d,"stairs":[%s],"items":[%s],"mobs":[%s],"mob_count":%d,"allies":[%s],"ally_count":%d,"skarmory_ok":%s,"diglett_ok":%s,"traps":%d,"verdict":"%s"}',self.seed,adventure_seed,map_seed,self.floor,tostring(_ZONE.CurrentZoneID),_ZONE.CurrentMapID.Segment,_ZONE.CurrentMapID.ID,w,h,quote(map.Music),sx,sy,table.concat(entry_parts,','),table.concat(terrain_parts,','),table.concat(effect_parts,','),walkable,reachable,walkable-reachable,table.concat(stair_parts,','),table.concat(item_parts,','),table.concat(mob_parts,','),mob_count,table.concat(ally_parts,','),ally_count,skarmory_ok and 'true' or 'false',diglett_ok and 'true' or 'false',#traps,verdict))
end
function V:OnInit()
 if not self.enabled then return end
 local f=io.open('/tmp/pmdred_mt_steel_native.jsonl','w');if f then f:close() end
...
function V:OnMapUpdate()
 ...
 self.task=TASK:BranchCoroutine(function()
  ...
  local ok,err=xpcall(function()
   ...
   GAME:EnterDungeon('mt_steel',0,self.floor,0,RogueEssence.Data.GameProgress.DungeonStakes.Risk,true,true)
  end,debug.traceback)
...
function V:Subscribe(med)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.Init,function()self.OnInit(self)end)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.NewGame,function()self.OnNewGame(self)end)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.GroundMapEnter,function()self.OnGroundMapEnter(self)end)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.DungeonFloorEnter,function()self.OnDungeonFloorEnter(self)end)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.Deinit,function()self.OnDeinit(self)end)
 med:Subscribe('MtSteelNativeValidator',EngineServiceEvents.GraphicsUnload,function()self.OnGraphicsUnload(self)end)
end
function V:UnSubscribe(med)end
MT_STEEL_NATIVE_VALIDATOR_INSTANCE=V:new()
PMDRED_EU_NATIVE_VALIDATOR_INSTANCE=MT_STEEL_NATIVE_VALIDATOR_INSTANCE
SCRIPT:AddService('MtSteelNativeValidator',MT_STEEL_NATIVE_VALIDATOR_INSTANCE)
return V
