#!/usr/bin/env bash
# Reproduce real PMDO FrameInput confirmation across the routed quiz menus.
set -euo pipefail
IFS=$'\n\t'
...
FIX="$ROOT/.runtime-cache/pmdred-eu-personality-quiz-input-fixture"
EVENTS=/tmp/pmdred_eu_personality_quiz_input.jsonl
EVIDENCE="$ROOT/docs/pmdred_eu/narrative_migration/personality_quiz/runtime_input"
...
"$PYTHON" tools/build_pmdred_eu_personality_quiz_input_fixture.py --output "$FIX"
PMDO_ENV=(
...
env "${PMDO_ENV[@]}" PMDO_NARRATIVE_VALIDATOR=pmdred_eu_personality_quiz_input \
 ...
 --status-json "$FIX/termination.json" --terminal-timeout 240 --reset-events
! grep -qE 'Exception Depth|Could not find value|RUNTIME_FAIL|Lua Trace' "$FIX/runtime.log"
"$PYTHON" tools/validate_pmdred_eu_personality_quiz_input_runtime.py --fixture "$FIX" --check-only
...
def semantic(values):return [{k:v for k,v in row.items() if k!='frame'} for row in values]
assert semantic(rows(fixture/'events.jsonl'))==semantic(rows(evidence/'events.jsonl'))
print('PERSONALITY_QUIZ_PHYSICAL_INPUT_SEMANTIC_REPRODUCTION_PASS')
PY
python3 tools/verify_pmdred_eu_narrative_migration_guard.py
echo PMDRED_EU_PERSONALITY_QUIZ_PHYSICAL_INPUT_REPRODUCTION_PASS_LIVE_CREATION_PARTIAL
