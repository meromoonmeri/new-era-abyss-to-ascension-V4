...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h09p02')
assert e == {'canonical_debug_id': 138, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00138', 'debug_string_address': '0x0827AA3C', 'ground_place_id': 9, 'map_file_id': 139, 'map_id': 133, 'stable_ground_id': 'h09p02', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_MYSTIC_LAKE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H09P02_IDENTITY_PASS map_id=133 map_file_id=139 symbol=MAP_FRIEND_AREA_MYSTIC_LAKE')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 8185
...
print('H09P02_EXHAUSTIVE_PASS samples=2721 unique_rgba=40 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
