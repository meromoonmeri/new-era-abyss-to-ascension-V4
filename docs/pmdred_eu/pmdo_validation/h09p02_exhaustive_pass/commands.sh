#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h09p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h09p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h09p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h09p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h09p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h09p02.rsground" | cut -d' ' -f1)" = 0b76a050c28d5c9046de2931c9ab5d3a88a0a6b9aefe3e048f2d3cbb56ae9685
test "$(sha256sum "$CANONICAL/tiles/h09p02_Base.tile" | cut -d' ' -f1)" = 521054dc3d97216193decc898d899952c15375c8ea8c199955f5eb7b29af076e
test "$(sha256sum RESERVE/red_grounds/h09p02.rsground | cut -d' ' -f1)" = 0a2f0006dc574ae5e841112b6000d6fa3bf62cf8ad9f5ff90ad58900325da175
test "$(sha256sum RESERVE/red_tiles/h09p02_Base.tile | cut -d' ' -f1)" = 521054dc3d97216193decc898d899952c15375c8ea8c199955f5eb7b29af076e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h09p02')
assert entry == {'canonical_debug_id': 138, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00138', 'debug_string_address': '0x0827AA3C', 'ground_place_id': 9, 'map_file_id': 139, 'map_id': 133, 'stable_ground_id': 'h09p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MYSTIC_LAKE' in header
print('H09P02_IDENTITY_PASS map_file_id=139 symbol=MAP_FRIEND_AREA_MYSTIC_LAKE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h09p02 --expected-screenshots 2721 \
...
print('H09P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 8188
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 2721
...
assert r['grounds']==['h09p02'] and r['sample_count']==2721 and len(primary)==2720
assert r['exact_sample_count']==2721 and r['fully_opaque_sample_count']==2721
assert len({s['actual_rgba_sha256'] for s in primary})==40
...
assert validations[('h09p02','primary')]['movement_probe']=='PASS'
assert validations[('h09p02','primary')]['blocked_probe']=='PASS'
assert validations[('h09p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H09P02_EXHAUSTIVE_PASS samples=2721 unique_rgba=40')
PY
...
test "$(sha256sum Data/Ground/h09p02.rsground | cut -d' ' -f1)" = 0b76a050c28d5c9046de2931c9ab5d3a88a0a6b9aefe3e048f2d3cbb56ae9685
test "$(sha256sum Content/Tile/h09p02_Base.tile | cut -d' ' -f1)" = 521054dc3d97216193decc898d899952c15375c8ea8c199955f5eb7b29af076e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h09p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h09p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h09p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 2721 comparative PNGs,
