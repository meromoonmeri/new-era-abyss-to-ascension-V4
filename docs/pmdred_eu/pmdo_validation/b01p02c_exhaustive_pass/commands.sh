#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b01p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b01p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b01p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b01p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b01p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b01p02c.rsground" | cut -d' ' -f1)" = 90ab72038c4a73a7d5ac89c99ee8bbc92d07526575403d584ff0ba3e450b8b16
test "$(sha256sum "$CANONICAL/tiles/b01p02c_Base.tile" | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
test "$(sha256sum RESERVE/red_grounds/b01p02c.rsground | cut -d' ' -f1)" = 2e280e57cc58999b6c4d502dce97a5e433de426cc5c5e1dd62bf8c4bed829aff
test "$(sha256sum RESERVE/red_tiles/b01p02c_Base.tile | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b01p02c')
assert entry == {'canonical_debug_id': 15, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00015', 'debug_string_address': '0x0827B8FC', 'ground_place_id': 11, 'map_file_id': 16, 'map_id': 15, 'stable_ground_id': 'b01p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MEOWTH_BASIC' in header
print('B01P02C_IDENTITY_PASS map_file_id=16 symbol=MAP_TEAM_BASE_MEOWTH_BASIC')
PYID
...
assert r['grounds']==['b01p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b01p02c','primary')]['movement_probe']=='PASS'
assert validations[('b01p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b01p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B01P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b01p02c.rsground | cut -d' ' -f1)" = 90ab72038c4a73a7d5ac89c99ee8bbc92d07526575403d584ff0ba3e450b8b16
test "$(sha256sum Content/Tile/b01p02c_Base.tile | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b01p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b01p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b01p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
