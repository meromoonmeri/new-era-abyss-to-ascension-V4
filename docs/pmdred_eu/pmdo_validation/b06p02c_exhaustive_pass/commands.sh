#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p02c.rsground" | cut -d' ' -f1)" = b2f6041b4ba3b97f8a89286cdddfc9e36326862cf6a4915339cfc3c44f4718c8
test "$(sha256sum "$CANONICAL/tiles/b06p02c_Base.tile" | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p02c')
assert entry == {'canonical_debug_id': 45, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00045', 'debug_string_address': '0x0827B53C', 'ground_place_id': 11, 'map_file_id': 46, 'map_id': 45, 'stable_ground_id': 'b06p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MUDKIP_BASIC' in header
print('B06P02C_IDENTITY_PASS map_id=45 map_file_id=46 symbol=MAP_TEAM_BASE_MUDKIP_BASIC')
PYID
...
assert r['grounds']==['b06p02c'] and r['sample_count']==1921 and len(primary)==1920
...
assert validations[('b06p02c','primary')]['movement_probe']=='PASS'
assert validations[('b06p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b06p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P02C_EXHAUSTIVE_PASS samples=1921 unique_rgba=180 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b06p02c.rsground | cut -d' ' -f1)" = b2f6041b4ba3b97f8a89286cdddfc9e36326862cf6a4915339cfc3c44f4718c8
test "$(sha256sum Content/Tile/b06p02c_Base.tile | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
