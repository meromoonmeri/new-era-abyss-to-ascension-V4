#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b04p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b04p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b04p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b04p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b04p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b04p02a.rsground" | cut -d' ' -f1)" = 5e216732747bfb0ec89a2ac463529b8232bc662b35c8b004ce16dd07766fceb1
test "$(sha256sum "$CANONICAL/tiles/b04p02a_Base.tile" | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
test "$(sha256sum RESERVE/red_grounds/b04p02a.rsground | cut -d' ' -f1)" = 621aea139e4f508d50ae62f4ca0f7f521afd6a779da19f6cd8e23258c35ed33f
test "$(sha256sum RESERVE/red_tiles/b04p02a_Base.tile | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b04p02a')
assert entry == {'canonical_debug_id': 31, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00031', 'debug_string_address': '0x0827B6FC', 'ground_place_id': 11, 'map_file_id': 32, 'map_id': 31, 'stable_ground_id': 'b04p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_SKITTY_CONSTRUCTION' in header
print('B04P02A_IDENTITY_PASS map_file_id=32 symbol=MAP_TEAM_BASE_INSIDE_SKITTY_CONSTRUCTION')
PYID
...
assert r['grounds']==['b04p02a'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b04p02a','primary')]['movement_probe']=='PASS'
assert validations[('b04p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b04p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B04P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b04p02a.rsground | cut -d' ' -f1)" = 5e216732747bfb0ec89a2ac463529b8232bc662b35c8b004ce16dd07766fceb1
test "$(sha256sum Content/Tile/b04p02a_Base.tile | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b04p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b04p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b04p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
