#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h05p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h05p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h05p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h05p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h05p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h05p01.rsground" | cut -d' ' -f1)" = c7361ba85bd167ea696feea158c1f2526f0876e4196a99a481653d456f6abbfe
test "$(sha256sum "$CANONICAL/tiles/h05p01_Base.tile" | cut -d' ' -f1)" = 2ce7c74ffac5022643165ec5beb6176704de29b563218bac2fc909848d899c90
test "$(sha256sum RESERVE/red_grounds/h05p01.rsground | cut -d' ' -f1)" = a7d91c480e2e6562536bcd1236e423a54f9e497ee2e40c765e559a72d7cc4bae
test "$(sha256sum RESERVE/red_tiles/h05p01_Base.tile | cut -d' ' -f1)" = 59079a6df0bd93856f7b2b2315f8b6b66fb71aab3a21e31511e79628a5a94a56
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h05p01')
assert entry == {'canonical_debug_id': 119, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00119', 'debug_string_address': '0x0827AC7C', 'ground_place_id': 9, 'map_file_id': 120, 'map_id': 115, 'stable_ground_id': 'h05p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_RAINBOW_PEAK' in header
print('H05P01_IDENTITY_PASS map_file_id=120 symbol=MAP_FRIEND_AREA_RAINBOW_PEAK')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h05p01 --expected-screenshots 81 \
...
print('H05P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 268
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 81
...
assert r['grounds']==['h05p01'] and r['sample_count']==81 and len(primary)==80
assert r['exact_sample_count']==81 and r['fully_opaque_sample_count']==81
...
assert validations[('h05p01','primary')]['movement_probe']=='PASS'
assert validations[('h05p01','primary')]['blocked_probe']=='PASS'
assert validations[('h05p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H05P01_EXHAUSTIVE_PASS samples=81 unique_rgba=12')
PY
...
test "$(sha256sum Data/Ground/h05p01.rsground | cut -d' ' -f1)" = c7361ba85bd167ea696feea158c1f2526f0876e4196a99a481653d456f6abbfe
test "$(sha256sum Content/Tile/h05p01_Base.tile | cut -d' ' -f1)" = 2ce7c74ffac5022643165ec5beb6176704de29b563218bac2fc909848d899c90
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h05p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h05p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h05p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 81 comparative PNGs,
