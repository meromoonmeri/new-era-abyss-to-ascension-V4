#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p02a.rsground" | cut -d' ' -f1)" = 30b4b4dc1a79757d34d456a60cf2eef5cf21fe79fc9dd0ef5ef70f122fcee25b
test "$(sha256sum "$CANONICAL/tiles/b12p02a_Base.tile" | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
test "$(sha256sum RESERVE/red_grounds/b12p02a.rsground | cut -d' ' -f1)" = 17f8799f276c3661d2f695196f624d252fc857a10019844bbcdc09b51f01d1e7
test "$(sha256sum RESERVE/red_tiles/b12p02a_Base.tile | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p02a')
assert entry == {'canonical_debug_id': 79, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00079', 'debug_string_address': '0x0827B0FC', 'ground_place_id': 11, 'map_file_id': 80, 'map_id': 79, 'stable_ground_id': 'b12p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CUBONE_CONSTRUCTION' in header
print('B12P02A_IDENTITY_PASS map_file_id=80 symbol=MAP_TEAM_BASE_INSIDE_CUBONE_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p02a --expected-screenshots 2 \
...
print('B12P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p02a'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b12p02a','primary')]['movement_probe']=='PASS'
assert validations[('b12p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b12p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b12p02a.rsground | cut -d' ' -f1)" = 30b4b4dc1a79757d34d456a60cf2eef5cf21fe79fc9dd0ef5ef70f122fcee25b
test "$(sha256sum Content/Tile/b12p02a_Base.tile | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
