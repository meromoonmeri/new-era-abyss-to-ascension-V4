#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d07p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d07p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d07p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d07p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d07p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d07p02.rsground" | cut -d' ' -f1)" = dda3fc36ff3d9789d90f8cb6e4ae5ae9a15a37fd00c0fd276d667c74f554cc70
test "$(sha256sum "$CANONICAL/tiles/d07p02_Base.tile" | cut -d' ' -f1)" = b376149a921c935a8f803b36559627302fb0fb5f00832a602d5c83a54af28adf
test "$(sha256sum RESERVE/red_grounds/d07p02.rsground | cut -d' ' -f1)" = 9003195fd9bdbea63060f7c349e07e5986d6df549795ef16dd271ecf618b9b87
test "$(sha256sum RESERVE/red_tiles/d07p02_Base.tile | cut -d' ' -f1)" = 98bfcfafcfde3d0abbff80861148c34699ba506de5f0aa0c10697a791166908d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d07p02')
assert entry == {'canonical_debug_id': 201, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00201', 'debug_string_address': '0x0827A2BC', 'ground_place_id': 21, 'map_file_id': 206, 'map_id': 197, 'stable_ground_id': 'd07p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MT_BLAZE_END' in header
print('D07P02_IDENTITY_PASS map_file_id=206 symbol=MAP_MT_BLAZE_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d07p02 --expected-screenshots 136 \
...
print('D07P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d07p02'] and r['sample_count']==136 and len(primary)==135
...
assert validations[('d07p02','primary')]['movement_probe']=='PASS'
assert validations[('d07p02','primary')]['blocked_probe']=='PASS'
assert validations[('d07p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D07P02_EXHAUSTIVE_PASS samples=136 unique_rgba=62')
PY
...
test "$(sha256sum Data/Ground/d07p02.rsground | cut -d' ' -f1)" = dda3fc36ff3d9789d90f8cb6e4ae5ae9a15a37fd00c0fd276d667c74f554cc70
test "$(sha256sum Content/Tile/d07p02_Base.tile | cut -d' ' -f1)" = b376149a921c935a8f803b36559627302fb0fb5f00832a602d5c83a54af28adf
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d07p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d07p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d07p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
