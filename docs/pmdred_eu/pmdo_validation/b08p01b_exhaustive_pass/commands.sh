#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b08p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b08p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b08p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b08p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b08p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b08p01b.rsground" | cut -d' ' -f1)" = 6fed61eef06bc26a837b6307c926b131534415df70f666c2eba3b3f7aae23dec
test "$(sha256sum "$CANONICAL/tiles/b08p01b_Base.tile" | cut -d' ' -f1)" = 5e51476307085eb94c96d604552211ddd4212245cab1e8aba71054272a680827
test "$(sha256sum RESERVE/red_grounds/b08p01b.rsground | cut -d' ' -f1)" = 30e8227776e6e5c0f9c9f4e66dd436cea9b10fe253c2ac0b499f9c68114a2eb4
test "$(sha256sum RESERVE/red_tiles/b08p01b_Base.tile | cut -d' ' -f1)" = 5e51476307085eb94c96d604552211ddd4212245cab1e8aba71054272a680827
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b08p01b')
assert entry == {'canonical_debug_id': 53, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00053', 'debug_string_address': '0x0827B43C', 'ground_place_id': 10, 'map_file_id': 54, 'map_id': 53, 'stable_ground_id': 'b08p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_PSYDUCK_FINAL' in header
print('B08P01B_IDENTITY_PASS map_file_id=54 symbol=MAP_TEAM_BASE_PSYDUCK_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b08p01b --expected-screenshots 51 \
...
print('B08P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b08p01b'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b08p01b','primary')]['movement_probe']=='PASS'
assert validations[('b08p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b08p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B08P01B_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b08p01b.rsground | cut -d' ' -f1)" = 6fed61eef06bc26a837b6307c926b131534415df70f666c2eba3b3f7aae23dec
test "$(sha256sum Content/Tile/b08p01b_Base.tile | cut -d' ' -f1)" = 5e51476307085eb94c96d604552211ddd4212245cab1e8aba71054272a680827
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b08p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b08p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b08p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
