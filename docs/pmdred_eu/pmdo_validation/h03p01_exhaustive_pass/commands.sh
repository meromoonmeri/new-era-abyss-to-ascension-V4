#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h03p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h03p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h03p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h03p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h03p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h03p01.rsground" | cut -d' ' -f1)" = a087193b5ebde6b33a779a83c3321736624c7fb4f83f628365d68fc441b2d141
test "$(sha256sum "$CANONICAL/tiles/h03p01_Base.tile" | cut -d' ' -f1)" = 20056a610fa0699ffa9dd4d5c63ca2b368fdd272f96b51f543948d65454ee39e
test "$(sha256sum RESERVE/red_grounds/h03p01.rsground | cut -d' ' -f1)" = c115485159f61d64b8d9747e9e32a822354eb9f0f469f0569c229dcf32397311
test "$(sha256sum RESERVE/red_tiles/h03p01_Base.tile | cut -d' ' -f1)" = 380828f3303fa63f03039c9bb4d34c81e3530be719a38b8b651e918444370b3e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h03p01')
assert entry == {'canonical_debug_id': 116, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00116', 'debug_string_address': '0x0827ACDC', 'ground_place_id': 9, 'map_file_id': 117, 'map_id': 112, 'stable_ground_id': 'h03p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MT_DEEPGREEN' in header
print('H03P01_IDENTITY_PASS map_file_id=117 symbol=MAP_FRIEND_AREA_MT_DEEPGREEN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h03p01 --expected-screenshots 257 \
...
print('H03P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 796
...
assert r['grounds']==['h03p01'] and r['sample_count']==257 and len(primary)==256
assert r['exact_sample_count']==257 and r['fully_opaque_sample_count']==257
assert len({s['actual_rgba_sha256'] for s in primary})==109
...
assert validations[('h03p01','primary')]['movement_probe']=='PASS'
assert validations[('h03p01','primary')]['blocked_probe']=='PASS'
assert validations[('h03p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H03P01_EXHAUSTIVE_PASS samples=257 unique_rgba=109')
PY
...
test "$(sha256sum Data/Ground/h03p01.rsground | cut -d' ' -f1)" = a087193b5ebde6b33a779a83c3321736624c7fb4f83f628365d68fc441b2d141
test "$(sha256sum Content/Tile/h03p01_Base.tile | cut -d' ' -f1)" = 20056a610fa0699ffa9dd4d5c63ca2b368fdd272f96b51f543948d65454ee39e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h03p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h03p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h03p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
