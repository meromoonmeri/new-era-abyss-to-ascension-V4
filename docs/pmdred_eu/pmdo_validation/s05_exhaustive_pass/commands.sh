...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s05')
assert e == {'canonical_debug_id': 239, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00239', 'debug_string_address': '0x08279E5C', 'ground_place_id': 37, 'map_file_id': 245, 'map_id': 236, 'stable_ground_id': 's05', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text(); assert 'MAP_COUNT' in header and 236 >= 229
print('S05_IDENTITY_PASS map_id=236 map_file_id=245 identity=ROM_EU_POST_ENUM_MAP_ID_236')
PYID
...
print('S05_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
