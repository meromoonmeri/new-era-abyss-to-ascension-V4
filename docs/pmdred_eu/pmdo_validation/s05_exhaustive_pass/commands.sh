#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s05 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s05-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s05-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s05-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s05-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s05.rsground" | cut -d' ' -f1)" = 58fde8e5fa5d815e1aa059be627d323a3d67debb61695b5de83668c0df3f5355
test "$(sha256sum "$CANONICAL/tiles/s05_Base.tile" | cut -d' ' -f1)" = 768c2a4d75761a48073fbead833635fe5892d0252fe7924cdf9137384b4128e6
test "$(sha256sum RESERVE/red_grounds/s05.rsground | cut -d' ' -f1)" = d2a1def65947443d012831b52b25459ae47e082a5d37c84ada992fd264a5b759
test "$(sha256sum RESERVE/red_tiles/s05_Base.tile | cut -d' ' -f1)" = cee16aecb013341901caf91899272b374c3fd976efa3d4b0c3f6b0c0ede88e6e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s05')
assert entry == {'canonical_debug_id': 239, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00239', 'debug_string_address': '0x08279E5C', 'ground_place_id': 37, 'map_file_id': 245, 'map_id': 236, 'stable_ground_id': 's05', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_LOGO_WARNING' in header
table=Path('.runtime-cache/pmd-red-reference/src/ground_map_conversion_table.c').read_text()
designated=table.split('[MAP_LOGO_WARNING] = {',1)[1].split('},',1)[0]
assert '__ground_amd_conversion_00239' in designated
print('S05_IDENTITY_PASS map_file_id=245 canonical_debug_id=239 symbol=MAP_LOGO_WARNING')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s05 --expected-screenshots 2 \
...
print('S05_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s05'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s05','primary')]['movement_probe']=='PASS'
assert validations[('s05','primary')]['blocked_probe']=='PASS'
assert validations[('s05','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S05_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s05.rsground | cut -d' ' -f1)" = 58fde8e5fa5d815e1aa059be627d323a3d67debb61695b5de83668c0df3f5355
test "$(sha256sum Content/Tile/s05_Base.tile | cut -d' ' -f1)" = 768c2a4d75761a48073fbead833635fe5892d0252fe7924cdf9137384b4128e6
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s05.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s05_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s05 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
