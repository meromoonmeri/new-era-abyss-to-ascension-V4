#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b15p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b15p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b15p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b15p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b15p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b15p02c.rsground" | cut -d' ' -f1)" = 6b0d07860dca8b90fd7eadfb29cbc300866a5dce92fa6e19b7b80dce005b7d89
test "$(sha256sum "$CANONICAL/tiles/b15p02c_Base.tile" | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test "$(sha256sum RESERVE/red_grounds/b15p02c.rsground | cut -d' ' -f1)" = f5b56bed572422d6282179664974a65c5cc5985653b34fb1f6d73261a3f56261
test "$(sha256sum RESERVE/red_tiles/b15p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b15p02c')
assert entry == {'canonical_debug_id': 99, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00099', 'debug_string_address': '0x0827AE7C', 'ground_place_id': 11, 'map_file_id': 100, 'map_id': 99, 'stable_ground_id': 'b15p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TREECKO_BASIC' in header
print('B15P02C_IDENTITY_PASS map_file_id=100 symbol=MAP_TEAM_BASE_TREECKO_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b15p02c --expected-screenshots 2 \
...
print('B15P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b15p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b15p02c','primary')]['movement_probe']=='PASS'
assert validations[('b15p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b15p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B15P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b15p02c.rsground | cut -d' ' -f1)" = 6b0d07860dca8b90fd7eadfb29cbc300866a5dce92fa6e19b7b80dce005b7d89
test "$(sha256sum Content/Tile/b15p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b15p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b15p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b15p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
