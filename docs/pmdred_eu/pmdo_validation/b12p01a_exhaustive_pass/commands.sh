#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p01a.rsground" | cut -d' ' -f1)" = e43e8d5b79ffb288c709c555f4c377bb46ca356876d5ae32b581df215e7dc9e5
test "$(sha256sum "$CANONICAL/tiles/b12p01a_Base.tile" | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
test "$(sha256sum RESERVE/red_grounds/b12p01a.rsground | cut -d' ' -f1)" = 4cb7f3b6f7efd3f592fd305a390ed31f819953a31339eb17e98c12e47db4268a
test "$(sha256sum RESERVE/red_tiles/b12p01a_Base.tile | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p01a')
assert entry == {'canonical_debug_id': 76, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00076', 'debug_string_address': '0x0827B15C', 'ground_place_id': 10, 'map_file_id': 77, 'map_id': 76, 'stable_ground_id': 'b12p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CUBONE_CONSTRUCTION' in header
print('B12P01A_IDENTITY_PASS map_file_id=77 symbol=MAP_TEAM_BASE_CUBONE_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p01a --expected-screenshots 25 \
...
print('B12P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b12p01a','primary')]['movement_probe']=='PASS'
assert validations[('b12p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b12p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b12p01a.rsground | cut -d' ' -f1)" = e43e8d5b79ffb288c709c555f4c377bb46ca356876d5ae32b581df215e7dc9e5
test "$(sha256sum Content/Tile/b12p01a_Base.tile | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
