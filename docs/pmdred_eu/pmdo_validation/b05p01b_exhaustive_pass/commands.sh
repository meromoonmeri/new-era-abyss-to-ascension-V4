#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b05p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b05p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b05p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b05p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b05p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b05p01b.rsground" | cut -d' ' -f1)" = 97294d20b1e8df42e440fd1bd32c4133ac477a2a39da1fcf78c5ee296ddb36e4
test "$(sha256sum "$CANONICAL/tiles/b05p01b_Base.tile" | cut -d' ' -f1)" = 12654bc24939fe1075de678bd7b382496c029c3b30caf8ec8f59042ef5338c79
test "$(sha256sum RESERVE/red_grounds/b05p01b.rsground | cut -d' ' -f1)" = 6dcc97ed6f230782aeff8ace7621e509834dbaf1aacb5003a295b5d165a9f09b
test "$(sha256sum RESERVE/red_tiles/b05p01b_Base.tile | cut -d' ' -f1)" = 12654bc24939fe1075de678bd7b382496c029c3b30caf8ec8f59042ef5338c79
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b05p01b')
assert entry == {'canonical_debug_id': 35, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00035', 'debug_string_address': '0x0827B67C', 'ground_place_id': 10, 'map_file_id': 36, 'map_id': 35, 'stable_ground_id': 'b05p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_SQUIRTLE_FINAL' in header
print('B05P01B_IDENTITY_PASS map_file_id=36 symbol=MAP_TEAM_BASE_SQUIRTLE_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b05p01b --expected-screenshots 51 \
...
print('B05P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b05p01b'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b05p01b','primary')]['movement_probe']=='PASS'
assert validations[('b05p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b05p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B05P01B_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b05p01b.rsground | cut -d' ' -f1)" = 97294d20b1e8df42e440fd1bd32c4133ac477a2a39da1fcf78c5ee296ddb36e4
test "$(sha256sum Content/Tile/b05p01b_Base.tile | cut -d' ' -f1)" = 12654bc24939fe1075de678bd7b382496c029c3b30caf8ec8f59042ef5338c79
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b05p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b05p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b05p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
