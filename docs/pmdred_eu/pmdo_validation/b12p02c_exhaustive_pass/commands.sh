#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p02c.rsground" | cut -d' ' -f1)" = a0fdcc9168b7f755046c5b8e1ad7d5bf7c0ccabd28b0b61b89e4cbe1c144c215
test "$(sha256sum "$CANONICAL/tiles/b12p02c_Base.tile" | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
test "$(sha256sum RESERVE/red_grounds/b12p02c.rsground | cut -d' ' -f1)" = 1aec75013f5629ff6587213ec0b266664cbb462581dd641e623b9d6e20c0b887
test "$(sha256sum RESERVE/red_tiles/b12p02c_Base.tile | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p02c')
assert entry == {'canonical_debug_id': 81, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00081', 'debug_string_address': '0x0827B0BC', 'ground_place_id': 11, 'map_file_id': 82, 'map_id': 81, 'stable_ground_id': 'b12p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MACHOP_BASIC' in header
print('B12P02C_IDENTITY_PASS map_file_id=82 symbol=MAP_TEAM_BASE_MACHOP_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p02c --expected-screenshots 2 \
...
print('B12P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b12p02c','primary')]['movement_probe']=='PASS'
assert validations[('b12p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b12p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b12p02c.rsground | cut -d' ' -f1)" = a0fdcc9168b7f755046c5b8e1ad7d5bf7c0ccabd28b0b61b89e4cbe1c144c215
test "$(sha256sum Content/Tile/b12p02c_Base.tile | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
