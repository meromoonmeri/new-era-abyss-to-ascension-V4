#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h14p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h14p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h14p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h14p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h14p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h14p01.rsground" | cut -d' ' -f1)" = 4af1c7ee7dec6357b788ec694e71520d882f5d7991bf4d5ceb7931ce59f2f373
test "$(sha256sum "$CANONICAL/tiles/h14p01_Base.tile" | cut -d' ' -f1)" = 3ea12b76f0b1468701ac30c888f39e240ddd3260b1a3283ded08729fd9b7751b
test "$(sha256sum RESERVE/red_grounds/h14p01.rsground | cut -d' ' -f1)" = d6bdaa1050716a115a3ec63112374a4400095336b9959f8770690021bc348bda
test "$(sha256sum RESERVE/red_tiles/h14p01_Base.tile | cut -d' ' -f1)" = 3ea12b76f0b1468701ac30c888f39e240ddd3260b1a3283ded08729fd9b7751b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h14p01')
assert entry == {'canonical_debug_id': 147, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00147', 'debug_string_address': '0x0827A91C', 'ground_place_id': 9, 'map_file_id': 148, 'map_id': 142, 'stable_ground_id': 'h14p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_DECREPIT_LAB' in header
print('H14P01_IDENTITY_PASS map_file_id=148 symbol=MAP_FRIEND_AREA_DECREPIT_LAB')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h14p01 --expected-screenshots 53 \
...
print('H14P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 184
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 53
...
assert r['grounds']==['h14p01'] and r['sample_count']==53 and len(primary)==52
assert r['exact_sample_count']==53 and r['fully_opaque_sample_count']==53
assert len({s['actual_rgba_sha256'] for s in primary})==13
...
assert validations[('h14p01','primary')]['movement_probe']=='PASS'
assert validations[('h14p01','primary')]['blocked_probe']=='PASS'
assert validations[('h14p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H14P01_EXHAUSTIVE_PASS samples=53 unique_rgba=13')
PY
...
test "$(sha256sum Data/Ground/h14p01.rsground | cut -d' ' -f1)" = 4af1c7ee7dec6357b788ec694e71520d882f5d7991bf4d5ceb7931ce59f2f373
test "$(sha256sum Content/Tile/h14p01_Base.tile | cut -d' ' -f1)" = 3ea12b76f0b1468701ac30c888f39e240ddd3260b1a3283ded08729fd9b7751b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h14p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h14p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h14p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 53 comparative PNGs,
