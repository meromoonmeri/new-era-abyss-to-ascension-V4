#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h02p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h02p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h02p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h02p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h02p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h02p02.rsground" | cut -d' ' -f1)" = 80c24e1b0dcccc06f2973529dd017e705a5c874f588e53ff938702e076886b35
test "$(sha256sum "$CANONICAL/tiles/h02p02_Base.tile" | cut -d' ' -f1)" = 9bd5d5ce607332171186302d47a4132f8e5aee08a2faaf20cf324029130bfe31
test "$(sha256sum RESERVE/red_grounds/h02p02.rsground | cut -d' ' -f1)" = 901b691b9ee0c071acf2a3dfa982a113c086bf30df20012d532388cb728f8741
test "$(sha256sum RESERVE/red_tiles/h02p02_Base.tile | cut -d' ' -f1)" = 9bd5d5ce607332171186302d47a4132f8e5aee08a2faaf20cf324029130bfe31
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h02p02')
assert entry == {'canonical_debug_id': 112, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00112', 'debug_string_address': '0x0827AD1C', 'ground_place_id': 9, 'map_file_id': 113, 'map_id': 110, 'stable_ground_id': 'h02p02', 'weather_id': 3}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SEAFLOOR_CAVE' in header
print('H02P02_IDENTITY_PASS map_file_id=113 symbol=MAP_FRIEND_AREA_SEAFLOOR_CAVE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h02p02 --expected-screenshots 2 \
...
print('H02P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h02p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h02p02','primary')]['movement_probe']=='PASS'
assert validations[('h02p02','primary')]['blocked_probe']=='PASS'
assert validations[('h02p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H02P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h02p02.rsground | cut -d' ' -f1)" = 80c24e1b0dcccc06f2973529dd017e705a5c874f588e53ff938702e076886b35
test "$(sha256sum Content/Tile/h02p02_Base.tile | cut -d' ' -f1)" = 9bd5d5ce607332171186302d47a4132f8e5aee08a2faaf20cf324029130bfe31
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h02p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h02p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h02p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
