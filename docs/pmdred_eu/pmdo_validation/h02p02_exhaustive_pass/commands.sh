...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h02p02')
assert e == {'canonical_debug_id': 112, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00112', 'debug_string_address': '0x0827AD1C', 'ground_place_id': 9, 'map_file_id': 113, 'map_id': 110, 'stable_ground_id': 'h02p02', 'weather_id': 3}
assert 'MAP_FRIEND_AREA_SEAFLOOR_CAVE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H02P02_IDENTITY_PASS map_id=110 map_file_id=113 symbol=MAP_FRIEND_AREA_SEAFLOOR_CAVE')
PYID
...
print('H02P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
