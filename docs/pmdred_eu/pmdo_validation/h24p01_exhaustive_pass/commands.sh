#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h24p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h24p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h24p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h24p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h24p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h24p01.rsground" | cut -d' ' -f1)" = fbf53ddd35a0f25115fe11b4e357369b4c7fc77afab42c92ee85b444e4e7d9ad
test "$(sha256sum "$CANONICAL/tiles/h24p01_Base.tile" | cut -d' ' -f1)" = 6866326a3933bdc01fc8177918b3c5ab11a7241de0290f1b4efd342e83bcf7c4
test "$(sha256sum RESERVE/red_grounds/h24p01.rsground | cut -d' ' -f1)" = 0d06f89955f4acdf995d4c4c1b3dd41db19ff8cb0495ddbaf70631bb6f86ef47
test "$(sha256sum RESERVE/red_tiles/h24p01_Base.tile | cut -d' ' -f1)" = 6866326a3933bdc01fc8177918b3c5ab11a7241de0290f1b4efd342e83bcf7c4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h24p01')
assert entry == {'canonical_debug_id': 160, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00160', 'debug_string_address': '0x0827A7BC', 'ground_place_id': 9, 'map_file_id': 161, 'map_id': 153, 'stable_ground_id': 'h24p01', 'weather_id': 10}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_ICE_FLOE_BEACH' in header
print('H24P01_IDENTITY_PASS map_file_id=161 symbol=MAP_FRIEND_AREA_ICE_FLOE_BEACH')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h24p01 --expected-screenshots 2 \
...
print('H24P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h24p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h24p01','primary')]['movement_probe']=='PASS'
assert validations[('h24p01','primary')]['blocked_probe']=='PASS'
assert validations[('h24p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H24P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h24p01.rsground | cut -d' ' -f1)" = fbf53ddd35a0f25115fe11b4e357369b4c7fc77afab42c92ee85b444e4e7d9ad
test "$(sha256sum Content/Tile/h24p01_Base.tile | cut -d' ' -f1)" = 6866326a3933bdc01fc8177918b3c5ab11a7241de0290f1b4efd342e83bcf7c4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h24p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h24p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h24p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
