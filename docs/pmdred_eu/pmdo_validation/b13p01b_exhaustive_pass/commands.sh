#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p01b.rsground" | cut -d' ' -f1)" = 225afef8d79b2ff3217ca5fdaee00d60f802afe0f1ce239fd0adac608f303b2f
test "$(sha256sum "$CANONICAL/tiles/b13p01b_Base.tile" | cut -d' ' -f1)" = f42098a166fbdca196be596ace678c4183fcaa25ebd60543c71e0a259700ba6b
test "$(sha256sum RESERVE/red_grounds/b13p01b.rsground | cut -d' ' -f1)" = a9958f27619cc03a8689e14b24db0ca63cb9b70ddbb22ab88c94fcd31c44d043
test "$(sha256sum RESERVE/red_tiles/b13p01b_Base.tile | cut -d' ' -f1)" = f42098a166fbdca196be596ace678c4183fcaa25ebd60543c71e0a259700ba6b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p01b')
assert entry == {'canonical_debug_id': 83, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00083', 'debug_string_address': '0x0827B07C', 'ground_place_id': 10, 'map_file_id': 84, 'map_id': 83, 'stable_ground_id': 'b13p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MACHOP_FINAL' in header
print('B13P01B_IDENTITY_PASS map_file_id=84 symbol=MAP_TEAM_BASE_MACHOP_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p01b --expected-screenshots 25 \
...
print('B13P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b13p01b','primary')]['movement_probe']=='PASS'
assert validations[('b13p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b13p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b13p01b.rsground | cut -d' ' -f1)" = 225afef8d79b2ff3217ca5fdaee00d60f802afe0f1ce239fd0adac608f303b2f
test "$(sha256sum Content/Tile/b13p01b_Base.tile | cut -d' ' -f1)" = f42098a166fbdca196be596ace678c4183fcaa25ebd60543c71e0a259700ba6b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
