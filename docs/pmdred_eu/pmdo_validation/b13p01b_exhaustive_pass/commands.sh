...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p01b')
assert e == {'canonical_debug_id': 83, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00083', 'debug_string_address': '0x0827B07C', 'ground_place_id': 10, 'map_file_id': 84, 'map_id': 83, 'stable_ground_id': 'b13p01b', 'weather_id': 9}
assert 'MAP_TEAM_BASE_MACHOP_FINAL' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B13P01B_IDENTITY_PASS map_id=83 map_file_id=84 symbol=MAP_TEAM_BASE_MACHOP_FINAL')
PYID
...
print('B13P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
