#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b10p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b10p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b10p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b10p01b.rsground" | cut -d' ' -f1)" = 30bc80f712fab7b8b462c9b5235bcb43e6b598a832807416a1fae323a8b2fd1b
test "$(sha256sum "$CANONICAL/tiles/b10p01b_Base.tile" | cut -d' ' -f1)" = cef571e1c5214869783a84fe203e9995675eb0316c91938e60ab6c7c7953521e
test "$(sha256sum RESERVE/red_grounds/b10p01b.rsground | cut -d' ' -f1)" = 38a9ac85051154903fa6ef942c21f3beb20dd9de0685163d1124f8dafef38689
test "$(sha256sum RESERVE/red_tiles/b10p01b_Base.tile | cut -d' ' -f1)" = cef571e1c5214869783a84fe203e9995675eb0316c91938e60ab6c7c7953521e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p01b')
assert entry == {'canonical_debug_id': 65, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00065', 'debug_string_address': '0x0827B2BC', 'ground_place_id': 10, 'map_file_id': 66, 'map_id': 65, 'stable_ground_id': 'b10p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TORCHIC_FINAL' in header
print('B10P01B_IDENTITY_PASS map_file_id=66 symbol=MAP_TEAM_BASE_TORCHIC_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b10p01b --expected-screenshots 25 \
...
print('B10P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b10p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b10p01b','primary')]['movement_probe']=='PASS'
assert validations[('b10p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b10p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B10P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b10p01b.rsground | cut -d' ' -f1)" = 30bc80f712fab7b8b462c9b5235bcb43e6b598a832807416a1fae323a8b2fd1b
test "$(sha256sum Content/Tile/b10p01b_Base.tile | cut -d' ' -f1)" = cef571e1c5214869783a84fe203e9995675eb0316c91938e60ab6c7c7953521e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b10p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b10p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b10p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
