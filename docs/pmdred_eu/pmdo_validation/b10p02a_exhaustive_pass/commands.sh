#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b10p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b10p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b10p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b10p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b10p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b10p02a.rsground" | cut -d' ' -f1)" = 30d7fb3a1d976507e627d84fa2fb2809afca208d3689b4fed8413eab14cba800
test "$(sha256sum "$CANONICAL/tiles/b10p02a_Base.tile" | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test "$(sha256sum RESERVE/red_grounds/b10p02a.rsground | cut -d' ' -f1)" = 72af91c2c5396824739f9b9fb5ccc0ae9b787a8c22e31fbffaf189cfd146c28d
test "$(sha256sum RESERVE/red_tiles/b10p02a_Base.tile | cut -d' ' -f1)" = 736104cafcda450a114285c37f4afd89c0da686118e1296e068603667727e2c9
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p02a')
assert entry == {'canonical_debug_id': 67, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00067', 'debug_string_address': '0x0827B27C', 'ground_place_id': 11, 'map_file_id': 68, 'map_id': 67, 'stable_ground_id': 'b10p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TORCHIC_CONSTRUCTION' in header
print('B10P02A_IDENTITY_PASS map_file_id=68 symbol=MAP_TEAM_BASE_INSIDE_TORCHIC_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b10p02a --expected-screenshots 1561 \
...
print('B10P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b10p02a'] and r['sample_count']==1561 and len(primary)==1560
...
assert validations[('b10p02a','primary')]['movement_probe']=='PASS'
assert validations[('b10p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b10p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B10P02A_EXHAUSTIVE_PASS samples=1561 unique_rgba=36')
PY
...
test "$(sha256sum Data/Ground/b10p02a.rsground | cut -d' ' -f1)" = 30d7fb3a1d976507e627d84fa2fb2809afca208d3689b4fed8413eab14cba800
test "$(sha256sum Content/Tile/b10p02a_Base.tile | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b10p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b10p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b10p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
