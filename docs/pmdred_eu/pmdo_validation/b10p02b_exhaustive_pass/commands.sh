#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b10p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b10p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b10p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b10p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b10p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b10p02b.rsground" | cut -d' ' -f1)" = 9c79f84752d6fd6fbf2435cfc1823b46f27a05ac88ee0160eaf986fcf0b45b59
test "$(sha256sum "$CANONICAL/tiles/b10p02b_Base.tile" | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test "$(sha256sum RESERVE/red_grounds/b10p02b.rsground | cut -d' ' -f1)" = 76d7fad54fd496d1a3f8e8fb7b35f809df9d96d3156195fed18877824ca2e6a0
test "$(sha256sum RESERVE/red_tiles/b10p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p02b')
assert entry == {'canonical_debug_id': 68, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00068', 'debug_string_address': '0x0827B25C', 'ground_place_id': 11, 'map_file_id': 69, 'map_id': 68, 'stable_ground_id': 'b10p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TORCHIC_FINAL' in header
print('B10P02B_IDENTITY_PASS map_file_id=69 symbol=MAP_TEAM_BASE_INSIDE_TORCHIC_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b10p02b --expected-screenshots 1561 \
...
print('B10P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b10p02b'] and r['sample_count']==1561 and len(primary)==1560
...
assert validations[('b10p02b','primary')]['movement_probe']=='PASS'
assert validations[('b10p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b10p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B10P02B_EXHAUSTIVE_PASS samples=1561 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b10p02b.rsground | cut -d' ' -f1)" = 9c79f84752d6fd6fbf2435cfc1823b46f27a05ac88ee0160eaf986fcf0b45b59
test "$(sha256sum Content/Tile/b10p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b10p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b10p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b10p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
