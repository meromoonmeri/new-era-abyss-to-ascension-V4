#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b03p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b03p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b03p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b03p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b03p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b03p02a.rsground" | cut -d' ' -f1)" = 114f641f238fc7d3b2d1c03b16f536e5105280a7fe5d62e30aaba5f6cf611fe8
test "$(sha256sum "$CANONICAL/tiles/b03p02a_Base.tile" | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
test "$(sha256sum RESERVE/red_grounds/b03p02a.rsground | cut -d' ' -f1)" = ea56cb1caf7c70465e80de2343bbf395d8c1e0132e05ee6426ccb068b399d745
test "$(sha256sum RESERVE/red_tiles/b03p02a_Base.tile | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b03p02a')
assert entry == {'canonical_debug_id': 25, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00025', 'debug_string_address': '0x0827B7BC', 'ground_place_id': 11, 'map_file_id': 26, 'map_id': 25, 'stable_ground_id': 'b03p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_EEVEE_CONSTRUCTION' in header
print('B03P02A_IDENTITY_PASS map_file_id=26 symbol=MAP_TEAM_BASE_INSIDE_EEVEE_CONSTRUCTION')
PYID
...
assert r['grounds']==['b03p02a'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b03p02a','primary')]['movement_probe']=='PASS'
assert validations[('b03p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b03p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B03P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b03p02a.rsground | cut -d' ' -f1)" = 114f641f238fc7d3b2d1c03b16f536e5105280a7fe5d62e30aaba5f6cf611fe8
test "$(sha256sum Content/Tile/b03p02a_Base.tile | cut -d' ' -f1)" = 7f260e40b2e42f19f5197bac1468fdf2a9eb39aee38e50470723a1391f913e94
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b03p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b03p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b03p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
