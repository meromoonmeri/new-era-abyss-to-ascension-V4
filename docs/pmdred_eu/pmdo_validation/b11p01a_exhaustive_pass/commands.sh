#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p01a.rsground" | cut -d' ' -f1)" = 9174774fc52ed1bbf56570a6781cc1433fa5674faa2efaf2a3dd2aa0dcdc084d
test "$(sha256sum "$CANONICAL/tiles/b11p01a_Base.tile" | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
test "$(sha256sum RESERVE/red_grounds/b11p01a.rsground | cut -d' ' -f1)" = 00dda09efc1ffac45bc4faed03654275e473c64f952b0da8efd823bb40167a96
test "$(sha256sum RESERVE/red_tiles/b11p01a_Base.tile | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p01a')
assert entry == {'canonical_debug_id': 70, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00070', 'debug_string_address': '0x0827B21C', 'ground_place_id': 10, 'map_file_id': 71, 'map_id': 70, 'stable_ground_id': 'b11p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CYNDAQUIL_CONSTRUCTION' in header
print('B11P01A_IDENTITY_PASS map_file_id=71 symbol=MAP_TEAM_BASE_CYNDAQUIL_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p01a --expected-screenshots 25 \
...
print('B11P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b11p01a','primary')]['movement_probe']=='PASS'
assert validations[('b11p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b11p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b11p01a.rsground | cut -d' ' -f1)" = 9174774fc52ed1bbf56570a6781cc1433fa5674faa2efaf2a3dd2aa0dcdc084d
test "$(sha256sum Content/Tile/b11p01a_Base.tile | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
