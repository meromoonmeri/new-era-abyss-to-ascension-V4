...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p01a')
assert e == {'canonical_debug_id': 70, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00070', 'debug_string_address': '0x0827B21C', 'ground_place_id': 10, 'map_file_id': 71, 'map_id': 70, 'stable_ground_id': 'b11p01a', 'weather_id': 9}
assert 'MAP_TEAM_BASE_CYNDAQUIL_CONSTRUCTION' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B11P01A_IDENTITY_PASS map_id=70 map_file_id=71 symbol=MAP_TEAM_BASE_CYNDAQUIL_CONSTRUCTION')
PYID
...
print('B11P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
