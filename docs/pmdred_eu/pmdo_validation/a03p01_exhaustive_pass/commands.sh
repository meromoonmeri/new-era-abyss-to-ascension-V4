...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a03p01')
assert e == {'canonical_debug_id': 177, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00177', 'debug_string_address': '0x0827A5BC', 'ground_place_id': 35, 'map_file_id': 178, 'map_id': 169, 'stable_ground_id': 'a03p01', 'weather_id': 10}
assert 'MAP_FUGITIVES_SNOW_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A03P01_IDENTITY_PASS map_id=169 map_file_id=178 symbol=MAP_FUGITIVES_SNOW_FOREST')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a03p01 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a03p01.rsground" | cut -d' ' -f1)" = b0217e3fa2963faf0ee340c639b80d8a2cd33f7c3a8a585db21f8eecc0be0beb
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --ids a03p01 \
  --output "$FIX"
"$PYTHON" tools/apply_pmdred_eu_fixture_sheet_alias.py   --fixture "$FIX" --candidate-root "$INTEGRATED" --id a03p01 --alias a03p01_pmdred_Base
...
print('A03P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/a03p01.rsground | cut -d' ' -f1)" = e88881d4a9e6c28affd44927ae963174f641ec95751b786ebf4442c12b0b1259
test "$(sha256sum Content/Tile/a03p01_pmdred_Base.tile | cut -d' ' -f1)" = 877fd56336e417ed7690db8baeeedf465657eaf37a345d2010fa356333bf8cf4
test "$(sha256sum Content/Tile/a03p01_Base.tile | cut -d' ' -f1)" = 4b8f2b3729a32685206ee5c37bc346e53b83685da4a9bdb6964bbb4d1f4642b0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp "$INTEGRATED/grounds/a03p01.rsground" "$POST_CANDIDATE/grounds/"
cp "$CANONICAL/tiles/a03p01_Base.tile" "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" \
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a03p01 --plan "$PLAN" \
  --ids a03p01 --output "$POST_FIX"
"$PYTHON" tools/apply_pmdred_eu_fixture_sheet_alias.py   --fixture "$POST_FIX" --candidate-root "$POST_CANDIDATE" --id a03p01 --alias a03p01_pmdred_Base
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
