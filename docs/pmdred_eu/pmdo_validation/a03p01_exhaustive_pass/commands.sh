#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a03p01 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a03p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a03p01.rsground" | cut -d' ' -f1)" = 36b389989c2ed916b95bc79aa5236a03c42911d81116d8f4f3c1f59679f1c3b3
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a03p01 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a03p01.rsground" | cut -d' ' -f1)" = b0217e3fa2963faf0ee340c639b80d8a2cd33f7c3a8a585db21f8eecc0be0beb
test "$(sha256sum "$CANONICAL/tiles/a03p01_Base.tile" | cut -d' ' -f1)" = 877fd56336e417ed7690db8baeeedf465657eaf37a345d2010fa356333bf8cf4
test "$(sha256sum RESERVE/red_grounds/a03p01.rsground | cut -d' ' -f1)" = b274410f90ad2ef7f0e9c2cbdbb4e68959db2dca25b94b3452f3d67a5a5280dc
test "$(sha256sum RESERVE/red_tiles/a03p01_Base.tile | cut -d' ' -f1)" = 877fd56336e417ed7690db8baeeedf465657eaf37a345d2010fa356333bf8cf4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a03p01')
assert entry == {'canonical_debug_id': 177, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00177', 'debug_string_address': '0x0827A5BC', 'ground_place_id': 35, 'map_file_id': 178, 'map_id': 169, 'stable_ground_id': 'a03p01', 'weather_id': 10}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_SNOW_FOREST' in header
print('A03P01_IDENTITY_PASS map_file_id=178 symbol=MAP_FUGITIVES_SNOW_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a03p01 --expected-screenshots 2 \
...
print('A03P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 31
...
assert r['grounds']==['a03p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a03p01','primary')]['movement_probe']=='PASS'
assert validations[('a03p01','primary')]['blocked_probe']=='PASS'
assert validations[('a03p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A03P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a03p01.rsground | cut -d' ' -f1)" = b0217e3fa2963faf0ee340c639b80d8a2cd33f7c3a8a585db21f8eecc0be0beb
test "$(sha256sum Content/Tile/a03p01_Base.tile | cut -d' ' -f1)" = 877fd56336e417ed7690db8baeeedf465657eaf37a345d2010fa356333bf8cf4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a03p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a03p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a03p01 --entity-integrated-ids a03p01 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p01/a03p01.rsground | cut -d' ' -f1)" = b274410f90ad2ef7f0e9c2cbdbb4e68959db2dca25b94b3452f3d67a5a5280dc
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p01/a03p01_Base.tile | cut -d' ' -f1)" = 4b8f2b3729a32685206ee5c37bc346e53b83685da4a9bdb6964bbb4d1f4642b0
