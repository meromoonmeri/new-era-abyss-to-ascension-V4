#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a05p03 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a05p03.rsground" | cut -d' ' -f1)" = ac270916ba76eefaa6b242d66092fe0a43cbe77637226896753f7dac5339a6a6
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a05p03 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a05p03.rsground" | cut -d' ' -f1)" = bb7a394689a9f70c556d54de0e8ca1679132174111446f14b2c6a39c305d5212
test "$(sha256sum "$CANONICAL/tiles/a05p03_Base.tile" | cut -d' ' -f1)" = 85be315dca72d574dc608c8685b1178653ae6123e4063008ae6fca75b0c21afc
test "$(sha256sum RESERVE/red_grounds/a05p03.rsground | cut -d' ' -f1)" = 9d3819a102ce3f141ffde72c69c4a24d9bf7043c8fc12d1dca970a27548fd813
test "$(sha256sum RESERVE/red_tiles/a05p03_Base.tile | cut -d' ' -f1)" = 85be315dca72d574dc608c8685b1178653ae6123e4063008ae6fca75b0c21afc
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p03')
assert entry == {'canonical_debug_id': 186, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00186', 'debug_string_address': '0x0827A49C', 'ground_place_id': 35, 'map_file_id': 187, 'map_id': 178, 'stable_ground_id': 'a05p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TINY_WOODS_ENTRY' in header
print('A05P03_IDENTITY_PASS map_file_id=187 symbol=MAP_TINY_WOODS_ENTRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a05p03 --expected-screenshots 2 \
...
print('A05P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a05p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a05p03','primary')]['movement_probe']=='PASS'
assert validations[('a05p03','primary')]['blocked_probe']=='PASS'
assert validations[('a05p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A05P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a05p03.rsground | cut -d' ' -f1)" = bb7a394689a9f70c556d54de0e8ca1679132174111446f14b2c6a39c305d5212
test "$(sha256sum Content/Tile/a05p03_Base.tile | cut -d' ' -f1)" = 85be315dca72d574dc608c8685b1178653ae6123e4063008ae6fca75b0c21afc
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a05p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a05p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a05p03 --entity-integrated-ids a05p03 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a05p03/a05p03.rsground | cut -d' ' -f1)" = 41d2183ff20d0dcaa9b2e70c7eb74f5ffeee3b4dca44c432f1bf38c5fdcabee8
test "$(sha256sum RESERVE/pmdred_pre_promotion/a05p03/A05p03_Base.tile | cut -d' ' -f1)" = 85be315dca72d574dc608c8685b1178653ae6123e4063008ae6fca75b0c21afc
