...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a05p03-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p03')
assert e == {'canonical_debug_id': 186, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00186', 'debug_string_address': '0x0827A49C', 'ground_place_id': 35, 'map_file_id': 187, 'map_id': 178, 'stable_ground_id': 'a05p03', 'weather_id': -1}
assert 'MAP_TINY_WOODS_ENTRY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A05P03_IDENTITY_PASS map_id=178 map_file_id=187 symbol=MAP_TINY_WOODS_ENTRY')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a05p03 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a05p03.rsground" | cut -d' ' -f1)" = bb7a394689a9f70c556d54de0e8ca1679132174111446f14b2c6a39c305d5212
...
print('A05P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a05p03 --plan "$PLAN" \
  --ids a05p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
