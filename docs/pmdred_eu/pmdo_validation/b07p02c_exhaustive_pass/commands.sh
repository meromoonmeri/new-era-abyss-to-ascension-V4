#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b07p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b07p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b07p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b07p02c.rsground" | cut -d' ' -f1)" = c1ccfc7e1e49a6b3ab48009c5ed31b8a6cc7a0e51662ed058ebec1001aeed095
test "$(sha256sum "$CANONICAL/tiles/b07p02c_Base.tile" | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b07p02c')
assert entry == {'canonical_debug_id': 51, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00051', 'debug_string_address': '0x0827B47C', 'ground_place_id': 11, 'map_file_id': 52, 'map_id': 51, 'stable_ground_id': 'b07p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_PSYDUCK_BASIC' in header
print('B07P02C_IDENTITY_PASS map_id=51 map_file_id=52 symbol=MAP_TEAM_BASE_PSYDUCK_BASIC')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 5785
...
assert r['grounds']==['b07p02c'] and r['sample_count']==1921 and len(primary)==1920
...
assert validations[('b07p02c','primary')]['movement_probe']=='PASS'
assert validations[('b07p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b07p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B07P02C_EXHAUSTIVE_PASS samples=1921 unique_rgba=180 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b07p02c.rsground | cut -d' ' -f1)" = c1ccfc7e1e49a6b3ab48009c5ed31b8a6cc7a0e51662ed058ebec1001aeed095
test "$(sha256sum Content/Tile/b07p02c_Base.tile | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b07p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b07p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b07p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
