#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p01a.rsground" | cut -d' ' -f1)" = cfdea1941de13bd44a05058d006cb87d258f812eedf9ee798be0e0e5a33c308d
test "$(sha256sum "$CANONICAL/tiles/b13p01a_Base.tile" | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
test "$(sha256sum RESERVE/red_grounds/b13p01a.rsground | cut -d' ' -f1)" = 95822e15f84dca1b101f1bdf45465cb34ff1ff22a8ada558fcfc87d016e3c00f
test "$(sha256sum RESERVE/red_tiles/b13p01a_Base.tile | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p01a')
assert entry == {'canonical_debug_id': 82, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00082', 'debug_string_address': '0x0827B09C', 'ground_place_id': 10, 'map_file_id': 83, 'map_id': 82, 'stable_ground_id': 'b13p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MACHOP_CONSTRUCTION' in header
print('B13P01A_IDENTITY_PASS map_file_id=83 symbol=MAP_TEAM_BASE_MACHOP_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p01a --expected-screenshots 25 \
...
print('B13P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b13p01a','primary')]['movement_probe']=='PASS'
assert validations[('b13p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b13p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b13p01a.rsground | cut -d' ' -f1)" = cfdea1941de13bd44a05058d006cb87d258f812eedf9ee798be0e0e5a33c308d
test "$(sha256sum Content/Tile/b13p01a_Base.tile | cut -d' ' -f1)" = 9d93b72a9a99b3357136ba83f5d14dc6173e6aa72f1fb31ab96db677c970f096
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
