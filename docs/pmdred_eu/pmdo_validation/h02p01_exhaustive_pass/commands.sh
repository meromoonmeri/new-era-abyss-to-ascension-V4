#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h02p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h02p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h02p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h02p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h02p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h02p01.rsground" | cut -d' ' -f1)" = 6edc54511d3c7c67783513d7f70a51ea7eb8da6b4007104d8ffec6029a31da09
test "$(sha256sum "$CANONICAL/tiles/h02p01_Base.tile" | cut -d' ' -f1)" = 2d0899da55864526a5bf2d8263737aaedf74f262460388824fc5926157c66b3a
test "$(sha256sum RESERVE/red_grounds/h02p01.rsground | cut -d' ' -f1)" = 14cfabab54c7fcc9895e88d17aad7c0b3994538c9c01eeb920069d9b6b6d9468
test "$(sha256sum RESERVE/red_tiles/h02p01_Base.tile | cut -d' ' -f1)" = 2d0899da55864526a5bf2d8263737aaedf74f262460388824fc5926157c66b3a
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h02p01')
assert entry == {'canonical_debug_id': 110, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00110', 'debug_string_address': '0x0827AD3C', 'ground_place_id': 9, 'map_file_id': 111, 'map_id': 109, 'stable_ground_id': 'h02p01', 'weather_id': 2}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_DEEP_SEA_CURRENT' in header
print('H02P01_IDENTITY_PASS map_file_id=111 symbol=MAP_FRIEND_AREA_DEEP_SEA_CURRENT')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h02p01 --expected-screenshots 2 \
...
print('H02P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h02p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h02p01','primary')]['movement_probe']=='PASS'
assert validations[('h02p01','primary')]['blocked_probe']=='PASS'
assert validations[('h02p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H02P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h02p01.rsground | cut -d' ' -f1)" = 6edc54511d3c7c67783513d7f70a51ea7eb8da6b4007104d8ffec6029a31da09
test "$(sha256sum Content/Tile/h02p01_Base.tile | cut -d' ' -f1)" = 2d0899da55864526a5bf2d8263737aaedf74f262460388824fc5926157c66b3a
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h02p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h02p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h02p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
