#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p04 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p04.rsground" | cut -d' ' -f1)" = d17ef948cab9249b2cd966167f2f2c8ce78ae12918522a21f990cf4c65421d59
test "$(sha256sum "$CANONICAL/tiles/h07p04_Base.tile" | cut -d' ' -f1)" = 82acaa4103e0855656fca0484165a910719c740233c9085fb7c9075de85d9f82
test "$(sha256sum RESERVE/red_grounds/h07p04.rsground | cut -d' ' -f1)" = fc8c4bca9ac6d6ee43da0d7ed728e2f980f609fc7d995d4856147c361f0f74a3
test "$(sha256sum RESERVE/red_tiles/h07p04_Base.tile | cut -d' ' -f1)" = 82acaa4103e0855656fca0484165a910719c740233c9085fb7c9075de85d9f82
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p04')
assert entry == {'canonical_debug_id': 130, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00130', 'debug_string_address': '0x0827AB1C', 'ground_place_id': 9, 'map_file_id': 131, 'map_id': 126, 'stable_ground_id': 'h07p04', 'weather_id': 5}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MUSHROOM_FOREST' in header
print('H07P04_IDENTITY_PASS map_file_id=131 symbol=MAP_FRIEND_AREA_MUSHROOM_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p04 --expected-screenshots 2 \
...
print('H07P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h07p04'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h07p04','primary')]['movement_probe']=='PASS'
assert validations[('h07p04','primary')]['blocked_probe']=='PASS'
assert validations[('h07p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h07p04.rsground | cut -d' ' -f1)" = d17ef948cab9249b2cd966167f2f2c8ce78ae12918522a21f990cf4c65421d59
test "$(sha256sum Content/Tile/h07p04_Base.tile | cut -d' ' -f1)" = 82acaa4103e0855656fca0484165a910719c740233c9085fb7c9075de85d9f82
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
