#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p01b.rsground" | cut -d' ' -f1)" = fe503070fcb328563aa1275c9a617a4ab55360e2438e97073be91c189d3fda4d
test "$(sha256sum "$CANONICAL/tiles/b06p01b_Base.tile" | cut -d' ' -f1)" = 0920e12315134ad5827e5e147746f2d413e79eaf112eafd90fd98910d12a1042
test "$(sha256sum RESERVE/red_grounds/b06p01b.rsground | cut -d' ' -f1)" = 60404810fe8e737e25e27bc2524c9ee918d61eb963ba4b225153bf51a1adda99
test "$(sha256sum RESERVE/red_tiles/b06p01b_Base.tile | cut -d' ' -f1)" = 0920e12315134ad5827e5e147746f2d413e79eaf112eafd90fd98910d12a1042
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p01b')
assert entry == {'canonical_debug_id': 41, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00041', 'debug_string_address': '0x0827B5BC', 'ground_place_id': 10, 'map_file_id': 42, 'map_id': 41, 'stable_ground_id': 'b06p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TOTODILE_FINAL' in header
print('B06P01B_IDENTITY_PASS map_file_id=42 symbol=MAP_TEAM_BASE_TOTODILE_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b06p01b --expected-screenshots 51 \
...
print('B06P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b06p01b'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b06p01b','primary')]['movement_probe']=='PASS'
assert validations[('b06p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b06p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P01B_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b06p01b.rsground | cut -d' ' -f1)" = fe503070fcb328563aa1275c9a617a4ab55360e2438e97073be91c189d3fda4d
test "$(sha256sum Content/Tile/b06p01b_Base.tile | cut -d' ' -f1)" = 0920e12315134ad5827e5e147746f2d413e79eaf112eafd90fd98910d12a1042
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
