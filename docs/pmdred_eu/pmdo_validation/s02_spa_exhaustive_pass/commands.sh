#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s02_spa milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s02_spa-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s02_spa-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s02_spa-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s02_spa-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s02_spa.rsground" | cut -d' ' -f1)" = 40996e801ac9ac074b6197e1b335f03d39f5e5e09d63e6f2c403f4e06723f156
test "$(sha256sum "$CANONICAL/tiles/s02_spa_Base.tile" | cut -d' ' -f1)" = feaac14d21d55f06470ef106f101250a54c02fbca8eef9dee0717d68c39aa111
test ! -e RESERVE/red_grounds/s02_spa.rsground
test ! -e RESERVE/red_tiles/s02_spa_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s02_spa')
assert entry == {'canonical_debug_id': 233, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00233', 'debug_string_address': '0x08279EBC', 'ground_place_id': 37, 'map_file_id': 242, 'map_id': 233, 'stable_ground_id': 's02_spa', 'weather_id': -1}
...
print('S02_SPA_IDENTITY_PASS map_file_id=242 canonical_debug_id=233 symbol=MAP_TITLE_SCREEN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s02_spa --expected-screenshots 2 \
...
print('S02_SPA_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s02_spa'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s02_spa','primary')]['movement_probe']=='PASS'
assert validations[('s02_spa','primary')]['blocked_probe']=='PASS'
assert validations[('s02_spa','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S02_SPA_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s02_spa.rsground | cut -d' ' -f1)" = 40996e801ac9ac074b6197e1b335f03d39f5e5e09d63e6f2c403f4e06723f156
test "$(sha256sum Content/Tile/s02_spa_Base.tile | cut -d' ' -f1)" = feaac14d21d55f06470ef106f101250a54c02fbca8eef9dee0717d68c39aa111
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s02_spa.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s02_spa_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s02_spa --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
