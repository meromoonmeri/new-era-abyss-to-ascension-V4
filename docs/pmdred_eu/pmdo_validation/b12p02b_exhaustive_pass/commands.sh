#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p02b.rsground" | cut -d' ' -f1)" = e420549cbbef7b48c2c63d0ffcf01937cd2664ec622a36d8d42c967eae0edde3
test "$(sha256sum "$CANONICAL/tiles/b12p02b_Base.tile" | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
test "$(sha256sum RESERVE/red_grounds/b12p02b.rsground | cut -d' ' -f1)" = e27fa90bf73698a51df66e83b47a773eafcfca74fea4f32f31fdc0727cd9187d
test "$(sha256sum RESERVE/red_tiles/b12p02b_Base.tile | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p02b')
assert entry == {'canonical_debug_id': 80, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00080', 'debug_string_address': '0x0827B0DC', 'ground_place_id': 11, 'map_file_id': 81, 'map_id': 80, 'stable_ground_id': 'b12p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CUBONE_FINAL' in header
print('B12P02B_IDENTITY_PASS map_file_id=81 symbol=MAP_TEAM_BASE_INSIDE_CUBONE_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p02b --expected-screenshots 2 \
...
print('B12P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b12p02b','primary')]['movement_probe']=='PASS'
assert validations[('b12p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b12p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b12p02b.rsground | cut -d' ' -f1)" = e420549cbbef7b48c2c63d0ffcf01937cd2664ec622a36d8d42c967eae0edde3
test "$(sha256sum Content/Tile/b12p02b_Base.tile | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
