#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b14p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b14p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b14p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b14p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b14p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b14p02a.rsground" | cut -d' ' -f1)" = 9b89d5fe0aba7ea6cfdbabca4f6512791b4aae3c816b0b4c4db9723d953c9217
test "$(sha256sum "$CANONICAL/tiles/b14p02a_Base.tile" | cut -d' ' -f1)" = c0fc5ae65267d782b3ff1f8ee8db4ebe8832a80a1708a6c48ac10b1366818dc7
test "$(sha256sum RESERVE/red_grounds/b14p02a.rsground | cut -d' ' -f1)" = 9c3a4c35643f72c76b0d8c4299c74b26855780c419884847a8688857ecb8862c
test "$(sha256sum RESERVE/red_tiles/b14p02a_Base.tile | cut -d' ' -f1)" = c0fc5ae65267d782b3ff1f8ee8db4ebe8832a80a1708a6c48ac10b1366818dc7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p02a')
assert entry == {'canonical_debug_id': 91, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00091', 'debug_string_address': '0x0827AF7C', 'ground_place_id': 11, 'map_file_id': 92, 'map_id': 91, 'stable_ground_id': 'b14p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_BULBASAUR_CONSTRUCTION' in header
print('B14P02A_IDENTITY_PASS map_file_id=92 symbol=MAP_TEAM_BASE_INSIDE_BULBASAUR_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b14p02a --expected-screenshots 2 \
...
print('B14P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b14p02a'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b14p02a','primary')]['movement_probe']=='PASS'
assert validations[('b14p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b14p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B14P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b14p02a.rsground | cut -d' ' -f1)" = 9b89d5fe0aba7ea6cfdbabca4f6512791b4aae3c816b0b4c4db9723d953c9217
test "$(sha256sum Content/Tile/b14p02a_Base.tile | cut -d' ' -f1)" = c0fc5ae65267d782b3ff1f8ee8db4ebe8832a80a1708a6c48ac10b1366818dc7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b14p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b14p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b14p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
