...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p02a')
assert e == {'canonical_debug_id': 91, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00091', 'debug_string_address': '0x0827AF7C', 'ground_place_id': 11, 'map_file_id': 92, 'map_id': 91, 'stable_ground_id': 'b14p02a', 'weather_id': -1}
assert 'MAP_TEAM_BASE_INSIDE_BULBASAUR_CONSTRUCTION' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B14P02A_IDENTITY_PASS map_id=91 map_file_id=92 symbol=MAP_TEAM_BASE_INSIDE_BULBASAUR_CONSTRUCTION')
PYID
...
print('B14P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
