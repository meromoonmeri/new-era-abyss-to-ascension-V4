#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b16p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b16p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b16p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b16p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b16p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b16p02b.rsground" | cut -d' ' -f1)" = 6e8364af902f8015a4a989025fe7fa3e7ad16fd3ae657b0ede7c6ec173c7784b
test "$(sha256sum "$CANONICAL/tiles/b16p02b_Base.tile" | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
test "$(sha256sum RESERVE/red_grounds/b16p02b.rsground | cut -d' ' -f1)" = 91183e63e93ca35a133ef315ca2e5acb028c22f14fed8cee26f60c6470829bd0
test "$(sha256sum RESERVE/red_tiles/b16p02b_Base.tile | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p02b')
assert entry == {'canonical_debug_id': 104, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00104', 'debug_string_address': '0x0827ADDC', 'ground_place_id': 11, 'map_file_id': 105, 'map_id': 104, 'stable_ground_id': 'b16p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TREECKO_FINAL' in header
print('B16P02B_IDENTITY_PASS map_file_id=105 symbol=MAP_TEAM_BASE_INSIDE_TREECKO_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b16p02b --expected-screenshots 2 \
...
print('B16P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b16p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b16p02b','primary')]['movement_probe']=='PASS'
assert validations[('b16p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b16p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B16P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b16p02b.rsground | cut -d' ' -f1)" = 6e8364af902f8015a4a989025fe7fa3e7ad16fd3ae657b0ede7c6ec173c7784b
test "$(sha256sum Content/Tile/b16p02b_Base.tile | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b16p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b16p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b16p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
