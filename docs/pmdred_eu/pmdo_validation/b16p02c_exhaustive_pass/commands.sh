#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b16p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b16p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b16p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b16p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b16p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b16p02c.rsground" | cut -d' ' -f1)" = e13c21ba5d13581a4ebed95a9b8169c3c5e5f3723b2d2adf7c4aa121df0d58f8
test "$(sha256sum "$CANONICAL/tiles/b16p02c_Base.tile" | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test "$(sha256sum RESERVE/red_grounds/b16p02c.rsground | cut -d' ' -f1)" = a59f7bdd253d914a3a0fdc5969f91cad29efdb72aa354ac1a7a3a776ece372bf
test "$(sha256sum RESERVE/red_tiles/b16p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p02c')
assert entry == {'canonical_debug_id': 105, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00105', 'debug_string_address': '0x0827ADBC', 'ground_place_id': 11, 'map_file_id': 106, 'map_id': 105, 'stable_ground_id': 'b16p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_BOUNTIFUL_SEA' in header
print('B16P02C_IDENTITY_PASS map_file_id=106 symbol=MAP_FRIEND_AREA_BOUNTIFUL_SEA')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b16p02c --expected-screenshots 2 \
...
print('B16P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b16p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b16p02c','primary')]['movement_probe']=='PASS'
assert validations[('b16p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b16p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B16P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b16p02c.rsground | cut -d' ' -f1)" = e13c21ba5d13581a4ebed95a9b8169c3c5e5f3723b2d2adf7c4aa121df0d58f8
test "$(sha256sum Content/Tile/b16p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b16p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b16p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b16p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
