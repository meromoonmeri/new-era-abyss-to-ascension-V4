...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p02c')
assert e == {'canonical_debug_id': 105, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00105', 'debug_string_address': '0x0827ADBC', 'ground_place_id': 11, 'map_file_id': 106, 'map_id': 105, 'stable_ground_id': 'b16p02c', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_BOUNTIFUL_SEA' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B16P02C_IDENTITY_PASS map_id=105 map_file_id=106 symbol=MAP_FRIEND_AREA_BOUNTIFUL_SEA')
PYID
...
print('B16P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
