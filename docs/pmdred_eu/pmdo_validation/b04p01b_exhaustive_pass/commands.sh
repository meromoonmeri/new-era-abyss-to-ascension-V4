#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b04p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b04p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b04p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b04p01b.rsground" | cut -d' ' -f1)" = 38fed8453dd3b4e958575dc595e1a5b117e62de85100abde62f94662584899e2
test "$(sha256sum "$CANONICAL/tiles/b04p01b_Base.tile" | cut -d' ' -f1)" = 917e0ffa07eced179eda7e362ca75a3f6aa53a877752b728cc86009300d5e4a1
test "$(sha256sum RESERVE/red_grounds/b04p01b.rsground | cut -d' ' -f1)" = c4a5784fc89657ad2e033e8223c475ebfc793e504a0121f5499dfeb70f74ba10
test "$(sha256sum RESERVE/red_tiles/b04p01b_Base.tile | cut -d' ' -f1)" = 917e0ffa07eced179eda7e362ca75a3f6aa53a877752b728cc86009300d5e4a1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b04p01b')
assert entry == {'canonical_debug_id': 29, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00029', 'debug_string_address': '0x0827B73C', 'ground_place_id': 10, 'map_file_id': 30, 'map_id': 29, 'stable_ground_id': 'b04p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_SKITTY_FINAL' in header
print('B04P01B_IDENTITY_PASS map_file_id=30 symbol=MAP_TEAM_BASE_SKITTY_FINAL')
PYID
...
assert r['grounds']==['b04p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b04p01b','primary')]['movement_probe']=='PASS'
assert validations[('b04p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b04p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B04P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b04p01b.rsground | cut -d' ' -f1)" = 38fed8453dd3b4e958575dc595e1a5b117e62de85100abde62f94662584899e2
test "$(sha256sum Content/Tile/b04p01b_Base.tile | cut -d' ' -f1)" = 917e0ffa07eced179eda7e362ca75a3f6aa53a877752b728cc86009300d5e4a1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b04p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b04p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b04p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
