...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p04')
assert e == {'canonical_debug_id': 170, 'conversion_type': 5, 'debug_string': '__ground_amd_conversion_00170', 'debug_string_address': '0x0827A69C', 'ground_place_id': 9, 'map_file_id': 171, 'map_id': 162, 'stable_ground_id': 'h29p04', 'weather_id': -1}
assert 'MAP_PERSONALITY_TEST_CYAN' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H29P04_IDENTITY_PASS map_id=162 map_file_id=171 symbol=MAP_PERSONALITY_TEST_CYAN')
PYID
...
print('H29P04_EXHAUSTIVE_PASS samples=73 unique_rgba=22 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
