#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h29p04 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h29p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h29p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h29p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h29p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h29p04.rsground" | cut -d' ' -f1)" = cb60d36384e7474513c716b010df2756a4abda7fc3559223bfbd82f5f25c7cb8
test "$(sha256sum "$CANONICAL/tiles/h29p04_Base.tile" | cut -d' ' -f1)" = 5801cbc2d045461c6a56f6b8cf5e2f81b7f3523ed8bcdd59a2d954bf7e0a3041
test "$(sha256sum RESERVE/red_grounds/h29p04.rsground | cut -d' ' -f1)" = af6c4c3e30ae27f73d9d063a2b23ba3d56cd5eede9b33661ab0c9ced215f710a
test "$(sha256sum RESERVE/red_tiles/h29p04_Base.tile | cut -d' ' -f1)" = 5801cbc2d045461c6a56f6b8cf5e2f81b7f3523ed8bcdd59a2d954bf7e0a3041
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p04')
assert entry == {'canonical_debug_id': 170, 'conversion_type': 5, 'debug_string': '__ground_amd_conversion_00170', 'debug_string_address': '0x0827A69C', 'ground_place_id': 9, 'map_file_id': 171, 'map_id': 162, 'stable_ground_id': 'h29p04', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_PERSONALITY_TEST_CYAN' in header
print('H29P04_IDENTITY_PASS map_file_id=171 symbol=MAP_PERSONALITY_TEST_CYAN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h29p04 --expected-screenshots 73 \
...
print('H29P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h29p04'] and r['sample_count']==73 and len(primary)==72
assert r['exact_sample_count']==73 and r['fully_opaque_sample_count']==73
assert len({s['actual_rgba_sha256'] for s in primary})==22
...
assert validations[('h29p04','primary')]['movement_probe']=='PASS'
assert validations[('h29p04','primary')]['blocked_probe']=='PASS'
assert validations[('h29p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H29P04_EXHAUSTIVE_PASS samples=73 unique_rgba=22')
PY
...
test "$(sha256sum Data/Ground/h29p04.rsground | cut -d' ' -f1)" = cb60d36384e7474513c716b010df2756a4abda7fc3559223bfbd82f5f25c7cb8
test "$(sha256sum Content/Tile/h29p04_Base.tile | cut -d' ' -f1)" = 5801cbc2d045461c6a56f6b8cf5e2f81b7f3523ed8bcdd59a2d954bf7e0a3041
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h29p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h29p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h29p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
