#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p01c.rsground" | cut -d' ' -f1)" = aa141d62abb713a588a5cb2e54a298d5c1d781ee693ded99a251dfc8017abfbf
test "$(sha256sum "$CANONICAL/tiles/b11p01c_Base.tile" | cut -d' ' -f1)" = 151b4962b6cb0a2b3443406dcdde5e67707e6fb216cc860a03615d0892fa2c94
test "$(sha256sum RESERVE/red_grounds/b11p01c.rsground | cut -d' ' -f1)" = aae7c3dc7f4eef712c4da0a763c031298f9335a52f03314cd20eec9d627298bc
test "$(sha256sum RESERVE/red_tiles/b11p01c_Base.tile | cut -d' ' -f1)" = 151b4962b6cb0a2b3443406dcdde5e67707e6fb216cc860a03615d0892fa2c94
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p01c')
assert entry == {'canonical_debug_id': 72, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00072', 'debug_string_address': '0x0827B1DC', 'ground_place_id': 10, 'map_file_id': 73, 'map_id': 72, 'stable_ground_id': 'b11p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CYNDAQUIL_BASIC' in header
print('B11P01C_IDENTITY_PASS map_file_id=73 symbol=MAP_TEAM_BASE_INSIDE_CYNDAQUIL_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p01c --expected-screenshots 25 \
...
print('B11P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b11p01c','primary')]['movement_probe']=='PASS'
assert validations[('b11p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b11p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b11p01c.rsground | cut -d' ' -f1)" = aa141d62abb713a588a5cb2e54a298d5c1d781ee693ded99a251dfc8017abfbf
test "$(sha256sum Content/Tile/b11p01c_Base.tile | cut -d' ' -f1)" = 151b4962b6cb0a2b3443406dcdde5e67707e6fb216cc860a03615d0892fa2c94
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
