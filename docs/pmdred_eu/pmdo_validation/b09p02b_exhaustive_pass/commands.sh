#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b09p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b09p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b09p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b09p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b09p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b09p02b.rsground" | cut -d' ' -f1)" = adcaa32a0f190e3e70d87d6268625a6f9883808173cee8c59252222f6d0ae140
test "$(sha256sum "$CANONICAL/tiles/b09p02b_Base.tile" | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test "$(sha256sum RESERVE/red_grounds/b09p02b.rsground | cut -d' ' -f1)" = 8dcdf7bf4d25b0fd0112d8af3d3fa59705c836327d54081fe8b27388dfbc4b6d
test "$(sha256sum RESERVE/red_tiles/b09p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b09p02b')
assert entry == {'canonical_debug_id': 62, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00062', 'debug_string_address': '0x0827B31C', 'ground_place_id': 11, 'map_file_id': 63, 'map_id': 62, 'stable_ground_id': 'b09p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CHARMANDER_FINAL' in header
print('B09P02B_IDENTITY_PASS map_file_id=63 symbol=MAP_TEAM_BASE_INSIDE_CHARMANDER_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b09p02b --expected-screenshots 1561 \
...
print('B09P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b09p02b'] and r['sample_count']==1561 and len(primary)==1560
assert r['exact_sample_count']==1561 and r['fully_opaque_sample_count']==1561
assert len({s['actual_rgba_sha256'] for s in primary})==6
...
assert validations[('b09p02b','primary')]['movement_probe']=='PASS'
assert validations[('b09p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b09p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B09P02B_EXHAUSTIVE_PASS samples=1561 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b09p02b.rsground | cut -d' ' -f1)" = adcaa32a0f190e3e70d87d6268625a6f9883808173cee8c59252222f6d0ae140
test "$(sha256sum Content/Tile/b09p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b09p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b09p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b09p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
