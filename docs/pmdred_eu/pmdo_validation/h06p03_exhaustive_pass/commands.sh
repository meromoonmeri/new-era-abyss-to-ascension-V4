#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p03.rsground" | cut -d' ' -f1)" = 2bb9374fa666f4b17d38fbfd69808b4983d18ba10e237d48f1ecc0c928db121e
test "$(sha256sum "$CANONICAL/tiles/h06p03_Base.tile" | cut -d' ' -f1)" = 7d561a91c33890cc765d783a49711550aa86b28333d8b84245a295a5333271d7
test "$(sha256sum RESERVE/red_grounds/h06p03.rsground | cut -d' ' -f1)" = 23871d52689f4f9639165ba04da4b1ec34c4cb623a5f68b4223d6478ab58a482
test "$(sha256sum RESERVE/red_tiles/h06p03_Base.tile | cut -d' ' -f1)" = 7d561a91c33890cc765d783a49711550aa86b28333d8b84245a295a5333271d7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p03')
assert entry == {'canonical_debug_id': 123, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00123', 'debug_string_address': '0x0827ABFC', 'ground_place_id': 9, 'map_file_id': 124, 'map_id': 119, 'stable_ground_id': 'h06p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SAFARI' in header
print('H06P03_IDENTITY_PASS map_file_id=124 symbol=MAP_FRIEND_AREA_SAFARI')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p03 --expected-screenshots 2 \
...
print('H06P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h06p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h06p03','primary')]['movement_probe']=='PASS'
assert validations[('h06p03','primary')]['blocked_probe']=='PASS'
assert validations[('h06p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h06p03.rsground | cut -d' ' -f1)" = 2bb9374fa666f4b17d38fbfd69808b4983d18ba10e237d48f1ecc0c928db121e
test "$(sha256sum Content/Tile/h06p03_Base.tile | cut -d' ' -f1)" = 7d561a91c33890cc765d783a49711550aa86b28333d8b84245a295a5333271d7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
