#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h01p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h01p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h01p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h01p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h01p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h01p01.rsground" | cut -d' ' -f1)" = 5e391a1b26d2594aeff73892e6c51ca900db3be341dfe2fe6050b6d737fb83bb
test "$(sha256sum "$CANONICAL/tiles/h01p01_Base.tile" | cut -d' ' -f1)" = 8f47475591aa8984930831820fcf7486baa2286fe3496f875f02330f3658e3b0
test "$(sha256sum RESERVE/red_grounds/h01p01.rsground | cut -d' ' -f1)" = 9df56a526c67c3b919fc10b7664993d8a13c373c80bd8a778355b9b5c342f600
test "$(sha256sum RESERVE/red_tiles/h01p01_Base.tile | cut -d' ' -f1)" = 8f47475591aa8984930831820fcf7486baa2286fe3496f875f02330f3658e3b0
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h01p01')
assert entry == {'canonical_debug_id': 106, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00106', 'debug_string_address': '0x0827AD9C', 'ground_place_id': 9, 'map_file_id': 107, 'map_id': 106, 'stable_ground_id': 'h01p01', 'weather_id': 1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_TREASURE_SEA' in header
print('H01P01_IDENTITY_PASS map_file_id=107 symbol=MAP_FRIEND_AREA_TREASURE_SEA')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h01p01 --expected-screenshots 2 \
...
print('H01P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h01p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h01p01','primary')]['movement_probe']=='PASS'
assert validations[('h01p01','primary')]['blocked_probe']=='PASS'
assert validations[('h01p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H01P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h01p01.rsground | cut -d' ' -f1)" = 5e391a1b26d2594aeff73892e6c51ca900db3be341dfe2fe6050b6d737fb83bb
test "$(sha256sum Content/Tile/h01p01_Base.tile | cut -d' ' -f1)" = 8f47475591aa8984930831820fcf7486baa2286fe3496f875f02330f3658e3b0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h01p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h01p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h01p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
