#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p07 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p07-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p07-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p07-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p07-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p07.rsground" | cut -d' ' -f1)" = 100e89ea0c6e00e37fbff782c3b3112e0862bdec79a8dbcc9f6f35a21bbc08ae
test "$(sha256sum "$CANONICAL/tiles/h07p07_Base.tile" | cut -d' ' -f1)" = 90f671b6297b8a937ff26c00b3994af6b74187f3680cf2a0d5ed1f01c473214d
test "$(sha256sum RESERVE/red_grounds/h07p07.rsground | cut -d' ' -f1)" = a248bec0aa00456bf8185560af83278185097c4b174ca6d7c6ea5ae1e8a1b815
test "$(sha256sum RESERVE/red_tiles/h07p07_Base.tile | cut -d' ' -f1)" = 90f671b6297b8a937ff26c00b3994af6b74187f3680cf2a0d5ed1f01c473214d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p07')
assert entry == {'canonical_debug_id': 134, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00134', 'debug_string_address': '0x0827AABC', 'ground_place_id': 9, 'map_file_id': 135, 'map_id': 129, 'stable_ground_id': 'h07p07', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SECRETIVE_FOREST' in header
print('H07P07_IDENTITY_PASS map_file_id=135 symbol=MAP_FRIEND_AREA_SECRETIVE_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p07 --expected-screenshots 129 \
...
print('H07P07_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 412
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 129
...
assert r['grounds']==['h07p07'] and r['sample_count']==129 and len(primary)==128
assert r['exact_sample_count']==129 and r['fully_opaque_sample_count']==129
...
assert validations[('h07p07','primary')]['movement_probe']=='PASS'
assert validations[('h07p07','primary')]['blocked_probe']=='PASS'
assert validations[('h07p07','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P07_EXHAUSTIVE_PASS samples=129 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/h07p07.rsground | cut -d' ' -f1)" = 100e89ea0c6e00e37fbff782c3b3112e0862bdec79a8dbcc9f6f35a21bbc08ae
test "$(sha256sum Content/Tile/h07p07_Base.tile | cut -d' ' -f1)" = 90f671b6297b8a937ff26c00b3994af6b74187f3680cf2a0d5ed1f01c473214d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p07.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p07_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p07 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 129 comparative PNGs,
