#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b07p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b07p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b07p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b07p02b.rsground" | cut -d' ' -f1)" = cbb1b5b946d4ab6d386e3fc52f21b57f97c434f896097c85ff7c0786a86ced95
test "$(sha256sum "$CANONICAL/tiles/b07p02b_Base.tile" | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b07p02b')
assert entry == {'canonical_debug_id': 50, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00050', 'debug_string_address': '0x0827B49C', 'ground_place_id': 11, 'map_file_id': 51, 'map_id': 50, 'stable_ground_id': 'b07p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MUDKIP_FINAL' in header
print('B07P02B_IDENTITY_PASS map_id=50 map_file_id=51 symbol=MAP_TEAM_BASE_INSIDE_MUDKIP_FINAL')
PYID
...
assert r['grounds']==['b07p02b'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b07p02b','primary')]['movement_probe']=='PASS'
assert validations[('b07p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b07p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B07P02B_EXHAUSTIVE_PASS samples=481 unique_rgba=233 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b07p02b.rsground | cut -d' ' -f1)" = cbb1b5b946d4ab6d386e3fc52f21b57f97c434f896097c85ff7c0786a86ced95
test "$(sha256sum Content/Tile/b07p02b_Base.tile | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b07p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b07p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b07p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
