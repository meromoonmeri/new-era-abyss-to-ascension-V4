#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p02.rsground" | cut -d' ' -f1)" = b5a24c3588f6e0c570b2822d1bbf36f555bb50fd2948ac5b98784358b53ca62f
test "$(sha256sum "$CANONICAL/tiles/h06p02_Base.tile" | cut -d' ' -f1)" = 51aa9af82852655de44faed5051982fb06825ed7d3346f151513dc21f479720a
test "$(sha256sum RESERVE/red_grounds/h06p02.rsground | cut -d' ' -f1)" = 701499516efc2300dde12268e700d70abb9f34af2c1fec4aeaaced3292174072
test "$(sha256sum RESERVE/red_tiles/h06p02_Base.tile | cut -d' ' -f1)" = 36924fec243a14145d72c19928bfa678aa0496d5bfd7892d6e8fa68d2156a610
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p02')
assert entry == {'canonical_debug_id': 122, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00122', 'debug_string_address': '0x0827AC1C', 'ground_place_id': 9, 'map_file_id': 123, 'map_id': 118, 'stable_ground_id': 'h06p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SKY_BLUE_PLAINS' in header
print('H06P02_IDENTITY_PASS map_file_id=123 symbol=MAP_FRIEND_AREA_SKY_BLUE_PLAINS')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p02 --expected-screenshots 41 \
...
print('H06P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 148
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 41
...
assert r['grounds']==['h06p02'] and r['sample_count']==41 and len(primary)==40
assert r['exact_sample_count']==41 and r['fully_opaque_sample_count']==41
assert len({s['actual_rgba_sha256'] for s in primary})==10
...
assert validations[('h06p02','primary')]['movement_probe']=='PASS'
assert validations[('h06p02','primary')]['blocked_probe']=='PASS'
assert validations[('h06p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P02_EXHAUSTIVE_PASS samples=41 unique_rgba=10')
PY
...
test "$(sha256sum Data/Ground/h06p02.rsground | cut -d' ' -f1)" = b5a24c3588f6e0c570b2822d1bbf36f555bb50fd2948ac5b98784358b53ca62f
test "$(sha256sum Content/Tile/h06p02_Base.tile | cut -d' ' -f1)" = 51aa9af82852655de44faed5051982fb06825ed7d3346f151513dc21f479720a
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 41 comparative PNGs,
