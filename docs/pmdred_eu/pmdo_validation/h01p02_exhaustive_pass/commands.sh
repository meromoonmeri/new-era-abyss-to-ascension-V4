...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h01p02')
assert e == {'canonical_debug_id': 108, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00108', 'debug_string_address': '0x0827AD7C', 'ground_place_id': 9, 'map_file_id': 109, 'map_id': 107, 'stable_ground_id': 'h01p02', 'weather_id': 14}
assert 'MAP_FRIEND_AREA_SERENE_SEA' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H01P02_IDENTITY_PASS map_id=107 map_file_id=109 symbol=MAP_FRIEND_AREA_SERENE_SEA')
PYID
...
print('H01P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
