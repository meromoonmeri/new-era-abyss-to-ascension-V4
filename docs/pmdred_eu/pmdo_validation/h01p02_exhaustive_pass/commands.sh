#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h01p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h01p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h01p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h01p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h01p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h01p02.rsground" | cut -d' ' -f1)" = 0f9a816363eff20e529f9e19b3abad72e8cd3cdc20d47af352c824fc3ac49808
test "$(sha256sum "$CANONICAL/tiles/h01p02_Base.tile" | cut -d' ' -f1)" = d19b1fa8d9149f1d19ecb9707967ef2129d931b46d8fa81ad6c78bd9859c7c31
test "$(sha256sum RESERVE/red_grounds/h01p02.rsground | cut -d' ' -f1)" = be4ee590853bb7bc40ee989d17651089b0c0d8b73901b6295f29dd95019d0cbd
test "$(sha256sum RESERVE/red_tiles/h01p02_Base.tile | cut -d' ' -f1)" = d19b1fa8d9149f1d19ecb9707967ef2129d931b46d8fa81ad6c78bd9859c7c31
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h01p02')
assert entry == {'canonical_debug_id': 108, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00108', 'debug_string_address': '0x0827AD7C', 'ground_place_id': 9, 'map_file_id': 109, 'map_id': 107, 'stable_ground_id': 'h01p02', 'weather_id': 14}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SERENE_SEA' in header
print('H01P02_IDENTITY_PASS map_file_id=109 symbol=MAP_FRIEND_AREA_SERENE_SEA')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h01p02 --expected-screenshots 2 \
...
print('H01P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h01p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h01p02','primary')]['movement_probe']=='PASS'
assert validations[('h01p02','primary')]['blocked_probe']=='PASS'
assert validations[('h01p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H01P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h01p02.rsground | cut -d' ' -f1)" = 0f9a816363eff20e529f9e19b3abad72e8cd3cdc20d47af352c824fc3ac49808
test "$(sha256sum Content/Tile/h01p02_Base.tile | cut -d' ' -f1)" = d19b1fa8d9149f1d19ecb9707967ef2129d931b46d8fa81ad6c78bd9859c7c31
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h01p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h01p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h01p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
