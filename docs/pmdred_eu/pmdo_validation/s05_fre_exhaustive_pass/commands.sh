#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s05_fre milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s05_fre-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s05_fre-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s05_fre-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s05_fre-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s05_fre.rsground" | cut -d' ' -f1)" = 9ea10fe451726313d93c95fc1336b07558cb0123e59561434c1cee2c487af86d
test "$(sha256sum "$CANONICAL/tiles/s05_fre_Base.tile" | cut -d' ' -f1)" = 7711a4b289d5923f6f5f4cd1f7d56aac79e0b0d35db2ece82bd5ca150f3d977f
test ! -e RESERVE/red_grounds/s05_fre.rsground
test ! -e RESERVE/red_tiles/s05_fre_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s05_fre')
assert entry == {'canonical_debug_id': 239, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00239', 'debug_string_address': '0x08279E5C', 'ground_place_id': 37, 'map_file_id': 247, 'map_id': 238, 'stable_ground_id': 's05_fre', 'weather_id': -1}
...
print('S05_FRE_IDENTITY_PASS map_file_id=247 canonical_debug_id=239 symbol=MAP_LOGO_WARNING')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s05_fre --expected-screenshots 2 \
...
print('S05_FRE_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s05_fre'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s05_fre','primary')]['movement_probe']=='PASS'
assert validations[('s05_fre','primary')]['blocked_probe']=='PASS'
assert validations[('s05_fre','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S05_FRE_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s05_fre.rsground | cut -d' ' -f1)" = 9ea10fe451726313d93c95fc1336b07558cb0123e59561434c1cee2c487af86d
test "$(sha256sum Content/Tile/s05_fre_Base.tile | cut -d' ' -f1)" = 7711a4b289d5923f6f5f4cd1f7d56aac79e0b0d35db2ece82bd5ca150f3d977f
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s05_fre.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s05_fre_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s05_fre --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
