#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b14p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b14p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b14p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b14p01a.rsground" | cut -d' ' -f1)" = 18d960e4e3386f50c7de86cab2466ddf95374c24b0185e1d94ceebae4744b459
test "$(sha256sum "$CANONICAL/tiles/b14p01a_Base.tile" | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
test "$(sha256sum RESERVE/red_grounds/b14p01a.rsground | cut -d' ' -f1)" = 5412245709f1517db85e4a1ab62b575c277ec4399226e9909f6b4c8cff812fa3
test "$(sha256sum RESERVE/red_tiles/b14p01a_Base.tile | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p01a')
assert entry == {'canonical_debug_id': 88, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00088', 'debug_string_address': '0x0827AFDC', 'ground_place_id': 10, 'map_file_id': 89, 'map_id': 88, 'stable_ground_id': 'b14p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_BULBASAUR_CONSTRUCTION' in header
print('B14P01A_IDENTITY_PASS map_file_id=89 symbol=MAP_TEAM_BASE_BULBASAUR_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b14p01a --expected-screenshots 25 \
...
print('B14P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b14p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b14p01a','primary')]['movement_probe']=='PASS'
assert validations[('b14p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b14p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B14P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b14p01a.rsground | cut -d' ' -f1)" = 18d960e4e3386f50c7de86cab2466ddf95374c24b0185e1d94ceebae4744b459
test "$(sha256sum Content/Tile/b14p01a_Base.tile | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b14p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b14p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b14p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
