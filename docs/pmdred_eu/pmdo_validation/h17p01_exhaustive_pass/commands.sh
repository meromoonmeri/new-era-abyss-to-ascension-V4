...
test "$(sha256sum RESERVE/red_grounds/h17p01.rsground | cut -d' ' -f1)" = a25320dab6218095f4c00e1de782b1cf9a58add99b743155cd6f1916d2b6d671
test "$(sha256sum RESERVE/red_tiles/h17p01_Base.tile | cut -d' ' -f1)" = 1868e04d61e382a54db847ba8091eae8cc9cb50031b836b0d7689cb6960fef9d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h17p01')
assert entry == {'canonical_debug_id': 150, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00150', 'debug_string_address': '0x0827A8BC', 'ground_place_id': 9, 'map_file_id': 151, 'map_id': 145, 'stable_ground_id': 'h17p01', 'weather_id': 6}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_POWER_PLANT' in header
print('H17P01_IDENTITY_PASS map_file_id=151 symbol=MAP_FRIEND_AREA_POWER_PLANT')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h17p01 --expected-screenshots 109 \
...
print('H17P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 352
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 109
...
print('H17P01_EXHAUSTIVE_PASS samples=109 unique_rgba=7')
PY
...
test "$(sha256sum Data/Ground/h17p01.rsground | cut -d' ' -f1)" = c35afe8b5b220dcfc5057ccbbadbba17ba8df0fd55f8b758fd93414fe9cd8599
test "$(sha256sum Content/Tile/h17p01_Base.tile | cut -d' ' -f1)" = 16c4a5f7b6971530817ec4735b9d575afe4b798d138975caafd9c5fc84751edf
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h17p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h17p01_Base.tile "$POST_CANDIDATE/tiles/"
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/h17p01/h17p01_Base.tile | cut -d' ' -f1)" = 1868e04d61e382a54db847ba8091eae8cc9cb50031b836b0d7689cb6960fef9d
