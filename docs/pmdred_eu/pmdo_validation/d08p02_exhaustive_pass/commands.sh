#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d08p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d08p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d08p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d08p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d08p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d08p02.rsground" | cut -d' ' -f1)" = dac79313e5c4ec08f8e63e947cd90e24d828a073202639e510aecd71bb961b01
test "$(sha256sum "$CANONICAL/tiles/d08p02_Base.tile" | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
test "$(sha256sum RESERVE/red_grounds/d08p02.rsground | cut -d' ' -f1)" = 64858f1b45c7ceb7a97c0eb0bd642c88e6fe266f7230951201dbdf3650a6b807
test "$(sha256sum RESERVE/red_tiles/d08p02_Base.tile | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d08p02')
assert entry == {'canonical_debug_id': 203, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00203', 'debug_string_address': '0x0827A27C', 'ground_place_id': 23, 'map_file_id': 208, 'map_id': 199, 'stable_ground_id': 'd08p02', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FROSTY_FOREST_MID' in header
print('D08P02_IDENTITY_PASS map_file_id=208 symbol=MAP_FROSTY_FOREST_MID')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d08p02 --expected-screenshots 2 \
...
print('D08P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d08p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d08p02','primary')]['movement_probe']=='PASS'
assert validations[('d08p02','primary')]['blocked_probe']=='PASS'
assert validations[('d08p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D08P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d08p02.rsground | cut -d' ' -f1)" = dac79313e5c4ec08f8e63e947cd90e24d828a073202639e510aecd71bb961b01
test "$(sha256sum Content/Tile/d08p02_Base.tile | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d08p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d08p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d08p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
