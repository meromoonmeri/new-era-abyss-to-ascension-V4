#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s02.rsground" | cut -d' ' -f1)" = ead4241fea20e55a895a6aa7293ca2d06dae256d3bf67f3cc063cad5036bb8e8
test "$(sha256sum "$CANONICAL/tiles/s02_Base.tile" | cut -d' ' -f1)" = 265794b0515744a44988996915a2b29968cdeed640a74f801a4295f902f6cb55
test "$(sha256sum RESERVE/red_grounds/s02.rsground | cut -d' ' -f1)" = e8f08b7bf560c7085cbd888448cc8f1640ac20676f646d24ce325e7c00eae517
test "$(sha256sum RESERVE/red_tiles/s02_Base.tile | cut -d' ' -f1)" = 0a37f2a759fa2f661ac1677810e052004a856fd7558c24e5e9e3ddc101cb5b91
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s02')
assert entry == {'canonical_debug_id': 233, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00233', 'debug_string_address': '0x08279EBC', 'ground_place_id': 37, 'map_file_id': 238, 'map_id': 229, 'stable_ground_id': 's02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TITLE_SCREEN' in header
table=Path('.runtime-cache/pmd-red-reference/src/ground_map_conversion_table.c').read_text()
designated=table.split('[MAP_TITLE_SCREEN] = {',1)[1].split('},',1)[0]
assert '__ground_amd_conversion_00233' in designated
print('S02_IDENTITY_PASS map_file_id=238 canonical_debug_id=233 symbol=MAP_TITLE_SCREEN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s02 --expected-screenshots 2 \
...
print('S02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s02','primary')]['movement_probe']=='PASS'
assert validations[('s02','primary')]['blocked_probe']=='PASS'
assert validations[('s02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s02.rsground | cut -d' ' -f1)" = ead4241fea20e55a895a6aa7293ca2d06dae256d3bf67f3cc063cad5036bb8e8
test "$(sha256sum Content/Tile/s02_Base.tile | cut -d' ' -f1)" = 265794b0515744a44988996915a2b29968cdeed640a74f801a4295f902f6cb55
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
