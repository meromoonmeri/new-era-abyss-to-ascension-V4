#!/usr/bin/env bash
# Exact recovery/validation recipe for the t01p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-t01p02b-full-fixture"
...
COMPARE="$ROOT/.runtime-cache/pmdred-eu-t01p02b-full-comparison"
...
test "$(sha256sum "$CANONICAL/grounds/t01p02b.rsground" | cut -d' ' -f1)" = 48690134d1035735fe8b7dc4341deb41382cfeefdda9fed81ccdc67f036f352f
test "$(sha256sum "$CANONICAL/tiles/t01p02b_Base.tile" | cut -d' ' -f1)" = b3d6c11fd882d957b18dac7f34fe9ae79b25b9f8395a03827015aee269f33243
test "$(sha256sum RESERVE/red_grounds/t01p02b.rsground | cut -d' ' -f1)" = 896f4bc4d1a5c821275a9cf611494e263002185ecdd8808810227fa79822383f
test "$(sha256sum RESERVE/red_tiles/t01p02b_Base.tile | cut -d' ' -f1)" = ed0ff744feb5ddba0f7d38e54b6a63352f84825e5cd96caaaa1c649ca49fdc11
...
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='t01p02b')
assert entry['canonical_debug_id']==3 and entry['map_id']==4 and entry['map_file_id']==4 and entry['ground_place_id']==3
...
assert symbols[4]=='MAP_PELIPPER_POST_OFFICE'
print('T01P02B_IDENTITY_PASS map_file_id=4 symbol=MAP_PELIPPER_POST_OFFICE')
PYID
...
import json
from pathlib import Path
r=json.loads(Path('.runtime-cache/pmdred-eu-t01p02b-full-comparison/report.json').read_text())
rt=r['runtime']
primary=[s for s in r['samples'] if s['phase'] == 'primary']
validations={(v['ground'],v['phase']):v for v in rt['validations']}
assert r['grounds'] == ['t01p02b']
...
assert validations[('t01p02b','primary')]['movement_probe'] == 'PASS'
assert validations[('t01p02b','primary')]['blocked_probe'] == 'PASS'
assert validations[('t01p02b','reload')]['load'] == 'LOAD_PASS'
...
print('T01P02B_EXHAUSTIVE_PASS samples=817 unique_rgba=336')
PY
...
test "$(sha256sum Data/Ground/t01p02b.rsground | cut -d' ' -f1)" = 48690134d1035735fe8b7dc4341deb41382cfeefdda9fed81ccdc67f036f352f
test "$(sha256sum Content/Tile/t01p02b_Base.tile | cut -d' ' -f1)" = b3d6c11fd882d957b18dac7f34fe9ae79b25b9f8395a03827015aee269f33243
"$PYTHON" - <<'PY'
...
old=json.loads(subprocess.check_output(['git','show','9dce133e1f75e982d5f08b773f18bec84da4327d:Data/Zone/master_zone.json']).decode('utf-8-sig'))['Object']['GroundMaps']
i=now.index('t01p02b')
assert len(now)==195 and now.count('t01p02b')==1 and now[i-1]=='t01p02a'
assert now[:i]+now[i+1:]==old
assert now.count('post_office')==1
assert Path('Data/Ground/post_office.rsground').read_bytes() == subprocess.check_output(['git','show','9dce133e1f75e982d5f08b773f18bec84da4327d:Data/Ground/post_office.rsground'])
print('T01P02B_ZONE_INTEGRATION_PASS canonical_index=%d project_post_office_index=%d'%(i,now.index('post_office')))
PY
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-t01p02b-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-t01p02b-promoted-integration-fixture"
rm -rf "$POST_CANDIDATE" "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/t01p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/t01p02b_Base.tile "$POST_CANDIDATE/tiles/"
