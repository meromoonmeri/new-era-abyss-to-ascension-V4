#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b10p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b10p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b10p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b10p01a.rsground" | cut -d' ' -f1)" = b96046c5a6c5b50e21863919d3822a1e262f76ea5ff230f211ab7bbb1a861325
test "$(sha256sum "$CANONICAL/tiles/b10p01a_Base.tile" | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
test "$(sha256sum RESERVE/red_grounds/b10p01a.rsground | cut -d' ' -f1)" = 9f0a452c30f48e423bde9f601ed3920040d7acfb09afe257070bda9cbed428f5
test "$(sha256sum RESERVE/red_tiles/b10p01a_Base.tile | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p01a')
assert entry == {'canonical_debug_id': 64, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00064', 'debug_string_address': '0x0827B2DC', 'ground_place_id': 10, 'map_file_id': 65, 'map_id': 64, 'stable_ground_id': 'b10p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TORCHIC_CONSTRUCTION' in header
print('B10P01A_IDENTITY_PASS map_file_id=65 symbol=MAP_TEAM_BASE_TORCHIC_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b10p01a --expected-screenshots 25 \
...
print('B10P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b10p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b10p01a','primary')]['movement_probe']=='PASS'
assert validations[('b10p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b10p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B10P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b10p01a.rsground | cut -d' ' -f1)" = b96046c5a6c5b50e21863919d3822a1e262f76ea5ff230f211ab7bbb1a861325
test "$(sha256sum Content/Tile/b10p01a_Base.tile | cut -d' ' -f1)" = ca79d4f527dd51eecdfdf2799ce94edb5fa8d09e2e684a7ac1acec2cab513f07
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b10p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b10p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b10p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
