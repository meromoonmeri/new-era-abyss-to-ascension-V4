#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s01.rsground" | cut -d' ' -f1)" = 1623ce2d7602906e7fd94936d3ec03f352bd44f1416583bbb5f10f48cd85cb96
test "$(sha256sum "$CANONICAL/tiles/s01_Base.tile" | cut -d' ' -f1)" = 1cacb50450710fcd5f6c4156ce0c5a260bd1edec88b85089d411f3816b0c4dbe
test "$(sha256sum RESERVE/red_grounds/s01.rsground | cut -d' ' -f1)" = bbf481a30f19d1ad832048604739672b906919d8b60401c383cf30abbe4ba80f
test "$(sha256sum RESERVE/red_tiles/s01_Base.tile | cut -d' ' -f1)" = 1cacb50450710fcd5f6c4156ce0c5a260bd1edec88b85089d411f3816b0c4dbe
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s01')
assert entry == {'canonical_debug_id': 232, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00232', 'debug_string_address': '0x08279EDC', 'ground_place_id': 37, 'map_file_id': 237, 'map_id': 228, 'stable_ground_id': 's01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_WALL_MAP' in header
print('S01_IDENTITY_PASS map_file_id=237 symbol=MAP_TEAM_BASE_WALL_MAP')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s01 --expected-screenshots 253 \
...
print('S01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 781
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 253
...
assert r['grounds']==['s01'] and r['sample_count']==253 and len(primary)==252
assert r['exact_sample_count']==253 and r['fully_opaque_sample_count']==253
assert len({s['actual_rgba_sha256'] for s in primary})==63
...
assert validations[('s01','primary')]['movement_probe']=='PASS'
assert validations[('s01','primary')]['blocked_probe']=='PASS'
assert validations[('s01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S01_EXHAUSTIVE_PASS samples=253 unique_rgba=63')
PY
...
test "$(sha256sum Data/Ground/s01.rsground | cut -d' ' -f1)" = 1623ce2d7602906e7fd94936d3ec03f352bd44f1416583bbb5f10f48cd85cb96
test "$(sha256sum Content/Tile/s01_Base.tile | cut -d' ' -f1)" = 1cacb50450710fcd5f6c4156ce0c5a260bd1edec88b85089d411f3816b0c4dbe
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 253 comparative PNGs,
