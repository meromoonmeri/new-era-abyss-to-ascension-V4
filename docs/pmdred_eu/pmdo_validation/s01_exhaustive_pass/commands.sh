...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s01')
assert e == {'canonical_debug_id': 232, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00232', 'debug_string_address': '0x08279EDC', 'ground_place_id': 37, 'map_file_id': 237, 'map_id': 228, 'stable_ground_id': 's01', 'weather_id': -1}
assert 'MAP_TEAM_BASE_WALL_MAP' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('S01_IDENTITY_PASS map_id=228 map_file_id=237 symbol=MAP_TEAM_BASE_WALL_MAP')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 778
...
print('S01_EXHAUSTIVE_PASS samples=253 unique_rgba=63 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
