#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b02p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b02p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b02p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b02p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b02p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b02p01b.rsground" | cut -d' ' -f1)" = 56ff8d17e40fda1739530a95a453a3aeb21aba4f70ef6d05f4854fdecb9f2cd7
test "$(sha256sum "$CANONICAL/tiles/b02p01b_Base.tile" | cut -d' ' -f1)" = 7355317e5fb617763b1f28ef578cb175ba7b9b95bdf49ce63708e3b2f7c26530
test "$(sha256sum RESERVE/red_grounds/b02p01b.rsground | cut -d' ' -f1)" = 394d79f34941b555f830cd85c7794834bb200169359fcd1fa4a830b333db9e86
test "$(sha256sum RESERVE/red_tiles/b02p01b_Base.tile | cut -d' ' -f1)" = 7355317e5fb617763b1f28ef578cb175ba7b9b95bdf49ce63708e3b2f7c26530
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b02p01b')
assert entry == {'canonical_debug_id': 17, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00017', 'debug_string_address': '0x0827B8BC', 'ground_place_id': 10, 'map_file_id': 18, 'map_id': 17, 'stable_ground_id': 'b02p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MEOWTH_FINAL' in header
print('B02P01B_IDENTITY_PASS map_file_id=18 symbol=MAP_TEAM_BASE_MEOWTH_FINAL')
PYID
...
assert r['grounds']==['b02p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b02p01b','primary')]['movement_probe']=='PASS'
assert validations[('b02p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b02p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B02P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b02p01b.rsground | cut -d' ' -f1)" = 56ff8d17e40fda1739530a95a453a3aeb21aba4f70ef6d05f4854fdecb9f2cd7
test "$(sha256sum Content/Tile/b02p01b_Base.tile | cut -d' ' -f1)" = 7355317e5fb617763b1f28ef578cb175ba7b9b95bdf49ce63708e3b2f7c26530
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b02p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b02p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b02p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
