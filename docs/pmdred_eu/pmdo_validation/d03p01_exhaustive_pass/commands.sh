#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d03p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d03p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d03p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d03p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d03p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d03p01.rsground" | cut -d' ' -f1)" = 38ae67d87f8e8d7eba531218094c6e5ccc0366fe1839fb148921788056d0c70e
test "$(sha256sum "$CANONICAL/tiles/d03p01_Base.tile" | cut -d' ' -f1)" = cb845052a3dd97ef5f4a31ac37599f4c90979e177d0996e8000aeda10b850a7e
test "$(sha256sum RESERVE/red_grounds/d03p01.rsground | cut -d' ' -f1)" = df9d6b0ce24119bd3eef9220ddd146b1ae89fd62a060c3c07f7734d7e0fc0b0b
test "$(sha256sum RESERVE/red_tiles/d03p01_Base.tile | cut -d' ' -f1)" = cb845052a3dd97ef5f4a31ac37599f4c90979e177d0996e8000aeda10b850a7e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d03p01')
assert entry == {'canonical_debug_id': 191, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00191', 'debug_string_address': '0x0827A3FC', 'ground_place_id': 15, 'map_file_id': 196, 'map_id': 187, 'stable_ground_id': 'd03p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_SILENT_CHASM_END' in header
print('D03P01_IDENTITY_PASS map_file_id=196 symbol=MAP_SILENT_CHASM_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d03p01 --expected-screenshots 2 \
...
print('D03P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d03p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d03p01','primary')]['movement_probe']=='PASS'
assert validations[('d03p01','primary')]['blocked_probe']=='PASS'
assert validations[('d03p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D03P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d03p01.rsground | cut -d' ' -f1)" = 38ae67d87f8e8d7eba531218094c6e5ccc0366fe1839fb148921788056d0c70e
test "$(sha256sum Content/Tile/d03p01_Base.tile | cut -d' ' -f1)" = cb845052a3dd97ef5f4a31ac37599f4c90979e177d0996e8000aeda10b850a7e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d03p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d03p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d03p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
