#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b02p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b02p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b02p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b02p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b02p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b02p02b.rsground" | cut -d' ' -f1)" = 28d28128dcc98ee7b3930e21020a74922e5be4fb9c1bd11fbe2553c8fe32ba25
test "$(sha256sum "$CANONICAL/tiles/b02p02b_Base.tile" | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
test "$(sha256sum RESERVE/red_grounds/b02p02b.rsground | cut -d' ' -f1)" = e85bd035fea7759571688049c28a4ded6b8ea685117eeef76f4cb6ea6375e95a
test "$(sha256sum RESERVE/red_tiles/b02p02b_Base.tile | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b02p02b')
assert entry == {'canonical_debug_id': 20, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00020', 'debug_string_address': '0x0827B85C', 'ground_place_id': 11, 'map_file_id': 21, 'map_id': 20, 'stable_ground_id': 'b02p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MEOWTH_FINAL' in header
print('B02P02B_IDENTITY_PASS map_file_id=21 symbol=MAP_TEAM_BASE_INSIDE_MEOWTH_FINAL')
PYID
...
assert r['grounds']==['b02p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b02p02b','primary')]['movement_probe']=='PASS'
assert validations[('b02p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b02p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B02P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b02p02b.rsground | cut -d' ' -f1)" = 28d28128dcc98ee7b3930e21020a74922e5be4fb9c1bd11fbe2553c8fe32ba25
test "$(sha256sum Content/Tile/b02p02b_Base.tile | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b02p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b02p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b02p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
