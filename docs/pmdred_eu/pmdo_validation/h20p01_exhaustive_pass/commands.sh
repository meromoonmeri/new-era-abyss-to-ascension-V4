#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h20p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h20p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h20p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h20p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h20p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h20p01.rsground" | cut -d' ' -f1)" = ec944dd50b1d35061d21759f007b4b93f28854759c9bbb301e8880bb492d311a
test "$(sha256sum "$CANONICAL/tiles/h20p01_Base.tile" | cut -d' ' -f1)" = e4c127a92007cd99c764b0e5714d083a0918b06d810bc417257ce61819b1fbde
test "$(sha256sum RESERVE/red_grounds/h20p01.rsground | cut -d' ' -f1)" = ab4f945e1e7571e732fd0b20b09065991a5b0ddfe96e266d1a9d02a96ea81b1a
test "$(sha256sum RESERVE/red_tiles/h20p01_Base.tile | cut -d' ' -f1)" = 79122019b2f2e86d7f10dedaaae603a33fa7996deab28f3fe9ccfdd377f89709
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h20p01')
assert entry == {'canonical_debug_id': 155, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00155', 'debug_string_address': '0x0827A85C', 'ground_place_id': 9, 'map_file_id': 156, 'map_id': 148, 'stable_ground_id': 'h20p01', 'weather_id': 14}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_AGED_CHAMBER_AN' in header
print('H20P01_IDENTITY_PASS map_file_id=156 symbol=MAP_FRIEND_AREA_AGED_CHAMBER_AN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h20p01 --expected-screenshots 49 \
...
print('H20P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 172
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 49
...
assert r['grounds']==['h20p01'] and r['sample_count']==49 and len(primary)==48
assert r['exact_sample_count']==49 and r['fully_opaque_sample_count']==49
assert len({s['actual_rgba_sha256'] for s in primary})==24
...
assert validations[('h20p01','primary')]['movement_probe']=='PASS'
assert validations[('h20p01','primary')]['blocked_probe']=='PASS'
assert validations[('h20p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H20P01_EXHAUSTIVE_PASS samples=49 unique_rgba=24')
PY
...
test "$(sha256sum Data/Ground/h20p01.rsground | cut -d' ' -f1)" = ec944dd50b1d35061d21759f007b4b93f28854759c9bbb301e8880bb492d311a
test "$(sha256sum Content/Tile/h20p01_Base.tile | cut -d' ' -f1)" = e4c127a92007cd99c764b0e5714d083a0918b06d810bc417257ce61819b1fbde
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h20p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h20p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h20p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 49 comparative PNGs,
