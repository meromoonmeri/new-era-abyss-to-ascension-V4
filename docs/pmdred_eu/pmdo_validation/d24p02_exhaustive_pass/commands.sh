#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d24p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d24p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d24p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d24p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d24p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d24p02.rsground" | cut -d' ' -f1)" = a09fa1f774c0c3c22c10e012c7ea0ff69b4ca025158b4b9d324eb967e44d8dea
test "$(sha256sum "$CANONICAL/tiles/d24p02_Base.tile" | cut -d' ' -f1)" = 23b276d9858db83bc1da4e17181233de867a8ce89e9436425293d8a92b727149
test "$(sha256sum RESERVE/red_grounds/d24p02.rsground | cut -d' ' -f1)" = 53e227e06ed877a10a717fe2b65869e5ffecde3a90b01e121fe1b40da2bcc8a1
test "$(sha256sum RESERVE/red_tiles/d24p02_Base.tile | cut -d' ' -f1)" = 23b276d9858db83bc1da4e17181233de867a8ce89e9436425293d8a92b727149
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d24p02')
assert entry == {'canonical_debug_id': 230, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00230', 'debug_string_address': '0x08279F1C', 'ground_place_id': 12, 'map_file_id': 235, 'map_id': 226, 'stable_ground_id': 'd24p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_LOGO_POKEMON_COMPANY' in header
print('D24P02_IDENTITY_PASS map_file_id=235 symbol=MAP_LOGO_POKEMON_COMPANY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d24p02 --expected-screenshots 2 \
...
print('D24P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d24p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d24p02','primary')]['movement_probe']=='PASS'
assert validations[('d24p02','primary')]['blocked_probe']=='PASS'
assert validations[('d24p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D24P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d24p02.rsground | cut -d' ' -f1)" = a09fa1f774c0c3c22c10e012c7ea0ff69b4ca025158b4b9d324eb967e44d8dea
test "$(sha256sum Content/Tile/d24p02_Base.tile | cut -d' ' -f1)" = 23b276d9858db83bc1da4e17181233de867a8ce89e9436425293d8a92b727149
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d24p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d24p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d24p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
