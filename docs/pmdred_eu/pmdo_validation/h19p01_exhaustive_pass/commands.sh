#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h19p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h19p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h19p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h19p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h19p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h19p01.rsground" | cut -d' ' -f1)" = e170a807f91633478d81e6f47c2c8bf02a1145ba41c247c8f98b2785034de6ff
test "$(sha256sum "$CANONICAL/tiles/h19p01_Base.tile" | cut -d' ' -f1)" = 65a60b82ecdf4ce3fb1f03b1a7a7d49aced8499bec02bac147c89b37991ffcd5
test "$(sha256sum RESERVE/red_grounds/h19p01.rsground | cut -d' ' -f1)" = 9359bf1daba2208e89c17f1571f228d760cbccb8e72b3910f981161bc55b729a
test "$(sha256sum RESERVE/red_tiles/h19p01_Base.tile | cut -d' ' -f1)" = 138077933a6b816392246e5395b53690db60c4d8d056d0616e317793aa024026
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h19p01')
assert entry == {'canonical_debug_id': 153, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00153', 'debug_string_address': '0x0827A87C', 'ground_place_id': 9, 'map_file_id': 154, 'map_id': 147, 'stable_ground_id': 'h19p01', 'weather_id': 7}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_FURNACE_DESERT' in header
print('H19P01_IDENTITY_PASS map_file_id=154 symbol=MAP_FRIEND_AREA_FURNACE_DESERT')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h19p01 --expected-screenshots 1858 \
...
print('H19P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 5599
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 1858
...
assert r['grounds']==['h19p01'] and r['sample_count']==1858 and len(primary)==1857
assert r['exact_sample_count']==1858 and r['fully_opaque_sample_count']==1858
assert len({s['actual_rgba_sha256'] for s in primary})==1237
...
assert validations[('h19p01','primary')]['movement_probe']=='PASS'
assert validations[('h19p01','primary')]['blocked_probe']=='PASS'
assert validations[('h19p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H19P01_EXHAUSTIVE_PASS samples=1858 unique_rgba=1237')
PY
...
test "$(sha256sum Data/Ground/h19p01.rsground | cut -d' ' -f1)" = e170a807f91633478d81e6f47c2c8bf02a1145ba41c247c8f98b2785034de6ff
test "$(sha256sum Content/Tile/h19p01_Base.tile | cut -d' ' -f1)" = 65a60b82ecdf4ce3fb1f03b1a7a7d49aced8499bec02bac147c89b37991ffcd5
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h19p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h19p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h19p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 1858 comparative PNGs,
