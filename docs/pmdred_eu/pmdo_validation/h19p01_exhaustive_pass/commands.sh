...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h19p01')
assert e == {'canonical_debug_id': 153, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00153', 'debug_string_address': '0x0827A87C', 'ground_place_id': 9, 'map_file_id': 154, 'map_id': 147, 'stable_ground_id': 'h19p01', 'weather_id': 7}
assert 'MAP_FRIEND_AREA_FURNACE_DESERT' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H19P01_IDENTITY_PASS map_id=147 map_file_id=154 symbol=MAP_FRIEND_AREA_FURNACE_DESERT')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 5596
...
print('H19P01_EXHAUSTIVE_PASS samples=1858 unique_rgba=1237 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
