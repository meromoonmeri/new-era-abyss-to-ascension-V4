#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h12p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h12p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h12p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h12p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h12p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h12p01.rsground" | cut -d' ' -f1)" = 3539d39afc657484ee984f01f71fbce4631fa5f46dfdae2b2020c41d24622060
test "$(sha256sum "$CANONICAL/tiles/h12p01_Base.tile" | cut -d' ' -f1)" = 020cc81db7fc86ad160c94d767e8ea96598f51c4e69211e6a40193edc15fb630
test "$(sha256sum RESERVE/red_grounds/h12p01.rsground | cut -d' ' -f1)" = 934b7d5a4b7fc1bbea1945c071dd8c8d899be4406916f7716c2e34414b5f445b
test "$(sha256sum RESERVE/red_tiles/h12p01_Base.tile | cut -d' ' -f1)" = 020cc81db7fc86ad160c94d767e8ea96598f51c4e69211e6a40193edc15fb630
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h12p01')
assert entry == {'canonical_debug_id': 142, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00142', 'debug_string_address': '0x0827A9BC', 'ground_place_id': 9, 'map_file_id': 143, 'map_id': 137, 'stable_ground_id': 'h12p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_ECHO_CAVE' in header
print('H12P01_IDENTITY_PASS map_file_id=143 symbol=MAP_FRIEND_AREA_ECHO_CAVE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h12p01 --expected-screenshots 1057 \
...
print('H12P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 3196
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 1057
...
assert r['grounds']==['h12p01'] and r['sample_count']==1057 and len(primary)==1056
assert r['exact_sample_count']==1057 and r['fully_opaque_sample_count']==1057
assert len({s['actual_rgba_sha256'] for s in primary})==54
...
assert validations[('h12p01','primary')]['movement_probe']=='PASS'
assert validations[('h12p01','primary')]['blocked_probe']=='PASS'
assert validations[('h12p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H12P01_EXHAUSTIVE_PASS samples=1057 unique_rgba=54')
PY
...
test "$(sha256sum Data/Ground/h12p01.rsground | cut -d' ' -f1)" = 3539d39afc657484ee984f01f71fbce4631fa5f46dfdae2b2020c41d24622060
test "$(sha256sum Content/Tile/h12p01_Base.tile | cut -d' ' -f1)" = 020cc81db7fc86ad160c94d767e8ea96598f51c4e69211e6a40193edc15fb630
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h12p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h12p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h12p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 1057 comparative PNGs,
