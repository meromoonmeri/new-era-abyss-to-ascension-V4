#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b01p00a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b01p00a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b01p00a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b01p00a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b01p00a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b01p00a.rsground" | cut -d' ' -f1)" = 5b52c0152eac8352bc364e362113d1ff9f73f710d558a64a06ba93d11aa4b449
test "$(sha256sum "$CANONICAL/tiles/b01p00a_Base.tile" | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test ! -e RESERVE/red_grounds/b01p00a.rsground
test ! -e RESERVE/red_tiles/b01p00a_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b01p00a')
assert entry == {
 'canonical_debug_id':10, 'conversion_type':1,
 'debug_string':'__ground_amd_conversion_00010',
 'debug_string_address':'0x0827B99C', 'ground_place_id':10,
 'map_file_id':10, 'map_id':9, 'stable_ground_id':'b01p00a', 'weather_id':9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_PIKACHU_BASIC' in header
print('B01P00A_IDENTITY_PASS map_file_id=10 symbol=MAP_TEAM_BASE_PIKACHU_BASIC')
PYID
...
assert r['grounds']==['b01p00a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b01p00a','primary')]['movement_probe']=='PASS'
assert validations[('b01p00a','primary')]['blocked_probe']=='PASS'
assert validations[('b01p00a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B01P00A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b01p00a.rsground | cut -d' ' -f1)" = 5b52c0152eac8352bc364e362113d1ff9f73f710d558a64a06ba93d11aa4b449
test "$(sha256sum Content/Tile/b01p00a_Base.tile | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b01p00a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b01p00a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b01p00a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
