#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h23p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h23p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h23p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h23p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h23p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h23p01.rsground" | cut -d' ' -f1)" = e612364a18cdbf0c3b75ddb6fd6b0d6f1223a21a0aae02e06188ce47ded274fc
test "$(sha256sum "$CANONICAL/tiles/h23p01_Base.tile" | cut -d' ' -f1)" = 7f8fe39ceb8f48658e7d195f2552d898837592ac70377e980bc3a90630ac17ae
test "$(sha256sum RESERVE/red_grounds/h23p01.rsground | cut -d' ' -f1)" = ef736974be0835edd6986d0f561de8eca1ddc8a1628ee58cb81a088a76d5f790
test "$(sha256sum RESERVE/red_tiles/h23p01_Base.tile | cut -d' ' -f1)" = 0d85854f4e962f5d4683721605848d7b9d6ea74c406596638d2250239c687626
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h23p01')
assert entry == {'canonical_debug_id': 159, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00159', 'debug_string_address': '0x0827A7DC', 'ground_place_id': 9, 'map_file_id': 160, 'map_id': 152, 'stable_ground_id': 'h23p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_FRIGID_CAVERN' in header
print('H23P01_IDENTITY_PASS map_file_id=160 symbol=MAP_FRIEND_AREA_FRIGID_CAVERN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h23p01 --expected-screenshots 117 \
...
print('H23P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 376
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 117
...
assert r['grounds']==['h23p01'] and r['sample_count']==117 and len(primary)==116
assert r['exact_sample_count']==117 and r['fully_opaque_sample_count']==117
assert len({s['actual_rgba_sha256'] for s in primary})==29
...
assert validations[('h23p01','primary')]['movement_probe']=='PASS'
assert validations[('h23p01','primary')]['blocked_probe']=='PASS'
assert validations[('h23p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H23P01_EXHAUSTIVE_PASS samples=117 unique_rgba=29')
PY
...
test "$(sha256sum Data/Ground/h23p01.rsground | cut -d' ' -f1)" = e612364a18cdbf0c3b75ddb6fd6b0d6f1223a21a0aae02e06188ce47ded274fc
test "$(sha256sum Content/Tile/h23p01_Base.tile | cut -d' ' -f1)" = 7f8fe39ceb8f48658e7d195f2552d898837592ac70377e980bc3a90630ac17ae
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h23p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h23p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h23p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 117 comparative PNGs,
