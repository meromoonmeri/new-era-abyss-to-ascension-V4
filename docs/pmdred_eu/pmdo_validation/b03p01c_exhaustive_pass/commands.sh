#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b03p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b03p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b03p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b03p01c.rsground" | cut -d' ' -f1)" = b478905d1321cdc3f01c49c04dc99cb6f79f26c3518a874ea6be6103395a9426
test "$(sha256sum "$CANONICAL/tiles/b03p01c_Base.tile" | cut -d' ' -f1)" = 1441dd9d579b9778c2d98e8e43e20272537399152d66db1fc0ad38bdb90f0e56
test "$(sha256sum RESERVE/red_grounds/b03p01c.rsground | cut -d' ' -f1)" = 94a37ad4290a247de161a8ccaa0e27aa2a14c9d69ff4a645dcc74b858a2ce2fb
test "$(sha256sum RESERVE/red_tiles/b03p01c_Base.tile | cut -d' ' -f1)" = 9863d6597da8f9c8db0a025d4b9cdcbdfcff949446c2e2dba79b231a0a886d8b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b03p01c')
assert entry == {'canonical_debug_id': 24, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00024', 'debug_string_address': '0x0827B7DC', 'ground_place_id': 10, 'map_file_id': 25, 'map_id': 24, 'stable_ground_id': 'b03p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_EEVEE_BASIC' in header
print('B03P01C_IDENTITY_PASS map_file_id=25 symbol=MAP_TEAM_BASE_INSIDE_EEVEE_BASIC')
PYID
...
assert r['grounds']==['b03p01c'] and r['sample_count']==67 and len(primary)==66
...
assert validations[('b03p01c','primary')]['movement_probe']=='PASS'
assert validations[('b03p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b03p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B03P01C_EXHAUSTIVE_PASS samples=67 unique_rgba=37')
PY
...
test "$(sha256sum Data/Ground/b03p01c.rsground | cut -d' ' -f1)" = b478905d1321cdc3f01c49c04dc99cb6f79f26c3518a874ea6be6103395a9426
test "$(sha256sum Content/Tile/b03p01c_Base.tile | cut -d' ' -f1)" = 1441dd9d579b9778c2d98e8e43e20272537399152d66db1fc0ad38bdb90f0e56
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b03p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b03p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b03p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
