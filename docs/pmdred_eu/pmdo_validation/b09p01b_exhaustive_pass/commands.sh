#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b09p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b09p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b09p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b09p01b.rsground" | cut -d' ' -f1)" = 575e7a7245f44197e2a8e2d82c32c93efcc70499c2eb93385993067ca9c95a64
test "$(sha256sum "$CANONICAL/tiles/b09p01b_Base.tile" | cut -d' ' -f1)" = ad3712dd43e2f934c89e818b77a278d6aea2cac7bcecdaf5f7a8be338591fc7b
test "$(sha256sum RESERVE/red_grounds/b09p01b.rsground | cut -d' ' -f1)" = 3959e00f18b18814c83525b27439c25db021bdf4561783a9cf5c69d90b36b510
test "$(sha256sum RESERVE/red_tiles/b09p01b_Base.tile | cut -d' ' -f1)" = ad3712dd43e2f934c89e818b77a278d6aea2cac7bcecdaf5f7a8be338591fc7b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b09p01b')
assert entry == {'canonical_debug_id': 59, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00059', 'debug_string_address': '0x0827B37C', 'ground_place_id': 10, 'map_file_id': 60, 'map_id': 59, 'stable_ground_id': 'b09p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CHARMANDER_FINAL' in header
print('B09P01B_IDENTITY_PASS map_file_id=60 symbol=MAP_TEAM_BASE_CHARMANDER_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b09p01b --expected-screenshots 25 \
...
print('B09P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b09p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b09p01b','primary')]['movement_probe']=='PASS'
assert validations[('b09p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b09p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B09P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b09p01b.rsground | cut -d' ' -f1)" = 575e7a7245f44197e2a8e2d82c32c93efcc70499c2eb93385993067ca9c95a64
test "$(sha256sum Content/Tile/b09p01b_Base.tile | cut -d' ' -f1)" = ad3712dd43e2f934c89e818b77a278d6aea2cac7bcecdaf5f7a8be338591fc7b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b09p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b09p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b09p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
