#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b07p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b07p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b07p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b07p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b07p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b07p01c.rsground" | cut -d' ' -f1)" = d75cfce108abf34deaf3aaaf14fdf327d0804efdc03e881d80af98b8c32fb714
test "$(sha256sum "$CANONICAL/tiles/b07p01c_Base.tile" | cut -d' ' -f1)" = 9a05ae5f91de685f4bcd3d0b3ccfc1720ca04074d7f0e038cb8308f46043c4eb
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b07p01c')
assert entry == {'canonical_debug_id': 48, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00048', 'debug_string_address': '0x0827B4DC', 'ground_place_id': 10, 'map_file_id': 49, 'map_id': 48, 'stable_ground_id': 'b07p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MUDKIP_BASIC' in header
print('B07P01C_IDENTITY_PASS map_id=48 map_file_id=49 symbol=MAP_TEAM_BASE_INSIDE_MUDKIP_BASIC')
PYID
...
assert r['grounds']==['b07p01c'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b07p01c','primary')]['movement_probe']=='PASS'
assert validations[('b07p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b07p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B07P01C_EXHAUSTIVE_PASS samples=51 unique_rgba=32 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b07p01c.rsground | cut -d' ' -f1)" = d75cfce108abf34deaf3aaaf14fdf327d0804efdc03e881d80af98b8c32fb714
test "$(sha256sum Content/Tile/b07p01c_Base.tile | cut -d' ' -f1)" = 9a05ae5f91de685f4bcd3d0b3ccfc1720ca04074d7f0e038cb8308f46043c4eb
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b07p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b07p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b07p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
