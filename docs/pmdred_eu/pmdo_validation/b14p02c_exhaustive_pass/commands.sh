#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b14p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b14p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b14p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b14p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b14p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b14p02c.rsground" | cut -d' ' -f1)" = 56bacd4428aeee0c59cf6d5ac5c52cbfeb5ef9436d95a71fe75604be2c1a197c
test "$(sha256sum "$CANONICAL/tiles/b14p02c_Base.tile" | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test "$(sha256sum RESERVE/red_grounds/b14p02c.rsground | cut -d' ' -f1)" = 96210bcf628ca54c97516fb3fd56705ce87045a3ebeeb26e4d6bd5918c5adbe1
test "$(sha256sum RESERVE/red_tiles/b14p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p02c')
assert entry == {'canonical_debug_id': 93, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00093', 'debug_string_address': '0x0827AF3C', 'ground_place_id': 11, 'map_file_id': 94, 'map_id': 93, 'stable_ground_id': 'b14p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CHIKORITA_BASIC' in header
print('B14P02C_IDENTITY_PASS map_file_id=94 symbol=MAP_TEAM_BASE_CHIKORITA_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b14p02c --expected-screenshots 2 \
...
print('B14P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b14p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b14p02c','primary')]['movement_probe']=='PASS'
assert validations[('b14p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b14p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B14P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b14p02c.rsground | cut -d' ' -f1)" = 56bacd4428aeee0c59cf6d5ac5c52cbfeb5ef9436d95a71fe75604be2c1a197c
test "$(sha256sum Content/Tile/b14p02c_Base.tile | cut -d' ' -f1)" = 97281c94a0674c9a2b9d694b18382573eb21bc0ffa0372a8634d3201b8dbe164
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b14p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b14p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b14p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
