#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d06p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d06p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d06p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d06p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d06p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d06p01.rsground" | cut -d' ' -f1)" = a367ee05eadd1059b7b6d0354f7c7adad10c4fdcfb64557436db5d361db72421
test "$(sha256sum "$CANONICAL/tiles/d06p01_Base.tile" | cut -d' ' -f1)" = e4d41521fca1b68aed1fb07cd67c3facd153310a7a881dd4288656a429de68dc
test "$(sha256sum RESERVE/red_grounds/d06p01.rsground | cut -d' ' -f1)" = f36e2cffe5999ad013a01f193d001a934dc40e107476615397ec16c6a6fb4be2
test "$(sha256sum RESERVE/red_tiles/d06p01_Base.tile | cut -d' ' -f1)" = e4d41521fca1b68aed1fb07cd67c3facd153310a7a881dd4288656a429de68dc
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d06p01')
assert entry == {'canonical_debug_id': 197, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00197', 'debug_string_address': '0x0827A33C', 'ground_place_id': 18, 'map_file_id': 202, 'map_id': 193, 'stable_ground_id': 'd06p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_LAPIS_CAVE_ENTRY' in header
print('D06P01_IDENTITY_PASS map_file_id=202 symbol=MAP_LAPIS_CAVE_ENTRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d06p01 --expected-screenshots 2 \
...
print('D06P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d06p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d06p01','primary')]['movement_probe']=='PASS'
assert validations[('d06p01','primary')]['blocked_probe']=='PASS'
assert validations[('d06p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D06P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d06p01.rsground | cut -d' ' -f1)" = a367ee05eadd1059b7b6d0354f7c7adad10c4fdcfb64557436db5d361db72421
test "$(sha256sum Content/Tile/d06p01_Base.tile | cut -d' ' -f1)" = e4d41521fca1b68aed1fb07cd67c3facd153310a7a881dd4288656a429de68dc
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d06p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d06p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d06p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
