#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b15p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b15p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b15p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b15p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b15p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b15p01c.rsground" | cut -d' ' -f1)" = 9562d435daa9981e85e1d33a5d690ebae74a888e7a05f2807c81ffa3fb496d2d
test "$(sha256sum "$CANONICAL/tiles/b15p01c_Base.tile" | cut -d' ' -f1)" = 59559498267b6ae7801d905318daa3c9417dfa8ef15500a60b39f3c702eb388b
test "$(sha256sum RESERVE/red_grounds/b15p01c.rsground | cut -d' ' -f1)" = f71526cff1a355c4c48de567c54e3f5aa074205f39e006437b10bece27639bb2
test "$(sha256sum RESERVE/red_tiles/b15p01c_Base.tile | cut -d' ' -f1)" = 59559498267b6ae7801d905318daa3c9417dfa8ef15500a60b39f3c702eb388b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b15p01c')
assert entry == {'canonical_debug_id': 96, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00096', 'debug_string_address': '0x0827AEDC', 'ground_place_id': 10, 'map_file_id': 97, 'map_id': 96, 'stable_ground_id': 'b15p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CHIKORITA_BASIC' in header
print('B15P01C_IDENTITY_PASS map_file_id=97 symbol=MAP_TEAM_BASE_INSIDE_CHIKORITA_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b15p01c --expected-screenshots 25 \
...
print('B15P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b15p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b15p01c','primary')]['movement_probe']=='PASS'
assert validations[('b15p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b15p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B15P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b15p01c.rsground | cut -d' ' -f1)" = 9562d435daa9981e85e1d33a5d690ebae74a888e7a05f2807c81ffa3fb496d2d
test "$(sha256sum Content/Tile/b15p01c_Base.tile | cut -d' ' -f1)" = 59559498267b6ae7801d905318daa3c9417dfa8ef15500a60b39f3c702eb388b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b15p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b15p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b15p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
