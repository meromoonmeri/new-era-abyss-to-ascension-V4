...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p01')
assert e == {'canonical_debug_id': 166, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00166', 'debug_string_address': '0x0827A6FC', 'ground_place_id': 9, 'map_file_id': 167, 'map_id': 159, 'stable_ground_id': 'h29p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_SOUTHERN_ISLAND' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H29P01_IDENTITY_PASS map_id=159 map_file_id=167 symbol=MAP_FRIEND_AREA_SOUTHERN_ISLAND')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 9625
...
print('H29P01_EXHAUSTIVE_PASS samples=3201 unique_rgba=288 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
