#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h29p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h29p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h29p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h29p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h29p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h29p01.rsground" | cut -d' ' -f1)" = ba7075f2df85d49ce5e4b711a7e57917257481dd18c7d0e9baebbcd09b80bd67
test "$(sha256sum "$CANONICAL/tiles/h29p01_Base.tile" | cut -d' ' -f1)" = 520662cab7438fe1f6b82d601806d3198f6f961479422e0bd2273f7f50e26b69
test "$(sha256sum RESERVE/red_grounds/h29p01.rsground | cut -d' ' -f1)" = 89bf4383862c1b125e6968582033f9e13bd62ffeaf92f3526c214009ddd744d4
test "$(sha256sum RESERVE/red_tiles/h29p01_Base.tile | cut -d' ' -f1)" = c685b51e27740618ac0f9ab8caa0558c0886a00dcd0ebf1e1c66ffd2a884b1fd
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p01')
assert entry == {'canonical_debug_id': 166, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00166', 'debug_string_address': '0x0827A6FC', 'ground_place_id': 9, 'map_file_id': 167, 'map_id': 159, 'stable_ground_id': 'h29p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SOUTHERN_ISLAND' in header
print('H29P01_IDENTITY_PASS map_file_id=167 symbol=MAP_FRIEND_AREA_SOUTHERN_ISLAND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h29p01 --expected-screenshots 3201 \
...
print('H29P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 9628
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 3201
...
assert r['grounds']==['h29p01'] and r['sample_count']==3201 and len(primary)==3200
assert r['exact_sample_count']==3201 and r['fully_opaque_sample_count']==3201
assert len({s['actual_rgba_sha256'] for s in primary})==288
...
assert validations[('h29p01','primary')]['movement_probe']=='PASS'
assert validations[('h29p01','primary')]['blocked_probe']=='PASS'
assert validations[('h29p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H29P01_EXHAUSTIVE_PASS samples=3201 unique_rgba=288')
PY
...
test "$(sha256sum Data/Ground/h29p01.rsground | cut -d' ' -f1)" = ba7075f2df85d49ce5e4b711a7e57917257481dd18c7d0e9baebbcd09b80bd67
test "$(sha256sum Content/Tile/h29p01_Base.tile | cut -d' ' -f1)" = 520662cab7438fe1f6b82d601806d3198f6f961479422e0bd2273f7f50e26b69
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h29p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h29p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h29p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 3201 comparative PNGs,
