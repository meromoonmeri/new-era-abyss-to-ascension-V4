#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d04p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d04p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d04p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d04p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d04p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d04p01.rsground" | cut -d' ' -f1)" = c6938c3ebc82f3e3332553ecdcef9ef808283f41f1a78584bfb1e28eb675f4f0
test "$(sha256sum "$CANONICAL/tiles/d04p01_Base.tile" | cut -d' ' -f1)" = b879546cf2a0784f94905ec98836dac480bf009291c6a97f7d218884554ae00d
test "$(sha256sum RESERVE/red_grounds/d04p01.rsground | cut -d' ' -f1)" = 96d50342d8ca40ed83c63450ff944ee104d96633f59a4c251efb026ff36323c0
test "$(sha256sum RESERVE/red_tiles/d04p01_Base.tile | cut -d' ' -f1)" = b879546cf2a0784f94905ec98836dac480bf009291c6a97f7d218884554ae00d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d04p01')
assert entry == {'canonical_debug_id': 193, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00193', 'debug_string_address': '0x0827A3BC', 'ground_place_id': 16, 'map_file_id': 198, 'map_id': 189, 'stable_ground_id': 'd04p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MT_THUNDER_MID' in header
print('D04P01_IDENTITY_PASS map_file_id=198 symbol=MAP_MT_THUNDER_MID')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d04p01 --expected-screenshots 2 \
...
print('D04P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d04p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d04p01','primary')]['movement_probe']=='PASS'
assert validations[('d04p01','primary')]['blocked_probe']=='PASS'
assert validations[('d04p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D04P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d04p01.rsground | cut -d' ' -f1)" = c6938c3ebc82f3e3332553ecdcef9ef808283f41f1a78584bfb1e28eb675f4f0
test "$(sha256sum Content/Tile/d04p01_Base.tile | cut -d' ' -f1)" = b879546cf2a0784f94905ec98836dac480bf009291c6a97f7d218884554ae00d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d04p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d04p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d04p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
