#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b01p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b01p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b01p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b01p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b01p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b01p01c.rsground" | cut -d' ' -f1)" = 25ed1c8ec607c839f05e7d4702164ed44f5abef32ec0c2b094ee7c61812e0a9f
test "$(sha256sum "$CANONICAL/tiles/b01p01c_Base.tile" | cut -d' ' -f1)" = 7b998782764c4c2f48eb2db9ab391ab63cb5dd5ffcaab38599ced5ea7f19a1c2
test "$(sha256sum RESERVE/red_grounds/b01p01c.rsground | cut -d' ' -f1)" = bd5aaaa8a68d33542cb0454047cebdf249f69515dee1980664c4b4b3ae5b517d
test "$(sha256sum RESERVE/red_tiles/b01p01c_Base.tile | cut -d' ' -f1)" = b6fe765e262c6b4ce7502424ec42089eeb014c8daef9cd87a6ad57aa23352944
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b01p01c')
assert entry == {'canonical_debug_id': 12, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00012', 'debug_string_address': '0x0827B95C', 'ground_place_id': 10, 'map_file_id': 13, 'map_id': 12, 'stable_ground_id': 'b01p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_PIKACHU_BASIC' in header
print('B01P01C_IDENTITY_PASS map_file_id=13 symbol=MAP_TEAM_BASE_INSIDE_PIKACHU_BASIC')
PYID
...
test "$(wc -l < "$FIX/events.jsonl")" -eq 223
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 67
...
assert r['grounds']==['b01p01c'] and r['sample_count']==67 and len(primary)==66
assert r['exact_sample_count']==67 and r['fully_opaque_sample_count']==67
assert len({s['actual_rgba_sha256'] for s in primary})==37
...
assert validations[('b01p01c','primary')]['movement_probe']=='PASS'
assert validations[('b01p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b01p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B01P01C_EXHAUSTIVE_PASS samples=67 unique_rgba=37')
PY
...
test "$(sha256sum Data/Ground/b01p01c.rsground | cut -d' ' -f1)" = 25ed1c8ec607c839f05e7d4702164ed44f5abef32ec0c2b094ee7c61812e0a9f
test "$(sha256sum Content/Tile/b01p01c_Base.tile | cut -d' ' -f1)" = 7b998782764c4c2f48eb2db9ab391ab63cb5dd5ffcaab38599ced5ea7f19a1c2
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b01p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b01p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b01p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 67 comparative PNGs,
