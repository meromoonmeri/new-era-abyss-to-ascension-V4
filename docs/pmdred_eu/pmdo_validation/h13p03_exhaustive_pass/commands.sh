#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h13p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h13p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h13p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h13p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h13p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h13p03.rsground" | cut -d' ' -f1)" = 2c8a352107a86d3f07625c13732a8c33d79170a6195b87801103b78472b7ac60
test "$(sha256sum "$CANONICAL/tiles/h13p03_Base.tile" | cut -d' ' -f1)" = dc890af005068c9299e528847896157e00823e367adaa7190f8c3fad6737b3e0
test "$(sha256sum RESERVE/red_grounds/h13p03.rsground | cut -d' ' -f1)" = d4480afda935f26b385e8a9415f4dce453813cc28a4b1cb570f8045a41a59fcc
test "$(sha256sum RESERVE/red_tiles/h13p03_Base.tile | cut -d' ' -f1)" = dc890af005068c9299e528847896157e00823e367adaa7190f8c3fad6737b3e0
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h13p03')
assert entry == {'canonical_debug_id': 145, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00145', 'debug_string_address': '0x0827A95C', 'ground_place_id': 9, 'map_file_id': 146, 'map_id': 140, 'stable_ground_id': 'h13p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_BOULDER_CAVE' in header
print('H13P03_IDENTITY_PASS map_file_id=146 symbol=MAP_FRIEND_AREA_BOULDER_CAVE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h13p03 --expected-screenshots 2 \
...
print('H13P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h13p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h13p03','primary')]['movement_probe']=='PASS'
assert validations[('h13p03','primary')]['blocked_probe']=='PASS'
assert validations[('h13p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H13P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h13p03.rsground | cut -d' ' -f1)" = 2c8a352107a86d3f07625c13732a8c33d79170a6195b87801103b78472b7ac60
test "$(sha256sum Content/Tile/h13p03_Base.tile | cut -d' ' -f1)" = dc890af005068c9299e528847896157e00823e367adaa7190f8c3fad6737b3e0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h13p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h13p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h13p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
