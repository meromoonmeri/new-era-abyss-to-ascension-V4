#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b15p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b15p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b15p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b15p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b15p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b15p02b.rsground" | cut -d' ' -f1)" = 057e5a220babd249bd58058902e1672757e170444393b6fcf867c5452ac07789
test "$(sha256sum "$CANONICAL/tiles/b15p02b_Base.tile" | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
test "$(sha256sum RESERVE/red_grounds/b15p02b.rsground | cut -d' ' -f1)" = 7c4077656e363de1023cf8e7fbf3951474bd7bdda1726cae066705fd3027b86b
test "$(sha256sum RESERVE/red_tiles/b15p02b_Base.tile | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b15p02b')
assert entry == {'canonical_debug_id': 98, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00098', 'debug_string_address': '0x0827AE9C', 'ground_place_id': 11, 'map_file_id': 99, 'map_id': 98, 'stable_ground_id': 'b15p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CHIKORITA_FINAL' in header
print('B15P02B_IDENTITY_PASS map_file_id=99 symbol=MAP_TEAM_BASE_INSIDE_CHIKORITA_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b15p02b --expected-screenshots 2 \
...
print('B15P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b15p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b15p02b','primary')]['movement_probe']=='PASS'
assert validations[('b15p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b15p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B15P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b15p02b.rsground | cut -d' ' -f1)" = 057e5a220babd249bd58058902e1672757e170444393b6fcf867c5452ac07789
test "$(sha256sum Content/Tile/b15p02b_Base.tile | cut -d' ' -f1)" = 8d2e7705b4c019e93f0fbee409aefcb63ee099f164cb9be120c688e4adbb0350
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b15p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b15p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b15p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
