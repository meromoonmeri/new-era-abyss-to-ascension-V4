#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p02b.rsground" | cut -d' ' -f1)" = 917ec49d2f31225777b8d76a3497ddb12b614b246a762cec6210c7b6a4dd67b4
test "$(sha256sum "$CANONICAL/tiles/b11p02b_Base.tile" | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test "$(sha256sum RESERVE/red_grounds/b11p02b.rsground | cut -d' ' -f1)" = db106639992ef5d082f29f689b959a40bfec462f3631a67a964c299d60d13387
test "$(sha256sum RESERVE/red_tiles/b11p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p02b')
assert entry == {'canonical_debug_id': 74, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00074', 'debug_string_address': '0x0827B19C', 'ground_place_id': 11, 'map_file_id': 75, 'map_id': 74, 'stable_ground_id': 'b11p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CYNDAQUIL_FINAL' in header
print('B11P02B_IDENTITY_PASS map_file_id=75 symbol=MAP_TEAM_BASE_INSIDE_CYNDAQUIL_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p02b --expected-screenshots 1561 \
...
print('B11P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p02b'] and r['sample_count']==1561 and len(primary)==1560
...
assert validations[('b11p02b','primary')]['movement_probe']=='PASS'
assert validations[('b11p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b11p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P02B_EXHAUSTIVE_PASS samples=1561 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b11p02b.rsground | cut -d' ' -f1)" = 917ec49d2f31225777b8d76a3497ddb12b614b246a762cec6210c7b6a4dd67b4
test "$(sha256sum Content/Tile/b11p02b_Base.tile | cut -d' ' -f1)" = de77a991e96498b3f1d7a714107a8f74a226e395f71ced915adbb811c736dab7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
