...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d11p01')
assert e == {'canonical_debug_id': 210, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00210', 'debug_string_address': '0x0827A19C', 'ground_place_id': 28, 'map_file_id': 215, 'map_id': 206, 'stable_ground_id': 'd11p01', 'weather_id': 10}
assert 'MAP_MAGMA_CAVERN_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D11P01_IDENTITY_PASS map_id=206 map_file_id=215 symbol=MAP_MAGMA_CAVERN_END')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 337
...
print('D11P01_EXHAUSTIVE_PASS samples=105 unique_rgba=26 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
