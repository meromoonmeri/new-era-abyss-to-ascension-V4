#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d11p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d11p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d11p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d11p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d11p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d11p01.rsground" | cut -d' ' -f1)" = ccfb212c50200f3a7a8f35f34b0549697a170df86306574f05b0560acae95a0e
test "$(sha256sum "$CANONICAL/tiles/d11p01_Base.tile" | cut -d' ' -f1)" = 7481e0fd69b1accfbee0fc04b164ad7368551d9b3f38cdd6e6b44c8e3375cf69
test "$(sha256sum RESERVE/red_grounds/d11p01.rsground | cut -d' ' -f1)" = e56aed737978d6f0066e7a6898f1798a1217e0867b8e660c0b66db71d8acab55
test "$(sha256sum RESERVE/red_tiles/d11p01_Base.tile | cut -d' ' -f1)" = 17d6d48e003d6a79370004b2663e3944bf74730d99ebd97fdbec13b167c821d2
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d11p01')
assert entry == {'canonical_debug_id': 210, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00210', 'debug_string_address': '0x0827A19C', 'ground_place_id': 28, 'map_file_id': 215, 'map_id': 206, 'stable_ground_id': 'd11p01', 'weather_id': 10}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MAGMA_CAVERN_END' in header
print('D11P01_IDENTITY_PASS map_file_id=215 symbol=MAP_MAGMA_CAVERN_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d11p01 --expected-screenshots 105 \
...
print('D11P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 340
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 105
...
assert r['grounds']==['d11p01'] and r['sample_count']==105 and len(primary)==104
assert r['exact_sample_count']==105 and r['fully_opaque_sample_count']==105
assert len({s['actual_rgba_sha256'] for s in primary})==26
...
assert validations[('d11p01','primary')]['movement_probe']=='PASS'
assert validations[('d11p01','primary')]['blocked_probe']=='PASS'
assert validations[('d11p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D11P01_EXHAUSTIVE_PASS samples=105 unique_rgba=26')
PY
...
test "$(sha256sum Data/Ground/d11p01.rsground | cut -d' ' -f1)" = ccfb212c50200f3a7a8f35f34b0549697a170df86306574f05b0560acae95a0e
test "$(sha256sum Content/Tile/d11p01_Base.tile | cut -d' ' -f1)" = 7481e0fd69b1accfbee0fc04b164ad7368551d9b3f38cdd6e6b44c8e3375cf69
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d11p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d11p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d11p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 105 comparative PNGs,
