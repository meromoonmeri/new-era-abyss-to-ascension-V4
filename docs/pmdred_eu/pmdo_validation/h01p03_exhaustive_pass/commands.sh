#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h01p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h01p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h01p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h01p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h01p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h01p03.rsground" | cut -d' ' -f1)" = 3e965d5100977605005ee603aed3c76cfab1b82702833c9fb975edc4db20c602
test "$(sha256sum "$CANONICAL/tiles/h01p03_Base.tile" | cut -d' ' -f1)" = 36eb836b0f3239310cfb33a17805db9619434dc29b322cec72ff3aed6298c2b4
test "$(sha256sum RESERVE/red_grounds/h01p03.rsground | cut -d' ' -f1)" = 57e041bb0d6ce52a2688e072bcffc61f9f815c3700474a072a56b2da4084cf5a
test "$(sha256sum RESERVE/red_tiles/h01p03_Base.tile | cut -d' ' -f1)" = 36eb836b0f3239310cfb33a17805db9619434dc29b322cec72ff3aed6298c2b4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h01p03')
assert entry == {'canonical_debug_id': 109, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00109', 'debug_string_address': '0x0827AD5C', 'ground_place_id': 9, 'map_file_id': 110, 'map_id': 108, 'stable_ground_id': 'h01p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_DEEP_SEA_FLOOR' in header
print('H01P03_IDENTITY_PASS map_file_id=110 symbol=MAP_FRIEND_AREA_DEEP_SEA_FLOOR')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h01p03 --expected-screenshots 2 \
...
print('H01P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h01p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h01p03','primary')]['movement_probe']=='PASS'
assert validations[('h01p03','primary')]['blocked_probe']=='PASS'
assert validations[('h01p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H01P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h01p03.rsground | cut -d' ' -f1)" = 3e965d5100977605005ee603aed3c76cfab1b82702833c9fb975edc4db20c602
test "$(sha256sum Content/Tile/h01p03_Base.tile | cut -d' ' -f1)" = 36eb836b0f3239310cfb33a17805db9619434dc29b322cec72ff3aed6298c2b4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h01p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h01p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h01p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
