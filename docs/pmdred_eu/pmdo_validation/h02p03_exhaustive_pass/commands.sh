...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h02p03')
assert e == {'canonical_debug_id': 114, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00114', 'debug_string_address': '0x0827ACFC', 'ground_place_id': 9, 'map_file_id': 115, 'map_id': 111, 'stable_ground_id': 'h02p03', 'weather_id': 4}
assert 'MAP_FRIEND_AREA_SHALLOW_BEACH' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H02P03_IDENTITY_PASS map_id=111 map_file_id=115 symbol=MAP_FRIEND_AREA_SHALLOW_BEACH')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 301
...
print('H02P03_EXHAUSTIVE_PASS samples=93 unique_rgba=18 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
