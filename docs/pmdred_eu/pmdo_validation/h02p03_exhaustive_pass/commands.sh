#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h02p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h02p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h02p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h02p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h02p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h02p03.rsground" | cut -d' ' -f1)" = ecef9ea2d11dcc3bca211a1e5ae49c3ba1accd472baf6b1caa1682f5878d2042
test "$(sha256sum "$CANONICAL/tiles/h02p03_Base.tile" | cut -d' ' -f1)" = abd013c720d7f36034d134321211968b5f1da234c8ad63018a52b4588fb2ada9
test "$(sha256sum RESERVE/red_grounds/h02p03.rsground | cut -d' ' -f1)" = 681af7bb0a1c40e8e725a801fb829398a7706ce8cc945323f56bd29d145f7b6c
test "$(sha256sum RESERVE/red_tiles/h02p03_Base.tile | cut -d' ' -f1)" = 70ec5c7deda239df957278653e5b732f531b53c752c9920fc1e98d4cecd49118
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h02p03')
assert entry == {'canonical_debug_id': 114, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00114', 'debug_string_address': '0x0827ACFC', 'ground_place_id': 9, 'map_file_id': 115, 'map_id': 111, 'stable_ground_id': 'h02p03', 'weather_id': 4}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SHALLOW_BEACH' in header
print('H02P03_IDENTITY_PASS map_file_id=115 symbol=MAP_FRIEND_AREA_SHALLOW_BEACH')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h02p03 --expected-screenshots 93 \
...
print('H02P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 304
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 93
...
assert r['grounds']==['h02p03'] and r['sample_count']==93 and len(primary)==92
assert r['exact_sample_count']==93 and r['fully_opaque_sample_count']==93
...
assert validations[('h02p03','primary')]['movement_probe']=='PASS'
assert validations[('h02p03','primary')]['blocked_probe']=='PASS'
assert validations[('h02p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H02P03_EXHAUSTIVE_PASS samples=93 unique_rgba=18')
PY
...
test "$(sha256sum Data/Ground/h02p03.rsground | cut -d' ' -f1)" = ecef9ea2d11dcc3bca211a1e5ae49c3ba1accd472baf6b1caa1682f5878d2042
test "$(sha256sum Content/Tile/h02p03_Base.tile | cut -d' ' -f1)" = abd013c720d7f36034d134321211968b5f1da234c8ad63018a52b4588fb2ada9
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h02p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h02p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h02p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 93 comparative PNGs,
