...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a03p02-reproduction-entity-migration"
...
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a03p02 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a03p02.rsground" | cut -d' ' -f1)" = 4c685a550e2cc6a9cf58f0b640ed3f206700639ac9dd0c39e1a35d1d8c16634c
...
test "$(sha256sum RESERVE/red_grounds/a03p02.rsground | cut -d' ' -f1)" = 62afb6275c527e997ec5225505328559f14a9671fd07a684b8b7b6de1d1bc749
test "$(sha256sum RESERVE/red_tiles/a03p02_Base.tile | cut -d' ' -f1)" = 076d64fb6b4f86ef10fc0861a1b2cf4e01f4d88a434b220c886575c6fe30898c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a03p02')
assert entry == {'canonical_debug_id': 178, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00178', 'debug_string_address': '0x0827A59C', 'ground_place_id': 35, 'map_file_id': 179, 'map_id': 170, 'stable_ground_id': 'a03p02', 'weather_id': 10}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_BLIZZARD_ROAD' in header
print('A03P02_IDENTITY_PASS map_file_id=179 symbol=MAP_FUGITIVES_BLIZZARD_ROAD')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a03p02 --expected-screenshots 2 \
...
print('A03P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
print('A03P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a03p02.rsground | cut -d' ' -f1)" = 4c685a550e2cc6a9cf58f0b640ed3f206700639ac9dd0c39e1a35d1d8c16634c
test "$(sha256sum Content/Tile/a03p02_Base.tile | cut -d' ' -f1)" = 076d64fb6b4f86ef10fc0861a1b2cf4e01f4d88a434b220c886575c6fe30898c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a03p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a03p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a03p02 --entity-integrated-ids a03p02 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p02/a03p02.rsground | cut -d' ' -f1)" = ca7278d2c16e3843a089ab20904b7c3b5d85542d24987573604ffe12e83c9ff3
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p02/a03p02_Base.tile | cut -d' ' -f1)" = 5d492bd32d1c0fbebce449bf85624865d333eb5743b0acb5adacf5ce7fb9cf2a
