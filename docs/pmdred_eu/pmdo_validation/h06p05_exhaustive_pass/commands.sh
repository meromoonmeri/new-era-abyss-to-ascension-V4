#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p05 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p05-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p05-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p05-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p05-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p05.rsground" | cut -d' ' -f1)" = df9ce45e1c55f251d023a2e933b7c217b21e6c8993ed43d7db32f94c74a0265e
test "$(sha256sum "$CANONICAL/tiles/h06p05_Base.tile" | cut -d' ' -f1)" = 1a799257fa4442a99e963037dc46d65c8a0cd50f4b619847da3cf873f5881210
test "$(sha256sum RESERVE/red_grounds/h06p05.rsground | cut -d' ' -f1)" = c1c3944f5435ef79fd1162433d9902fba2a2521dd12e2a6e83276901fc7e36f6
test "$(sha256sum RESERVE/red_tiles/h06p05_Base.tile | cut -d' ' -f1)" = f87726f30aae6e8a8958631d6bfb7322673d269281da3d6a0b20673eec863ab0
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p05')
assert entry == {'canonical_debug_id': 125, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00125', 'debug_string_address': '0x0827ABBC', 'ground_place_id': 9, 'map_file_id': 126, 'map_id': 121, 'stable_ground_id': 'h06p05', 'weather_id': 13}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SACRED_FIELD' in header
print('H06P05_IDENTITY_PASS map_file_id=126 symbol=MAP_FRIEND_AREA_SACRED_FIELD')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p05 --expected-screenshots 57 \
...
print('H06P05_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 196
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 57
...
assert r['grounds']==['h06p05'] and r['sample_count']==57 and len(primary)==56
assert r['exact_sample_count']==57 and r['fully_opaque_sample_count']==57
...
assert validations[('h06p05','primary')]['movement_probe']=='PASS'
assert validations[('h06p05','primary')]['blocked_probe']=='PASS'
assert validations[('h06p05','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P05_EXHAUSTIVE_PASS samples=57 unique_rgba=36')
PY
...
test "$(sha256sum Data/Ground/h06p05.rsground | cut -d' ' -f1)" = df9ce45e1c55f251d023a2e933b7c217b21e6c8993ed43d7db32f94c74a0265e
test "$(sha256sum Content/Tile/h06p05_Base.tile | cut -d' ' -f1)" = 1a799257fa4442a99e963037dc46d65c8a0cd50f4b619847da3cf873f5881210
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p05.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p05_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p05 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 57 comparative PNGs,
