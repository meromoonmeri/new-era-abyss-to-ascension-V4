#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a05p03f milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03f-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a05p03f-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a05p03f-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03f-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a05p03f.rsground" | cut -d' ' -f1)" = cde4df3fc3f501d68a28048e5c539953c1e2185d3b72954f96fda3bee776f4dd
test "$(sha256sum "$CANONICAL/tiles/a05p03f_Base.tile" | cut -d' ' -f1)" = c09c12bb5cae283dea4e4a6edbf98e201be37986faedfb9f5abed7e93cd1b830
test ! -e RESERVE/red_grounds/a05p03f.rsground
test ! -e RESERVE/red_tiles/a05p03f_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p03f')
assert entry == {'canonical_debug_id': 186, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00186', 'debug_string_address': '0x0827A49C', 'ground_place_id': 35, 'map_file_id': 189, 'map_id': 180, 'stable_ground_id': 'a05p03f', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_THUNDERWAVE_CAVE_ENTRY' in header
print('A05P03F_IDENTITY_PASS map_file_id=189 symbol=MAP_THUNDERWAVE_CAVE_ENTRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a05p03f --expected-screenshots 2 \
...
print('A05P03F_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a05p03f'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a05p03f','primary')]['movement_probe']=='PASS'
assert validations[('a05p03f','primary')]['blocked_probe']=='PASS'
assert validations[('a05p03f','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A05P03F_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a05p03f.rsground | cut -d' ' -f1)" = cde4df3fc3f501d68a28048e5c539953c1e2185d3b72954f96fda3bee776f4dd
test "$(sha256sum Content/Tile/a05p03f_Base.tile | cut -d' ' -f1)" = c09c12bb5cae283dea4e4a6edbf98e201be37986faedfb9f5abed7e93cd1b830
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a05p03f.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a05p03f_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a05p03f --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
