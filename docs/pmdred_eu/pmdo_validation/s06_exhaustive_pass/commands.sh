#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s06 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s06-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s06-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s06-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s06-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s06.rsground" | cut -d' ' -f1)" = c87b931615455df278a6745bf5c0fbbe9528f473fd53b1be5e40a0c49e3bb308
test "$(sha256sum "$CANONICAL/tiles/s06_Base.tile" | cut -d' ' -f1)" = 0a4838e70a0ee8afdaa92dce2bb48edf112f5cc7dfa0833c4a3f79a836974f9c
test "$(sha256sum RESERVE/red_grounds/s06.rsground | cut -d' ' -f1)" = 721287b25d622fb4681a76520410d60b8216060aad476659a2e14faca97fc301
test "$(sha256sum RESERVE/red_tiles/s06_Base.tile | cut -d' ' -f1)" = 0a4838e70a0ee8afdaa92dce2bb48edf112f5cc7dfa0833c4a3f79a836974f9c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s06')
assert entry == {'canonical_debug_id': 240, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00240', 'debug_string_address': '0x08279E3C', 'ground_place_id': 37, 'map_file_id': 250, 'map_id': 241, 'stable_ground_id': 's06', 'weather_id': -1}
...
designated=table.split('[MAP_TEAM_BASE_WALL_MAP] = {',1)[1].split('},',1)[0]
assert '__ground_amd_conversion_00240' in designated
print('S06_IDENTITY_PASS map_file_id=250 canonical_debug_id=240 symbol=MAP_TEAM_BASE_WALL_MAP')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s06 --expected-screenshots 2 \
...
print('S06_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s06'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s06','primary')]['movement_probe']=='PASS'
assert validations[('s06','primary')]['blocked_probe']=='PASS'
assert validations[('s06','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S06_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s06.rsground | cut -d' ' -f1)" = c87b931615455df278a6745bf5c0fbbe9528f473fd53b1be5e40a0c49e3bb308
test "$(sha256sum Content/Tile/s06_Base.tile | cut -d' ' -f1)" = 0a4838e70a0ee8afdaa92dce2bb48edf112f5cc7dfa0833c4a3f79a836974f9c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s06.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s06_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s06 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
