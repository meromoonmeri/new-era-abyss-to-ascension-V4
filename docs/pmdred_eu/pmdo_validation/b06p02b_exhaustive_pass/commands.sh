#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p02b.rsground" | cut -d' ' -f1)" = ab960fe04722222d3257f42fb0e252defad095c4438ff0d817536e9690ac289c
test "$(sha256sum "$CANONICAL/tiles/b06p02b_Base.tile" | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p02b')
assert entry == {'canonical_debug_id': 44, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00044', 'debug_string_address': '0x0827B55C', 'ground_place_id': 11, 'map_file_id': 45, 'map_id': 44, 'stable_ground_id': 'b06p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TOTODILE_FINAL' in header
print('B06P02B_IDENTITY_PASS map_id=44 map_file_id=45 symbol=MAP_TEAM_BASE_INSIDE_TOTODILE_FINAL')
PYID
...
assert r['grounds']==['b06p02b'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b06p02b','primary')]['movement_probe']=='PASS'
assert validations[('b06p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b06p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P02B_EXHAUSTIVE_PASS samples=481 unique_rgba=233 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b06p02b.rsground | cut -d' ' -f1)" = ab960fe04722222d3257f42fb0e252defad095c4438ff0d817536e9690ac289c
test "$(sha256sum Content/Tile/b06p02b_Base.tile | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
