#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h08p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h08p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h08p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h08p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h08p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h08p01.rsground" | cut -d' ' -f1)" = eea163564c8fac00b5d462195c6ba7597da4323a469022cdfc45ba5b2361b134
test "$(sha256sum "$CANONICAL/tiles/h08p01_Base.tile" | cut -d' ' -f1)" = 605a5888b2a7b67e759bde86e95c42955182572913a65fd59754e4ededdd4e00
test "$(sha256sum RESERVE/red_grounds/h08p01.rsground | cut -d' ' -f1)" = b59eb52bba2e31d224a432da1fe1f72c1b304d44e567831a7974be1d6716c54b
test "$(sha256sum RESERVE/red_tiles/h08p01_Base.tile | cut -d' ' -f1)" = 605a5888b2a7b67e759bde86e95c42955182572913a65fd59754e4ededdd4e00
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h08p01')
assert entry == {'canonical_debug_id': 136, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00136', 'debug_string_address': '0x0827AA7C', 'ground_place_id': 9, 'map_file_id': 137, 'map_id': 131, 'stable_ground_id': 'h08p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_TADPOLE_POND' in header
print('H08P01_IDENTITY_PASS map_file_id=137 symbol=MAP_FRIEND_AREA_TADPOLE_POND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h08p01 --expected-screenshots 918 \
...
print('H08P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 2779
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 918
...
assert r['grounds']==['h08p01'] and r['sample_count']==918 and len(primary)==917
assert r['exact_sample_count']==918 and r['fully_opaque_sample_count']==918
assert len({s['actual_rgba_sha256'] for s in primary})==634
...
assert validations[('h08p01','primary')]['movement_probe']=='PASS'
assert validations[('h08p01','primary')]['blocked_probe']=='PASS'
assert validations[('h08p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H08P01_EXHAUSTIVE_PASS samples=918 unique_rgba=634')
PY
...
test "$(sha256sum Data/Ground/h08p01.rsground | cut -d' ' -f1)" = eea163564c8fac00b5d462195c6ba7597da4323a469022cdfc45ba5b2361b134
test "$(sha256sum Content/Tile/h08p01_Base.tile | cut -d' ' -f1)" = 605a5888b2a7b67e759bde86e95c42955182572913a65fd59754e4ededdd4e00
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h08p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h08p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h08p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 918 comparative PNGs,
