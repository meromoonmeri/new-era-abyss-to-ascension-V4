...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h08p01')
assert e == {'canonical_debug_id': 136, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00136', 'debug_string_address': '0x0827AA7C', 'ground_place_id': 9, 'map_file_id': 137, 'map_id': 131, 'stable_ground_id': 'h08p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_TADPOLE_POND' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H08P01_IDENTITY_PASS map_id=131 map_file_id=137 symbol=MAP_FRIEND_AREA_TADPOLE_POND')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 2776
...
print('H08P01_EXHAUSTIVE_PASS samples=918 unique_rgba=634 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
