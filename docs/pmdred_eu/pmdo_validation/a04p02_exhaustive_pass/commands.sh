#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a04p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a04p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a04p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a04p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a04p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a04p02.rsground" | cut -d' ' -f1)" = f5ce40b13dd3ec9a838f7b6e052c7cb24d56a933f1ec3f45ced743546e99de30
test "$(sha256sum "$CANONICAL/tiles/a04p02_Base.tile" | cut -d' ' -f1)" = cd3701af37bc622846de18ec28bb3d9073ae7efa96a88bdd589afc408e142bca
test "$(sha256sum RESERVE/red_grounds/a04p02.rsground | cut -d' ' -f1)" = 17a1491cd2334d0b74b65eef8b4bf0981c4d8b55f214acfd12b9565ba80b037d
test "$(sha256sum RESERVE/red_tiles/a04p02_Base.tile | cut -d' ' -f1)" = cd3701af37bc622846de18ec28bb3d9073ae7efa96a88bdd589afc408e142bca
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p02')
assert entry == {'canonical_debug_id': 181, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00181', 'debug_string_address': '0x0827A53C', 'ground_place_id': 35, 'map_file_id': 182, 'map_id': 173, 'stable_ground_id': 'a04p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_COMET' in header
print('A04P02_IDENTITY_PASS map_file_id=182 symbol=MAP_COMET')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a04p02 --expected-screenshots 2 \
...
print('A04P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a04p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a04p02','primary')]['movement_probe']=='PASS'
assert validations[('a04p02','primary')]['blocked_probe']=='PASS'
assert validations[('a04p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A04P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a04p02.rsground | cut -d' ' -f1)" = f5ce40b13dd3ec9a838f7b6e052c7cb24d56a933f1ec3f45ced743546e99de30
test "$(sha256sum Content/Tile/a04p02_Base.tile | cut -d' ' -f1)" = cd3701af37bc622846de18ec28bb3d9073ae7efa96a88bdd589afc408e142bca
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a04p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a04p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a04p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
