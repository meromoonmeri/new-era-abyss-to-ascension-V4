...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p02')
assert e == {'canonical_debug_id': 181, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00181', 'debug_string_address': '0x0827A53C', 'ground_place_id': 35, 'map_file_id': 182, 'map_id': 173, 'stable_ground_id': 'a04p02', 'weather_id': -1}
assert 'MAP_COMET' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A04P02_IDENTITY_PASS map_id=173 map_file_id=182 symbol=MAP_COMET')
PYID
...
print('A04P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
