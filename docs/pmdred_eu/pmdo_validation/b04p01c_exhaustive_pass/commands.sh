#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b04p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b04p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b04p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b04p01c.rsground" | cut -d' ' -f1)" = 301106a61d582b0406ca1117a11f5252c87ac78cc737797696156fad886c1aab
test "$(sha256sum "$CANONICAL/tiles/b04p01c_Base.tile" | cut -d' ' -f1)" = ab0fecd3745354fa2cef6cc3ac009023617549e62b7215be77599b554c121c25
test "$(sha256sum RESERVE/red_grounds/b04p01c.rsground | cut -d' ' -f1)" = b4c1d7ff41cf3b3ea4196a97ee6d266c8cee12389ac925c9b977300e218064d3
test "$(sha256sum RESERVE/red_tiles/b04p01c_Base.tile | cut -d' ' -f1)" = 032ec67447f36b0ddaeb562c090203c74b1dab3ad41612878a55c98622a0d7f3
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b04p01c')
assert entry == {'canonical_debug_id': 30, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00030', 'debug_string_address': '0x0827B71C', 'ground_place_id': 10, 'map_file_id': 31, 'map_id': 30, 'stable_ground_id': 'b04p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_SKITTY_BASIC' in header
print('B04P01C_IDENTITY_PASS map_file_id=31 symbol=MAP_TEAM_BASE_INSIDE_SKITTY_BASIC')
PYID
...
assert r['grounds']==['b04p01c'] and r['sample_count']==67 and len(primary)==66
...
assert validations[('b04p01c','primary')]['movement_probe']=='PASS'
assert validations[('b04p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b04p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B04P01C_EXHAUSTIVE_PASS samples=67 unique_rgba=37')
PY
...
test "$(sha256sum Data/Ground/b04p01c.rsground | cut -d' ' -f1)" = 301106a61d582b0406ca1117a11f5252c87ac78cc737797696156fad886c1aab
test "$(sha256sum Content/Tile/b04p01c_Base.tile | cut -d' ' -f1)" = ab0fecd3745354fa2cef6cc3ac009023617549e62b7215be77599b554c121c25
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b04p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b04p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b04p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
