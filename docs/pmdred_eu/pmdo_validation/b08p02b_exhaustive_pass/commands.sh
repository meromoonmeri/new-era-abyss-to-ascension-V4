#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b08p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b08p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b08p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b08p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b08p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b08p02b.rsground" | cut -d' ' -f1)" = c3b06a21afd3d3a87e7e752d006838e6bd611e80c147a60f65f514d07e1c741d
test "$(sha256sum "$CANONICAL/tiles/b08p02b_Base.tile" | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test "$(sha256sum RESERVE/red_grounds/b08p02b.rsground | cut -d' ' -f1)" = daf6842164542d65689b52ae19567f1317957a8a671df3dbac2ba17b7fea5b98
test "$(sha256sum RESERVE/red_tiles/b08p02b_Base.tile | cut -d' ' -f1)" = b589283f5822564edf2d3f39633c3dd11fb720a760263f667a883a8f58142e37
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b08p02b')
assert entry == {'canonical_debug_id': 56, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00056', 'debug_string_address': '0x0827B3DC', 'ground_place_id': 11, 'map_file_id': 57, 'map_id': 56, 'stable_ground_id': 'b08p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_PSYDUCK_FINAL' in header
print('B08P02B_IDENTITY_PASS map_file_id=57 symbol=MAP_TEAM_BASE_INSIDE_PSYDUCK_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b08p02b --expected-screenshots 481 \
...
print('B08P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b08p02b'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b08p02b','primary')]['movement_probe']=='PASS'
assert validations[('b08p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b08p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B08P02B_EXHAUSTIVE_PASS samples=481 unique_rgba=233')
PY
...
test "$(sha256sum Data/Ground/b08p02b.rsground | cut -d' ' -f1)" = c3b06a21afd3d3a87e7e752d006838e6bd611e80c147a60f65f514d07e1c741d
test "$(sha256sum Content/Tile/b08p02b_Base.tile | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b08p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b08p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b08p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
