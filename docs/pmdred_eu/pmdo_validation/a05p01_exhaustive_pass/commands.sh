#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a05p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a05p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a05p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a05p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a05p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a05p01.rsground" | cut -d' ' -f1)" = a42bde3b9b871577c24dc78fd266a46c13747b44f9692b0ce2821795be0db5ab
test "$(sha256sum "$CANONICAL/tiles/a05p01_Base.tile" | cut -d' ' -f1)" = 650f8c1b67fad28c0f0059a45c57a2969ec0761ddf1e6f709e618c359a02944e
test "$(sha256sum RESERVE/red_grounds/a05p01.rsground | cut -d' ' -f1)" = 8a5c42febb7c7749ef01454d73668ac0e62b4063b7d5140a7b6abe4da9ce825f
test "$(sha256sum RESERVE/red_tiles/a05p01_Base.tile | cut -d' ' -f1)" = e36dab3aae172fb32142de1f473e5b2b3fee7bef015642f25c4db1fce3cf9a5d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p01')
assert entry == {'canonical_debug_id': 184, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00184', 'debug_string_address': '0x0827A4DC', 'ground_place_id': 35, 'map_file_id': 185, 'map_id': 176, 'stable_ground_id': 'a05p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_NIGHT_SKY_2' in header
print('A05P01_IDENTITY_PASS map_file_id=185 symbol=MAP_NIGHT_SKY_2')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a05p01 --expected-screenshots 9121 \
...
print('A05P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 27385
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 9121
...
assert r['grounds']==['a05p01'] and r['sample_count']==9121 and len(primary)==9120
assert r['exact_sample_count']==9121 and r['fully_opaque_sample_count']==9121
assert len({s['actual_rgba_sha256'] for s in primary})==228
...
assert validations[('a05p01','primary')]['movement_probe']=='PASS'
assert validations[('a05p01','primary')]['blocked_probe']=='PASS'
assert validations[('a05p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A05P01_EXHAUSTIVE_PASS samples=9121 unique_rgba=228')
PY
...
test "$(sha256sum Data/Ground/a05p01.rsground | cut -d' ' -f1)" = a42bde3b9b871577c24dc78fd266a46c13747b44f9692b0ce2821795be0db5ab
test "$(sha256sum Content/Tile/a05p01_Base.tile | cut -d' ' -f1)" = 650f8c1b67fad28c0f0059a45c57a2969ec0761ddf1e6f709e618c359a02944e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a05p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a05p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a05p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 9121 comparative PNGs,
