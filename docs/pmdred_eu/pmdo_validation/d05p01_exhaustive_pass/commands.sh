#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d05p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d05p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d05p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d05p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d05p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d05p01.rsground" | cut -d' ' -f1)" = e8166d54a0f79f431917772a1e32617c3be938389496bb05307ba2fd37f05860
test "$(sha256sum "$CANONICAL/tiles/d05p01_Base.tile" | cut -d' ' -f1)" = 9ca91e5c119eaeb5921f76fd279a60014af2de0a94f9243ca0dd77e361b96bf6
test "$(sha256sum RESERVE/red_grounds/d05p01.rsground | cut -d' ' -f1)" = e501c4333fb4c525f4d3f0ff8dac46906aff73316f3c756c4d3eb0037922e650
test "$(sha256sum RESERVE/red_tiles/d05p01_Base.tile | cut -d' ' -f1)" = 0460e866419feb423761905bf0d9a8d61df13e365f604149bb33c3af9745b303
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d05p01')
assert entry == {'canonical_debug_id': 195, 'conversion_type': 6, 'debug_string': '__ground_amd_conversion_00195', 'debug_string_address': '0x0827A37C', 'ground_place_id': 17, 'map_file_id': 200, 'map_id': 191, 'stable_ground_id': 'd05p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_GREAT_CANYON_ENTRY' in header
print('D05P01_IDENTITY_PASS map_file_id=200 symbol=MAP_GREAT_CANYON_ENTRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d05p01 --expected-screenshots 2 \
...
print('D05P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d05p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d05p01','primary')]['movement_probe']=='PASS'
assert validations[('d05p01','primary')]['blocked_probe']=='PASS'
assert validations[('d05p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D05P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d05p01.rsground | cut -d' ' -f1)" = e8166d54a0f79f431917772a1e32617c3be938389496bb05307ba2fd37f05860
test "$(sha256sum Content/Tile/d05p01_Base.tile | cut -d' ' -f1)" = 9ca91e5c119eaeb5921f76fd279a60014af2de0a94f9243ca0dd77e361b96bf6
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d05p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d05p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d05p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
