#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b09p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b09p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b09p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b09p01a.rsground" | cut -d' ' -f1)" = 3e1563a0a12855ee15bf03c5ca0e8c6908212779b094282d706da28a87d2d22d
test "$(sha256sum "$CANONICAL/tiles/b09p01a_Base.tile" | cut -d' ' -f1)" = 556c13c01482ffcf0ee8dd632d17fd49da502e994dbff888cd9d61f3fde18281
test "$(sha256sum RESERVE/red_grounds/b09p01a.rsground | cut -d' ' -f1)" = 50271f4f7af9cde49aa800d31b950808e238094e773b86d10fcf592a705f086b
test "$(sha256sum RESERVE/red_tiles/b09p01a_Base.tile | cut -d' ' -f1)" = 556c13c01482ffcf0ee8dd632d17fd49da502e994dbff888cd9d61f3fde18281
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b09p01a')
assert entry == {'canonical_debug_id': 58, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00058', 'debug_string_address': '0x0827B39C', 'ground_place_id': 10, 'map_file_id': 59, 'map_id': 58, 'stable_ground_id': 'b09p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CHARMANDER_CONSTRUCTION' in header
print('B09P01A_IDENTITY_PASS map_file_id=59 symbol=MAP_TEAM_BASE_CHARMANDER_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b09p01a --expected-screenshots 25 \
...
print('B09P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 100
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 25
...
assert r['grounds']==['b09p01a'] and r['sample_count']==25 and len(primary)==24
assert r['exact_sample_count']==25 and r['fully_opaque_sample_count']==25
...
assert validations[('b09p01a','primary')]['movement_probe']=='PASS'
assert validations[('b09p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b09p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B09P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b09p01a.rsground | cut -d' ' -f1)" = 3e1563a0a12855ee15bf03c5ca0e8c6908212779b094282d706da28a87d2d22d
test "$(sha256sum Content/Tile/b09p01a_Base.tile | cut -d' ' -f1)" = 556c13c01482ffcf0ee8dd632d17fd49da502e994dbff888cd9d61f3fde18281
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b09p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b09p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b09p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 25 comparative PNGs,
