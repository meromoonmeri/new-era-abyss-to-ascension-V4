#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b02p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b02p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b02p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b02p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b02p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b02p01c.rsground" | cut -d' ' -f1)" = dd04e5f2a37629c087d709209965204b2164c40276d9aedc8a33d45bebb8a744
test "$(sha256sum "$CANONICAL/tiles/b02p01c_Base.tile" | cut -d' ' -f1)" = 0cb36c93b111d3def5fb46bd355a6ac9fedda9482725d6b82fcb9de3b342bbe8
test "$(sha256sum RESERVE/red_grounds/b02p01c.rsground | cut -d' ' -f1)" = a0970c4ce8201e1251ec248d38c3876d815fe5558521759c3295b79d668fcada
test "$(sha256sum RESERVE/red_tiles/b02p01c_Base.tile | cut -d' ' -f1)" = b31cab976de89db462835ee40aa0d2e42af0752e36d68eea063a94fa6380063b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b02p01c')
assert entry == {'canonical_debug_id': 18, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00018', 'debug_string_address': '0x0827B89C', 'ground_place_id': 10, 'map_file_id': 19, 'map_id': 18, 'stable_ground_id': 'b02p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MEOWTH_BASIC' in header
print('B02P01C_IDENTITY_PASS map_file_id=19 symbol=MAP_TEAM_BASE_INSIDE_MEOWTH_BASIC')
PYID
...
assert r['grounds']==['b02p01c'] and r['sample_count']==67 and len(primary)==66
...
assert validations[('b02p01c','primary')]['movement_probe']=='PASS'
assert validations[('b02p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b02p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B02P01C_EXHAUSTIVE_PASS samples=67 unique_rgba=37')
PY
...
test "$(sha256sum Data/Ground/b02p01c.rsground | cut -d' ' -f1)" = dd04e5f2a37629c087d709209965204b2164c40276d9aedc8a33d45bebb8a744
test "$(sha256sum Content/Tile/b02p01c_Base.tile | cut -d' ' -f1)" = 0cb36c93b111d3def5fb46bd355a6ac9fedda9482725d6b82fcb9de3b342bbe8
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b02p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b02p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b02p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
