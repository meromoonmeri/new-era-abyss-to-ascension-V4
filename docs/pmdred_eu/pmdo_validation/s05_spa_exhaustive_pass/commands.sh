#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s05_spa milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s05_spa-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s05_spa-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s05_spa-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s05_spa-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s05_spa.rsground" | cut -d' ' -f1)" = 2d7416d93cf59bfa5526a50c70738fbdad8d2ba6848662327e841c85c4df134a
test "$(sha256sum "$CANONICAL/tiles/s05_spa_Base.tile" | cut -d' ' -f1)" = 87ccbab6933311ed55b3fbf7febce75c4e2e2d18fbe53dc1f007e5a2d5f09230
test ! -e RESERVE/red_grounds/s05_spa.rsground
test ! -e RESERVE/red_tiles/s05_spa_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s05_spa')
assert entry == {'canonical_debug_id': 239, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00239', 'debug_string_address': '0x08279E5C', 'ground_place_id': 37, 'map_file_id': 249, 'map_id': 240, 'stable_ground_id': 's05_spa', 'weather_id': -1}
...
print('S05_SPA_IDENTITY_PASS map_file_id=249 canonical_debug_id=239 symbol=MAP_LOGO_WARNING')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s05_spa --expected-screenshots 2 \
...
print('S05_SPA_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s05_spa'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s05_spa','primary')]['movement_probe']=='PASS'
assert validations[('s05_spa','primary')]['blocked_probe']=='PASS'
assert validations[('s05_spa','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S05_SPA_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s05_spa.rsground | cut -d' ' -f1)" = 2d7416d93cf59bfa5526a50c70738fbdad8d2ba6848662327e841c85c4df134a
test "$(sha256sum Content/Tile/s05_spa_Base.tile | cut -d' ' -f1)" = 87ccbab6933311ed55b3fbf7febce75c4e2e2d18fbe53dc1f007e5a2d5f09230
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s05_spa.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s05_spa_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s05_spa --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
