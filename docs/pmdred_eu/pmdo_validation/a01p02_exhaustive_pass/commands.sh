...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a01p02')
assert e == {'canonical_debug_id': 172, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00172', 'debug_string_address': '0x0827A65C', 'ground_place_id': 35, 'map_file_id': 173, 'map_id': 164, 'stable_ground_id': 'a01p02', 'weather_id': -1}
assert 'MAP_FUGITIVES_FISSURE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A01P02_IDENTITY_PASS map_id=164 map_file_id=173 symbol=MAP_FUGITIVES_FISSURE')
PYID
...
print('A01P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
