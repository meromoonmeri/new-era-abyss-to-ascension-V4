#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a01p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a01p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a01p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a01p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a01p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a01p02.rsground" | cut -d' ' -f1)" = e5aaf9d09f98955a09f9c53ea0c7ce8663289c511edec0ae503f7b181100896c
test "$(sha256sum "$CANONICAL/tiles/a01p02_Base.tile" | cut -d' ' -f1)" = e501d4761b634b4f5103d484dff170fc1b9606438b67e6e69f8ee5aef144c06f
test "$(sha256sum RESERVE/red_grounds/a01p02.rsground | cut -d' ' -f1)" = 2f1e472a58b5b2359e5ccb4e36b7e2a923b15ec44d368b91e80de78c0a5a6ae5
test "$(sha256sum RESERVE/red_tiles/a01p02_Base.tile | cut -d' ' -f1)" = e501d4761b634b4f5103d484dff170fc1b9606438b67e6e69f8ee5aef144c06f
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a01p02')
assert entry == {'canonical_debug_id': 172, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00172', 'debug_string_address': '0x0827A65C', 'ground_place_id': 35, 'map_file_id': 173, 'map_id': 164, 'stable_ground_id': 'a01p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_FISSURE' in header
print('A01P02_IDENTITY_PASS map_file_id=173 symbol=MAP_FUGITIVES_FISSURE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a01p02 --expected-screenshots 2 \
...
print('A01P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a01p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a01p02','primary')]['movement_probe']=='PASS'
assert validations[('a01p02','primary')]['blocked_probe']=='PASS'
assert validations[('a01p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A01P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a01p02.rsground | cut -d' ' -f1)" = e5aaf9d09f98955a09f9c53ea0c7ce8663289c511edec0ae503f7b181100896c
test "$(sha256sum Content/Tile/a01p02_Base.tile | cut -d' ' -f1)" = e501d4761b634b4f5103d484dff170fc1b9606438b67e6e69f8ee5aef144c06f
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a01p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a01p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a01p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
