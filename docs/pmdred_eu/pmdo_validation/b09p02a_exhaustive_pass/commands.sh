#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b09p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b09p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b09p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b09p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b09p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b09p02a.rsground" | cut -d' ' -f1)" = 199a1846af2b159cc4e701d41538784226a552f00ac36b891fdd2900e6e76c4f
test "$(sha256sum "$CANONICAL/tiles/b09p02a_Base.tile" | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test "$(sha256sum RESERVE/red_grounds/b09p02a.rsground | cut -d' ' -f1)" = 33b2f9dddb8b5efb7814b49606431b48754226e6aeecfeb17b59e3629a534dbb
test "$(sha256sum RESERVE/red_tiles/b09p02a_Base.tile | cut -d' ' -f1)" = 736104cafcda450a114285c37f4afd89c0da686118e1296e068603667727e2c9
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b09p02a')
assert entry == {'canonical_debug_id': 61, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00061', 'debug_string_address': '0x0827B33C', 'ground_place_id': 11, 'map_file_id': 62, 'map_id': 61, 'stable_ground_id': 'b09p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CHARMANDER_CONSTRUCTION' in header
print('B09P02A_IDENTITY_PASS map_file_id=62 symbol=MAP_TEAM_BASE_INSIDE_CHARMANDER_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b09p02a --expected-screenshots 1561 \
...
print('B09P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 4708
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 1561
...
assert r['grounds']==['b09p02a'] and r['sample_count']==1561 and len(primary)==1560
assert r['exact_sample_count']==1561 and r['fully_opaque_sample_count']==1561
assert len({s['actual_rgba_sha256'] for s in primary})==36
...
assert validations[('b09p02a','primary')]['movement_probe']=='PASS'
assert validations[('b09p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b09p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B09P02A_EXHAUSTIVE_PASS samples=1561 unique_rgba=36')
PY
...
test "$(sha256sum Data/Ground/b09p02a.rsground | cut -d' ' -f1)" = 199a1846af2b159cc4e701d41538784226a552f00ac36b891fdd2900e6e76c4f
test "$(sha256sum Content/Tile/b09p02a_Base.tile | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b09p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b09p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b09p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 1561 comparative PNGs,
