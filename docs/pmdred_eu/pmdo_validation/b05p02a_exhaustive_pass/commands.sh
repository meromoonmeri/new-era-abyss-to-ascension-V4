#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b05p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b05p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b05p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b05p02a.rsground" | cut -d' ' -f1)" = 5f76d3decc993635843a55cdc50fed1986db45c146d9612c453a7f57c36e0832
test "$(sha256sum "$CANONICAL/tiles/b05p02a_Base.tile" | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test "$(sha256sum RESERVE/red_grounds/b05p02a.rsground | cut -d' ' -f1)" = d9cd667a5df9ac09bce39124d53ce9e2eb857ce71472683a404a03d6adc1db9b
test "$(sha256sum RESERVE/red_tiles/b05p02a_Base.tile | cut -d' ' -f1)" = 9c6f1beb39b7034bb768b1239413b0b7342fb345065e368eb779fe109d8c10f8
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b05p02a')
assert entry == {'canonical_debug_id': 37, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00037', 'debug_string_address': '0x0827B63C', 'ground_place_id': 11, 'map_file_id': 38, 'map_id': 37, 'stable_ground_id': 'b05p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_SQUIRTLE_CONSTRUCTION' in header
print('B05P02A_IDENTITY_PASS map_file_id=38 symbol=MAP_TEAM_BASE_INSIDE_SQUIRTLE_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b05p02a --expected-screenshots 481 \
...
print('B05P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 1468
...
assert r['grounds']==['b05p02a'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b05p02a','primary')]['movement_probe']=='PASS'
assert validations[('b05p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b05p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B05P02A_EXHAUSTIVE_PASS samples=481 unique_rgba=233')
PY
...
test "$(sha256sum Data/Ground/b05p02a.rsground | cut -d' ' -f1)" = 5f76d3decc993635843a55cdc50fed1986db45c146d9612c453a7f57c36e0832
test "$(sha256sum Content/Tile/b05p02a_Base.tile | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b05p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b05p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b05p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
