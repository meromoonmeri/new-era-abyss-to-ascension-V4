#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h13p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h13p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h13p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h13p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h13p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h13p02.rsground" | cut -d' ' -f1)" = 23e8e9bd249aaef292a365e42553b2945688c1a80cf8107bf5cb1bbe3fbf7c8b
test "$(sha256sum "$CANONICAL/tiles/h13p02_Base.tile" | cut -d' ' -f1)" = e095237f77710aa73f4f5959bb358da62e07e1ea077444188e1fcbb335cc3616
test "$(sha256sum RESERVE/red_grounds/h13p02.rsground | cut -d' ' -f1)" = 7759a2c7ae2d6533f82162da4169e91e8d091bc679dbe825057f33b3746a3e1e
test "$(sha256sum RESERVE/red_tiles/h13p02_Base.tile | cut -d' ' -f1)" = e095237f77710aa73f4f5959bb358da62e07e1ea077444188e1fcbb335cc3616
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h13p02')
assert entry == {'canonical_debug_id': 144, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00144', 'debug_string_address': '0x0827A97C', 'ground_place_id': 9, 'map_file_id': 145, 'map_id': 139, 'stable_ground_id': 'h13p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_DRAGON_CAVE' in header
print('H13P02_IDENTITY_PASS map_file_id=145 symbol=MAP_FRIEND_AREA_DRAGON_CAVE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h13p02 --expected-screenshots 2 \
...
print('H13P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h13p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h13p02','primary')]['movement_probe']=='PASS'
assert validations[('h13p02','primary')]['blocked_probe']=='PASS'
assert validations[('h13p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H13P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h13p02.rsground | cut -d' ' -f1)" = 23e8e9bd249aaef292a365e42553b2945688c1a80cf8107bf5cb1bbe3fbf7c8b
test "$(sha256sum Content/Tile/h13p02_Base.tile | cut -d' ' -f1)" = e095237f77710aa73f4f5959bb358da62e07e1ea077444188e1fcbb335cc3616
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h13p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h13p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h13p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
