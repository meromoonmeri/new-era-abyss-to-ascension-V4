...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d02p01')
assert e == {'canonical_debug_id': 189, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00189', 'debug_string_address': '0x0827A43C', 'ground_place_id': 14, 'map_file_id': 194, 'map_id': 185, 'stable_ground_id': 'd02p01', 'weather_id': -1}
assert 'MAP_SINISTER_WOODS_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D02P01_IDENTITY_PASS map_id=185 map_file_id=194 symbol=MAP_SINISTER_WOODS_END')
PYID
...
print('D02P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
