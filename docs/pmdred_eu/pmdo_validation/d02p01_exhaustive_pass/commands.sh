#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d02p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d02p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d02p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d02p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d02p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d02p01.rsground" | cut -d' ' -f1)" = e9ce13b63db26790d4b6e3ef4e1f8fb48684aefdcc752ada1293bda80f35da70
test "$(sha256sum "$CANONICAL/tiles/d02p01_Base.tile" | cut -d' ' -f1)" = b3326c75641f4b9800e78911e7b512de431f708a335ba15a0df68b3849bd25b6
test "$(sha256sum RESERVE/red_grounds/d02p01.rsground | cut -d' ' -f1)" = d8b62c091d55091b9878875b3d47531064b820386c4b63946076157166ddebe8
test "$(sha256sum RESERVE/red_tiles/d02p01_Base.tile | cut -d' ' -f1)" = b3326c75641f4b9800e78911e7b512de431f708a335ba15a0df68b3849bd25b6
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d02p01')
assert entry == {'canonical_debug_id': 189, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00189', 'debug_string_address': '0x0827A43C', 'ground_place_id': 14, 'map_file_id': 194, 'map_id': 185, 'stable_ground_id': 'd02p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_SINISTER_WOODS_END' in header
print('D02P01_IDENTITY_PASS map_file_id=194 symbol=MAP_SINISTER_WOODS_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d02p01 --expected-screenshots 2 \
...
print('D02P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d02p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d02p01','primary')]['movement_probe']=='PASS'
assert validations[('d02p01','primary')]['blocked_probe']=='PASS'
assert validations[('d02p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D02P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d02p01.rsground | cut -d' ' -f1)" = e9ce13b63db26790d4b6e3ef4e1f8fb48684aefdcc752ada1293bda80f35da70
test "$(sha256sum Content/Tile/d02p01_Base.tile | cut -d' ' -f1)" = b3326c75641f4b9800e78911e7b512de431f708a335ba15a0df68b3849bd25b6
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d02p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d02p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d02p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
