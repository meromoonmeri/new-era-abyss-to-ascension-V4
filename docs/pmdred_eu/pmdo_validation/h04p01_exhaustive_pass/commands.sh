#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h04p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h04p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h04p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h04p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h04p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h04p01.rsground" | cut -d' ' -f1)" = ca703e7dec616302082a411e9201d4415a1b9adfe26406280424578f2799fee8
test "$(sha256sum "$CANONICAL/tiles/h04p01_Base.tile" | cut -d' ' -f1)" = 807537e29bf8e3d814b0d1800dab9dde8e09ae1aa3b4c3db482c60a4d8829a73
test "$(sha256sum RESERVE/red_grounds/h04p01.rsground | cut -d' ' -f1)" = e3c52880138d1cc2a0b41312f2554fec7285ea477e69f26c34879e0e04c08000
test "$(sha256sum RESERVE/red_tiles/h04p01_Base.tile | cut -d' ' -f1)" = 807537e29bf8e3d814b0d1800dab9dde8e09ae1aa3b4c3db482c60a4d8829a73
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h04p01')
assert entry == {'canonical_debug_id': 117, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00117', 'debug_string_address': '0x0827ACBC', 'ground_place_id': 9, 'map_file_id': 118, 'map_id': 113, 'stable_ground_id': 'h04p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MT_CLEFT' in header
print('H04P01_IDENTITY_PASS map_file_id=118 symbol=MAP_FRIEND_AREA_MT_CLEFT')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h04p01 --expected-screenshots 2 \
...
print('H04P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h04p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h04p01','primary')]['movement_probe']=='PASS'
assert validations[('h04p01','primary')]['blocked_probe']=='PASS'
assert validations[('h04p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H04P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h04p01.rsground | cut -d' ' -f1)" = ca703e7dec616302082a411e9201d4415a1b9adfe26406280424578f2799fee8
test "$(sha256sum Content/Tile/h04p01_Base.tile | cut -d' ' -f1)" = 807537e29bf8e3d814b0d1800dab9dde8e09ae1aa3b4c3db482c60a4d8829a73
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h04p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h04p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h04p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
