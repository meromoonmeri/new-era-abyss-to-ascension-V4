#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p02b.rsground" | cut -d' ' -f1)" = 28ebd8a9c6307d9db258c4c114969465cb00d1be731b3cf87f38f988b39a598b
test "$(sha256sum "$CANONICAL/tiles/b13p02b_Base.tile" | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
test "$(sha256sum RESERVE/red_grounds/b13p02b.rsground | cut -d' ' -f1)" = b058f9cf3963ba18d919761cd22f90dba9d226a5f7050c870770dc52528d404a
test "$(sha256sum RESERVE/red_tiles/b13p02b_Base.tile | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02b')
assert entry == {'canonical_debug_id': 86, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00086', 'debug_string_address': '0x0827B01C', 'ground_place_id': 11, 'map_file_id': 87, 'map_id': 86, 'stable_ground_id': 'b13p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_FINAL' in header
print('B13P02B_IDENTITY_PASS map_file_id=87 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p02b --expected-screenshots 2 \
...
print('B13P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b13p02b','primary')]['movement_probe']=='PASS'
assert validations[('b13p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b13p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b13p02b.rsground | cut -d' ' -f1)" = 28ebd8a9c6307d9db258c4c114969465cb00d1be731b3cf87f38f988b39a598b
test "$(sha256sum Content/Tile/b13p02b_Base.tile | cut -d' ' -f1)" = 47a9022017af4fa5b9b1dad3144488376961ca40c0d658a99dd0b3818d7f2081
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
