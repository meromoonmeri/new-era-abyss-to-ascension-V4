#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b16p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b16p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b16p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b16p01b.rsground" | cut -d' ' -f1)" = 862849529ba0b4b7d8e3da9657c27adb74a4a8fe218479978c371243c25f0784
test "$(sha256sum "$CANONICAL/tiles/b16p01b_Base.tile" | cut -d' ' -f1)" = ef9f78608165cb285cf0495e77d2cd6e8c96e9499c19206e2548de93fb0ed623
test "$(sha256sum RESERVE/red_grounds/b16p01b.rsground | cut -d' ' -f1)" = 15e6a4c48a41ec7598bff66b0d59a5d686a6d60ea27fb46980ca6f6a6d4eb2e2
test "$(sha256sum RESERVE/red_tiles/b16p01b_Base.tile | cut -d' ' -f1)" = ef9f78608165cb285cf0495e77d2cd6e8c96e9499c19206e2548de93fb0ed623
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p01b')
assert entry == {'canonical_debug_id': 101, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00101', 'debug_string_address': '0x0827AE3C', 'ground_place_id': 10, 'map_file_id': 102, 'map_id': 101, 'stable_ground_id': 'b16p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TREECKO_FINAL' in header
print('B16P01B_IDENTITY_PASS map_file_id=102 symbol=MAP_TEAM_BASE_TREECKO_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b16p01b --expected-screenshots 25 \
...
print('B16P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b16p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b16p01b','primary')]['movement_probe']=='PASS'
assert validations[('b16p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b16p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B16P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b16p01b.rsground | cut -d' ' -f1)" = 862849529ba0b4b7d8e3da9657c27adb74a4a8fe218479978c371243c25f0784
test "$(sha256sum Content/Tile/b16p01b_Base.tile | cut -d' ' -f1)" = ef9f78608165cb285cf0495e77d2cd6e8c96e9499c19206e2548de93fb0ed623
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b16p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b16p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b16p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
