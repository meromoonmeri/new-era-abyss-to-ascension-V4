#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p04 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p04.rsground" | cut -d' ' -f1)" = a69c6c00e133bc28b80b2a4ac818eeaf1a33fb24d9e6ec09f067a4ae745f465f
test "$(sha256sum "$CANONICAL/tiles/h06p04_Base.tile" | cut -d' ' -f1)" = ed69356fa15276b2d86f3df27aaafd563ae56dd75e3ba9a70ac5cab93ddc1bf7
test "$(sha256sum RESERVE/red_grounds/h06p04.rsground | cut -d' ' -f1)" = 2a814a17f015da8e4cc4875266961fbb2072428a39c8a29408f37d52d9cb5b60
test "$(sha256sum RESERVE/red_tiles/h06p04_Base.tile | cut -d' ' -f1)" = ed69356fa15276b2d86f3df27aaafd563ae56dd75e3ba9a70ac5cab93ddc1bf7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p04')
assert entry == {'canonical_debug_id': 124, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00124', 'debug_string_address': '0x0827ABDC', 'ground_place_id': 9, 'map_file_id': 125, 'map_id': 120, 'stable_ground_id': 'h06p04', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_SCORCHED_PLAINS' in header
print('H06P04_IDENTITY_PASS map_file_id=125 symbol=MAP_FRIEND_AREA_SCORCHED_PLAINS')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p04 --expected-screenshots 2 \
...
print('H06P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h06p04'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h06p04','primary')]['movement_probe']=='PASS'
assert validations[('h06p04','primary')]['blocked_probe']=='PASS'
assert validations[('h06p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h06p04.rsground | cut -d' ' -f1)" = a69c6c00e133bc28b80b2a4ac818eeaf1a33fb24d9e6ec09f067a4ae745f465f
test "$(sha256sum Content/Tile/h06p04_Base.tile | cut -d' ' -f1)" = ed69356fa15276b2d86f3df27aaafd563ae56dd75e3ba9a70ac5cab93ddc1bf7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
