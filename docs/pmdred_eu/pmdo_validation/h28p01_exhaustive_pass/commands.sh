...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h28p01')
assert e == {'canonical_debug_id': 164, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00164', 'debug_string_address': '0x0827A73C', 'ground_place_id': 9, 'map_file_id': 165, 'map_id': 157, 'stable_ground_id': 'h28p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_MAGNETIC_QUARRY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H28P01_IDENTITY_PASS map_id=157 map_file_id=165 symbol=MAP_FRIEND_AREA_MAGNETIC_QUARRY')
PYID
...
print('H28P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
