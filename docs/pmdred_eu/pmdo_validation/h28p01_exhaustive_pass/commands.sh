#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h28p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h28p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h28p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h28p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h28p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h28p01.rsground" | cut -d' ' -f1)" = c3793e9d3b5751a6052d772d866b37ffb6cc7bc32b60811fd9f5d5323bcc5192
test "$(sha256sum "$CANONICAL/tiles/h28p01_Base.tile" | cut -d' ' -f1)" = 8a1b702d5cd0e12fe4d8cc20f5b594600ea9ab7db212b60de98af7e65d023ff7
test "$(sha256sum RESERVE/red_grounds/h28p01.rsground | cut -d' ' -f1)" = c0596704d5ebe133e325cd2d23bf52395c85093d17a9f89fb9ba835606916aed
test "$(sha256sum RESERVE/red_tiles/h28p01_Base.tile | cut -d' ' -f1)" = 8a1b702d5cd0e12fe4d8cc20f5b594600ea9ab7db212b60de98af7e65d023ff7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h28p01')
assert entry == {'canonical_debug_id': 164, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00164', 'debug_string_address': '0x0827A73C', 'ground_place_id': 9, 'map_file_id': 165, 'map_id': 157, 'stable_ground_id': 'h28p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MAGNETIC_QUARRY' in header
print('H28P01_IDENTITY_PASS map_file_id=165 symbol=MAP_FRIEND_AREA_MAGNETIC_QUARRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h28p01 --expected-screenshots 2 \
...
print('H28P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h28p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h28p01','primary')]['movement_probe']=='PASS'
assert validations[('h28p01','primary')]['blocked_probe']=='PASS'
assert validations[('h28p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H28P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h28p01.rsground | cut -d' ' -f1)" = c3793e9d3b5751a6052d772d866b37ffb6cc7bc32b60811fd9f5d5323bcc5192
test "$(sha256sum Content/Tile/h28p01_Base.tile | cut -d' ' -f1)" = 8a1b702d5cd0e12fe4d8cc20f5b594600ea9ab7db212b60de98af7e65d023ff7
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h28p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h28p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h28p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
