#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b01p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b01p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b01p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b01p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b01p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b01p02b.rsground" | cut -d' ' -f1)" = 7d9c238334e06763df5468c85ed67568e7a41104d5f74a3d42fb96d0c176dc6e
test "$(sha256sum "$CANONICAL/tiles/b01p02b_Base.tile" | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
test "$(sha256sum RESERVE/red_grounds/b01p02b.rsground | cut -d' ' -f1)" = b23349ac4e73e2f8ea0ddbae56945924cdfb283ad1d5d0c1e11dc406296615dd
test "$(sha256sum RESERVE/red_tiles/b01p02b_Base.tile | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b01p02b')
assert entry == {'canonical_debug_id': 14, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00014', 'debug_string_address': '0x0827B91C', 'ground_place_id': 11, 'map_file_id': 15, 'map_id': 14, 'stable_ground_id': 'b01p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_PIKACHU_FINAL' in header
print('B01P02B_IDENTITY_PASS map_file_id=15 symbol=MAP_TEAM_BASE_INSIDE_PIKACHU_FINAL')
PYID
...
assert r['grounds']==['b01p02b'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b01p02b','primary')]['movement_probe']=='PASS'
assert validations[('b01p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b01p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B01P02B_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b01p02b.rsground | cut -d' ' -f1)" = 7d9c238334e06763df5468c85ed67568e7a41104d5f74a3d42fb96d0c176dc6e
test "$(sha256sum Content/Tile/b01p02b_Base.tile | cut -d' ' -f1)" = ec8f5ab4cfea9b73de418a74b4c104b3eb564386ad556a9e391e0e4f70aa6cbe
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b01p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b01p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b01p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
