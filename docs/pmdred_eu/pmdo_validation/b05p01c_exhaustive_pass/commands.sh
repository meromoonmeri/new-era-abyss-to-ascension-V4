#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b05p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b05p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b05p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b05p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b05p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b05p01c.rsground" | cut -d' ' -f1)" = 88018d6116f0332b16f577d217167b448cb3d5420d984a04ae4e19b345094327
test "$(sha256sum "$CANONICAL/tiles/b05p01c_Base.tile" | cut -d' ' -f1)" = 3466ff6dbae5bcfce5dca4bed313265859834ada5f402aaebab6cae8967264ad
test "$(sha256sum RESERVE/red_grounds/b05p01c.rsground | cut -d' ' -f1)" = df7c512e5449b4813eee2eed513e6134adedda153cb2791efbe376ba4900513b
test "$(sha256sum RESERVE/red_tiles/b05p01c_Base.tile | cut -d' ' -f1)" = 3466ff6dbae5bcfce5dca4bed313265859834ada5f402aaebab6cae8967264ad
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b05p01c')
assert entry == {'canonical_debug_id': 36, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00036', 'debug_string_address': '0x0827B65C', 'ground_place_id': 10, 'map_file_id': 37, 'map_id': 36, 'stable_ground_id': 'b05p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_SQUIRTLE_BASIC' in header
print('B05P01C_IDENTITY_PASS map_file_id=37 symbol=MAP_TEAM_BASE_INSIDE_SQUIRTLE_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b05p01c --expected-screenshots 51 \
...
print('B05P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 178
...
assert r['grounds']==['b05p01c'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b05p01c','primary')]['movement_probe']=='PASS'
assert validations[('b05p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b05p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B05P01C_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b05p01c.rsground | cut -d' ' -f1)" = 88018d6116f0332b16f577d217167b448cb3d5420d984a04ae4e19b345094327
test "$(sha256sum Content/Tile/b05p01c_Base.tile | cut -d' ' -f1)" = 3466ff6dbae5bcfce5dca4bed313265859834ada5f402aaebab6cae8967264ad
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b05p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b05p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b05p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
