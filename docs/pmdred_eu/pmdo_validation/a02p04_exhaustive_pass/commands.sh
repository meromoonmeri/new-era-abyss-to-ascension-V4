#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a02p04 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a02p04.rsground" | cut -d' ' -f1)" = 15d18cb20ccaadb38887e2c3c722e2b5983d3184f1409960bed15c951451a209
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a02p04 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a02p04.rsground" | cut -d' ' -f1)" = 9add5df8383ddb077c832ddae9e06287ad29cd6db695437b7d1f655af8a2c790
test "$(sha256sum "$CANONICAL/tiles/a02p04_Base.tile" | cut -d' ' -f1)" = 2270fe64e93dca7b7c15dca06c8e87e7d4b1aeae4ff558ad9c86853207eb7ea0
test "$(sha256sum RESERVE/red_grounds/a02p04.rsground | cut -d' ' -f1)" = 5a91344deb7e0c8682aa421486d603592d53d12df5b8c8476ffb1b026f88daed
test "$(sha256sum RESERVE/red_tiles/a02p04_Base.tile | cut -d' ' -f1)" = 2270fe64e93dca7b7c15dca06c8e87e7d4b1aeae4ff558ad9c86853207eb7ea0
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p04')
assert entry == {'canonical_debug_id': 176, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00176', 'debug_string_address': '0x0827A5DC', 'ground_place_id': 35, 'map_file_id': 177, 'map_id': 168, 'stable_ground_id': 'a02p04', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_SNOW_ROAD' in header
print('A02P04_IDENTITY_PASS map_file_id=177 symbol=MAP_FUGITIVES_SNOW_ROAD')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a02p04 --expected-screenshots 641 \
...
print('A02P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 1945
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 641
...
assert r['grounds']==['a02p04'] and r['sample_count']==641 and len(primary)==640
assert r['exact_sample_count']==641 and r['fully_opaque_sample_count']==641
assert len({s['actual_rgba_sha256'] for s in primary})==150
...
assert validations[('a02p04','primary')]['movement_probe']=='PASS'
assert validations[('a02p04','primary')]['blocked_probe']=='PASS'
assert validations[('a02p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A02P04_EXHAUSTIVE_PASS samples=641 unique_rgba=150')
PY
...
test "$(sha256sum Data/Ground/a02p04.rsground | cut -d' ' -f1)" = 9add5df8383ddb077c832ddae9e06287ad29cd6db695437b7d1f655af8a2c790
test "$(sha256sum Content/Tile/a02p04_Base.tile | cut -d' ' -f1)" = 2270fe64e93dca7b7c15dca06c8e87e7d4b1aeae4ff558ad9c86853207eb7ea0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a02p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a02p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a02p04 --entity-integrated-ids a02p04 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 641 comparative PNGs,
# rather than the three durable representative captures, are desired.
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p04/a02p04.rsground | cut -d' ' -f1)" = b792870dc610761c89fe9fd5c26fcee343714311b04d9a17a52521fa5c9f738f
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p04/A02p04_Base.tile | cut -d' ' -f1)" = 2270fe64e93dca7b7c15dca06c8e87e7d4b1aeae4ff558ad9c86853207eb7ea0
