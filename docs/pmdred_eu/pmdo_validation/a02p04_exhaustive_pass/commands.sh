...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p04-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p04')
assert e == {'canonical_debug_id': 176, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00176', 'debug_string_address': '0x0827A5DC', 'ground_place_id': 35, 'map_file_id': 177, 'map_id': 168, 'stable_ground_id': 'a02p04', 'weather_id': -1}
assert 'MAP_FUGITIVES_SNOW_ROAD' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A02P04_IDENTITY_PASS map_id=168 map_file_id=177 symbol=MAP_FUGITIVES_SNOW_ROAD')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a02p04 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a02p04.rsground" | cut -d' ' -f1)" = 9add5df8383ddb077c832ddae9e06287ad29cd6db695437b7d1f655af8a2c790
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 1942
...
print('A02P04_EXHAUSTIVE_PASS samples=641 unique_rgba=150 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a02p04 --plan "$PLAN" \
  --ids a02p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
