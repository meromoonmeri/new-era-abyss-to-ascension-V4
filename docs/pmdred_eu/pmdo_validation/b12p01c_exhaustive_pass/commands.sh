#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p01c.rsground" | cut -d' ' -f1)" = 0c8dadff5ef13798abbd8825d0b5701fe70933a42ce896c692a9a842e5835aad
test "$(sha256sum "$CANONICAL/tiles/b12p01c_Base.tile" | cut -d' ' -f1)" = ce807381860336ed36eec49d4c11f3d42709a3a7e521589cef4e3cb6445a9751
test "$(sha256sum RESERVE/red_grounds/b12p01c.rsground | cut -d' ' -f1)" = 6d759ac09e396f87b3df4eb35f15606d4309b5462c3935d31a148ba9935e0fff
test "$(sha256sum RESERVE/red_tiles/b12p01c_Base.tile | cut -d' ' -f1)" = ce807381860336ed36eec49d4c11f3d42709a3a7e521589cef4e3cb6445a9751
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p01c')
assert entry == {'canonical_debug_id': 78, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00078', 'debug_string_address': '0x0827B11C', 'ground_place_id': 10, 'map_file_id': 79, 'map_id': 78, 'stable_ground_id': 'b12p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CUBONE_BASIC' in header
print('B12P01C_IDENTITY_PASS map_file_id=79 symbol=MAP_TEAM_BASE_INSIDE_CUBONE_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p01c --expected-screenshots 25 \
...
print('B12P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b12p01c','primary')]['movement_probe']=='PASS'
assert validations[('b12p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b12p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b12p01c.rsground | cut -d' ' -f1)" = 0c8dadff5ef13798abbd8825d0b5701fe70933a42ce896c692a9a842e5835aad
test "$(sha256sum Content/Tile/b12p01c_Base.tile | cut -d' ' -f1)" = ce807381860336ed36eec49d4c11f3d42709a3a7e521589cef4e3cb6445a9751
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
