#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p02a.rsground" | cut -d' ' -f1)" = 588671612d683ce17537921ed05731de582302572593678bd3440839bfc4e46d
test "$(sha256sum "$CANONICAL/tiles/b06p02a_Base.tile" | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p02a')
assert entry == {'canonical_debug_id': 43, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00043', 'debug_string_address': '0x0827B57C', 'ground_place_id': 11, 'map_file_id': 44, 'map_id': 43, 'stable_ground_id': 'b06p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TOTODILE_CONSTRUCTION' in header
print('B06P02A_IDENTITY_PASS map_id=43 map_file_id=44 symbol=MAP_TEAM_BASE_INSIDE_TOTODILE_CONSTRUCTION')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 1465
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 481
"$PYTHON" - "$FIX/termination.json" <<'PYTERM'
import json,sys
t=json.load(open(sys.argv[1]));x=t['termination']
assert t['terminal_seen'] and t['requested_signal_name']=='SIGINT'
assert t['requested_signal_sent'] and not t['residual_processes_before_cleanup']
assert not t['residual_processes_after_cleanup']
assert x['result']=='PASS' and x['kind'] in ('EXPECTED_SIGINT_SIGNAL','EXPECTED_SIGINT_EXIT_CODE')
assert not x['segmentation_fault'] and not x['forced_kill']
print('PMDO_SIGINT_TERMINATION_PASS kind='+x['kind'])
PYTERM
...
assert r['grounds']==['b06p02a'] and r['sample_count']==481 and len(primary)==480
assert r['exact_sample_count']==481 and r['fully_opaque_sample_count']==481
assert len({s['actual_rgba_sha256'] for s in primary})==233
...
assert validations[('b06p02a','primary')]['movement_probe']=='PASS'
assert validations[('b06p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b06p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P02A_EXHAUSTIVE_PASS samples=481 unique_rgba=233 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b06p02a.rsground | cut -d' ' -f1)" = 588671612d683ce17537921ed05731de582302572593678bd3440839bfc4e46d
test "$(sha256sum Content/Tile/b06p02a_Base.tile | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 481 comparative PNGs,
