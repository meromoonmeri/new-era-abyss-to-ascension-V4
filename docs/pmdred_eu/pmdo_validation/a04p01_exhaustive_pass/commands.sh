#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a04p01 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a04p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a04p01.rsground" | cut -d' ' -f1)" = b511cfe2a90b46a3c01ba4809582bfcef63aff4898b73ff5970e8c5d08086c92
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a04p01 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a04p01.rsground" | cut -d' ' -f1)" = c6b2b6938f37f2c5f4d10f0686c7b7734aab500144ed036ecc5eeee53c79d6af
test "$(sha256sum "$CANONICAL/tiles/a04p01_Base.tile" | cut -d' ' -f1)" = b376149a921c935a8f803b36559627302fb0fb5f00832a602d5c83a54af28adf
test "$(sha256sum RESERVE/red_grounds/a04p01.rsground | cut -d' ' -f1)" = 8ca6d9d60743b11b98c8d30fabd47c8108871187a2018f8f7df678ef7162f230
test "$(sha256sum RESERVE/red_tiles/a04p01_Base.tile | cut -d' ' -f1)" = 98bfcfafcfde3d0abbff80861148c34699ba506de5f0aa0c10697a791166908d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p01')
assert entry == {'canonical_debug_id': 180, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00180', 'debug_string_address': '0x0827A55C', 'ground_place_id': 21, 'map_file_id': 181, 'map_id': 172, 'stable_ground_id': 'a04p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_SKY_TOWER' in header
print('A04P01_IDENTITY_PASS map_file_id=181 symbol=MAP_SKY_TOWER')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a04p01 --expected-screenshots 136 \
...
print('A04P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 430
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 136
...
assert r['grounds']==['a04p01'] and r['sample_count']==136 and len(primary)==135
assert r['exact_sample_count']==136 and r['fully_opaque_sample_count']==136
assert len({s['actual_rgba_sha256'] for s in primary})==62
...
assert validations[('a04p01','primary')]['movement_probe']=='PASS'
assert validations[('a04p01','primary')]['blocked_probe']=='PASS'
assert validations[('a04p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A04P01_EXHAUSTIVE_PASS samples=136 unique_rgba=62')
PY
...
test "$(sha256sum Data/Ground/a04p01.rsground | cut -d' ' -f1)" = c6b2b6938f37f2c5f4d10f0686c7b7734aab500144ed036ecc5eeee53c79d6af
test "$(sha256sum Content/Tile/a04p01_Base.tile | cut -d' ' -f1)" = b376149a921c935a8f803b36559627302fb0fb5f00832a602d5c83a54af28adf
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a04p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a04p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a04p01 --entity-integrated-ids a04p01 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 136 comparative PNGs,
# rather than the three durable representative captures, are desired.
test "$(sha256sum RESERVE/pmdred_pre_promotion/a04p01/a04p01.rsground | cut -d' ' -f1)" = c44840e6dca2e1462f98b63204f4d60f5b6a86304b04c81faa379ae416039a5b
test "$(sha256sum RESERVE/pmdred_pre_promotion/a04p01/a04p01_Base.tile | cut -d' ' -f1)" = b376149a921c935a8f803b36559627302fb0fb5f00832a602d5c83a54af28adf
