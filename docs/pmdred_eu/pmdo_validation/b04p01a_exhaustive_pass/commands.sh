#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b04p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b04p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b04p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b04p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b04p01a.rsground" | cut -d' ' -f1)" = 26ec7bf821c6c2dd997abc97e401f293ee50c45c73e144c054ccb4134104d6e9
test "$(sha256sum "$CANONICAL/tiles/b04p01a_Base.tile" | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test "$(sha256sum RESERVE/red_grounds/b04p01a.rsground | cut -d' ' -f1)" = edfc021ad0ff0d7af4286a7cb873a9a208d1e083bc1b4f5e936b5a74d066f408
test "$(sha256sum RESERVE/red_tiles/b04p01a_Base.tile | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b04p01a')
assert entry == {'canonical_debug_id': 28, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00028', 'debug_string_address': '0x0827B75C', 'ground_place_id': 10, 'map_file_id': 29, 'map_id': 28, 'stable_ground_id': 'b04p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_SKITTY_CONSTRUCTION' in header
print('B04P01A_IDENTITY_PASS map_file_id=29 symbol=MAP_TEAM_BASE_SKITTY_CONSTRUCTION')
PYID
...
assert r['grounds']==['b04p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b04p01a','primary')]['movement_probe']=='PASS'
assert validations[('b04p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b04p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B04P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b04p01a.rsground | cut -d' ' -f1)" = 26ec7bf821c6c2dd997abc97e401f293ee50c45c73e144c054ccb4134104d6e9
test "$(sha256sum Content/Tile/b04p01a_Base.tile | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b04p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b04p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b04p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
