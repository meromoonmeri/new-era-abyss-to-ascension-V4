#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b14p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b14p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b14p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b14p01b.rsground" | cut -d' ' -f1)" = af824627636f0f3ee05c8007e2366125f5953a64d6683cab1cb9ac5ba423e263
test "$(sha256sum "$CANONICAL/tiles/b14p01b_Base.tile" | cut -d' ' -f1)" = 2da8007cb08166c1978a830cae00560e83ea3e0a220a7b6138ea46aa66de8e7d
test "$(sha256sum RESERVE/red_grounds/b14p01b.rsground | cut -d' ' -f1)" = a2118f85bfc3ed9280588d3b6688550ce5ed32ef15a1a9fe7e8693246833f694
test "$(sha256sum RESERVE/red_tiles/b14p01b_Base.tile | cut -d' ' -f1)" = 2da8007cb08166c1978a830cae00560e83ea3e0a220a7b6138ea46aa66de8e7d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p01b')
assert entry == {'canonical_debug_id': 89, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00089', 'debug_string_address': '0x0827AFBC', 'ground_place_id': 10, 'map_file_id': 90, 'map_id': 89, 'stable_ground_id': 'b14p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_BULBASAUR_FINAL' in header
print('B14P01B_IDENTITY_PASS map_file_id=90 symbol=MAP_TEAM_BASE_BULBASAUR_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b14p01b --expected-screenshots 25 \
...
print('B14P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b14p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b14p01b','primary')]['movement_probe']=='PASS'
assert validations[('b14p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b14p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B14P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b14p01b.rsground | cut -d' ' -f1)" = af824627636f0f3ee05c8007e2366125f5953a64d6683cab1cb9ac5ba423e263
test "$(sha256sum Content/Tile/b14p01b_Base.tile | cut -d' ' -f1)" = 2da8007cb08166c1978a830cae00560e83ea3e0a220a7b6138ea46aa66de8e7d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b14p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b14p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b14p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
