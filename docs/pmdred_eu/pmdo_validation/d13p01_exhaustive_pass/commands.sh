#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d13p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d13p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d13p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d13p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d13p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d13p01.rsground" | cut -d' ' -f1)" = b38c698355225a1218310029108798e023309bc9c1b1c27fdbde4f4cb664e5f4
test "$(sha256sum "$CANONICAL/tiles/d13p01_Base.tile" | cut -d' ' -f1)" = 65d558e37ce64b04503878f9ab205c7dc0822ecefa39055ea796f7513bfc4c35
test "$(sha256sum RESERVE/red_grounds/d13p01.rsground | cut -d' ' -f1)" = dddf6faa9f9197f66645fcf6d5c150c451942e9b0c2016861e656afe023cd921
test "$(sha256sum RESERVE/red_tiles/d13p01_Base.tile | cut -d' ' -f1)" = ea329f82be27340d6d186e3f9d0e1e09a5337ebc9ef5666619c16daddf810969
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d13p01')
assert entry == {'canonical_debug_id': 216, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00216', 'debug_string_address': '0x0827A0DC', 'ground_place_id': 32, 'map_file_id': 221, 'map_id': 212, 'stable_ground_id': 'd13p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_D16' in header
print('D13P01_IDENTITY_PASS map_file_id=221 symbol=MAP_D16')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d13p01 --expected-screenshots 33 \
...
print('D13P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d13p01'] and r['sample_count']==33 and len(primary)==32
...
assert validations[('d13p01','primary')]['movement_probe']=='PASS'
assert validations[('d13p01','primary')]['blocked_probe']=='PASS'
assert validations[('d13p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D13P01_EXHAUSTIVE_PASS samples=33 unique_rgba=7')
PY
...
test "$(sha256sum Data/Ground/d13p01.rsground | cut -d' ' -f1)" = b38c698355225a1218310029108798e023309bc9c1b1c27fdbde4f4cb664e5f4
test "$(sha256sum Content/Tile/d13p01_Base.tile | cut -d' ' -f1)" = 65d558e37ce64b04503878f9ab205c7dc0822ecefa39055ea796f7513bfc4c35
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d13p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d13p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d13p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
