...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d13p01')
assert e == {'canonical_debug_id': 216, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00216', 'debug_string_address': '0x0827A0DC', 'ground_place_id': 32, 'map_file_id': 221, 'map_id': 212, 'stable_ground_id': 'd13p01', 'weather_id': -1}
assert 'MAP_D16' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D13P01_IDENTITY_PASS map_id=212 map_file_id=221 symbol=MAP_D16')
PYID
...
print('D13P01_EXHAUSTIVE_PASS samples=33 unique_rgba=7 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
