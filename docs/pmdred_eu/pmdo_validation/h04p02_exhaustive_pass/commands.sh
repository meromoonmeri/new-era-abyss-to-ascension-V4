#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h04p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h04p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h04p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h04p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h04p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h04p02.rsground" | cut -d' ' -f1)" = 369b48f6b2239820a3ee5e51836e1d8e4cac9f125cf403c06e4ca2940725ab3d
test "$(sha256sum "$CANONICAL/tiles/h04p02_Base.tile" | cut -d' ' -f1)" = 6abbbbb25d241379b2cc8788263fa621a03428e388e4371bceddd551bf49ffee
test "$(sha256sum RESERVE/red_grounds/h04p02.rsground | cut -d' ' -f1)" = c8ef6d355930bfff017811154a8dfc6a41fa195fb3bf61e1dd5882440a3468cc
test "$(sha256sum RESERVE/red_tiles/h04p02_Base.tile | cut -d' ' -f1)" = 6abbbbb25d241379b2cc8788263fa621a03428e388e4371bceddd551bf49ffee
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h04p02')
assert entry == {'canonical_debug_id': 118, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00118', 'debug_string_address': '0x0827AC9C', 'ground_place_id': 9, 'map_file_id': 119, 'map_id': 114, 'stable_ground_id': 'h04p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MT_MOONVIEW' in header
print('H04P02_IDENTITY_PASS map_file_id=119 symbol=MAP_FRIEND_AREA_MT_MOONVIEW')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h04p02 --expected-screenshots 2 \
...
print('H04P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h04p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h04p02','primary')]['movement_probe']=='PASS'
assert validations[('h04p02','primary')]['blocked_probe']=='PASS'
assert validations[('h04p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H04P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h04p02.rsground | cut -d' ' -f1)" = 369b48f6b2239820a3ee5e51836e1d8e4cac9f125cf403c06e4ca2940725ab3d
test "$(sha256sum Content/Tile/h04p02_Base.tile | cut -d' ' -f1)" = 6abbbbb25d241379b2cc8788263fa621a03428e388e4371bceddd551bf49ffee
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h04p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h04p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h04p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
