#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b01p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b01p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b01p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b01p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b01p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b01p01b.rsground" | cut -d' ' -f1)" = 7ad1758b66c7aec996a35d56b2298bc35a7605f2c1b01c5fd66753f71edbaabb
test "$(sha256sum "$CANONICAL/tiles/b01p01b_Base.tile" | cut -d' ' -f1)" = 61c7bf8768db5e0b0769e5235392d493ba16310355fd3bdf79a9d5015119f812
test "$(sha256sum RESERVE/red_grounds/b01p01b.rsground | cut -d' ' -f1)" = c32e48d187a76dbd9d3fa50cbaf1cec48bcc33db83307a9627c77d867b70e4c0
test "$(sha256sum RESERVE/red_tiles/b01p01b_Base.tile | cut -d' ' -f1)" = 61c7bf8768db5e0b0769e5235392d493ba16310355fd3bdf79a9d5015119f812
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b01p01b')
assert entry == {'canonical_debug_id': 11, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00011', 'debug_string_address': '0x0827B97C', 'ground_place_id': 10, 'map_file_id': 12, 'map_id': 11, 'stable_ground_id': 'b01p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_PIKACHU_FINAL' in header
print('B01P01B_IDENTITY_PASS map_file_id=12 symbol=MAP_TEAM_BASE_PIKACHU_FINAL')
PYID
...
assert r['grounds']==['b01p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b01p01b','primary')]['movement_probe']=='PASS'
assert validations[('b01p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b01p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B01P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b01p01b.rsground | cut -d' ' -f1)" = 7ad1758b66c7aec996a35d56b2298bc35a7605f2c1b01c5fd66753f71edbaabb
test "$(sha256sum Content/Tile/b01p01b_Base.tile | cut -d' ' -f1)" = 61c7bf8768db5e0b0769e5235392d493ba16310355fd3bdf79a9d5015119f812
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b01p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b01p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b01p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
