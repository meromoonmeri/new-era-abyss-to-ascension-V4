...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h05p02')
assert e == {'canonical_debug_id': 120, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00120', 'debug_string_address': '0x0827AC5C', 'ground_place_id': 9, 'map_file_id': 121, 'map_id': 116, 'stable_ground_id': 'h05p02', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_WILD_PLAINS' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H05P02_IDENTITY_PASS map_id=116 map_file_id=121 symbol=MAP_FRIEND_AREA_WILD_PLAINS')
PYID
...
print('H05P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
