#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h05p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h05p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h05p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h05p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h05p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h05p02.rsground" | cut -d' ' -f1)" = 10496681f6b5f911ef6a3fbe08800a479199627f64b5fbb4e48914999bd919db
test "$(sha256sum "$CANONICAL/tiles/h05p02_Base.tile" | cut -d' ' -f1)" = a8f57ae94079804d759d2e7d97b01008353cb9d85df6e0bcd4e23b7490103484
test "$(sha256sum RESERVE/red_grounds/h05p02.rsground | cut -d' ' -f1)" = 53601a1613289cae8d8e7bfa4916264b8268888b0331be81ccab6d0da9832ac1
test "$(sha256sum RESERVE/red_tiles/h05p02_Base.tile | cut -d' ' -f1)" = a8f57ae94079804d759d2e7d97b01008353cb9d85df6e0bcd4e23b7490103484
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h05p02')
assert entry == {'canonical_debug_id': 120, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00120', 'debug_string_address': '0x0827AC5C', 'ground_place_id': 9, 'map_file_id': 121, 'map_id': 116, 'stable_ground_id': 'h05p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_WILD_PLAINS' in header
print('H05P02_IDENTITY_PASS map_file_id=121 symbol=MAP_FRIEND_AREA_WILD_PLAINS')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h05p02 --expected-screenshots 2 \
...
print('H05P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h05p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h05p02','primary')]['movement_probe']=='PASS'
assert validations[('h05p02','primary')]['blocked_probe']=='PASS'
assert validations[('h05p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H05P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h05p02.rsground | cut -d' ' -f1)" = 10496681f6b5f911ef6a3fbe08800a479199627f64b5fbb4e48914999bd919db
test "$(sha256sum Content/Tile/h05p02_Base.tile | cut -d' ' -f1)" = a8f57ae94079804d759d2e7d97b01008353cb9d85df6e0bcd4e23b7490103484
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h05p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h05p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h05p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
