#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b08p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b08p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b08p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b08p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b08p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b08p02a.rsground" | cut -d' ' -f1)" = 8fcc0e9bfea0139fc9fee6b76cb9d37dedfee84d8ae1fef26d5bc89cafadd42b
test "$(sha256sum "$CANONICAL/tiles/b08p02a_Base.tile" | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test "$(sha256sum RESERVE/red_grounds/b08p02a.rsground | cut -d' ' -f1)" = 52f0fc0c2cea2a90a945a4a0f42f0da5f3c2de2a8d4a9ad0cd2a610be8a53fb1
test "$(sha256sum RESERVE/red_tiles/b08p02a_Base.tile | cut -d' ' -f1)" = 9c6f1beb39b7034bb768b1239413b0b7342fb345065e368eb779fe109d8c10f8
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b08p02a')
assert entry == {'canonical_debug_id': 55, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00055', 'debug_string_address': '0x0827B3FC', 'ground_place_id': 11, 'map_file_id': 56, 'map_id': 55, 'stable_ground_id': 'b08p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_PSYDUCK_CONSTRUCTION' in header
print('B08P02A_IDENTITY_PASS map_file_id=56 symbol=MAP_TEAM_BASE_INSIDE_PSYDUCK_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b08p02a --expected-screenshots 481 \
...
print('B08P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b08p02a'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b08p02a','primary')]['movement_probe']=='PASS'
assert validations[('b08p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b08p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B08P02A_EXHAUSTIVE_PASS samples=481 unique_rgba=233')
PY
...
test "$(sha256sum Data/Ground/b08p02a.rsground | cut -d' ' -f1)" = 8fcc0e9bfea0139fc9fee6b76cb9d37dedfee84d8ae1fef26d5bc89cafadd42b
test "$(sha256sum Content/Tile/b08p02a_Base.tile | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b08p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b08p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b08p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
