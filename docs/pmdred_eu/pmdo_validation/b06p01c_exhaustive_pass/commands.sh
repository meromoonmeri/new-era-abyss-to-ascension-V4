#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p01c.rsground" | cut -d' ' -f1)" = 3825ed3f36959a7ef70a2ca84dc410536d8dd3543d458172cdf7daaaae60f23b
test "$(sha256sum "$CANONICAL/tiles/b06p01c_Base.tile" | cut -d' ' -f1)" = 2398db32660589463a0b9212e4ffe7ada44e80f2d9481488a50679c98c81935a
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p01c')
assert entry == {'canonical_debug_id': 42, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00042', 'debug_string_address': '0x0827B59C', 'ground_place_id': 10, 'map_file_id': 43, 'map_id': 42, 'stable_ground_id': 'b06p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TOTODILE_BASIC' in header
print('B06P01C_IDENTITY_PASS map_id=42 map_file_id=43 symbol=MAP_TEAM_BASE_INSIDE_TOTODILE_BASIC')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 175
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 51
...
assert r['grounds']==['b06p01c'] and r['sample_count']==51 and len(primary)==50
assert r['exact_sample_count']==51 and r['fully_opaque_sample_count']==51
assert len({s['actual_rgba_sha256'] for s in primary})==32
...
assert validations[('b06p01c','primary')]['movement_probe']=='PASS'
assert validations[('b06p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b06p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P01C_EXHAUSTIVE_PASS samples=51 unique_rgba=32 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b06p01c.rsground | cut -d' ' -f1)" = 3825ed3f36959a7ef70a2ca84dc410536d8dd3543d458172cdf7daaaae60f23b
test "$(sha256sum Content/Tile/b06p01c_Base.tile | cut -d' ' -f1)" = 2398db32660589463a0b9212e4ffe7ada44e80f2d9481488a50679c98c81935a
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 51 comparative PNGs,
