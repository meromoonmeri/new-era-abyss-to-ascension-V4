#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h10p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h10p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h10p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h10p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h10p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h10p01.rsground" | cut -d' ' -f1)" = 20d4db49693d33c65cd328797a92efbc34b1a85db1c4c8149d8c44e9d456f80c
test "$(sha256sum "$CANONICAL/tiles/h10p01_Base.tile" | cut -d' ' -f1)" = 0f195d6273424eb748df4c77031ed4db0e621f0cc80cf04b16d4149005e1ad2c
test "$(sha256sum RESERVE/red_grounds/h10p01.rsground | cut -d' ' -f1)" = 3191d0c0ce1d8c5744c7e7fc6dbbb144e09aae9e5c480aff72e563f1fecf91c2
test "$(sha256sum RESERVE/red_tiles/h10p01_Base.tile | cut -d' ' -f1)" = 0955816ce2aeeab408c8a566edccce7f82566b95e5edd99ef7bc139a04a6081e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h10p01')
assert entry == {'canonical_debug_id': 139, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00139', 'debug_string_address': '0x0827AA1C', 'ground_place_id': 9, 'map_file_id': 140, 'map_id': 134, 'stable_ground_id': 'h10p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_WATERFALL_LAKE' in header
print('H10P01_IDENTITY_PASS map_file_id=140 symbol=MAP_FRIEND_AREA_WATERFALL_LAKE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h10p01 --expected-screenshots 353 \
...
print('H10P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 1084
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 353
...
assert r['grounds']==['h10p01'] and r['sample_count']==353 and len(primary)==352
assert r['exact_sample_count']==353 and r['fully_opaque_sample_count']==353
assert len({s['actual_rgba_sha256'] for s in primary})==42
...
assert validations[('h10p01','primary')]['movement_probe']=='PASS'
assert validations[('h10p01','primary')]['blocked_probe']=='PASS'
assert validations[('h10p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H10P01_EXHAUSTIVE_PASS samples=353 unique_rgba=42')
PY
...
test "$(sha256sum Data/Ground/h10p01.rsground | cut -d' ' -f1)" = 20d4db49693d33c65cd328797a92efbc34b1a85db1c4c8149d8c44e9d456f80c
test "$(sha256sum Content/Tile/h10p01_Base.tile | cut -d' ' -f1)" = 0f195d6273424eb748df4c77031ed4db0e621f0cc80cf04b16d4149005e1ad2c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h10p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h10p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h10p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 353 comparative PNGs,
