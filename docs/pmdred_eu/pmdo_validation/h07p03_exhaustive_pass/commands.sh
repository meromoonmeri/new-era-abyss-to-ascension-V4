#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p03.rsground" | cut -d' ' -f1)" = 327409f8adb1352baf8216dbf2a7052d7bd290ee5072dfe3d66e25b35bde8961
test "$(sha256sum "$CANONICAL/tiles/h07p03_Base.tile" | cut -d' ' -f1)" = cfdb987bd91e902fe1a4c7fe361c7372efd7d61f0c85b94bcbcf7dc745cfcb59
test "$(sha256sum RESERVE/red_grounds/h07p03.rsground | cut -d' ' -f1)" = 80e4cfd8802e6413e4e794619dd1d53b98714a8b0d18fd4ee4f57e09d82829c8
test "$(sha256sum RESERVE/red_tiles/h07p03_Base.tile | cut -d' ' -f1)" = cfdb987bd91e902fe1a4c7fe361c7372efd7d61f0c85b94bcbcf7dc745cfcb59
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p03')
assert entry == {'canonical_debug_id': 129, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00129', 'debug_string_address': '0x0827AB3C', 'ground_place_id': 9, 'map_file_id': 130, 'map_id': 125, 'stable_ground_id': 'h07p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_ENERGETIC_FOREST' in header
print('H07P03_IDENTITY_PASS map_file_id=130 symbol=MAP_FRIEND_AREA_ENERGETIC_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p03 --expected-screenshots 2 \
...
print('H07P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h07p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h07p03','primary')]['movement_probe']=='PASS'
assert validations[('h07p03','primary')]['blocked_probe']=='PASS'
assert validations[('h07p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h07p03.rsground | cut -d' ' -f1)" = 327409f8adb1352baf8216dbf2a7052d7bd290ee5072dfe3d66e25b35bde8961
test "$(sha256sum Content/Tile/h07p03_Base.tile | cut -d' ' -f1)" = cfdb987bd91e902fe1a4c7fe361c7372efd7d61f0c85b94bcbcf7dc745cfcb59
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
