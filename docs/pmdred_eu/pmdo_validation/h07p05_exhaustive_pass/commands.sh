#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p05 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p05-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p05-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p05-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p05-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p05.rsground" | cut -d' ' -f1)" = 05ee55e06556fffc38c25d4a091533cf5ac3d82a4f61e25a1bc2a8dcddf5879d
test "$(sha256sum "$CANONICAL/tiles/h07p05_Base.tile" | cut -d' ' -f1)" = 6e750b15c15d541d9fdd56413a072df0898c48e8e1e98d29376ec4b7859beb9a
test "$(sha256sum RESERVE/red_grounds/h07p05.rsground | cut -d' ' -f1)" = 76f9c4b30703e61f7be6c17aab1f49b6294d4e4db1e9780ccd515648166e67d6
test "$(sha256sum RESERVE/red_tiles/h07p05_Base.tile | cut -d' ' -f1)" = 6e750b15c15d541d9fdd56413a072df0898c48e8e1e98d29376ec4b7859beb9a
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p05')
assert entry == {'canonical_debug_id': 132, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00132', 'debug_string_address': '0x0827AAFC', 'ground_place_id': 9, 'map_file_id': 133, 'map_id': 127, 'stable_ground_id': 'h07p05', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_HEALING_FOREST' in header
print('H07P05_IDENTITY_PASS map_file_id=133 symbol=MAP_FRIEND_AREA_HEALING_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p05 --expected-screenshots 2 \
...
print('H07P05_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h07p05'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h07p05','primary')]['movement_probe']=='PASS'
assert validations[('h07p05','primary')]['blocked_probe']=='PASS'
assert validations[('h07p05','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P05_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h07p05.rsground | cut -d' ' -f1)" = 05ee55e06556fffc38c25d4a091533cf5ac3d82a4f61e25a1bc2a8dcddf5879d
test "$(sha256sum Content/Tile/h07p05_Base.tile | cut -d' ' -f1)" = 6e750b15c15d541d9fdd56413a072df0898c48e8e1e98d29376ec4b7859beb9a
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p05.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p05_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p05 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
