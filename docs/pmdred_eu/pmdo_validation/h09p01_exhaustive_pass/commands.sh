#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h09p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h09p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h09p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h09p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h09p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h09p01.rsground" | cut -d' ' -f1)" = b9b1e3c83e02756de51c6d3914f80c4ee508264a650ece8db2f8736953acb3b5
test "$(sha256sum "$CANONICAL/tiles/h09p01_Base.tile" | cut -d' ' -f1)" = 10bfce2e89ddfcad7a28c2aa4bcb8c8e40f575e9bc07dde7e71753795c1deff4
test "$(sha256sum RESERVE/red_grounds/h09p01.rsground | cut -d' ' -f1)" = f786a4067d52ddead64982d32586d1ff955769a29d0e2f8e6c3e8ab15156abe2
test "$(sha256sum RESERVE/red_tiles/h09p01_Base.tile | cut -d' ' -f1)" = 9f329d176fb017c47a31db928a816a09b62a225ef487dd38418a3cf0ac5253f6
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h09p01')
assert entry == {'canonical_debug_id': 137, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00137', 'debug_string_address': '0x0827AA5C', 'ground_place_id': 9, 'map_file_id': 138, 'map_id': 132, 'stable_ground_id': 'h09p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_TURTLESHELL_POND' in header
print('H09P01_IDENTITY_PASS map_file_id=138 symbol=MAP_FRIEND_AREA_TURTLESHELL_POND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h09p01 --expected-screenshots 98 \
...
print('H09P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 319
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 98
...
assert r['grounds']==['h09p01'] and r['sample_count']==98 and len(primary)==97
assert r['exact_sample_count']==98 and r['fully_opaque_sample_count']==98
...
assert validations[('h09p01','primary')]['movement_probe']=='PASS'
assert validations[('h09p01','primary')]['blocked_probe']=='PASS'
assert validations[('h09p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H09P01_EXHAUSTIVE_PASS samples=98 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/h09p01.rsground | cut -d' ' -f1)" = b9b1e3c83e02756de51c6d3914f80c4ee508264a650ece8db2f8736953acb3b5
test "$(sha256sum Content/Tile/h09p01_Base.tile | cut -d' ' -f1)" = 10bfce2e89ddfcad7a28c2aa4bcb8c8e40f575e9bc07dde7e71753795c1deff4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h09p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h09p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h09p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 98 comparative PNGs,
