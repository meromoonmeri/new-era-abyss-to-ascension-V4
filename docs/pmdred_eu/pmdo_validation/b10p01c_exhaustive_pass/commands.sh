...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p01c')
assert e == {'canonical_debug_id': 66, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00066', 'debug_string_address': '0x0827B29C', 'ground_place_id': 10, 'map_file_id': 67, 'map_id': 66, 'stable_ground_id': 'b10p01c', 'weather_id': 9}
assert 'MAP_TEAM_BASE_INSIDE_TORCHIC_BASIC' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B10P01C_IDENTITY_PASS map_id=66 map_file_id=67 symbol=MAP_TEAM_BASE_INSIDE_TORCHIC_BASIC')
PYID
...
print('B10P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
