#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b10p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b10p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b10p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b10p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b10p01c.rsground" | cut -d' ' -f1)" = 13f3859be815a2ac585bcc35fde3964c8e1f3e12609601b5c2b69e52eb04671c
test "$(sha256sum "$CANONICAL/tiles/b10p01c_Base.tile" | cut -d' ' -f1)" = 03a401a44892dad3a651cf45797361bdfd24ef909e5f5f878606eba27e5b2c7e
test "$(sha256sum RESERVE/red_grounds/b10p01c.rsground | cut -d' ' -f1)" = ef6ad460a12c922d36c266b2ee63ca97c33eb8ea23b703df4e4d8a318ce98ca8
test "$(sha256sum RESERVE/red_tiles/b10p01c_Base.tile | cut -d' ' -f1)" = 03a401a44892dad3a651cf45797361bdfd24ef909e5f5f878606eba27e5b2c7e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b10p01c')
assert entry == {'canonical_debug_id': 66, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00066', 'debug_string_address': '0x0827B29C', 'ground_place_id': 10, 'map_file_id': 67, 'map_id': 66, 'stable_ground_id': 'b10p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TORCHIC_BASIC' in header
print('B10P01C_IDENTITY_PASS map_file_id=67 symbol=MAP_TEAM_BASE_INSIDE_TORCHIC_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b10p01c --expected-screenshots 25 \
...
print('B10P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b10p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b10p01c','primary')]['movement_probe']=='PASS'
assert validations[('b10p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b10p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B10P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b10p01c.rsground | cut -d' ' -f1)" = 13f3859be815a2ac585bcc35fde3964c8e1f3e12609601b5c2b69e52eb04671c
test "$(sha256sum Content/Tile/b10p01c_Base.tile | cut -d' ' -f1)" = 03a401a44892dad3a651cf45797361bdfd24ef909e5f5f878606eba27e5b2c7e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b10p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b10p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b10p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
