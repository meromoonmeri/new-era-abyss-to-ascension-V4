...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p02')
assert e == {'canonical_debug_id': 185, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00185', 'debug_string_address': '0x0827A4BC', 'ground_place_id': 35, 'map_file_id': 186, 'map_id': 177, 'stable_ground_id': 'a05p02', 'weather_id': -1}
assert 'MAP_THE_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A05P02_IDENTITY_PASS map_id=177 map_file_id=186 symbol=MAP_THE_END')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 46
...
print('A05P02_EXHAUSTIVE_PASS samples=9 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
