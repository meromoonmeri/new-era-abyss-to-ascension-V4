#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a05p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a05p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a05p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a05p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a05p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a05p02.rsground" | cut -d' ' -f1)" = 17750d5ab3f6db0e0f5544ea93cda61358d0ce1d86f5a726d276529fad953335
test "$(sha256sum "$CANONICAL/tiles/a05p02_Base.tile" | cut -d' ' -f1)" = cd697ec328cb37d619c7d5d3e4cc795ba8f9f9c94a5dec050babd0f13f2f0a93
test "$(sha256sum RESERVE/red_grounds/a05p02.rsground | cut -d' ' -f1)" = af7ab7d68e5eaa8655481f7ff61545956fda755dc517f49aa6464ad61ee098e6
test "$(sha256sum RESERVE/red_tiles/a05p02_Base.tile | cut -d' ' -f1)" = cd697ec328cb37d619c7d5d3e4cc795ba8f9f9c94a5dec050babd0f13f2f0a93
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p02')
assert entry == {'canonical_debug_id': 185, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00185', 'debug_string_address': '0x0827A4BC', 'ground_place_id': 35, 'map_file_id': 186, 'map_id': 177, 'stable_ground_id': 'a05p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_THE_END' in header
print('A05P02_IDENTITY_PASS map_file_id=186 symbol=MAP_THE_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a05p02 --expected-screenshots 9 \
...
print('A05P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 49
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 9
...
assert r['grounds']==['a05p02'] and r['sample_count']==9 and len(primary)==8
assert r['exact_sample_count']==9 and r['fully_opaque_sample_count']==9
...
assert validations[('a05p02','primary')]['movement_probe']=='PASS'
assert validations[('a05p02','primary')]['blocked_probe']=='PASS'
assert validations[('a05p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A05P02_EXHAUSTIVE_PASS samples=9 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a05p02.rsground | cut -d' ' -f1)" = 17750d5ab3f6db0e0f5544ea93cda61358d0ce1d86f5a726d276529fad953335
test "$(sha256sum Content/Tile/a05p02_Base.tile | cut -d' ' -f1)" = cd697ec328cb37d619c7d5d3e4cc795ba8f9f9c94a5dec050babd0f13f2f0a93
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a05p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a05p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a05p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 9 comparative PNGs,
