#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d24p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d24p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d24p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d24p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d24p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d24p01.rsground" | cut -d' ' -f1)" = cd9e6c1aecb6bdbda154d4cbb7efb54e00e23168cee402c63bdcffc20b35025f
test "$(sha256sum "$CANONICAL/tiles/d24p01_Base.tile" | cut -d' ' -f1)" = 32bab429129d1f61f2d35459718bd82d4dcf439ea15fb8f383be458bf6a4ebf1
test "$(sha256sum RESERVE/red_grounds/d24p01.rsground | cut -d' ' -f1)" = 390a9088524b5bccb655a306f35d2e0618e824b0709be0391b6302801673a727
test "$(sha256sum RESERVE/red_tiles/d24p01_Base.tile | cut -d' ' -f1)" = 32bab429129d1f61f2d35459718bd82d4dcf439ea15fb8f383be458bf6a4ebf1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d24p01')
assert entry == {'canonical_debug_id': 229, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00229', 'debug_string_address': '0x08279F3C', 'ground_place_id': 12, 'map_file_id': 234, 'map_id': 225, 'stable_ground_id': 'd24p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_INTRO' in header
print('D24P01_IDENTITY_PASS map_file_id=234 symbol=MAP_INTRO')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d24p01 --expected-screenshots 2 \
...
print('D24P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d24p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d24p01','primary')]['movement_probe']=='PASS'
assert validations[('d24p01','primary')]['blocked_probe']=='PASS'
assert validations[('d24p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D24P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d24p01.rsground | cut -d' ' -f1)" = cd9e6c1aecb6bdbda154d4cbb7efb54e00e23168cee402c63bdcffc20b35025f
test "$(sha256sum Content/Tile/d24p01_Base.tile | cut -d' ' -f1)" = 32bab429129d1f61f2d35459718bd82d4dcf439ea15fb8f383be458bf6a4ebf1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d24p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d24p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d24p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
