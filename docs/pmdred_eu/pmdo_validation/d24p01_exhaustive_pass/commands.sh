...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d24p01')
assert e == {'canonical_debug_id': 229, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00229', 'debug_string_address': '0x08279F3C', 'ground_place_id': 12, 'map_file_id': 234, 'map_id': 225, 'stable_ground_id': 'd24p01', 'weather_id': -1}
assert 'MAP_INTRO' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D24P01_IDENTITY_PASS map_id=225 map_file_id=234 symbol=MAP_INTRO')
PYID
...
print('D24P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
