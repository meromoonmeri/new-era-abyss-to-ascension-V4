#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p08 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p08-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p08-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p08-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p08-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p08.rsground" | cut -d' ' -f1)" = 794f244eb749a247123b620cf3d8c33fbfad17591b55b3d5b599fb0a237341b7
test "$(sha256sum "$CANONICAL/tiles/h07p08_Base.tile" | cut -d' ' -f1)" = 1b39a124211a4014abbf7edfafdf7fcefc8292d0d624328ae08adee1e2bdb257
test "$(sha256sum RESERVE/red_grounds/h07p08.rsground | cut -d' ' -f1)" = afd688da60a01ee94ac6b3b52c8c81e93fc982f9d85526cad9c663aacf447961
test "$(sha256sum RESERVE/red_tiles/h07p08_Base.tile | cut -d' ' -f1)" = 1b39a124211a4014abbf7edfafdf7fcefc8292d0d624328ae08adee1e2bdb257
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p08')
assert entry == {'canonical_debug_id': 135, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00135', 'debug_string_address': '0x0827AA9C', 'ground_place_id': 9, 'map_file_id': 136, 'map_id': 130, 'stable_ground_id': 'h07p08', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_RUB_A_DUB_RIVER' in header
print('H07P08_IDENTITY_PASS map_file_id=136 symbol=MAP_FRIEND_AREA_RUB_A_DUB_RIVER')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p08 --expected-screenshots 2 \
...
print('H07P08_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h07p08'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h07p08','primary')]['movement_probe']=='PASS'
assert validations[('h07p08','primary')]['blocked_probe']=='PASS'
assert validations[('h07p08','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P08_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h07p08.rsground | cut -d' ' -f1)" = 794f244eb749a247123b620cf3d8c33fbfad17591b55b3d5b599fb0a237341b7
test "$(sha256sum Content/Tile/h07p08_Base.tile | cut -d' ' -f1)" = 1b39a124211a4014abbf7edfafdf7fcefc8292d0d624328ae08adee1e2bdb257
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p08.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p08_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p08 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
