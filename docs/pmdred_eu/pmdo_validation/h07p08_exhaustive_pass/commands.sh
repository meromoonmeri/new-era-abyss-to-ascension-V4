...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p08')
assert e == {'canonical_debug_id': 135, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00135', 'debug_string_address': '0x0827AA9C', 'ground_place_id': 9, 'map_file_id': 136, 'map_id': 130, 'stable_ground_id': 'h07p08', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_RUB_A_DUB_RIVER' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H07P08_IDENTITY_PASS map_id=130 map_file_id=136 symbol=MAP_FRIEND_AREA_RUB_A_DUB_RIVER')
PYID
...
print('H07P08_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
