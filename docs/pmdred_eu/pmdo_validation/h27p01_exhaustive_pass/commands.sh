#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h27p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h27p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h27p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h27p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h27p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h27p01.rsground" | cut -d' ' -f1)" = c3e670bbcf3bb2c554eb2e93d5a9798ec7ee7d7dc30d4fa7fb27915369b5834c
test "$(sha256sum "$CANONICAL/tiles/h27p01_Base.tile" | cut -d' ' -f1)" = 090f29330690b6ac6a5fbf0923c275dcb1502cc27ac2e2c9ba4ead6bbf0deda9
test "$(sha256sum RESERVE/red_grounds/h27p01.rsground | cut -d' ' -f1)" = 97b18dc49a6164aa0aec5b003196ea6b43ebe34a6d6deee5c4f96ba4749356d0
test "$(sha256sum RESERVE/red_tiles/h27p01_Base.tile | cut -d' ' -f1)" = bc1638d0c758163ede03fa2050ecd68c9550a464780391699b56f928441b8883
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h27p01')
assert entry == {'canonical_debug_id': 163, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00163', 'debug_string_address': '0x0827A75C', 'ground_place_id': 9, 'map_file_id': 164, 'map_id': 156, 'stable_ground_id': 'h27p01', 'weather_id': 14}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_RAVAGED_FIELD' in header
print('H27P01_IDENTITY_PASS map_file_id=164 symbol=MAP_FRIEND_AREA_RAVAGED_FIELD')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h27p01 --expected-screenshots 609 \
...
print('H27P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 1852
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 609
...
assert r['grounds']==['h27p01'] and r['sample_count']==609 and len(primary)==608
assert r['exact_sample_count']==609 and r['fully_opaque_sample_count']==609
assert len({s['actual_rgba_sha256'] for s in primary})==90
...
assert validations[('h27p01','primary')]['movement_probe']=='PASS'
assert validations[('h27p01','primary')]['blocked_probe']=='PASS'
assert validations[('h27p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H27P01_EXHAUSTIVE_PASS samples=609 unique_rgba=90')
PY
...
test "$(sha256sum Data/Ground/h27p01.rsground | cut -d' ' -f1)" = c3e670bbcf3bb2c554eb2e93d5a9798ec7ee7d7dc30d4fa7fb27915369b5834c
test "$(sha256sum Content/Tile/h27p01_Base.tile | cut -d' ' -f1)" = 090f29330690b6ac6a5fbf0923c275dcb1502cc27ac2e2c9ba4ead6bbf0deda9
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h27p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h27p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h27p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 609 comparative PNGs,
