...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h27p01')
assert e == {'canonical_debug_id': 163, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00163', 'debug_string_address': '0x0827A75C', 'ground_place_id': 9, 'map_file_id': 164, 'map_id': 156, 'stable_ground_id': 'h27p01', 'weather_id': 14}
assert 'MAP_FRIEND_AREA_RAVAGED_FIELD' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H27P01_IDENTITY_PASS map_id=156 map_file_id=164 symbol=MAP_FRIEND_AREA_RAVAGED_FIELD')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 1849
...
print('H27P01_EXHAUSTIVE_PASS samples=609 unique_rgba=90 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
