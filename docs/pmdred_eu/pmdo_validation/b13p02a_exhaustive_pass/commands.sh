#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p02a.rsground" | cut -d' ' -f1)" = 5cea1d09f25da8a9c9214ce7dc5a6ad5082ba2f6ab8fc3db2257c7d1399f2181
test "$(sha256sum "$CANONICAL/tiles/b13p02a_Base.tile" | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
test "$(sha256sum RESERVE/red_grounds/b13p02a.rsground | cut -d' ' -f1)" = a72cea3ead7414791dc69656ab05af4692b7795590e801600b73af7b1377ac57
test "$(sha256sum RESERVE/red_tiles/b13p02a_Base.tile | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02a')
assert entry == {'canonical_debug_id': 85, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00085', 'debug_string_address': '0x0827B03C', 'ground_place_id': 11, 'map_file_id': 86, 'map_id': 85, 'stable_ground_id': 'b13p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_CONSTRUCTION' in header
print('B13P02A_IDENTITY_PASS map_file_id=86 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p02a --expected-screenshots 2 \
...
print('B13P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p02a'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b13p02a','primary')]['movement_probe']=='PASS'
assert validations[('b13p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b13p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P02A_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b13p02a.rsground | cut -d' ' -f1)" = 5cea1d09f25da8a9c9214ce7dc5a6ad5082ba2f6ab8fc3db2257c7d1399f2181
test "$(sha256sum Content/Tile/b13p02a_Base.tile | cut -d' ' -f1)" = 4ac92b15ff0220543cd25d2fe05ce6dc155f8f8fae85b6cb253ff495aa30e58c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
