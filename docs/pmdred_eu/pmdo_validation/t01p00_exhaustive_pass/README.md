...
Repository records still conflict around the legacy Pokémon Square alias: `docs/pmdred_extraction_tracabilite.md` identifies `T00P01` as Pokémon Square, while `docs/canonical/red/upstream_ground_aliases.json` maps `place_pokemon.rsground` to `t00p01.rsground`. In addition, the adjacent `t01p01` record shares canonical debug ID 1 and Ground place ID 2 but uses map ID/map file ID 2. This promotion records only the proven map-ID-1 identity. It does not rewrite the alias or infer the adjacent variant's role. The alias remains untouched until `t01p01` receives equivalent exact runtime proof.
