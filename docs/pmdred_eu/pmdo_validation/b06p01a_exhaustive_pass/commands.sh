#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b06p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b06p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b06p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b06p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b06p01a.rsground" | cut -d' ' -f1)" = 34dafd0f7abc393d2f38df7087b8d94264705d04d5c2cba2e28858c6177cfadd
test "$(sha256sum "$CANONICAL/tiles/b06p01a_Base.tile" | cut -d' ' -f1)" = ff7f4ddc12bc5c32551f77194c9cbf262a94d295a81d9cbb6275640aed0fd94c
test "$(sha256sum RESERVE/red_grounds/b06p01a.rsground | cut -d' ' -f1)" = 72f8d203c06621160fa66c4751661cad960c3cc1aaa09080cc54eca7cc61c6ab
test "$(sha256sum RESERVE/red_tiles/b06p01a_Base.tile | cut -d' ' -f1)" = ff7f4ddc12bc5c32551f77194c9cbf262a94d295a81d9cbb6275640aed0fd94c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b06p01a')
assert entry == {'canonical_debug_id': 40, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00040', 'debug_string_address': '0x0827B5DC', 'ground_place_id': 10, 'map_file_id': 41, 'map_id': 40, 'stable_ground_id': 'b06p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TOTODILE_CONSTRUCTION' in header
print('B06P01A_IDENTITY_PASS map_file_id=41 symbol=MAP_TEAM_BASE_TOTODILE_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b06p01a --expected-screenshots 51 \
...
print('B06P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b06p01a'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b06p01a','primary')]['movement_probe']=='PASS'
assert validations[('b06p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b06p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B06P01A_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b06p01a.rsground | cut -d' ' -f1)" = 34dafd0f7abc393d2f38df7087b8d94264705d04d5c2cba2e28858c6177cfadd
test "$(sha256sum Content/Tile/b06p01a_Base.tile | cut -d' ' -f1)" = ff7f4ddc12bc5c32551f77194c9cbf262a94d295a81d9cbb6275640aed0fd94c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b06p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b06p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b06p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
