#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h11p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h11p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h11p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h11p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h11p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h11p01.rsground" | cut -d' ' -f1)" = 04ea01cb7c42d95b0297592fc1a748742494e663d615b1be52e4eb07acb23d07
test "$(sha256sum "$CANONICAL/tiles/h11p01_Base.tile" | cut -d' ' -f1)" = fc86684f7b08b2ff83b55f641117219cbb78f7079393933cdb4ecb9bbe0540a4
test "$(sha256sum RESERVE/red_grounds/h11p01.rsground | cut -d' ' -f1)" = acd8fe7e8f2bc57658412467039110518dba632dee145b7a4783b4268e1f1c4b
test "$(sha256sum RESERVE/red_tiles/h11p01_Base.tile | cut -d' ' -f1)" = e9fe76708d3085a0904e60ba0d99352001cc8d38ee326ad90be34ac7c9a9a020
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h11p01')
assert entry == {'canonical_debug_id': 141, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00141', 'debug_string_address': '0x0827A9DC', 'ground_place_id': 9, 'map_file_id': 142, 'map_id': 136, 'stable_ground_id': 'h11p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_POISON_SWAMP' in header
print('H11P01_IDENTITY_PASS map_file_id=142 symbol=MAP_FRIEND_AREA_POISON_SWAMP')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h11p01 --expected-screenshots 49 \
...
print('H11P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h11p01'] and r['sample_count']==49 and len(primary)==48
assert r['exact_sample_count']==49 and r['fully_opaque_sample_count']==49
assert len({s['actual_rgba_sha256'] for s in primary})==12
...
assert validations[('h11p01','primary')]['movement_probe']=='PASS'
assert validations[('h11p01','primary')]['blocked_probe']=='PASS'
assert validations[('h11p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H11P01_EXHAUSTIVE_PASS samples=49 unique_rgba=12')
PY
...
test "$(sha256sum Data/Ground/h11p01.rsground | cut -d' ' -f1)" = 04ea01cb7c42d95b0297592fc1a748742494e663d615b1be52e4eb07acb23d07
test "$(sha256sum Content/Tile/h11p01_Base.tile | cut -d' ' -f1)" = fc86684f7b08b2ff83b55f641117219cbb78f7079393933cdb4ecb9bbe0540a4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h11p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h11p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h11p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
