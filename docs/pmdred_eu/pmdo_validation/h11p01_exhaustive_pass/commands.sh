...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h11p01')
assert e == {'canonical_debug_id': 141, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00141', 'debug_string_address': '0x0827A9DC', 'ground_place_id': 9, 'map_file_id': 142, 'map_id': 136, 'stable_ground_id': 'h11p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_POISON_SWAMP' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H11P01_IDENTITY_PASS map_id=136 map_file_id=142 symbol=MAP_FRIEND_AREA_POISON_SWAMP')
PYID
...
print('H11P01_EXHAUSTIVE_PASS samples=49 unique_rgba=12 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
