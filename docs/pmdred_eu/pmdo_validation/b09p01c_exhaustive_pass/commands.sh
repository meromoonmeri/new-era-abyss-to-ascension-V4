#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b09p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b09p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b09p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b09p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b09p01c.rsground" | cut -d' ' -f1)" = 19f880be1bd395edbdfb8f5e7ce432910fc0e4ab344bd7af329aaacb5d400838
test "$(sha256sum "$CANONICAL/tiles/b09p01c_Base.tile" | cut -d' ' -f1)" = 1bef4bef796b7ad78a58a71abfab202c704345754342338fd576d21663829878
test "$(sha256sum RESERVE/red_grounds/b09p01c.rsground | cut -d' ' -f1)" = 1a20a67edb876b37f3bec29d190ed795ef2ccf132820d65a93616b05d9f0914c
test "$(sha256sum RESERVE/red_tiles/b09p01c_Base.tile | cut -d' ' -f1)" = 1bef4bef796b7ad78a58a71abfab202c704345754342338fd576d21663829878
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b09p01c')
assert entry == {'canonical_debug_id': 60, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00060', 'debug_string_address': '0x0827B35C', 'ground_place_id': 10, 'map_file_id': 61, 'map_id': 60, 'stable_ground_id': 'b09p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CHARMANDER_BASIC' in header
print('B09P01C_IDENTITY_PASS map_file_id=61 symbol=MAP_TEAM_BASE_INSIDE_CHARMANDER_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b09p01c --expected-screenshots 25 \
...
print('B09P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b09p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b09p01c','primary')]['movement_probe']=='PASS'
assert validations[('b09p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b09p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B09P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b09p01c.rsground | cut -d' ' -f1)" = 19f880be1bd395edbdfb8f5e7ce432910fc0e4ab344bd7af329aaacb5d400838
test "$(sha256sum Content/Tile/b09p01c_Base.tile | cut -d' ' -f1)" = 1bef4bef796b7ad78a58a71abfab202c704345754342338fd576d21663829878
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b09p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b09p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b09p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
