#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h28p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h28p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h28p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h28p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h28p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h28p02.rsground" | cut -d' ' -f1)" = 0768871862cd19a8752191cbbacc8e4b8eb655fcfd5cab04427d11136598edda
test "$(sha256sum "$CANONICAL/tiles/h28p02_Base.tile" | cut -d' ' -f1)" = d5d108438713b76bc2b8e262c84f9d0692e89ec0597cbc92c5fa68d35cac54c5
test "$(sha256sum RESERVE/red_grounds/h28p02.rsground | cut -d' ' -f1)" = f0e3892f458445c52489e5aa8720c89692128a3d9dbe488b54c330094bb2fc21
test "$(sha256sum RESERVE/red_tiles/h28p02_Base.tile | cut -d' ' -f1)" = d5d108438713b76bc2b8e262c84f9d0692e89ec0597cbc92c5fa68d35cac54c5
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h28p02')
assert entry == {'canonical_debug_id': 165, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00165', 'debug_string_address': '0x0827A71C', 'ground_place_id': 9, 'map_file_id': 166, 'map_id': 158, 'stable_ground_id': 'h28p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_LEGENDARY_ISLAND' in header
print('H28P02_IDENTITY_PASS map_file_id=166 symbol=MAP_FRIEND_AREA_LEGENDARY_ISLAND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h28p02 --expected-screenshots 2 \
...
print('H28P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h28p02'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h28p02','primary')]['movement_probe']=='PASS'
assert validations[('h28p02','primary')]['blocked_probe']=='PASS'
assert validations[('h28p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H28P02_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h28p02.rsground | cut -d' ' -f1)" = 0768871862cd19a8752191cbbacc8e4b8eb655fcfd5cab04427d11136598edda
test "$(sha256sum Content/Tile/h28p02_Base.tile | cut -d' ' -f1)" = d5d108438713b76bc2b8e262c84f9d0692e89ec0597cbc92c5fa68d35cac54c5
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h28p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h28p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h28p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
