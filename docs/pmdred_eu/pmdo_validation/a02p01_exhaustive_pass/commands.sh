#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a02p01 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a02p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a02p01.rsground" | cut -d' ' -f1)" = 4cb74d9c04c7473359f8b8360caac80df825dff91775917dbfe0d5dd201fc389
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a02p01 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a02p01.rsground" | cut -d' ' -f1)" = 016551d87ddf6b5556a4f9181ec8061a5af2df1bda97e3f43e376f3dc64dc3b2
test "$(sha256sum "$CANONICAL/tiles/a02p01_Base.tile" | cut -d' ' -f1)" = 7f80a67d0d6823541dbed67e77b68d1d005eb09b53d79a0d3a66e7a1bae027e3
test "$(sha256sum RESERVE/red_grounds/a02p01.rsground | cut -d' ' -f1)" = 51e08f36ecc2975ba6ca9b0ae4fc2590400eb559608bf7f38246a7168e8aa9fb
test "$(sha256sum RESERVE/red_tiles/a02p01_Base.tile | cut -d' ' -f1)" = 7f80a67d0d6823541dbed67e77b68d1d005eb09b53d79a0d3a66e7a1bae027e3
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p01')
assert entry == {'canonical_debug_id': 173, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00173', 'debug_string_address': '0x0827A63C', 'ground_place_id': 35, 'map_file_id': 174, 'map_id': 165, 'stable_ground_id': 'a02p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_FIRE' in header
print('A02P01_IDENTITY_PASS map_file_id=174 symbol=MAP_FUGITIVES_FIRE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a02p01 --expected-screenshots 2 \
...
print('A02P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a02p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a02p01','primary')]['movement_probe']=='PASS'
assert validations[('a02p01','primary')]['blocked_probe']=='PASS'
assert validations[('a02p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A02P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a02p01.rsground | cut -d' ' -f1)" = 016551d87ddf6b5556a4f9181ec8061a5af2df1bda97e3f43e376f3dc64dc3b2
test "$(sha256sum Content/Tile/a02p01_Base.tile | cut -d' ' -f1)" = 7f80a67d0d6823541dbed67e77b68d1d005eb09b53d79a0d3a66e7a1bae027e3
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a02p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a02p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a02p01 --entity-integrated-ids a02p01 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p01/a02p01.rsground | cut -d' ' -f1)" = 2b6d651a9fd7d25f159f50fc9d1cd6fa7006899988822eda217f4bf60923aad7
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p01/A02p01_Base.tile | cut -d' ' -f1)" = 7f80a67d0d6823541dbed67e77b68d1d005eb09b53d79a0d3a66e7a1bae027e3
