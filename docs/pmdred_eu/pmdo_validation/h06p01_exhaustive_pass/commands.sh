#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p01.rsground" | cut -d' ' -f1)" = a641e05e4cb0cb646170929c8aa7f972683680591f7bb89e53af7fd91b352f80
test "$(sha256sum "$CANONICAL/tiles/h06p01_Base.tile" | cut -d' ' -f1)" = 403de7d3c7d18b44edfec6df8db05119c2004d7bad1ba77e5a05ca8511898ada
test "$(sha256sum RESERVE/red_grounds/h06p01.rsground | cut -d' ' -f1)" = 97232127088fd81ec3f74c1b99aa1009c3bb335003109c8b84f134cd817c29b1
test "$(sha256sum RESERVE/red_tiles/h06p01_Base.tile | cut -d' ' -f1)" = 235567977524b3f8716040ee08b0ccb0e0a17226e873a96cecd208bcdb67bdb7
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p01')
assert entry == {'canonical_debug_id': 121, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00121', 'debug_string_address': '0x0827AC3C', 'ground_place_id': 9, 'map_file_id': 122, 'map_id': 117, 'stable_ground_id': 'h06p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_BEAU_PLAINS' in header
print('H06P01_IDENTITY_PASS map_file_id=122 symbol=MAP_FRIEND_AREA_BEAU_PLAINS')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p01 --expected-screenshots 361 \
...
print('H06P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 1108
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 361
...
assert r['grounds']==['h06p01'] and r['sample_count']==361 and len(primary)==360
assert r['exact_sample_count']==361 and r['fully_opaque_sample_count']==361
assert len({s['actual_rgba_sha256'] for s in primary})==17
...
assert validations[('h06p01','primary')]['movement_probe']=='PASS'
assert validations[('h06p01','primary')]['blocked_probe']=='PASS'
assert validations[('h06p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P01_EXHAUSTIVE_PASS samples=361 unique_rgba=17')
PY
...
test "$(sha256sum Data/Ground/h06p01.rsground | cut -d' ' -f1)" = a641e05e4cb0cb646170929c8aa7f972683680591f7bb89e53af7fd91b352f80
test "$(sha256sum Content/Tile/h06p01_Base.tile | cut -d' ' -f1)" = 403de7d3c7d18b44edfec6df8db05119c2004d7bad1ba77e5a05ca8511898ada
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 361 comparative PNGs,
