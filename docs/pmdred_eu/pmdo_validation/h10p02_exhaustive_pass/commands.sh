...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h10p02')
assert e == {'canonical_debug_id': 140, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00140', 'debug_string_address': '0x0827A9FC', 'ground_place_id': 9, 'map_file_id': 141, 'map_id': 135, 'stable_ground_id': 'h10p02', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_PEANUT_SWAMP' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H10P02_IDENTITY_PASS map_id=135 map_file_id=141 symbol=MAP_FRIEND_AREA_PEANUT_SWAMP')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 5065
...
print('H10P02_EXHAUSTIVE_PASS samples=1681 unique_rgba=781 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
