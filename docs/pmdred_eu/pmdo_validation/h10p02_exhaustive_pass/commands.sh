#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h10p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h10p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h10p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h10p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h10p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h10p02.rsground" | cut -d' ' -f1)" = 3ab15246fcdfc0187b74a471f43e0a3c8520d367461148996570271238c33b4f
test "$(sha256sum "$CANONICAL/tiles/h10p02_Base.tile" | cut -d' ' -f1)" = c3f2fe5cb4e39f98ef4e5f86039430f0d090a5d7c11723be9e33a44c68b37792
test "$(sha256sum RESERVE/red_grounds/h10p02.rsground | cut -d' ' -f1)" = 7f672e58a570ea0b45338d821ca667a85162a32944b1b656101cb6ee3f8ada46
test "$(sha256sum RESERVE/red_tiles/h10p02_Base.tile | cut -d' ' -f1)" = c3f2fe5cb4e39f98ef4e5f86039430f0d090a5d7c11723be9e33a44c68b37792
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h10p02')
assert entry == {'canonical_debug_id': 140, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00140', 'debug_string_address': '0x0827A9FC', 'ground_place_id': 9, 'map_file_id': 141, 'map_id': 135, 'stable_ground_id': 'h10p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_PEANUT_SWAMP' in header
print('H10P02_IDENTITY_PASS map_file_id=141 symbol=MAP_FRIEND_AREA_PEANUT_SWAMP')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h10p02 --expected-screenshots 1681 \
...
print('H10P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 5068
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 1681
...
assert r['grounds']==['h10p02'] and r['sample_count']==1681 and len(primary)==1680
assert r['exact_sample_count']==1681 and r['fully_opaque_sample_count']==1681
assert len({s['actual_rgba_sha256'] for s in primary})==781
...
assert validations[('h10p02','primary')]['movement_probe']=='PASS'
assert validations[('h10p02','primary')]['blocked_probe']=='PASS'
assert validations[('h10p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H10P02_EXHAUSTIVE_PASS samples=1681 unique_rgba=781')
PY
...
test "$(sha256sum Data/Ground/h10p02.rsground | cut -d' ' -f1)" = 3ab15246fcdfc0187b74a471f43e0a3c8520d367461148996570271238c33b4f
test "$(sha256sum Content/Tile/h10p02_Base.tile | cut -d' ' -f1)" = c3f2fe5cb4e39f98ef4e5f86039430f0d090a5d7c11723be9e33a44c68b37792
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h10p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h10p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h10p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 1681 comparative PNGs,
