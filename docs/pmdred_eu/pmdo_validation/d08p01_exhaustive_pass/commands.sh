...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d08p01')
assert e == {'canonical_debug_id': 202, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00202', 'debug_string_address': '0x0827A29C', 'ground_place_id': 22, 'map_file_id': 207, 'map_id': 198, 'stable_ground_id': 'd08p01', 'weather_id': 9}
assert 'MAP_FROSTY_FOREST_ENTRY' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D08P01_IDENTITY_PASS map_id=198 map_file_id=207 symbol=MAP_FROSTY_FOREST_ENTRY')
PYID
...
print('D08P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
