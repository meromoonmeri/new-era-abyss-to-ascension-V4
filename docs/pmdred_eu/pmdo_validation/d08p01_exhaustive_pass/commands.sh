#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d08p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d08p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d08p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d08p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d08p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d08p01.rsground" | cut -d' ' -f1)" = 0f3c504855d3a51d8f13c0f4b802dc91fbc16604abc6a201c3e1fa6c9e37d682
test "$(sha256sum "$CANONICAL/tiles/d08p01_Base.tile" | cut -d' ' -f1)" = 7cdbddd4e85c0b145985571799ae55f167e35220bc2b0939b9a8d2f9b6e9aba1
test "$(sha256sum RESERVE/red_grounds/d08p01.rsground | cut -d' ' -f1)" = 5494bc0cdd7b3425485f7681a549a5c0d6ed4cc4867836788319c1272f3891a0
test "$(sha256sum RESERVE/red_tiles/d08p01_Base.tile | cut -d' ' -f1)" = 7cdbddd4e85c0b145985571799ae55f167e35220bc2b0939b9a8d2f9b6e9aba1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d08p01')
assert entry == {'canonical_debug_id': 202, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00202', 'debug_string_address': '0x0827A29C', 'ground_place_id': 22, 'map_file_id': 207, 'map_id': 198, 'stable_ground_id': 'd08p01', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FROSTY_FOREST_ENTRY' in header
print('D08P01_IDENTITY_PASS map_file_id=207 symbol=MAP_FROSTY_FOREST_ENTRY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d08p01 --expected-screenshots 2 \
...
print('D08P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d08p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d08p01','primary')]['movement_probe']=='PASS'
assert validations[('d08p01','primary')]['blocked_probe']=='PASS'
assert validations[('d08p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D08P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d08p01.rsground | cut -d' ' -f1)" = 0f3c504855d3a51d8f13c0f4b802dc91fbc16604abc6a201c3e1fa6c9e37d682
test "$(sha256sum Content/Tile/d08p01_Base.tile | cut -d' ' -f1)" = 7cdbddd4e85c0b145985571799ae55f167e35220bc2b0939b9a8d2f9b6e9aba1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d08p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d08p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d08p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
