#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b03p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b03p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b03p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b03p01b.rsground" | cut -d' ' -f1)" = 6ce5f31699cb3d5946898116c5deca1ecf8851a79ae91b07d9a84fa7cf28c8e6
test "$(sha256sum "$CANONICAL/tiles/b03p01b_Base.tile" | cut -d' ' -f1)" = 5f99e441ae5d88ad97797422ca4dc19635017b590b2bef18c75586bb55b902ff
test "$(sha256sum RESERVE/red_grounds/b03p01b.rsground | cut -d' ' -f1)" = b4c9875f18468112187abc42948fe6afc63a68adc24601075d71a5e6d7cbb074
test "$(sha256sum RESERVE/red_tiles/b03p01b_Base.tile | cut -d' ' -f1)" = 5f99e441ae5d88ad97797422ca4dc19635017b590b2bef18c75586bb55b902ff
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b03p01b')
assert entry == {'canonical_debug_id': 23, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00023', 'debug_string_address': '0x0827B7FC', 'ground_place_id': 10, 'map_file_id': 24, 'map_id': 23, 'stable_ground_id': 'b03p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_EEVEE_FINAL' in header
print('B03P01B_IDENTITY_PASS map_file_id=24 symbol=MAP_TEAM_BASE_EEVEE_FINAL')
PYID
...
assert r['grounds']==['b03p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b03p01b','primary')]['movement_probe']=='PASS'
assert validations[('b03p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b03p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B03P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b03p01b.rsground | cut -d' ' -f1)" = 6ce5f31699cb3d5946898116c5deca1ecf8851a79ae91b07d9a84fa7cf28c8e6
test "$(sha256sum Content/Tile/b03p01b_Base.tile | cut -d' ' -f1)" = 5f99e441ae5d88ad97797422ca4dc19635017b590b2bef18c75586bb55b902ff
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b03p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b03p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b03p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
