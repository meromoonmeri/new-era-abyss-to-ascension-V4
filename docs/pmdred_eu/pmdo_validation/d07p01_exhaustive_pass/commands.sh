#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d07p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d07p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d07p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d07p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d07p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d07p01.rsground" | cut -d' ' -f1)" = 5dfbaac562d870a81e4df4523b336e2c45d0573d128b921d833de7a4995a304f
test "$(sha256sum "$CANONICAL/tiles/d07p01_Base.tile" | cut -d' ' -f1)" = aad06c8cebb4c28b22c8954111a6d26e266601d8d1c0c1a2cc797f0079b09fed
test "$(sha256sum RESERVE/red_grounds/d07p01.rsground | cut -d' ' -f1)" = 7d96a8970dc39cd22de3a8d94769a0d0a1b8619682717f569c5a97c5e2d63b7e
test "$(sha256sum RESERVE/red_tiles/d07p01_Base.tile | cut -d' ' -f1)" = aad06c8cebb4c28b22c8954111a6d26e266601d8d1c0c1a2cc797f0079b09fed
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d07p01')
assert entry == {'canonical_debug_id': 200, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00200', 'debug_string_address': '0x0827A2DC', 'ground_place_id': 20, 'map_file_id': 205, 'map_id': 196, 'stable_ground_id': 'd07p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MT_BLAZE_MID' in header
print('D07P01_IDENTITY_PASS map_file_id=205 symbol=MAP_MT_BLAZE_MID')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d07p01 --expected-screenshots 2 \
...
print('D07P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d07p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d07p01','primary')]['movement_probe']=='PASS'
assert validations[('d07p01','primary')]['blocked_probe']=='PASS'
assert validations[('d07p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D07P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d07p01.rsground | cut -d' ' -f1)" = 5dfbaac562d870a81e4df4523b336e2c45d0573d128b921d833de7a4995a304f
test "$(sha256sum Content/Tile/d07p01_Base.tile | cut -d' ' -f1)" = aad06c8cebb4c28b22c8954111a6d26e266601d8d1c0c1a2cc797f0079b09fed
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d07p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d07p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d07p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
