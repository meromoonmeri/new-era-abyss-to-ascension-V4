...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d07p01')
assert e == {'canonical_debug_id': 200, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00200', 'debug_string_address': '0x0827A2DC', 'ground_place_id': 20, 'map_file_id': 205, 'map_id': 196, 'stable_ground_id': 'd07p01', 'weather_id': -1}
assert 'MAP_MT_BLAZE_MID' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D07P01_IDENTITY_PASS map_id=196 map_file_id=205 symbol=MAP_MT_BLAZE_MID')
PYID
...
print('D07P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
