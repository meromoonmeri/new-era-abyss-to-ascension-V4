#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p02c.rsground" | cut -d' ' -f1)" = d51289943319760998767e9e90612cd3c379f3e154450d901a6de4d979e02118
test "$(sha256sum "$CANONICAL/tiles/b11p02c_Base.tile" | cut -d' ' -f1)" = a6aef8ccf4a4ababbfe2c698c453a5a922f939d5b3de69b2d421d13ae9f790df
test "$(sha256sum RESERVE/red_grounds/b11p02c.rsground | cut -d' ' -f1)" = fefa644b5e2cac1e072eb8472ad51d0d68e50af4ed5d117a66ac7da961dab071
test "$(sha256sum RESERVE/red_tiles/b11p02c_Base.tile | cut -d' ' -f1)" = a6aef8ccf4a4ababbfe2c698c453a5a922f939d5b3de69b2d421d13ae9f790df
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p02c')
assert entry == {'canonical_debug_id': 75, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00075', 'debug_string_address': '0x0827B17C', 'ground_place_id': 11, 'map_file_id': 76, 'map_id': 75, 'stable_ground_id': 'b11p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CUBONE_BASIC' in header
print('B11P02C_IDENTITY_PASS map_file_id=76 symbol=MAP_TEAM_BASE_CUBONE_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p02c --expected-screenshots 59 \
...
print('B11P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p02c'] and r['sample_count']==59 and len(primary)==58
...
assert validations[('b11p02c','primary')]['movement_probe']=='PASS'
assert validations[('b11p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b11p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P02C_EXHAUSTIVE_PASS samples=59 unique_rgba=27')
PY
...
test "$(sha256sum Data/Ground/b11p02c.rsground | cut -d' ' -f1)" = d51289943319760998767e9e90612cd3c379f3e154450d901a6de4d979e02118
test "$(sha256sum Content/Tile/b11p02c_Base.tile | cut -d' ' -f1)" = a6aef8ccf4a4ababbfe2c698c453a5a922f939d5b3de69b2d421d13ae9f790df
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
