...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p06')
assert e == {'canonical_debug_id': 133, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00133', 'debug_string_address': '0x0827AADC', 'ground_place_id': 9, 'map_file_id': 134, 'map_id': 128, 'stable_ground_id': 'h07p06', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_TRANSFORM_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H07P06_IDENTITY_PASS map_id=128 map_file_id=134 symbol=MAP_FRIEND_AREA_TRANSFORM_FOREST')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 11929
...
print('H07P06_EXHAUSTIVE_PASS samples=3969 unique_rgba=884 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
