#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p02c.rsground" | cut -d' ' -f1)" = 158b389fe3f8ef93e7bbae602812cb5bb97d3e0df4cd4289af3d48657d594a8c
test "$(sha256sum "$CANONICAL/tiles/b13p02c_Base.tile" | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
test "$(sha256sum RESERVE/red_grounds/b13p02c.rsground | cut -d' ' -f1)" = fed986d8a2e18cfa49a69ca4e31d4154fc0d0c63924b6d64c2ab41fa13db2083
test "$(sha256sum RESERVE/red_tiles/b13p02c_Base.tile | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02c')
assert entry == {'canonical_debug_id': 87, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00087', 'debug_string_address': '0x0827AFFC', 'ground_place_id': 11, 'map_file_id': 88, 'map_id': 87, 'stable_ground_id': 'b13p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_BULBASAUR_BASIC' in header
print('B13P02C_IDENTITY_PASS map_file_id=88 symbol=MAP_TEAM_BASE_BULBASAUR_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p02c --expected-screenshots 2 \
...
print('B13P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b13p02c','primary')]['movement_probe']=='PASS'
assert validations[('b13p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b13p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b13p02c.rsground | cut -d' ' -f1)" = 158b389fe3f8ef93e7bbae602812cb5bb97d3e0df4cd4289af3d48657d594a8c
test "$(sha256sum Content/Tile/b13p02c_Base.tile | cut -d' ' -f1)" = c29d9fb9f2f081d216c0b49991b4e9a3287574ea7bcd9598294df20d17f4a372
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
