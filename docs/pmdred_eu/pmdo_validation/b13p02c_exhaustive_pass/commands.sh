...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p02c')
assert e == {'canonical_debug_id': 87, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00087', 'debug_string_address': '0x0827AFFC', 'ground_place_id': 11, 'map_file_id': 88, 'map_id': 87, 'stable_ground_id': 'b13p02c', 'weather_id': -1}
assert 'MAP_TEAM_BASE_BULBASAUR_BASIC' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B13P02C_IDENTITY_PASS map_id=87 map_file_id=88 symbol=MAP_TEAM_BASE_BULBASAUR_BASIC')
PYID
...
print('B13P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
