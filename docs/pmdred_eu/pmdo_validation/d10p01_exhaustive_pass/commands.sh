...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d10p01')
assert e == {'canonical_debug_id': 207, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00207', 'debug_string_address': '0x0827A1FC', 'ground_place_id': 26, 'map_file_id': 212, 'map_id': 203, 'stable_ground_id': 'd10p01', 'weather_id': 10}
assert 'MAP_MT_FREEZE_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D10P01_IDENTITY_PASS map_id=203 map_file_id=212 symbol=MAP_MT_FREEZE_END')
PYID
...
print('D10P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
