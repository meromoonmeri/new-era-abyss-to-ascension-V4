#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d10p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d10p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d10p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d10p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d10p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d10p01.rsground" | cut -d' ' -f1)" = 9b6e1d3a233461a52c42fa766e78f78b16e880a92ce6abe691ae67494fc7c698
test "$(sha256sum "$CANONICAL/tiles/d10p01_Base.tile" | cut -d' ' -f1)" = 076d64fb6b4f86ef10fc0861a1b2cf4e01f4d88a434b220c886575c6fe30898c
test "$(sha256sum RESERVE/red_grounds/d10p01.rsground | cut -d' ' -f1)" = c65d31be89696f8db3cd93b93a72f6b5c946c59fd381cd64fc83b11d009787a2
test "$(sha256sum RESERVE/red_tiles/d10p01_Base.tile | cut -d' ' -f1)" = 076d64fb6b4f86ef10fc0861a1b2cf4e01f4d88a434b220c886575c6fe30898c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d10p01')
assert entry == {'canonical_debug_id': 207, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00207', 'debug_string_address': '0x0827A1FC', 'ground_place_id': 26, 'map_file_id': 212, 'map_id': 203, 'stable_ground_id': 'd10p01', 'weather_id': 10}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MT_FREEZE_END' in header
print('D10P01_IDENTITY_PASS map_file_id=212 symbol=MAP_MT_FREEZE_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d10p01 --expected-screenshots 2 \
...
print('D10P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d10p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d10p01','primary')]['movement_probe']=='PASS'
assert validations[('d10p01','primary')]['blocked_probe']=='PASS'
assert validations[('d10p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D10P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d10p01.rsground | cut -d' ' -f1)" = 9b6e1d3a233461a52c42fa766e78f78b16e880a92ce6abe691ae67494fc7c698
test "$(sha256sum Content/Tile/d10p01_Base.tile | cut -d' ' -f1)" = 076d64fb6b4f86ef10fc0861a1b2cf4e01f4d88a434b220c886575c6fe30898c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d10p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d10p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d10p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
