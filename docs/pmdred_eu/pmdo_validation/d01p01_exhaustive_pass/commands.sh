...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d01p01')
assert e == {'canonical_debug_id': 187, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00187', 'debug_string_address': '0x0827A47C', 'ground_place_id': 13, 'map_file_id': 192, 'map_id': 183, 'stable_ground_id': 'd01p01', 'weather_id': -1}
assert 'MAP_MT_STEEL_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D01P01_IDENTITY_PASS map_id=183 map_file_id=192 symbol=MAP_MT_STEEL_END')
PYID
...
print('D01P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
