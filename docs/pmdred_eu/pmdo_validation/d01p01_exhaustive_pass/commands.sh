#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d01p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d01p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d01p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d01p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d01p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d01p01.rsground" | cut -d' ' -f1)" = 85b5d0ffaec562825adc08f7b011d3bc2727caad9b4dd0c7ffc77b3f2d53359e
test "$(sha256sum "$CANONICAL/tiles/d01p01_Base.tile" | cut -d' ' -f1)" = 1ab9bac0f3256b52f5dc58fed78684d3b4297fafc2e69a8e32dfbe7d898be39b
test "$(sha256sum RESERVE/red_grounds/d01p01.rsground | cut -d' ' -f1)" = 2fb9c6d60404146ca849cde31773ed6e9f95e956de266e633bc34d2ff4f360ee
test "$(sha256sum RESERVE/red_tiles/d01p01_Base.tile | cut -d' ' -f1)" = 1ab9bac0f3256b52f5dc58fed78684d3b4297fafc2e69a8e32dfbe7d898be39b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d01p01')
assert entry == {'canonical_debug_id': 187, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00187', 'debug_string_address': '0x0827A47C', 'ground_place_id': 13, 'map_file_id': 192, 'map_id': 183, 'stable_ground_id': 'd01p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_MT_STEEL_END' in header
print('D01P01_IDENTITY_PASS map_file_id=192 symbol=MAP_MT_STEEL_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d01p01 --expected-screenshots 2 \
...
print('D01P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d01p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d01p01','primary')]['movement_probe']=='PASS'
assert validations[('d01p01','primary')]['blocked_probe']=='PASS'
assert validations[('d01p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D01P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d01p01.rsground | cut -d' ' -f1)" = 85b5d0ffaec562825adc08f7b011d3bc2727caad9b4dd0c7ffc77b3f2d53359e
test "$(sha256sum Content/Tile/d01p01_Base.tile | cut -d' ' -f1)" = 1ab9bac0f3256b52f5dc58fed78684d3b4297fafc2e69a8e32dfbe7d898be39b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d01p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d01p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d01p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
