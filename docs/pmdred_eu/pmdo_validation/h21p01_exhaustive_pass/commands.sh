#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h21p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h21p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h21p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h21p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h21p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h21p01.rsground" | cut -d' ' -f1)" = bf0fc76bb9c18d573b82a4ab6fb96b4c9fe3ee0933437c6fe68971637cc61471
test "$(sha256sum "$CANONICAL/tiles/h21p01_Base.tile" | cut -d' ' -f1)" = bdda233c00af4b07c955aae03638a801ead33d35ca10134aa319c0216d5d1fb2
test "$(sha256sum RESERVE/red_grounds/h21p01.rsground | cut -d' ' -f1)" = 25b0ce92ccbda98cae1b25bef2919c3548b173832bcd2e5cd5652b5b5b7344db
test "$(sha256sum RESERVE/red_tiles/h21p01_Base.tile | cut -d' ' -f1)" = da9d616b8a3bf61233a4c718a524906eb3403dd58b558d5fe15721609b085b9d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h21p01')
assert entry == {'canonical_debug_id': 156, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00156', 'debug_string_address': '0x0827A83C', 'ground_place_id': 9, 'map_file_id': 157, 'map_id': 149, 'stable_ground_id': 'h21p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_AGED_CHAMBER_O_EXCLAIM' in header
print('H21P01_IDENTITY_PASS map_file_id=157 symbol=MAP_FRIEND_AREA_AGED_CHAMBER_O_EXCLAIM')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h21p01 --expected-screenshots 61 \
...
print('H21P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 208
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 61
...
assert r['grounds']==['h21p01'] and r['sample_count']==61 and len(primary)==60
assert r['exact_sample_count']==61 and r['fully_opaque_sample_count']==61
assert len({s['actual_rgba_sha256'] for s in primary})==14
...
assert validations[('h21p01','primary')]['movement_probe']=='PASS'
assert validations[('h21p01','primary')]['blocked_probe']=='PASS'
assert validations[('h21p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H21P01_EXHAUSTIVE_PASS samples=61 unique_rgba=14')
PY
...
test "$(sha256sum Data/Ground/h21p01.rsground | cut -d' ' -f1)" = bf0fc76bb9c18d573b82a4ab6fb96b4c9fe3ee0933437c6fe68971637cc61471
test "$(sha256sum Content/Tile/h21p01_Base.tile | cut -d' ' -f1)" = bdda233c00af4b07c955aae03638a801ead33d35ca10134aa319c0216d5d1fb2
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h21p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h21p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h21p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 61 comparative PNGs,
