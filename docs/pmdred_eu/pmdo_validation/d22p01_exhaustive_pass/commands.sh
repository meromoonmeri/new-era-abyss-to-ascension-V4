#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d22p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d22p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d22p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d22p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d22p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d22p01.rsground" | cut -d' ' -f1)" = ac5e2cb3665a902d7058cc38f41e695b09715f61353f7a6c135900c9f8f69d05
test "$(sha256sum "$CANONICAL/tiles/d22p01_Base.tile" | cut -d' ' -f1)" = dff0f69cfcf41e6571d308a850099ec018a657253905b7f783d2c041da8c0c7d
test "$(sha256sum RESERVE/red_grounds/d22p01.rsground | cut -d' ' -f1)" = 11081705a5121cb39aedf6ccd9d1b0030951de4d68fbb8599edd8c0be47ea002
test "$(sha256sum RESERVE/red_tiles/d22p01_Base.tile | cut -d' ' -f1)" = dff0f69cfcf41e6571d308a850099ec018a657253905b7f783d2c041da8c0c7d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d22p01')
assert entry == {'canonical_debug_id': 227, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00227', 'debug_string_address': '0x08279F7C', 'ground_place_id': 12, 'map_file_id': 232, 'map_id': 223, 'stable_ground_id': 'd22p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_PERSONALITY_TEST_MULTICOLOR' in header
print('D22P01_IDENTITY_PASS map_file_id=232 symbol=MAP_PERSONALITY_TEST_MULTICOLOR')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d22p01 --expected-screenshots 2 \
...
print('D22P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d22p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('d22p01','primary')]['movement_probe']=='PASS'
assert validations[('d22p01','primary')]['blocked_probe']=='PASS'
assert validations[('d22p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D22P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/d22p01.rsground | cut -d' ' -f1)" = ac5e2cb3665a902d7058cc38f41e695b09715f61353f7a6c135900c9f8f69d05
test "$(sha256sum Content/Tile/d22p01_Base.tile | cut -d' ' -f1)" = dff0f69cfcf41e6571d308a850099ec018a657253905b7f783d2c041da8c0c7d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d22p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d22p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d22p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
