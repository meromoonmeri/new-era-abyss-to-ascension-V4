...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d22p01')
assert e == {'canonical_debug_id': 227, 'conversion_type': 3, 'debug_string': '__ground_amd_conversion_00227', 'debug_string_address': '0x08279F7C', 'ground_place_id': 12, 'map_file_id': 232, 'map_id': 223, 'stable_ground_id': 'd22p01', 'weather_id': -1}
assert 'MAP_PERSONALITY_TEST_MULTICOLOR' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D22P01_IDENTITY_PASS map_id=223 map_file_id=232 symbol=MAP_PERSONALITY_TEST_MULTICOLOR')
PYID
...
print('D22P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
