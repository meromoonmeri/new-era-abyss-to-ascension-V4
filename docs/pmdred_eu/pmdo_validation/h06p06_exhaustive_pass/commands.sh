#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h06p06 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h06p06-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h06p06-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h06p06-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h06p06-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h06p06.rsground" | cut -d' ' -f1)" = 3ee8c2b92943513ecaf0c1ae7f7aa1db1f23df640c17fec320e91f16a2e9b2b7
test "$(sha256sum "$CANONICAL/tiles/h06p06_Base.tile" | cut -d' ' -f1)" = 800957191d3128e99be84a2597f396c19ddbd34a9b811fd82b9d089a59c62a66
test "$(sha256sum RESERVE/red_grounds/h06p06.rsground | cut -d' ' -f1)" = cceace077f2a6f51b56ac16b2472e7f89bbf7ae2ebd327d6d6437f0455e283ee
test "$(sha256sum RESERVE/red_tiles/h06p06_Base.tile | cut -d' ' -f1)" = 800957191d3128e99be84a2597f396c19ddbd34a9b811fd82b9d089a59c62a66
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h06p06')
assert entry == {'canonical_debug_id': 126, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00126', 'debug_string_address': '0x0827AB9C', 'ground_place_id': 9, 'map_file_id': 127, 'map_id': 122, 'stable_ground_id': 'h06p06', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MIST_RISE_FOREST' in header
print('H06P06_IDENTITY_PASS map_file_id=127 symbol=MAP_FRIEND_AREA_MIST_RISE_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h06p06 --expected-screenshots 2 \
...
print('H06P06_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h06p06'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h06p06','primary')]['movement_probe']=='PASS'
assert validations[('h06p06','primary')]['blocked_probe']=='PASS'
assert validations[('h06p06','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H06P06_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h06p06.rsground | cut -d' ' -f1)" = 3ee8c2b92943513ecaf0c1ae7f7aa1db1f23df640c17fec320e91f16a2e9b2b7
test "$(sha256sum Content/Tile/h06p06_Base.tile | cut -d' ' -f1)" = 800957191d3128e99be84a2597f396c19ddbd34a9b811fd82b9d089a59c62a66
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h06p06.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h06p06_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h06p06 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
