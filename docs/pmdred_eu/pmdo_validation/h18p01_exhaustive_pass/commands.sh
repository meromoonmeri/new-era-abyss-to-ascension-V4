...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h18p01')
assert e == {'canonical_debug_id': 152, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00152', 'debug_string_address': '0x0827A89C', 'ground_place_id': 9, 'map_file_id': 153, 'map_id': 146, 'stable_ground_id': 'h18p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_CRATER' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H18P01_IDENTITY_PASS map_id=146 map_file_id=153 symbol=MAP_FRIEND_AREA_CRATER')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 160
...
print('H18P01_EXHAUSTIVE_PASS samples=46 unique_rgba=33 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
