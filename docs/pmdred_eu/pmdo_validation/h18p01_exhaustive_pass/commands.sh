#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h18p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h18p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h18p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h18p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h18p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h18p01.rsground" | cut -d' ' -f1)" = 81bec39947e9a90abdbf48e224b58dfe89a95ca09fa7b4bcaa9a074c78a225c4
test "$(sha256sum "$CANONICAL/tiles/h18p01_Base.tile" | cut -d' ' -f1)" = a0f68e18a8a2db88eb7bc02341539a61cd08811b9f5ac3332dfc484a41ce9403
test "$(sha256sum RESERVE/red_grounds/h18p01.rsground | cut -d' ' -f1)" = cd8af860fa8cf8512c833532d8e9c2aaf2e81fffd3ed658a2b70e4f9fcf0e105
test "$(sha256sum RESERVE/red_tiles/h18p01_Base.tile | cut -d' ' -f1)" = fb59cc53911dd3e66832b1fac5e6b10aae9785902ab93865243c811a902abffd
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h18p01')
assert entry == {'canonical_debug_id': 152, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00152', 'debug_string_address': '0x0827A89C', 'ground_place_id': 9, 'map_file_id': 153, 'map_id': 146, 'stable_ground_id': 'h18p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_CRATER' in header
print('H18P01_IDENTITY_PASS map_file_id=153 symbol=MAP_FRIEND_AREA_CRATER')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h18p01 --expected-screenshots 46 \
...
print('H18P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 163
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 46
...
assert r['grounds']==['h18p01'] and r['sample_count']==46 and len(primary)==45
assert r['exact_sample_count']==46 and r['fully_opaque_sample_count']==46
assert len({s['actual_rgba_sha256'] for s in primary})==33
...
assert validations[('h18p01','primary')]['movement_probe']=='PASS'
assert validations[('h18p01','primary')]['blocked_probe']=='PASS'
assert validations[('h18p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H18P01_EXHAUSTIVE_PASS samples=46 unique_rgba=33')
PY
...
test "$(sha256sum Data/Ground/h18p01.rsground | cut -d' ' -f1)" = 81bec39947e9a90abdbf48e224b58dfe89a95ca09fa7b4bcaa9a074c78a225c4
test "$(sha256sum Content/Tile/h18p01_Base.tile | cut -d' ' -f1)" = a0f68e18a8a2db88eb7bc02341539a61cd08811b9f5ac3332dfc484a41ce9403
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h18p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h18p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h18p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 46 comparative PNGs,
