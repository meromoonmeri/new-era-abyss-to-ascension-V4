#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a04p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a04p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a04p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a04p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a04p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a04p03.rsground" | cut -d' ' -f1)" = 082fd713704a37b0a971382cdcaed6ad07beabf5d889dd8bc98eaa772c4fb909
test "$(sha256sum "$CANONICAL/tiles/a04p03_Base.tile" | cut -d' ' -f1)" = e566229a6550297f5dcd953a250a315a50da20149001a59c19ee776bbd534999
test "$(sha256sum RESERVE/red_grounds/a04p03.rsground | cut -d' ' -f1)" = 2962ec601ffbeb84ddcab439f347433cd6f457aed449115a0f2d2ece1a046bc1
test "$(sha256sum RESERVE/red_tiles/a04p03_Base.tile | cut -d' ' -f1)" = e566229a6550297f5dcd953a250a315a50da20149001a59c19ee776bbd534999
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p03')
assert entry == {'canonical_debug_id': 182, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00182', 'debug_string_address': '0x0827A51C', 'ground_place_id': 35, 'map_file_id': 183, 'map_id': 174, 'stable_ground_id': 'a04p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_NIGHTMARE' in header
print('A04P03_IDENTITY_PASS map_file_id=183 symbol=MAP_NIGHTMARE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a04p03 --expected-screenshots 481 \
...
print('A04P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a04p03'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('a04p03','primary')]['movement_probe']=='PASS'
assert validations[('a04p03','primary')]['blocked_probe']=='PASS'
assert validations[('a04p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A04P03_EXHAUSTIVE_PASS samples=481 unique_rgba=24')
PY
...
test "$(sha256sum Data/Ground/a04p03.rsground | cut -d' ' -f1)" = 082fd713704a37b0a971382cdcaed6ad07beabf5d889dd8bc98eaa772c4fb909
test "$(sha256sum Content/Tile/a04p03_Base.tile | cut -d' ' -f1)" = e566229a6550297f5dcd953a250a315a50da20149001a59c19ee776bbd534999
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a04p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a04p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a04p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
