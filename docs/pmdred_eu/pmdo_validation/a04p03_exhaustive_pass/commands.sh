...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p03')
assert e == {'canonical_debug_id': 182, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00182', 'debug_string_address': '0x0827A51C', 'ground_place_id': 35, 'map_file_id': 183, 'map_id': 174, 'stable_ground_id': 'a04p03', 'weather_id': -1}
assert 'MAP_NIGHTMARE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A04P03_IDENTITY_PASS map_id=174 map_file_id=183 symbol=MAP_NIGHTMARE')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 1462
...
print('A04P03_EXHAUSTIVE_PASS samples=481 unique_rgba=24 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
