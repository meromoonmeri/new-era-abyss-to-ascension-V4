#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b07p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b07p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b07p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b07p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b07p02a.rsground" | cut -d' ' -f1)" = 2f81249377f7bd34a7a7017ee77518f56b80598656a6414c0a92edc8cb5c13e9
test "$(sha256sum "$CANONICAL/tiles/b07p02a_Base.tile" | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b07p02a')
assert entry == {'canonical_debug_id': 49, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00049', 'debug_string_address': '0x0827B4BC', 'ground_place_id': 11, 'map_file_id': 50, 'map_id': 49, 'stable_ground_id': 'b07p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MUDKIP_CONSTRUCTION' in header
print('B07P02A_IDENTITY_PASS map_id=49 map_file_id=50 symbol=MAP_TEAM_BASE_INSIDE_MUDKIP_CONSTRUCTION')
PYID
...
assert r['grounds']==['b07p02a'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b07p02a','primary')]['movement_probe']=='PASS'
assert validations[('b07p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b07p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B07P02A_EXHAUSTIVE_PASS samples=481 unique_rgba=233 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b07p02a.rsground | cut -d' ' -f1)" = 2f81249377f7bd34a7a7017ee77518f56b80598656a6414c0a92edc8cb5c13e9
test "$(sha256sum Content/Tile/b07p02a_Base.tile | cut -d' ' -f1)" = 1387a0495a57cc1f7e07baf995632054b4a0e71329ef099ce5e65fb0def38e46
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b07p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b07p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b07p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
