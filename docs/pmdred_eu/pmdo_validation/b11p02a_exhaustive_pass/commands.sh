#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p02a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p02a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p02a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p02a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p02a.rsground" | cut -d' ' -f1)" = 0371b26a604bdb8ad97ee3bc833a9c1616b9ad6d4ca13cc0c21ef3a0b128ae3b
test "$(sha256sum "$CANONICAL/tiles/b11p02a_Base.tile" | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test "$(sha256sum RESERVE/red_grounds/b11p02a.rsground | cut -d' ' -f1)" = 3ea9593f8a094651be311409f7e2a0ca7efbd7fe9d881d50dc6334d2f522c719
test "$(sha256sum RESERVE/red_tiles/b11p02a_Base.tile | cut -d' ' -f1)" = 736104cafcda450a114285c37f4afd89c0da686118e1296e068603667727e2c9
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p02a')
assert entry == {'canonical_debug_id': 73, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00073', 'debug_string_address': '0x0827B1BC', 'ground_place_id': 11, 'map_file_id': 74, 'map_id': 73, 'stable_ground_id': 'b11p02a', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_CYNDAQUIL_CONSTRUCTION' in header
print('B11P02A_IDENTITY_PASS map_file_id=74 symbol=MAP_TEAM_BASE_INSIDE_CYNDAQUIL_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p02a --expected-screenshots 1561 \
...
print('B11P02A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p02a'] and r['sample_count']==1561 and len(primary)==1560
...
assert validations[('b11p02a','primary')]['movement_probe']=='PASS'
assert validations[('b11p02a','primary')]['blocked_probe']=='PASS'
assert validations[('b11p02a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P02A_EXHAUSTIVE_PASS samples=1561 unique_rgba=36')
PY
...
test "$(sha256sum Data/Ground/b11p02a.rsground | cut -d' ' -f1)" = 0371b26a604bdb8ad97ee3bc833a9c1616b9ad6d4ca13cc0c21ef3a0b128ae3b
test "$(sha256sum Content/Tile/b11p02a_Base.tile | cut -d' ' -f1)" = 9706a7b3404e05ba899ea5ff8282252c5c3e8a9569a8b8335676d72a4da4dbf0
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p02a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p02a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p02a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
