#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b12p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b12p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b12p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b12p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b12p01b.rsground" | cut -d' ' -f1)" = f19ed32128f515fba1e88c455d6d6fc108464a5e79febbb13e1866b51475ad3a
test "$(sha256sum "$CANONICAL/tiles/b12p01b_Base.tile" | cut -d' ' -f1)" = 516511d98fedb769f141f7b21ba9149d559d485cd51f584adad2280a23fa954d
test "$(sha256sum RESERVE/red_grounds/b12p01b.rsground | cut -d' ' -f1)" = 9bb75d4722f08b3a39d57dacd11408b0a1e6d85f992f7d9d82aeb6f834013e9c
test "$(sha256sum RESERVE/red_tiles/b12p01b_Base.tile | cut -d' ' -f1)" = 516511d98fedb769f141f7b21ba9149d559d485cd51f584adad2280a23fa954d
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b12p01b')
assert entry == {'canonical_debug_id': 77, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00077', 'debug_string_address': '0x0827B13C', 'ground_place_id': 10, 'map_file_id': 78, 'map_id': 77, 'stable_ground_id': 'b12p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CUBONE_FINAL' in header
print('B12P01B_IDENTITY_PASS map_file_id=78 symbol=MAP_TEAM_BASE_CUBONE_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b12p01b --expected-screenshots 25 \
...
print('B12P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b12p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b12p01b','primary')]['movement_probe']=='PASS'
assert validations[('b12p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b12p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B12P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b12p01b.rsground | cut -d' ' -f1)" = f19ed32128f515fba1e88c455d6d6fc108464a5e79febbb13e1866b51475ad3a
test "$(sha256sum Content/Tile/b12p01b_Base.tile | cut -d' ' -f1)" = 516511d98fedb769f141f7b21ba9149d559d485cd51f584adad2280a23fa954d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b12p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b12p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b12p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
