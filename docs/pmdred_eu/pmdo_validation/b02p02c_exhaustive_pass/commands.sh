#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b02p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b02p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b02p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b02p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b02p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b02p02c.rsground" | cut -d' ' -f1)" = 298913218035d5dbbc89679187654a515d32b479b7d0e206dfd1e5cc8e590ff1
test "$(sha256sum "$CANONICAL/tiles/b02p02c_Base.tile" | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
test "$(sha256sum RESERVE/red_grounds/b02p02c.rsground | cut -d' ' -f1)" = c3357308d2f02c7cc49d579dea7a221018df30ac79162e31d57fcbf733dc2443
test "$(sha256sum RESERVE/red_tiles/b02p02c_Base.tile | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b02p02c')
assert entry == {'canonical_debug_id': 21, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00021', 'debug_string_address': '0x0827B83C', 'ground_place_id': 11, 'map_file_id': 22, 'map_id': 21, 'stable_ground_id': 'b02p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_EEVEE_BASIC' in header
print('B02P02C_IDENTITY_PASS map_file_id=22 symbol=MAP_TEAM_BASE_EEVEE_BASIC')
PYID
...
assert r['grounds']==['b02p02c'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('b02p02c','primary')]['movement_probe']=='PASS'
assert validations[('b02p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b02p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B02P02C_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/b02p02c.rsground | cut -d' ' -f1)" = 298913218035d5dbbc89679187654a515d32b479b7d0e206dfd1e5cc8e590ff1
test "$(sha256sum Content/Tile/b02p02c_Base.tile | cut -d' ' -f1)" = ad1ffe29272457999ae976b16b5d447183fbf83b4d09bb4f508e1d097f37f142
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b02p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b02p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b02p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
