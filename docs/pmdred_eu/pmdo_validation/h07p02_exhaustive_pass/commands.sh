#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p02.rsground" | cut -d' ' -f1)" = 569848c346b09531a892d5672b0738d7a00bc23632ba712c87c4d573cac2348a
test "$(sha256sum "$CANONICAL/tiles/h07p02_Base.tile" | cut -d' ' -f1)" = d479ac29fa1dced7fb092eaa509156504f23714c8a5de1a76bef79c7debb8247
test "$(sha256sum RESERVE/red_grounds/h07p02.rsground | cut -d' ' -f1)" = 0ba371b822d8967a87b297daae35a358cd8ed2bb4617ddebee0b63672ad4dcb8
test "$(sha256sum RESERVE/red_tiles/h07p02_Base.tile | cut -d' ' -f1)" = d479ac29fa1dced7fb092eaa509156504f23714c8a5de1a76bef79c7debb8247
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p02')
assert entry == {'canonical_debug_id': 128, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00128', 'debug_string_address': '0x0827AB5C', 'ground_place_id': 9, 'map_file_id': 129, 'map_id': 124, 'stable_ground_id': 'h07p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_OVERGROWN_FOREST' in header
print('H07P02_IDENTITY_PASS map_file_id=129 symbol=MAP_FRIEND_AREA_OVERGROWN_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p02 --expected-screenshots 33 \
...
print('H07P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 124
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 33
...
assert r['grounds']==['h07p02'] and r['sample_count']==33 and len(primary)==32
assert r['exact_sample_count']==33 and r['fully_opaque_sample_count']==33
...
assert validations[('h07p02','primary')]['movement_probe']=='PASS'
assert validations[('h07p02','primary')]['blocked_probe']=='PASS'
assert validations[('h07p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P02_EXHAUSTIVE_PASS samples=33 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/h07p02.rsground | cut -d' ' -f1)" = 569848c346b09531a892d5672b0738d7a00bc23632ba712c87c4d573cac2348a
test "$(sha256sum Content/Tile/h07p02_Base.tile | cut -d' ' -f1)" = d479ac29fa1dced7fb092eaa509156504f23714c8a5de1a76bef79c7debb8247
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 33 comparative PNGs,
