#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a04p04 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a04p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a04p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a04p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a04p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a04p04.rsground" | cut -d' ' -f1)" = bd11eb579038a7edb165b3a3e3431207e8a7c049c29244f9891b559b1c07ef82
test "$(sha256sum "$CANONICAL/tiles/a04p04_Base.tile" | cut -d' ' -f1)" = 73b12eb18692df73095cb580c78ba581da23e414c38493b99f040641f32abb75
test "$(sha256sum RESERVE/red_grounds/a04p04.rsground | cut -d' ' -f1)" = 60f3169a2fd9950ebcc4d68157cd189f37a277df2be7efd0672c36264ce16568
test "$(sha256sum RESERVE/red_tiles/a04p04_Base.tile | cut -d' ' -f1)" = 73b12eb18692df73095cb580c78ba581da23e414c38493b99f040641f32abb75
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a04p04')
assert entry == {'canonical_debug_id': 183, 'conversion_type': 6, 'debug_string': '__ground_amd_conversion_00183', 'debug_string_address': '0x0827A4FC', 'ground_place_id': 35, 'map_file_id': 184, 'map_id': 175, 'stable_ground_id': 'a04p04', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_NIGHT_SKY_1' in header
print('A04P04_IDENTITY_PASS map_file_id=184 symbol=MAP_NIGHT_SKY_1')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a04p04 --expected-screenshots 2 \
...
print('A04P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a04p04'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a04p04','primary')]['movement_probe']=='PASS'
assert validations[('a04p04','primary')]['blocked_probe']=='PASS'
assert validations[('a04p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A04P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a04p04.rsground | cut -d' ' -f1)" = bd11eb579038a7edb165b3a3e3431207e8a7c049c29244f9891b559b1c07ef82
test "$(sha256sum Content/Tile/a04p04_Base.tile | cut -d' ' -f1)" = 73b12eb18692df73095cb580c78ba581da23e414c38493b99f040641f32abb75
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a04p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a04p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a04p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
