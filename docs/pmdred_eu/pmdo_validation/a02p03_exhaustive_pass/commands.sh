#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a02p03 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a02p03.rsground" | cut -d' ' -f1)" = b6cfa6d2cfc5e550cf85f23f321af8ffe5c862b11a8332ec6d1444dd84c32bbc
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a02p03 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a02p03.rsground" | cut -d' ' -f1)" = f939c874590008a7db4217f7fd77f65d548a32ba5ccfa0701e77a0da070982a3
test "$(sha256sum "$CANONICAL/tiles/a02p03_Base.tile" | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
test "$(sha256sum RESERVE/red_grounds/a02p03.rsground | cut -d' ' -f1)" = 3904f90e0454b5863c74aab0d6455e09253e7bafdb747bbc816cefa1f2b20191
test "$(sha256sum RESERVE/red_tiles/a02p03_Base.tile | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p03')
assert entry == {'canonical_debug_id': 175, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00175', 'debug_string_address': '0x0827A5FC', 'ground_place_id': 35, 'map_file_id': 176, 'map_id': 167, 'stable_ground_id': 'a02p03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FUGITIVES_VOLCANO' in header
print('A02P03_IDENTITY_PASS map_file_id=176 symbol=MAP_FUGITIVES_VOLCANO')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a02p03 --expected-screenshots 2 \
...
print('A02P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a02p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a02p03','primary')]['movement_probe']=='PASS'
assert validations[('a02p03','primary')]['blocked_probe']=='PASS'
assert validations[('a02p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A02P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a02p03.rsground | cut -d' ' -f1)" = f939c874590008a7db4217f7fd77f65d548a32ba5ccfa0701e77a0da070982a3
test "$(sha256sum Content/Tile/a02p03_Base.tile | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a02p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a02p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a02p03 --entity-integrated-ids a02p03 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p03/a02p03.rsground | cut -d' ' -f1)" = 7f91bdf9d4ff3574ee00ffe8fcacd30aae83bc9bdec8fb2c18b0135c06ba456e
test "$(sha256sum RESERVE/pmdred_pre_promotion/a02p03/A02p03_Base.tile | cut -d' ' -f1)" = 94b275262765e0b7ea1fd209bc9f1d1dac89452d3f3e36228ed46d2f30c12e7b
