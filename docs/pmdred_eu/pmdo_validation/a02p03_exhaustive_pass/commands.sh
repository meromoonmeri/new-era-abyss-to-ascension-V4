...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a02p03-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a02p03')
assert e == {'canonical_debug_id': 175, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00175', 'debug_string_address': '0x0827A5FC', 'ground_place_id': 35, 'map_file_id': 176, 'map_id': 167, 'stable_ground_id': 'a02p03', 'weather_id': -1}
assert 'MAP_FUGITIVES_VOLCANO' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A02P03_IDENTITY_PASS map_id=167 map_file_id=176 symbol=MAP_FUGITIVES_VOLCANO')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a02p03 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a02p03.rsground" | cut -d' ' -f1)" = f939c874590008a7db4217f7fd77f65d548a32ba5ccfa0701e77a0da070982a3
...
print('A02P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a02p03 --plan "$PLAN" \
  --ids a02p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
