...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p01')
assert e == {'canonical_debug_id': 127, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00127', 'debug_string_address': '0x0827AB7C', 'ground_place_id': 9, 'map_file_id': 128, 'map_id': 123, 'stable_ground_id': 'h07p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_FLYAWAY_FOREST' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H07P01_IDENTITY_PASS map_id=123 map_file_id=128 symbol=MAP_FRIEND_AREA_FLYAWAY_FOREST')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 385
...
print('H07P01_EXHAUSTIVE_PASS samples=121 unique_rgba=19 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
