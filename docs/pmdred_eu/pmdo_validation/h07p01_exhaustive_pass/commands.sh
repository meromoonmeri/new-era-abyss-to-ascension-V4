#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h07p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h07p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h07p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h07p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h07p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h07p01.rsground" | cut -d' ' -f1)" = a1452f4b6dd0a78adb6344e79460e20b1066bc3beaaa188d3c2a53494a822fa5
test "$(sha256sum "$CANONICAL/tiles/h07p01_Base.tile" | cut -d' ' -f1)" = 420413fcafc49a434058dba0cd6021c9e3c6ac82687695e9f5de3fa5c0f3b15d
test "$(sha256sum RESERVE/red_grounds/h07p01.rsground | cut -d' ' -f1)" = 0c4ae99173a08d02de5a6150b393a7060fc0b126a4154453d18673aaba5accb1
test "$(sha256sum RESERVE/red_tiles/h07p01_Base.tile | cut -d' ' -f1)" = 0c0eeb2e84584e12b5bf491eede33ce8afc250ae61a827977577b18b070a208c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h07p01')
assert entry == {'canonical_debug_id': 127, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00127', 'debug_string_address': '0x0827AB7C', 'ground_place_id': 9, 'map_file_id': 128, 'map_id': 123, 'stable_ground_id': 'h07p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_FLYAWAY_FOREST' in header
print('H07P01_IDENTITY_PASS map_file_id=128 symbol=MAP_FRIEND_AREA_FLYAWAY_FOREST')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h07p01 --expected-screenshots 121 \
...
print('H07P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 388
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 121
...
assert r['grounds']==['h07p01'] and r['sample_count']==121 and len(primary)==120
assert r['exact_sample_count']==121 and r['fully_opaque_sample_count']==121
assert len({s['actual_rgba_sha256'] for s in primary})==19
...
assert validations[('h07p01','primary')]['movement_probe']=='PASS'
assert validations[('h07p01','primary')]['blocked_probe']=='PASS'
assert validations[('h07p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H07P01_EXHAUSTIVE_PASS samples=121 unique_rgba=19')
PY
...
test "$(sha256sum Data/Ground/h07p01.rsground | cut -d' ' -f1)" = a1452f4b6dd0a78adb6344e79460e20b1066bc3beaaa188d3c2a53494a822fa5
test "$(sha256sum Content/Tile/h07p01_Base.tile | cut -d' ' -f1)" = 420413fcafc49a434058dba0cd6021c9e3c6ac82687695e9f5de3fa5c0f3b15d
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h07p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h07p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h07p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 121 comparative PNGs,
