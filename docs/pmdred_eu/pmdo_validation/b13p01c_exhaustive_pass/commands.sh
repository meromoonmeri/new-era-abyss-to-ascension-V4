#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b13p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b13p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b13p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b13p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b13p01c.rsground" | cut -d' ' -f1)" = a9ac7d252ee9d9034006f278363e8c9de8aff213c28c28f3313456d711a4c78a
test "$(sha256sum "$CANONICAL/tiles/b13p01c_Base.tile" | cut -d' ' -f1)" = 2da9141d73b058e9f54976d7c9925e0e90a4ed33ef82c5c9aa5cbf6c313de757
test "$(sha256sum RESERVE/red_grounds/b13p01c.rsground | cut -d' ' -f1)" = 7a10fddd0106d3af2112364deec6fa24a553e6d499947f955c21f956d6755ebc
test "$(sha256sum RESERVE/red_tiles/b13p01c_Base.tile | cut -d' ' -f1)" = 2da9141d73b058e9f54976d7c9925e0e90a4ed33ef82c5c9aa5cbf6c313de757
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b13p01c')
assert entry == {'canonical_debug_id': 84, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00084', 'debug_string_address': '0x0827B05C', 'ground_place_id': 10, 'map_file_id': 85, 'map_id': 84, 'stable_ground_id': 'b13p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_MACHOP_BASIC' in header
print('B13P01C_IDENTITY_PASS map_file_id=85 symbol=MAP_TEAM_BASE_INSIDE_MACHOP_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b13p01c --expected-screenshots 25 \
...
print('B13P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b13p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b13p01c','primary')]['movement_probe']=='PASS'
assert validations[('b13p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b13p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B13P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b13p01c.rsground | cut -d' ' -f1)" = a9ac7d252ee9d9034006f278363e8c9de8aff213c28c28f3313456d711a4c78a
test "$(sha256sum Content/Tile/b13p01c_Base.tile | cut -d' ' -f1)" = 2da9141d73b058e9f54976d7c9925e0e90a4ed33ef82c5c9aa5cbf6c313de757
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b13p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b13p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b13p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
