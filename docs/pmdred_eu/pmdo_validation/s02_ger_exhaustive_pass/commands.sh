#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s02_ger milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s02_ger-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s02_ger-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s02_ger-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s02_ger-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s02_ger.rsground" | cut -d' ' -f1)" = 331c9bd4e213706a36b89e571acaa9ec8ae9fcff4c686b4b60f790e6196fab97
test "$(sha256sum "$CANONICAL/tiles/s02_ger_Base.tile" | cut -d' ' -f1)" = 082a11d69f896f283d105c15e4223ac7f0993a3cec7662ca04db6ecfe3df14ed
test ! -e RESERVE/red_grounds/s02_ger.rsground
test ! -e RESERVE/red_tiles/s02_ger_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s02_ger')
assert entry == {'canonical_debug_id': 233, 'conversion_type': 9, 'debug_string': '__ground_amd_conversion_00233', 'debug_string_address': '0x08279EBC', 'ground_place_id': 37, 'map_file_id': 239, 'map_id': 230, 'stable_ground_id': 's02_ger', 'weather_id': -1}
...
print('S02_GER_IDENTITY_PASS map_file_id=239 canonical_debug_id=233 symbol=MAP_TITLE_SCREEN')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s02_ger --expected-screenshots 2 \
...
print('S02_GER_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s02_ger'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s02_ger','primary')]['movement_probe']=='PASS'
assert validations[('s02_ger','primary')]['blocked_probe']=='PASS'
assert validations[('s02_ger','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S02_GER_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s02_ger.rsground | cut -d' ' -f1)" = 331c9bd4e213706a36b89e571acaa9ec8ae9fcff4c686b4b60f790e6196fab97
test "$(sha256sum Content/Tile/s02_ger_Base.tile | cut -d' ' -f1)" = 082a11d69f896f283d105c15e4223ac7f0993a3cec7662ca04db6ecfe3df14ed
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s02_ger.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s02_ger_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s02_ger --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
