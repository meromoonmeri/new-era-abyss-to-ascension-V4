#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b16p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b16p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b16p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b16p01a.rsground" | cut -d' ' -f1)" = 1586306eb5d35375ee967f8d30514b86fa861320a31d57d0ebdacce311bfc9fe
test "$(sha256sum "$CANONICAL/tiles/b16p01a_Base.tile" | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
test "$(sha256sum RESERVE/red_grounds/b16p01a.rsground | cut -d' ' -f1)" = 4970f1aac5fc9ab7ca66b744ed0417b2d8ecdb9be9e77a252580a5117e1b5a3e
test "$(sha256sum RESERVE/red_tiles/b16p01a_Base.tile | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p01a')
assert entry == {'canonical_debug_id': 100, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00100', 'debug_string_address': '0x0827AE5C', 'ground_place_id': 10, 'map_file_id': 101, 'map_id': 100, 'stable_ground_id': 'b16p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TREECKO_CONSTRUCTION' in header
print('B16P01A_IDENTITY_PASS map_file_id=101 symbol=MAP_TEAM_BASE_TREECKO_CONSTRUCTION')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b16p01a --expected-screenshots 25 \
...
print('B16P01A_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b16p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b16p01a','primary')]['movement_probe']=='PASS'
assert validations[('b16p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b16p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B16P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b16p01a.rsground | cut -d' ' -f1)" = 1586306eb5d35375ee967f8d30514b86fa861320a31d57d0ebdacce311bfc9fe
test "$(sha256sum Content/Tile/b16p01a_Base.tile | cut -d' ' -f1)" = 2411e7b7eeb1cacb5cd8803e8b91ddf0df96fdd03de0f01a7e9e3172207b985b
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b16p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b16p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b16p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
