...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p01a')
assert e == {'canonical_debug_id': 100, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00100', 'debug_string_address': '0x0827AE5C', 'ground_place_id': 10, 'map_file_id': 101, 'map_id': 100, 'stable_ground_id': 'b16p01a', 'weather_id': 9}
assert 'MAP_TEAM_BASE_TREECKO_CONSTRUCTION' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('B16P01A_IDENTITY_PASS map_id=100 map_file_id=101 symbol=MAP_TEAM_BASE_TREECKO_CONSTRUCTION')
PYID
...
print('B16P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
