#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h22p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h22p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h22p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h22p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h22p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h22p01.rsground" | cut -d' ' -f1)" = 851b9ba6bb9a41d966f363937c79aa0d028dd3ac113377332bd5ca12de84f8d9
test "$(sha256sum "$CANONICAL/tiles/h22p01_Base.tile" | cut -d' ' -f1)" = 2a8b85c396ea7af1ed60f2bfb077e12381873e7d4d021a027a86ac77e1b2b5e5
test "$(sha256sum RESERVE/red_grounds/h22p01.rsground | cut -d' ' -f1)" = ed1f7ec744e7fcb9d7585063391ae94161a99ee5f870c681398973b54554e7d7
test "$(sha256sum RESERVE/red_tiles/h22p01_Base.tile | cut -d' ' -f1)" = 2a8b85c396ea7af1ed60f2bfb077e12381873e7d4d021a027a86ac77e1b2b5e5
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h22p01')
assert entry == {'canonical_debug_id': 158, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00158', 'debug_string_address': '0x0827A7FC', 'ground_place_id': 9, 'map_file_id': 159, 'map_id': 151, 'stable_ground_id': 'h22p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_DARKNESS_RIDGE' in header
print('H22P01_IDENTITY_PASS map_file_id=159 symbol=MAP_FRIEND_AREA_DARKNESS_RIDGE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h22p01 --expected-screenshots 73 \
...
print('H22P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 244
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 73
...
assert r['grounds']==['h22p01'] and r['sample_count']==73 and len(primary)==72
assert r['exact_sample_count']==73 and r['fully_opaque_sample_count']==73
assert len({s['actual_rgba_sha256'] for s in primary})==18
...
assert validations[('h22p01','primary')]['movement_probe']=='PASS'
assert validations[('h22p01','primary')]['blocked_probe']=='PASS'
assert validations[('h22p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H22P01_EXHAUSTIVE_PASS samples=73 unique_rgba=18')
PY
...
test "$(sha256sum Data/Ground/h22p01.rsground | cut -d' ' -f1)" = 851b9ba6bb9a41d966f363937c79aa0d028dd3ac113377332bd5ca12de84f8d9
test "$(sha256sum Content/Tile/h22p01_Base.tile | cut -d' ' -f1)" = 2a8b85c396ea7af1ed60f2bfb077e12381873e7d4d021a027a86ac77e1b2b5e5
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h22p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h22p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h22p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 73 comparative PNGs,
