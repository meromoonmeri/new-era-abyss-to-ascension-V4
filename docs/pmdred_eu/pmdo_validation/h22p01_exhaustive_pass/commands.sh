...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h22p01')
assert e == {'canonical_debug_id': 158, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00158', 'debug_string_address': '0x0827A7FC', 'ground_place_id': 9, 'map_file_id': 159, 'map_id': 151, 'stable_ground_id': 'h22p01', 'weather_id': -1}
assert 'MAP_FRIEND_AREA_DARKNESS_RIDGE' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H22P01_IDENTITY_PASS map_id=151 map_file_id=159 symbol=MAP_FRIEND_AREA_DARKNESS_RIDGE')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 241
...
print('H22P01_EXHAUSTIVE_PASS samples=73 unique_rgba=18 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
