...
test "$(sha256sum RESERVE/red_grounds/s04.rsground | cut -d' ' -f1)" = 06c709a9bacff3fafca057afcc0e7c56b844e7bf11804dcb3591f261f9c225be
test "$(sha256sum RESERVE/red_tiles/s04_Base.tile | cut -d' ' -f1)" = a88590a282cfd116177d60e0dc828ea2e620e491a7838830c4fc708224b41bc4
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s04')
assert entry == {'canonical_debug_id': 237, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00237', 'debug_string_address': '0x08279E7C', 'ground_place_id': 37, 'map_file_id': 244, 'map_id': 235, 'stable_ground_id': 's04', 'weather_id': -1}
...
designated=table.split('[MAP_LOGO_POKEMON_COMPANY] = {',1)[1].split('},',1)[0]
assert '__ground_amd_conversion_00237' in designated
print('S04_IDENTITY_PASS map_file_id=244 canonical_debug_id=237 symbol=MAP_LOGO_POKEMON_COMPANY')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s04 --expected-screenshots 2 \
...
print('S04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
print('S04_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
