...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d12p01')
assert e == {'canonical_debug_id': 213, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00213', 'debug_string_address': '0x0827A13C', 'ground_place_id': 30, 'map_file_id': 218, 'map_id': 209, 'stable_ground_id': 'd12p01', 'weather_id': -1}
assert 'MAP_SKY_TOWER_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D12P01_IDENTITY_PASS map_id=209 map_file_id=218 symbol=MAP_SKY_TOWER_END')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 697
...
print('D12P01_EXHAUSTIVE_PASS samples=225 unique_rgba=32 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
