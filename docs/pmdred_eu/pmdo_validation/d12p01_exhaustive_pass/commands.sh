#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d12p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d12p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d12p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d12p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d12p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d12p01.rsground" | cut -d' ' -f1)" = b9db0e2c99fcb3491e55bf4ea05378ad7820c228ae4a42e640ef8a05e0376d26
test "$(sha256sum "$CANONICAL/tiles/d12p01_Base.tile" | cut -d' ' -f1)" = 5a51688f7f54d91488f8ee2d6d1f00ab2dfb1d30249cb6a83e2f0449e0aec76e
test "$(sha256sum RESERVE/red_grounds/d12p01.rsground | cut -d' ' -f1)" = b5f012e78ab6515be6b382bf87b69b3e77b5a45a81e909dc57c12898a84746e9
test "$(sha256sum RESERVE/red_tiles/d12p01_Base.tile | cut -d' ' -f1)" = 5a51688f7f54d91488f8ee2d6d1f00ab2dfb1d30249cb6a83e2f0449e0aec76e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d12p01')
assert entry == {'canonical_debug_id': 213, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00213', 'debug_string_address': '0x0827A13C', 'ground_place_id': 30, 'map_file_id': 218, 'map_id': 209, 'stable_ground_id': 'd12p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_SKY_TOWER_END' in header
print('D12P01_IDENTITY_PASS map_file_id=218 symbol=MAP_SKY_TOWER_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d12p01 --expected-screenshots 225 \
...
print('D12P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 700
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 225
...
assert r['grounds']==['d12p01'] and r['sample_count']==225 and len(primary)==224
assert r['exact_sample_count']==225 and r['fully_opaque_sample_count']==225
...
assert validations[('d12p01','primary')]['movement_probe']=='PASS'
assert validations[('d12p01','primary')]['blocked_probe']=='PASS'
assert validations[('d12p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D12P01_EXHAUSTIVE_PASS samples=225 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/d12p01.rsground | cut -d' ' -f1)" = b9db0e2c99fcb3491e55bf4ea05378ad7820c228ae4a42e640ef8a05e0376d26
test "$(sha256sum Content/Tile/d12p01_Base.tile | cut -d' ' -f1)" = 5a51688f7f54d91488f8ee2d6d1f00ab2dfb1d30249cb6a83e2f0449e0aec76e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d12p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d12p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d12p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 225 comparative PNGs,
