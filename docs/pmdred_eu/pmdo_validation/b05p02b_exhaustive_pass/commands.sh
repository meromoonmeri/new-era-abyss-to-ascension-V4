#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b05p02b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b05p02b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b05p02b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b05p02b.rsground" | cut -d' ' -f1)" = 1b0a8c19401817acc0bea9024e94ee92b5fd0df375393af0fc1ca1880e3e3ae7
test "$(sha256sum "$CANONICAL/tiles/b05p02b_Base.tile" | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test "$(sha256sum RESERVE/red_grounds/b05p02b.rsground | cut -d' ' -f1)" = 353c01f916a681a91af300ca8f2f33ab13ec48f2b15d41b08a3d9875e8835de2
test "$(sha256sum RESERVE/red_tiles/b05p02b_Base.tile | cut -d' ' -f1)" = b589283f5822564edf2d3f39633c3dd11fb720a760263f667a883a8f58142e37
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b05p02b')
assert entry == {'canonical_debug_id': 38, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00038', 'debug_string_address': '0x0827B61C', 'ground_place_id': 11, 'map_file_id': 39, 'map_id': 38, 'stable_ground_id': 'b05p02b', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_SQUIRTLE_FINAL' in header
print('B05P02B_IDENTITY_PASS map_file_id=39 symbol=MAP_TEAM_BASE_INSIDE_SQUIRTLE_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b05p02b --expected-screenshots 481 \
...
print('B05P02B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b05p02b'] and r['sample_count']==481 and len(primary)==480
...
assert validations[('b05p02b','primary')]['movement_probe']=='PASS'
assert validations[('b05p02b','primary')]['blocked_probe']=='PASS'
assert validations[('b05p02b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B05P02B_EXHAUSTIVE_PASS samples=481 unique_rgba=233')
PY
...
test "$(sha256sum Data/Ground/b05p02b.rsground | cut -d' ' -f1)" = 1b0a8c19401817acc0bea9024e94ee92b5fd0df375393af0fc1ca1880e3e3ae7
test "$(sha256sum Content/Tile/b05p02b_Base.tile | cut -d' ' -f1)" = dea8e4f2bb697348525eee8629b920d7a7f0cee45c607704099c99761f0256a4
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b05p02b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b05p02b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b05p02b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
