#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h15p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h15p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h15p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h15p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h15p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h15p01.rsground" | cut -d' ' -f1)" = 331e9f10dd2c3437fc4c6951ef5b8025736a9196c40bb57acd82883a26fb3ac3
test "$(sha256sum "$CANONICAL/tiles/h15p01_Base.tile" | cut -d' ' -f1)" = 0fa47c5a9f48508e0ee14bd9b0b3c8da42387782a526c367171ced30ba255b79
test "$(sha256sum RESERVE/red_grounds/h15p01.rsground | cut -d' ' -f1)" = 3e1132d3399400adcead3f2f2832c4dc0da43f6e9a69ff0b1f8b97049b4ab397
test "$(sha256sum RESERVE/red_tiles/h15p01_Base.tile | cut -d' ' -f1)" = 3037837ce656cde5767911f44f5ecedf2e9b10da37ae86db02d219cb0e6e85f1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h15p01')
assert entry == {'canonical_debug_id': 148, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00148', 'debug_string_address': '0x0827A8FC', 'ground_place_id': 9, 'map_file_id': 149, 'map_id': 143, 'stable_ground_id': 'h15p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_MT_DISCIPLINE' in header
print('H15P01_IDENTITY_PASS map_file_id=149 symbol=MAP_FRIEND_AREA_MT_DISCIPLINE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h15p01 --expected-screenshots 209 \
...
print('H15P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 652
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 209
...
assert r['grounds']==['h15p01'] and r['sample_count']==209 and len(primary)==208
assert r['exact_sample_count']==209 and r['fully_opaque_sample_count']==209
assert len({s['actual_rgba_sha256'] for s in primary})==175
...
assert validations[('h15p01','primary')]['movement_probe']=='PASS'
assert validations[('h15p01','primary')]['blocked_probe']=='PASS'
assert validations[('h15p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H15P01_EXHAUSTIVE_PASS samples=209 unique_rgba=175')
PY
...
test "$(sha256sum Data/Ground/h15p01.rsground | cut -d' ' -f1)" = 331e9f10dd2c3437fc4c6951ef5b8025736a9196c40bb57acd82883a26fb3ac3
test "$(sha256sum Content/Tile/h15p01_Base.tile | cut -d' ' -f1)" = 0fa47c5a9f48508e0ee14bd9b0b3c8da42387782a526c367171ced30ba255b79
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h15p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h15p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h15p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 209 comparative PNGs,
