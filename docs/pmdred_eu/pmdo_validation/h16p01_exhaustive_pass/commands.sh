#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h16p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h16p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h16p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h16p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h16p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h16p01.rsground" | cut -d' ' -f1)" = 959afe5c81aa370f12ff20e3a95956b6e98e9557ac64384c52d5892a5d21f92d
test "$(sha256sum "$CANONICAL/tiles/h16p01_Base.tile" | cut -d' ' -f1)" = d2d216101d10bca063404b938e4066ad4ac8bd6eafdc76ec990331155cccc45a
test "$(sha256sum RESERVE/red_grounds/h16p01.rsground | cut -d' ' -f1)" = dd7a660ca8f71393a583f1d00c1bd7a1ad34c061899785d9c3836dbd52a0be87
test "$(sha256sum RESERVE/red_tiles/h16p01_Base.tile | cut -d' ' -f1)" = d2d216101d10bca063404b938e4066ad4ac8bd6eafdc76ec990331155cccc45a
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h16p01')
assert entry == {'canonical_debug_id': 149, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00149', 'debug_string_address': '0x0827A8DC', 'ground_place_id': 9, 'map_file_id': 150, 'map_id': 144, 'stable_ground_id': 'h16p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_THUNDER_MEADOW' in header
print('H16P01_IDENTITY_PASS map_file_id=150 symbol=MAP_FRIEND_AREA_THUNDER_MEADOW')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h16p01 --expected-screenshots 2 \
...
print('H16P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h16p01'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h16p01','primary')]['movement_probe']=='PASS'
assert validations[('h16p01','primary')]['blocked_probe']=='PASS'
assert validations[('h16p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H16P01_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h16p01.rsground | cut -d' ' -f1)" = 959afe5c81aa370f12ff20e3a95956b6e98e9557ac64384c52d5892a5d21f92d
test "$(sha256sum Content/Tile/h16p01_Base.tile | cut -d' ' -f1)" = d2d216101d10bca063404b938e4066ad4ac8bd6eafdc76ec990331155cccc45a
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h16p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h16p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h16p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
