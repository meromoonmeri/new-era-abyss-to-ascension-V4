#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s05_ger milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s05_ger-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s05_ger-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s05_ger-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s05_ger-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s05_ger.rsground" | cut -d' ' -f1)" = 59bead85745bc5ccebf87d601a822b9ecbe29d2c517b627c7565b3d8c7c1ad4a
test "$(sha256sum "$CANONICAL/tiles/s05_ger_Base.tile" | cut -d' ' -f1)" = b833d91c70e573bac86f2887e77765a4bcfed68caf29c4690c50b8e75f6a029e
test ! -e RESERVE/red_grounds/s05_ger.rsground
test ! -e RESERVE/red_tiles/s05_ger_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s05_ger')
assert entry == {'canonical_debug_id': 239, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00239', 'debug_string_address': '0x08279E5C', 'ground_place_id': 37, 'map_file_id': 246, 'map_id': 237, 'stable_ground_id': 's05_ger', 'weather_id': -1}
...
print('S05_GER_IDENTITY_PASS map_file_id=246 canonical_debug_id=239 symbol=MAP_LOGO_WARNING')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s05_ger --expected-screenshots 2 \
...
print('S05_GER_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s05_ger'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s05_ger','primary')]['movement_probe']=='PASS'
assert validations[('s05_ger','primary')]['blocked_probe']=='PASS'
assert validations[('s05_ger','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S05_GER_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s05_ger.rsground | cut -d' ' -f1)" = 59bead85745bc5ccebf87d601a822b9ecbe29d2c517b627c7565b3d8c7c1ad4a
test "$(sha256sum Content/Tile/s05_ger_Base.tile | cut -d' ' -f1)" = b833d91c70e573bac86f2887e77765a4bcfed68caf29c4690c50b8e75f6a029e
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s05_ger.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s05_ger_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s05_ger --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
