#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h13p04 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h13p04-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h13p04-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h13p04-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h13p04-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h13p04.rsground" | cut -d' ' -f1)" = 737a49b8820269952285878df88ace93acdfcfd6f6e1c9f321455676a2058cf6
test "$(sha256sum "$CANONICAL/tiles/h13p04_Base.tile" | cut -d' ' -f1)" = 87b53a881d1d32e4c2e61a5cac4d648889c7cc5034e2555c828d862b6f8a1466
test "$(sha256sum RESERVE/red_grounds/h13p04.rsground | cut -d' ' -f1)" = cc51d16e00e80f6b1488ba5efd67d6a74dd69b2c32e9c2a22f58acda371eff2d
test "$(sha256sum RESERVE/red_tiles/h13p04_Base.tile | cut -d' ' -f1)" = 87b53a881d1d32e4c2e61a5cac4d648889c7cc5034e2555c828d862b6f8a1466
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h13p04')
assert entry == {'canonical_debug_id': 146, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00146', 'debug_string_address': '0x0827A93C', 'ground_place_id': 9, 'map_file_id': 147, 'map_id': 141, 'stable_ground_id': 'h13p04', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_JUNGLE' in header
print('H13P04_IDENTITY_PASS map_file_id=147 symbol=MAP_FRIEND_AREA_JUNGLE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h13p04 --expected-screenshots 2 \
...
print('H13P04_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h13p04'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('h13p04','primary')]['movement_probe']=='PASS'
assert validations[('h13p04','primary')]['blocked_probe']=='PASS'
assert validations[('h13p04','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H13P04_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/h13p04.rsground | cut -d' ' -f1)" = 737a49b8820269952285878df88ace93acdfcfd6f6e1c9f321455676a2058cf6
test "$(sha256sum Content/Tile/h13p04_Base.tile | cut -d' ' -f1)" = 87b53a881d1d32e4c2e61a5cac4d648889c7cc5034e2555c828d862b6f8a1466
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h13p04.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h13p04_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h13p04 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
