#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h21p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h21p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h21p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h21p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h21p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h21p02.rsground" | cut -d' ' -f1)" = 7d0a5fef8dcd26bb2d1d06d9ffdb2d4fd5f859c88fe20cce1feef54df442a17e
test "$(sha256sum "$CANONICAL/tiles/h21p02_Base.tile" | cut -d' ' -f1)" = 9d935b5dc97260d47b4ff6a90bfd1e4e3532dcff8cd72379f80e53e5f0f20f8c
test "$(sha256sum RESERVE/red_grounds/h21p02.rsground | cut -d' ' -f1)" = 649065a3b4999204403d2ef1871f9ad0e786963040aaa0cdedfb27cb1d68fce3
test "$(sha256sum RESERVE/red_tiles/h21p02_Base.tile | cut -d' ' -f1)" = f8fc781f6f23573c9dff58aabcab6848ec3ce709bd8112905a5dec774fc0ee0f
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h21p02')
assert entry == {'canonical_debug_id': 157, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00157', 'debug_string_address': '0x0827A81C', 'ground_place_id': 9, 'map_file_id': 158, 'map_id': 150, 'stable_ground_id': 'h21p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_ANCIENT_RELIC' in header
print('H21P02_IDENTITY_PASS map_file_id=158 symbol=MAP_FRIEND_AREA_ANCIENT_RELIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h21p02 --expected-screenshots 85 \
...
print('H21P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 280
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 85
...
assert r['grounds']==['h21p02'] and r['sample_count']==85 and len(primary)==84
assert r['exact_sample_count']==85 and r['fully_opaque_sample_count']==85
...
assert validations[('h21p02','primary')]['movement_probe']=='PASS'
assert validations[('h21p02','primary')]['blocked_probe']=='PASS'
assert validations[('h21p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H21P02_EXHAUSTIVE_PASS samples=85 unique_rgba=14')
PY
...
test "$(sha256sum Data/Ground/h21p02.rsground | cut -d' ' -f1)" = 7d0a5fef8dcd26bb2d1d06d9ffdb2d4fd5f859c88fe20cce1feef54df442a17e
test "$(sha256sum Content/Tile/h21p02_Base.tile | cut -d' ' -f1)" = 9d935b5dc97260d47b4ff6a90bfd1e4e3532dcff8cd72379f80e53e5f0f20f8c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h21p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h21p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h21p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 85 comparative PNGs,
