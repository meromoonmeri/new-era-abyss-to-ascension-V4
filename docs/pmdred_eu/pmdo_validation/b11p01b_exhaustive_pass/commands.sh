#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b11p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b11p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b11p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b11p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b11p01b.rsground" | cut -d' ' -f1)" = 8c854b07e2569fe5894fc8fe6059d3b4f8733007b4941773cdc617e9f4ee787d
test "$(sha256sum "$CANONICAL/tiles/b11p01b_Base.tile" | cut -d' ' -f1)" = e121b6b6c70cafa4ce4f35a1c7dc4b96f5e3b7bd18c1e472ac941c77eccff2ac
test "$(sha256sum RESERVE/red_grounds/b11p01b.rsground | cut -d' ' -f1)" = 959f720e25baaf6a808289547101ed61027a3f902e7dde0020a373ff2d73e0e7
test "$(sha256sum RESERVE/red_tiles/b11p01b_Base.tile | cut -d' ' -f1)" = e121b6b6c70cafa4ce4f35a1c7dc4b96f5e3b7bd18c1e472ac941c77eccff2ac
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b11p01b')
assert entry == {'canonical_debug_id': 71, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00071', 'debug_string_address': '0x0827B1FC', 'ground_place_id': 10, 'map_file_id': 72, 'map_id': 71, 'stable_ground_id': 'b11p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CYNDAQUIL_FINAL' in header
print('B11P01B_IDENTITY_PASS map_file_id=72 symbol=MAP_TEAM_BASE_CYNDAQUIL_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b11p01b --expected-screenshots 25 \
...
print('B11P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b11p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b11p01b','primary')]['movement_probe']=='PASS'
assert validations[('b11p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b11p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B11P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b11p01b.rsground | cut -d' ' -f1)" = 8c854b07e2569fe5894fc8fe6059d3b4f8733007b4941773cdc617e9f4ee787d
test "$(sha256sum Content/Tile/b11p01b_Base.tile | cut -d' ' -f1)" = e121b6b6c70cafa4ce4f35a1c7dc4b96f5e3b7bd18c1e472ac941c77eccff2ac
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b11p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b11p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b11p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
