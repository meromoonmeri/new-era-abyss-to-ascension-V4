#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b14p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b14p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b14p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b14p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b14p01c.rsground" | cut -d' ' -f1)" = da2a32e2e72fe3822051dd8e5ec63396feca4ca27713d34fd9509192f10b84cf
test "$(sha256sum "$CANONICAL/tiles/b14p01c_Base.tile" | cut -d' ' -f1)" = 292f73cde0aefc2e8bbbeba9a2ec190f18126edb629432403cb05e462e09d50c
test "$(sha256sum RESERVE/red_grounds/b14p01c.rsground | cut -d' ' -f1)" = 26c6fa517f2baff5e23cbe0a6fd4732050160497b8be9593d48c76a9df1c4643
test "$(sha256sum RESERVE/red_tiles/b14p01c_Base.tile | cut -d' ' -f1)" = 292f73cde0aefc2e8bbbeba9a2ec190f18126edb629432403cb05e462e09d50c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b14p01c')
assert entry == {'canonical_debug_id': 90, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00090', 'debug_string_address': '0x0827AF9C', 'ground_place_id': 10, 'map_file_id': 91, 'map_id': 90, 'stable_ground_id': 'b14p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_BULBASAUR_BASIC' in header
print('B14P01C_IDENTITY_PASS map_file_id=91 symbol=MAP_TEAM_BASE_INSIDE_BULBASAUR_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b14p01c --expected-screenshots 25 \
...
print('B14P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b14p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b14p01c','primary')]['movement_probe']=='PASS'
assert validations[('b14p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b14p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B14P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b14p01c.rsground | cut -d' ' -f1)" = da2a32e2e72fe3822051dd8e5ec63396feca4ca27713d34fd9509192f10b84cf
test "$(sha256sum Content/Tile/b14p01c_Base.tile | cut -d' ' -f1)" = 292f73cde0aefc2e8bbbeba9a2ec190f18126edb629432403cb05e462e09d50c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b14p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b14p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b14p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
