#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b05p02c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b05p02c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b05p02c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b05p02c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b05p02c.rsground" | cut -d' ' -f1)" = b14b8d5d9b4ddf57f95f660810951ad7cc61e9c28d408a4248c8a114abe1b4b5
test "$(sha256sum "$CANONICAL/tiles/b05p02c_Base.tile" | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
test "$(sha256sum RESERVE/red_grounds/b05p02c.rsground | cut -d' ' -f1)" = 57bc4be8710e1b01f4ba7d888060b3203c788f5a49e7b8b40310db68b4980f32
test "$(sha256sum RESERVE/red_tiles/b05p02c_Base.tile | cut -d' ' -f1)" = 4a7eb9c34fe92861dc80230da6961c60903fce06f0466f2602b8bcafebe0172a
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b05p02c')
assert entry == {'canonical_debug_id': 39, 'conversion_type': 2, 'debug_string': '__ground_amd_conversion_00039', 'debug_string_address': '0x0827B5FC', 'ground_place_id': 11, 'map_file_id': 40, 'map_id': 39, 'stable_ground_id': 'b05p02c', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_TOTODILE_BASIC' in header
print('B05P02C_IDENTITY_PASS map_file_id=40 symbol=MAP_TEAM_BASE_TOTODILE_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b05p02c --expected-screenshots 1921 \
...
print('B05P02C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b05p02c'] and r['sample_count']==1921 and len(primary)==1920
...
assert validations[('b05p02c','primary')]['movement_probe']=='PASS'
assert validations[('b05p02c','primary')]['blocked_probe']=='PASS'
assert validations[('b05p02c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B05P02C_EXHAUSTIVE_PASS samples=1921 unique_rgba=180')
PY
...
test "$(sha256sum Data/Ground/b05p02c.rsground | cut -d' ' -f1)" = b14b8d5d9b4ddf57f95f660810951ad7cc61e9c28d408a4248c8a114abe1b4b5
test "$(sha256sum Content/Tile/b05p02c_Base.tile | cut -d' ' -f1)" = 11eb7a8724b4dd0a971e69161d62e7f47ac6eac57e8d246bfecb9f1fee46f2e1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b05p02c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b05p02c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b05p02c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
