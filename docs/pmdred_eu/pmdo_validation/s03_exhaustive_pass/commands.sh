#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s03.rsground" | cut -d' ' -f1)" = bdc56155688944d5861f33f72e8b76fc08b6c5c8d341c440cb1c181ed271aa82
test "$(sha256sum "$CANONICAL/tiles/s03_Base.tile" | cut -d' ' -f1)" = f83eec868021876d0e198d39c5c40ca1b9743a3fdd130e634e8905ad1f985c81
test "$(sha256sum RESERVE/red_grounds/s03.rsground | cut -d' ' -f1)" = 9c1e960e5241e6274868c156f228b23be3ea93bc656e693b0bd72c3e8a45cb79
test "$(sha256sum RESERVE/red_tiles/s03_Base.tile | cut -d' ' -f1)" = f83eec868021876d0e198d39c5c40ca1b9743a3fdd130e634e8905ad1f985c81
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s03')
assert entry == {'canonical_debug_id': 235, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00235', 'debug_string_address': '0x08279E9C', 'ground_place_id': 37, 'map_file_id': 243, 'map_id': 234, 'stable_ground_id': 's03', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_INTRO' in header
table=Path('.runtime-cache/pmd-red-reference/src/ground_map_conversion_table.c').read_text()
designated=table.split('[MAP_INTRO] = {',1)[1].split('},',1)[0]
assert '__ground_amd_conversion_00235' in designated
print('S03_IDENTITY_PASS map_file_id=243 canonical_debug_id=235 symbol=MAP_INTRO')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s03 --expected-screenshots 257 \
...
print('S03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 793
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 257
...
assert r['grounds']==['s03'] and r['sample_count']==257 and len(primary)==256
assert r['exact_sample_count']==257 and r['fully_opaque_sample_count']==257
assert len({s['actual_rgba_sha256'] for s in primary})==23
...
assert validations[('s03','primary')]['movement_probe']=='PASS'
assert validations[('s03','primary')]['blocked_probe']=='PASS'
assert validations[('s03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S03_EXHAUSTIVE_PASS samples=257 unique_rgba=23')
PY
...
test "$(sha256sum Data/Ground/s03.rsground | cut -d' ' -f1)" = bdc56155688944d5861f33f72e8b76fc08b6c5c8d341c440cb1c181ed271aa82
test "$(sha256sum Content/Tile/s03_Base.tile | cut -d' ' -f1)" = f83eec868021876d0e198d39c5c40ca1b9743a3fdd130e634e8905ad1f985c81
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 257 comparative PNGs,
