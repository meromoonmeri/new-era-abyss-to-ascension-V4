#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h29p02 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h29p02-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h29p02-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h29p02-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h29p02-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h29p02.rsground" | cut -d' ' -f1)" = ebdd35834ca870c56efaa054149a2d9165b79028e3a1a45ee48f3252a81a93ed
test "$(sha256sum "$CANONICAL/tiles/h29p02_Base.tile" | cut -d' ' -f1)" = 24f86c9b3ffbacee915bee76e0f3aacb3ad9f0b678e93d4050e484766b83b196
test "$(sha256sum RESERVE/red_grounds/h29p02.rsground | cut -d' ' -f1)" = 6828356aa34fccae96bb74f026d8867ec459e4f51a590d7fa11497428b9e4285
test "$(sha256sum RESERVE/red_tiles/h29p02_Base.tile | cut -d' ' -f1)" = e5e02b5877bd94a7323cdb407b27127ae3a67a6d52f36f7d5cac647635902209
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p02')
assert entry == {'canonical_debug_id': 167, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00167', 'debug_string_address': '0x0827A6DC', 'ground_place_id': 9, 'map_file_id': 168, 'map_id': 160, 'stable_ground_id': 'h29p02', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_ENCLOSED_ISLAND' in header
print('H29P02_IDENTITY_PASS map_file_id=168 symbol=MAP_FRIEND_AREA_ENCLOSED_ISLAND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h29p02 --expected-screenshots 3277 \
...
print('H29P02_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 9856
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 3277
...
assert r['grounds']==['h29p02'] and r['sample_count']==3277 and len(primary)==3276
assert r['exact_sample_count']==3277 and r['fully_opaque_sample_count']==3277
assert len({s['actual_rgba_sha256'] for s in primary})==1186
...
assert validations[('h29p02','primary')]['movement_probe']=='PASS'
assert validations[('h29p02','primary')]['blocked_probe']=='PASS'
assert validations[('h29p02','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H29P02_EXHAUSTIVE_PASS samples=3277 unique_rgba=1186')
PY
...
test "$(sha256sum Data/Ground/h29p02.rsground | cut -d' ' -f1)" = ebdd35834ca870c56efaa054149a2d9165b79028e3a1a45ee48f3252a81a93ed
test "$(sha256sum Content/Tile/h29p02_Base.tile | cut -d' ' -f1)" = 24f86c9b3ffbacee915bee76e0f3aacb3ad9f0b678e93d4050e484766b83b196
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h29p02.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h29p02_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h29p02 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 3277 comparative PNGs,
