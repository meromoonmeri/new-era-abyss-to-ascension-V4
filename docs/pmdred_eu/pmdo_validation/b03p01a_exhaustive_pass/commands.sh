#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b03p01a milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01a-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b03p01a-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b03p01a-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b03p01a-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b03p01a.rsground" | cut -d' ' -f1)" = f317ceb5e36d9a63970cb29632c653d5627a6efa297890b89ce3a2f9d083a211
test "$(sha256sum "$CANONICAL/tiles/b03p01a_Base.tile" | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test "$(sha256sum RESERVE/red_grounds/b03p01a.rsground | cut -d' ' -f1)" = 2a9f644832d9c4915a2c896a476252e6c2a45e19aece625d6fd96a94b5226a6c
test "$(sha256sum RESERVE/red_tiles/b03p01a_Base.tile | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b03p01a')
assert entry == {'canonical_debug_id': 22, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00022', 'debug_string_address': '0x0827B81C', 'ground_place_id': 10, 'map_file_id': 23, 'map_id': 22, 'stable_ground_id': 'b03p01a', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_EEVEE_CONSTRUCTION' in header
print('B03P01A_IDENTITY_PASS map_file_id=23 symbol=MAP_TEAM_BASE_EEVEE_CONSTRUCTION')
PYID
...
assert r['grounds']==['b03p01a'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b03p01a','primary')]['movement_probe']=='PASS'
assert validations[('b03p01a','primary')]['blocked_probe']=='PASS'
assert validations[('b03p01a','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B03P01A_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b03p01a.rsground | cut -d' ' -f1)" = f317ceb5e36d9a63970cb29632c653d5627a6efa297890b89ce3a2f9d083a211
test "$(sha256sum Content/Tile/b03p01a_Base.tile | cut -d' ' -f1)" = d6a7088978d33196fab79b0285b505b84a6de86b01914033c8d0ac502fa7d4ba
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b03p01a.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b03p01a_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b03p01a --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
