#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b07p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b07p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b07p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b07p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b07p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b07p01b.rsground" | cut -d' ' -f1)" = 391a22fbd5a609f8a9864d3bd4d22a8af75319aa9a89ec882edf949096dc543d
test "$(sha256sum "$CANONICAL/tiles/b07p01b_Base.tile" | cut -d' ' -f1)" = 83f25ae821e8055af5bf31a714a07ee7dabf766f72036ed61e6d342a1e89c251
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b07p01b')
assert entry == {'canonical_debug_id': 47, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00047', 'debug_string_address': '0x0827B4FC', 'ground_place_id': 10, 'map_file_id': 48, 'map_id': 47, 'stable_ground_id': 'b07p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_MUDKIP_FINAL' in header
print('B07P01B_IDENTITY_PASS map_id=47 map_file_id=48 symbol=MAP_TEAM_BASE_MUDKIP_FINAL')
PYID
...
assert r['grounds']==['b07p01b'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b07p01b','primary')]['movement_probe']=='PASS'
assert validations[('b07p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b07p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B07P01B_EXHAUSTIVE_PASS samples=51 unique_rgba=32 termination=SIGINT_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/b07p01b.rsground | cut -d' ' -f1)" = 391a22fbd5a609f8a9864d3bd4d22a8af75319aa9a89ec882edf949096dc543d
test "$(sha256sum Content/Tile/b07p01b_Base.tile | cut -d' ' -f1)" = 83f25ae821e8055af5bf31a714a07ee7dabf766f72036ed61e6d342a1e89c251
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b07p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b07p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b07p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
