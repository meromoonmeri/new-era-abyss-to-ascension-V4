...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h25p01')
assert e == {'canonical_debug_id': 161, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00161', 'debug_string_address': '0x0827A79C', 'ground_place_id': 9, 'map_file_id': 162, 'map_id': 154, 'stable_ground_id': 'h25p01', 'weather_id': 12}
assert 'MAP_FRIEND_AREA_VOLCANIC_PIT' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H25P01_IDENTITY_PASS map_id=154 map_file_id=162 symbol=MAP_FRIEND_AREA_VOLCANIC_PIT')
PYID
...
print('H25P01_EXHAUSTIVE_PASS samples=129 unique_rgba=49 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
