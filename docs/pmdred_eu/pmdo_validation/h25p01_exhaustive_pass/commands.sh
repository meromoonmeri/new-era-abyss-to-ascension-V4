#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h25p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h25p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h25p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h25p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h25p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h25p01.rsground" | cut -d' ' -f1)" = 976ce9e46aa537fdc99fbaf7c4af74cdeb00301a9a29a81077142a59aeae0aec
test "$(sha256sum "$CANONICAL/tiles/h25p01_Base.tile" | cut -d' ' -f1)" = a243627bbb4cefb94a867d0a7b54c5a18e8b58956e12cf840dbe4b99abeae9b8
test "$(sha256sum RESERVE/red_grounds/h25p01.rsground | cut -d' ' -f1)" = 6d80052e712acf53ec39c2a6253350ef8ea85f0a59e23e9ae6e3561e78aa0a56
test "$(sha256sum RESERVE/red_tiles/h25p01_Base.tile | cut -d' ' -f1)" = 211f7083d8a61728074d857989b00b97862585e64a5a9818c9f3794e5f1a4469
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h25p01')
assert entry == {'canonical_debug_id': 161, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00161', 'debug_string_address': '0x0827A79C', 'ground_place_id': 9, 'map_file_id': 162, 'map_id': 154, 'stable_ground_id': 'h25p01', 'weather_id': 12}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_VOLCANIC_PIT' in header
print('H25P01_IDENTITY_PASS map_file_id=162 symbol=MAP_FRIEND_AREA_VOLCANIC_PIT')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h25p01 --expected-screenshots 129 \
...
print('H25P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['h25p01'] and r['sample_count']==129 and len(primary)==128
assert r['exact_sample_count']==129 and r['fully_opaque_sample_count']==129
assert len({s['actual_rgba_sha256'] for s in primary})==49
...
assert validations[('h25p01','primary')]['movement_probe']=='PASS'
assert validations[('h25p01','primary')]['blocked_probe']=='PASS'
assert validations[('h25p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H25P01_EXHAUSTIVE_PASS samples=129 unique_rgba=49')
PY
...
test "$(sha256sum Data/Ground/h25p01.rsground | cut -d' ' -f1)" = 976ce9e46aa537fdc99fbaf7c4af74cdeb00301a9a29a81077142a59aeae0aec
test "$(sha256sum Content/Tile/h25p01_Base.tile | cut -d' ' -f1)" = a243627bbb4cefb94a867d0a7b54c5a18e8b58956e12cf840dbe4b99abeae9b8
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h25p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h25p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h25p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
