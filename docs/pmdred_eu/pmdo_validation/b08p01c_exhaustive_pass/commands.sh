#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b08p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b08p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b08p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b08p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b08p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b08p01c.rsground" | cut -d' ' -f1)" = 3e4a53493197d74bd2968e92b94b25cbbfd86c4d8bc5c91be94ad295303d2f0b
test "$(sha256sum "$CANONICAL/tiles/b08p01c_Base.tile" | cut -d' ' -f1)" = 869e3676829c05f63ea982c4655975027a5cc5e0edf43b8322ba5e281ba43d79
test "$(sha256sum RESERVE/red_grounds/b08p01c.rsground | cut -d' ' -f1)" = 80a14f4225944deb19734462540bbd268964b69dc29d6b06ed7ca1fdf3192484
test "$(sha256sum RESERVE/red_tiles/b08p01c_Base.tile | cut -d' ' -f1)" = 869e3676829c05f63ea982c4655975027a5cc5e0edf43b8322ba5e281ba43d79
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b08p01c')
assert entry == {'canonical_debug_id': 54, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00054', 'debug_string_address': '0x0827B41C', 'ground_place_id': 10, 'map_file_id': 55, 'map_id': 54, 'stable_ground_id': 'b08p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_PSYDUCK_BASIC' in header
print('B08P01C_IDENTITY_PASS map_file_id=55 symbol=MAP_TEAM_BASE_INSIDE_PSYDUCK_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b08p01c --expected-screenshots 51 \
...
print('B08P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b08p01c'] and r['sample_count']==51 and len(primary)==50
...
assert validations[('b08p01c','primary')]['movement_probe']=='PASS'
assert validations[('b08p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b08p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B08P01C_EXHAUSTIVE_PASS samples=51 unique_rgba=32')
PY
...
test "$(sha256sum Data/Ground/b08p01c.rsground | cut -d' ' -f1)" = 3e4a53493197d74bd2968e92b94b25cbbfd86c4d8bc5c91be94ad295303d2f0b
test "$(sha256sum Content/Tile/b08p01c_Base.tile | cut -d' ' -f1)" = 869e3676829c05f63ea982c4655975027a5cc5e0edf43b8322ba5e281ba43d79
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b08p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b08p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b08p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
