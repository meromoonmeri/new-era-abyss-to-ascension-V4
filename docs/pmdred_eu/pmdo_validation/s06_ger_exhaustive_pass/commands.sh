#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the s06_ger milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-s06_ger-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-s06_ger-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-s06_ger-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-s06_ger-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/s06_ger.rsground" | cut -d' ' -f1)" = e4d1e7049a85dd2527f7978f4f18931aa042c873b47819bd4f821f75e816c8e1
test "$(sha256sum "$CANONICAL/tiles/s06_ger_Base.tile" | cut -d' ' -f1)" = 2095dc70f2ac4f55fe5fe28c4a1cb4889851a64ac515005d0527eebcc4dcca25
test ! -e RESERVE/red_grounds/s06_ger.rsground
test ! -e RESERVE/red_tiles/s06_ger_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='s06_ger')
assert entry == {'canonical_debug_id': 240, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00240', 'debug_string_address': '0x08279E3C', 'ground_place_id': 37, 'map_file_id': 251, 'map_id': 242, 'stable_ground_id': 's06_ger', 'weather_id': -1}
...
print('S06_GER_IDENTITY_PASS map_file_id=251 canonical_debug_id=240 symbol=MAP_TEAM_BASE_WALL_MAP')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground s06_ger --expected-screenshots 2 \
...
print('S06_GER_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['s06_ger'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('s06_ger','primary')]['movement_probe']=='PASS'
assert validations[('s06_ger','primary')]['blocked_probe']=='PASS'
assert validations[('s06_ger','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('S06_GER_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/s06_ger.rsground | cut -d' ' -f1)" = e4d1e7049a85dd2527f7978f4f18931aa042c873b47819bd4f821f75e816c8e1
test "$(sha256sum Content/Tile/s06_ger_Base.tile | cut -d' ' -f1)" = 2095dc70f2ac4f55fe5fe28c4a1cb4889851a64ac515005d0527eebcc4dcca25
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/s06_ger.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/s06_ger_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids s06_ger --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
