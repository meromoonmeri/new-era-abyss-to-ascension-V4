#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a03p03 milestone.
...
MIGRATED="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-entity-migration"
PLAN="$ROOT/.runtime-cache/pmdred-eu-reference-plan-v2.json"
SOURCE="$ROOT/.runtime-cache/pmdred-eu-ground"
FIX="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a03p03.rsground" | cut -d' ' -f1)" = 4025e48c9cf24be8a321da31115e933b05f994b89bcf01f827f4c46af3a8d22a
test ! -e "$MIGRATED"
"$PYTHON" tools/build_pmdred_eu_entity_migration.py --ground a03p03 --output "$MIGRATED"
test "$(sha256sum "$MIGRATED/grounds/a03p03.rsground" | cut -d' ' -f1)" = 8a0124252466f4adcdbfad6ab2097920e34d863c663e141772a034e602039589
test "$(sha256sum "$CANONICAL/tiles/a03p03_Base.tile" | cut -d' ' -f1)" = fa9b2e916bf28714e4614964f7589d6ef83e81754ee358c147c09c975a7a5281
test "$(sha256sum RESERVE/red_grounds/a03p03.rsground | cut -d' ' -f1)" = 056e711e49f8f9ae7b5067458d66ba2c758e42ca3b96fb60e0e58946c9bb52ae
test "$(sha256sum RESERVE/red_tiles/a03p03_Base.tile | cut -d' ' -f1)" = fa9b2e916bf28714e4614964f7589d6ef83e81754ee358c147c09c975a7a5281
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a03p03')
assert entry == {'canonical_debug_id': 179, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00179', 'debug_string_address': '0x0827A57C', 'ground_place_id': 35, 'map_file_id': 180, 'map_id': 171, 'stable_ground_id': 'a03p03', 'weather_id': 12}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_SUMMIT_SUNSET' in header
print('A03P03_IDENTITY_PASS map_file_id=180 symbol=MAP_SUMMIT_SUNSET')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a03p03 --expected-screenshots 2 \
...
print('A03P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a03p03'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a03p03','primary')]['movement_probe']=='PASS'
assert validations[('a03p03','primary')]['blocked_probe']=='PASS'
assert validations[('a03p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A03P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a03p03.rsground | cut -d' ' -f1)" = 8a0124252466f4adcdbfad6ab2097920e34d863c663e141772a034e602039589
test "$(sha256sum Content/Tile/a03p03_Base.tile | cut -d' ' -f1)" = fa9b2e916bf28714e4614964f7589d6ef83e81754ee358c147c09c975a7a5281
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a03p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a03p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a03p03 --entity-integrated-ids a03p03 --canonical-baseline-root "$CANONICAL" --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p03/a03p03.rsground | cut -d' ' -f1)" = 056e711e49f8f9ae7b5067458d66ba2c758e42ca3b96fb60e0e58946c9bb52ae
test "$(sha256sum RESERVE/pmdred_pre_promotion/a03p03/a03p03_Base.tile | cut -d' ' -f1)" = 314207f03b49c0908a801ee10ee0cbcb29c700d8500539444625e56da05489f9
