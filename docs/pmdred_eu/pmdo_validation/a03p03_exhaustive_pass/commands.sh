...
INTEGRATED="$ROOT/.runtime-cache/pmdred-eu-a03p03-reproduction-integrated-candidate"
...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a03p03')
assert e == {'canonical_debug_id': 179, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00179', 'debug_string_address': '0x0827A57C', 'ground_place_id': 35, 'map_file_id': 180, 'map_id': 171, 'stable_ground_id': 'a03p03', 'weather_id': 12}
assert 'MAP_SUMMIT_SUNSET' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A03P03_IDENTITY_PASS map_id=171 map_file_id=180 symbol=MAP_SUMMIT_SUNSET')
PYID

# Recreate the exact integration-preserving candidate from the promoted project entities.
test ! -e "$INTEGRATED"
"$PYTHON" tools/materialize_pmdred_eu_entity_integration.py --id a03p03 --output "$INTEGRATED"
test "$(sha256sum "$INTEGRATED/grounds/a03p03.rsground" | cut -d' ' -f1)" = 8a0124252466f4adcdbfad6ab2097920e34d863c663e141772a034e602039589
...
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  ...
  --ids a03p03 \
  --output "$FIX"
"$PYTHON" tools/apply_pmdred_eu_fixture_sheet_alias.py   --fixture "$FIX" --candidate-root "$INTEGRATED" --id a03p03 --alias a03p03_pmdred_Base
...
print('A03P03_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
...
test "$(sha256sum Data/Ground/a03p03.rsground | cut -d' ' -f1)" = 8ed1b765f91416f760ff84d4e16309c9bc3aef4d4c896f17a77ccddca341d80f
test "$(sha256sum Content/Tile/a03p03_pmdred_Base.tile | cut -d' ' -f1)" = fa9b2e916bf28714e4614964f7589d6ef83e81754ee358c147c09c975a7a5281
test "$(sha256sum Content/Tile/a03p03_Base.tile | cut -d' ' -f1)" = 314207f03b49c0908a801ee10ee0cbcb29c700d8500539444625e56da05489f9
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp "$INTEGRATED/grounds/a03p03.rsground" "$POST_CANDIDATE/grounds/"
cp "$CANONICAL/tiles/a03p03_Base.tile" "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" \
  --canonical-baseline-root "$CANONICAL" --entity-integrated-ids a03p03 --plan "$PLAN" \
  --ids a03p03 --output "$POST_FIX"
"$PYTHON" tools/apply_pmdred_eu_fixture_sheet_alias.py   --fixture "$POST_FIX" --candidate-root "$POST_CANDIDATE" --id a03p03 --alias a03p03_pmdred_Base
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
