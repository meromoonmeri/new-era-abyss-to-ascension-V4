#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b15p01b milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b15p01b-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b15p01b-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b15p01b-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b15p01b-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b15p01b.rsground" | cut -d' ' -f1)" = b6b35f6d7c1b6f3785812be0208c5f5e5494878a00b12b034e0efb6ae3a67dd9
test "$(sha256sum "$CANONICAL/tiles/b15p01b_Base.tile" | cut -d' ' -f1)" = 5097da1992aa25d0b0c00ec6fccf4f6ca2e0be6716c30014b71333e7b7f4431c
test "$(sha256sum RESERVE/red_grounds/b15p01b.rsground | cut -d' ' -f1)" = 4bd971df69c0ceeac6a385b7afeced4e3854f069e3d7c8afb79b9c7756ec7aa5
test "$(sha256sum RESERVE/red_tiles/b15p01b_Base.tile | cut -d' ' -f1)" = 5097da1992aa25d0b0c00ec6fccf4f6ca2e0be6716c30014b71333e7b7f4431c
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b15p01b')
assert entry == {'canonical_debug_id': 95, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00095', 'debug_string_address': '0x0827AEFC', 'ground_place_id': 10, 'map_file_id': 96, 'map_id': 95, 'stable_ground_id': 'b15p01b', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_CHIKORITA_FINAL' in header
print('B15P01B_IDENTITY_PASS map_file_id=96 symbol=MAP_TEAM_BASE_CHIKORITA_FINAL')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b15p01b --expected-screenshots 25 \
...
print('B15P01B_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b15p01b'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b15p01b','primary')]['movement_probe']=='PASS'
assert validations[('b15p01b','primary')]['blocked_probe']=='PASS'
assert validations[('b15p01b','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B15P01B_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b15p01b.rsground | cut -d' ' -f1)" = b6b35f6d7c1b6f3785812be0208c5f5e5494878a00b12b034e0efb6ae3a67dd9
test "$(sha256sum Content/Tile/b15p01b_Base.tile | cut -d' ' -f1)" = 5097da1992aa25d0b0c00ec6fccf4f6ca2e0be6716c30014b71333e7b7f4431c
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b15p01b.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b15p01b_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b15p01b --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
