...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p03i')
assert e == {'canonical_debug_id': 186, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00186', 'debug_string_address': '0x0827A49C', 'ground_place_id': 35, 'map_file_id': 190, 'map_id': 181, 'stable_ground_id': 'a05p03i', 'weather_id': -1}
assert 'MAP_THUNDERWAVE_CAVE_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('A05P03I_IDENTITY_PASS map_id=181 map_file_id=190 symbol=MAP_THUNDERWAVE_CAVE_END')
PYID
...
print('A05P03I_EXHAUSTIVE_PASS samples=2 unique_rgba=1 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
