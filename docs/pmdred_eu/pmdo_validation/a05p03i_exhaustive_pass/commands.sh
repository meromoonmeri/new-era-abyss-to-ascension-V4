#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the a05p03i milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03i-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-a05p03i-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-a05p03i-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-a05p03i-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/a05p03i.rsground" | cut -d' ' -f1)" = 19d02839cb1626f472cd07c5531d24bd4d0a899d282f9cc55de75e521280b90c
test "$(sha256sum "$CANONICAL/tiles/a05p03i_Base.tile" | cut -d' ' -f1)" = 175046fecc03e2d92a7d4b3c26704a1cc5070e4ea3daac676b9032d4c2319191
test ! -e RESERVE/red_grounds/a05p03i.rsground
test ! -e RESERVE/red_tiles/a05p03i_Base.tile
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='a05p03i')
assert entry == {'canonical_debug_id': 186, 'conversion_type': 4, 'debug_string': '__ground_amd_conversion_00186', 'debug_string_address': '0x0827A49C', 'ground_place_id': 35, 'map_file_id': 190, 'map_id': 181, 'stable_ground_id': 'a05p03i', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_THUNDERWAVE_CAVE_END' in header
print('A05P03I_IDENTITY_PASS map_file_id=190 symbol=MAP_THUNDERWAVE_CAVE_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground a05p03i --expected-screenshots 2 \
...
print('A05P03I_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['a05p03i'] and r['sample_count']==2 and len(primary)==1
...
assert validations[('a05p03i','primary')]['movement_probe']=='PASS'
assert validations[('a05p03i','primary')]['blocked_probe']=='PASS'
assert validations[('a05p03i','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('A05P03I_EXHAUSTIVE_PASS samples=2 unique_rgba=1')
PY
...
test "$(sha256sum Data/Ground/a05p03i.rsground | cut -d' ' -f1)" = 19d02839cb1626f472cd07c5531d24bd4d0a899d282f9cc55de75e521280b90c
test "$(sha256sum Content/Tile/a05p03i_Base.tile | cut -d' ' -f1)" = 175046fecc03e2d92a7d4b3c26704a1cc5070e4ea3daac676b9032d4c2319191
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/a05p03i.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/a05p03i_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids a05p03i --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
