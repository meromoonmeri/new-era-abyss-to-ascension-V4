#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h29p03 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h29p03-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h29p03-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h29p03-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h29p03-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h29p03.rsground" | cut -d' ' -f1)" = b583321d1d543d8921d0dc0b969e653b245fb9d08ca4b02c62350ee4b0510d5b
test "$(sha256sum "$CANONICAL/tiles/h29p03_Base.tile" | cut -d' ' -f1)" = f81d9c6fdc54f2b877b00144d9605b7b7cf95ba5124890c1f602ebda8c3a3962
test "$(sha256sum RESERVE/red_grounds/h29p03.rsground | cut -d' ' -f1)" = 29ed584623ff0a63f66a06a9427a9ed7fa8733313d80d9d74a9fe55844d1e768
test "$(sha256sum RESERVE/red_tiles/h29p03_Base.tile | cut -d' ' -f1)" = 754004298e6040655172126e7c160c825b5476d8c0fad73b3ce884d32438c24e
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p03')
assert entry == {'canonical_debug_id': 168, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00168', 'debug_string_address': '0x0827A6BC', 'ground_place_id': 9, 'map_file_id': 169, 'map_id': 161, 'stable_ground_id': 'h29p03', 'weather_id': 8}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_FINAL_ISLAND' in header
print('H29P03_IDENTITY_PASS map_file_id=169 symbol=MAP_FRIEND_AREA_FINAL_ISLAND')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h29p03 --expected-screenshots 1009 \
...
print('H29P03_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 3052
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 1009
...
assert r['grounds']==['h29p03'] and r['sample_count']==1009 and len(primary)==1008
assert r['exact_sample_count']==1009 and r['fully_opaque_sample_count']==1009
assert len({s['actual_rgba_sha256'] for s in primary})==44
...
assert validations[('h29p03','primary')]['movement_probe']=='PASS'
assert validations[('h29p03','primary')]['blocked_probe']=='PASS'
assert validations[('h29p03','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H29P03_EXHAUSTIVE_PASS samples=1009 unique_rgba=44')
PY
...
test "$(sha256sum Data/Ground/h29p03.rsground | cut -d' ' -f1)" = b583321d1d543d8921d0dc0b969e653b245fb9d08ca4b02c62350ee4b0510d5b
test "$(sha256sum Content/Tile/h29p03_Base.tile | cut -d' ' -f1)" = f81d9c6fdc54f2b877b00144d9605b7b7cf95ba5124890c1f602ebda8c3a3962
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h29p03.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h29p03_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h29p03 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 1009 comparative PNGs,
