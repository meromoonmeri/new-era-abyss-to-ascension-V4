...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h29p03')
assert e == {'canonical_debug_id': 168, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00168', 'debug_string_address': '0x0827A6BC', 'ground_place_id': 9, 'map_file_id': 169, 'map_id': 161, 'stable_ground_id': 'h29p03', 'weather_id': 8}
assert 'MAP_FRIEND_AREA_FINAL_ISLAND' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('H29P03_IDENTITY_PASS map_id=161 map_file_id=169 symbol=MAP_FRIEND_AREA_FINAL_ISLAND')
PYID
...
env "${PMDO_ENV[@]}" PMDO_GROUND_VALIDATOR=pmdred_eu_native_fixture \
  ...
  --reset-events
test "$(wc -l < "$FIX/events.jsonl")" -eq 3049
...
print('H29P03_EXHAUSTIVE_PASS samples=1009 unique_rgba=44 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
