#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the d09p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-d09p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-d09p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-d09p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-d09p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/d09p01.rsground" | cut -d' ' -f1)" = caab96b8bda330d0339ca5b905e195b5713dfad7addce37ccae2feee043e8a9b
test "$(sha256sum "$CANONICAL/tiles/d09p01_Base.tile" | cut -d' ' -f1)" = 71fb57ca94187b2924419998c809c6588849ab53c376cc7dae7310c476e88aac
test "$(sha256sum RESERVE/red_grounds/d09p01.rsground | cut -d' ' -f1)" = b5989b9626eec7b923abad732d0b540bb716bc3dcbc0c22159e7b0547f14aca4
test "$(sha256sum RESERVE/red_tiles/d09p01_Base.tile | cut -d' ' -f1)" = 71fb57ca94187b2924419998c809c6588849ab53c376cc7dae7310c476e88aac
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d09p01')
assert entry == {'canonical_debug_id': 204, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00204', 'debug_string_address': '0x0827A25C', 'ground_place_id': 24, 'map_file_id': 209, 'map_id': 200, 'stable_ground_id': 'd09p01', 'weather_id': 13}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FROSTY_FOREST_END' in header
print('D09P01_IDENTITY_PASS map_file_id=209 symbol=MAP_FROSTY_FOREST_END')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground d09p01 --expected-screenshots 25 \
...
print('D09P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['d09p01'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('d09p01','primary')]['movement_probe']=='PASS'
assert validations[('d09p01','primary')]['blocked_probe']=='PASS'
assert validations[('d09p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('D09P01_EXHAUSTIVE_PASS samples=25 unique_rgba=4')
PY
...
test "$(sha256sum Data/Ground/d09p01.rsground | cut -d' ' -f1)" = caab96b8bda330d0339ca5b905e195b5713dfad7addce37ccae2feee043e8a9b
test "$(sha256sum Content/Tile/d09p01_Base.tile | cut -d' ' -f1)" = 71fb57ca94187b2924419998c809c6588849ab53c376cc7dae7310c476e88aac
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/d09p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/d09p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids d09p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
