...
e=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='d09p01')
assert e == {'canonical_debug_id': 204, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00204', 'debug_string_address': '0x0827A25C', 'ground_place_id': 24, 'map_file_id': 209, 'map_id': 200, 'stable_ground_id': 'd09p01', 'weather_id': 13}
assert 'MAP_FROSTY_FOREST_END' in Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
print('D09P01_IDENTITY_PASS map_id=200 map_file_id=209 symbol=MAP_FROSTY_FOREST_END')
PYID
...
print('D09P01_EXHAUSTIVE_PASS samples=25 unique_rgba=4 termination=NORMAL_EXIT_0_NO_SIGNAL_NO_SIGSEGV_NO_ORPHANS')
PY
