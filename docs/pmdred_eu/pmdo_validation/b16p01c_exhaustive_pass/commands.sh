#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the b16p01c milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01c-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-b16p01c-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-b16p01c-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-b16p01c-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/b16p01c.rsground" | cut -d' ' -f1)" = 0407c158b69f19a547b3c9bc18c45f1eda59c50ec3cd19a6dee4929a39c0244a
test "$(sha256sum "$CANONICAL/tiles/b16p01c_Base.tile" | cut -d' ' -f1)" = 338462fb10130f2085acbf7e8791c8f85f34cacb014cb0ccc673060906d6da58
test "$(sha256sum RESERVE/red_grounds/b16p01c.rsground | cut -d' ' -f1)" = 759a390a376ac750704f6cd418d0ed4289f2302b2388be6dfa26fb15eb1003c2
test "$(sha256sum RESERVE/red_tiles/b16p01c_Base.tile | cut -d' ' -f1)" = 338462fb10130f2085acbf7e8791c8f85f34cacb014cb0ccc673060906d6da58
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='b16p01c')
assert entry == {'canonical_debug_id': 102, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00102', 'debug_string_address': '0x0827AE1C', 'ground_place_id': 10, 'map_file_id': 103, 'map_id': 102, 'stable_ground_id': 'b16p01c', 'weather_id': 9}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_TEAM_BASE_INSIDE_TREECKO_BASIC' in header
print('B16P01C_IDENTITY_PASS map_file_id=103 symbol=MAP_TEAM_BASE_INSIDE_TREECKO_BASIC')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground b16p01c --expected-screenshots 25 \
...
print('B16P01C_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
...
assert r['grounds']==['b16p01c'] and r['sample_count']==25 and len(primary)==24
...
assert validations[('b16p01c','primary')]['movement_probe']=='PASS'
assert validations[('b16p01c','primary')]['blocked_probe']=='PASS'
assert validations[('b16p01c','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('B16P01C_EXHAUSTIVE_PASS samples=25 unique_rgba=6')
PY
...
test "$(sha256sum Data/Ground/b16p01c.rsground | cut -d' ' -f1)" = 0407c158b69f19a547b3c9bc18c45f1eda59c50ec3cd19a6dee4929a39c0244a
test "$(sha256sum Content/Tile/b16p01c_Base.tile | cut -d' ' -f1)" = 338462fb10130f2085acbf7e8791c8f85f34cacb014cb0ccc673060906d6da58
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/b16p01c.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/b16p01c_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids b16p01c --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
