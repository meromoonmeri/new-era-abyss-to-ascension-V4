#!/usr/bin/env bash
# Exact create-only recovery and reproduction recipe for the h13p01 milestone.
...
FIX="$ROOT/.runtime-cache/pmdred-eu-h13p01-reproduction-fixture"
COMPARE="$ROOT/.runtime-cache/pmdred-eu-h13p01-reproduction-comparison"
POST_CANDIDATE="$ROOT/.runtime-cache/pmdred-eu-h13p01-reproduction-promoted-candidate"
POST_FIX="$ROOT/.runtime-cache/pmdred-eu-h13p01-reproduction-promoted-fixture"
...
test "$(sha256sum "$CANONICAL/grounds/h13p01.rsground" | cut -d' ' -f1)" = 26aa27ed7b672be74a2295b4232a343ae9010df2eb795c9dd6811f60232bd717
test "$(sha256sum "$CANONICAL/tiles/h13p01_Base.tile" | cut -d' ' -f1)" = b04de3ec4470fd865d4344bc714ca9171f3ecbd8189fdbd911ee3ff461328ef1
test "$(sha256sum RESERVE/red_grounds/h13p01.rsground | cut -d' ' -f1)" = e52f52c450f855f1bdd3c5e4f4b3905ff37095b082dd83a6bc6c68d136ca191d
test "$(sha256sum RESERVE/red_tiles/h13p01_Base.tile | cut -d' ' -f1)" = b04de3ec4470fd865d4344bc714ca9171f3ecbd8189fdbd911ee3ff461328ef1
"$PYTHON" - <<'PYID'
import json
from pathlib import Path
entry=next(x for x in json.loads(Path('docs/pmdred_eu/ground_manifest.json').read_text())['ground_conversion_table']['entries'] if x['stable_ground_id']=='h13p01')
assert entry == {'canonical_debug_id': 143, 'conversion_type': 1, 'debug_string': '__ground_amd_conversion_00143', 'debug_string_address': '0x0827A99C', 'ground_place_id': 9, 'map_file_id': 144, 'map_id': 138, 'stable_ground_id': 'h13p01', 'weather_id': -1}
header=Path('.runtime-cache/pmd-red-reference/include/constants/ground_map.h').read_text()
assert 'MAP_FRIEND_AREA_CRYPTIC_CAVE' in header
print('H13P01_IDENTITY_PASS map_file_id=144 symbol=MAP_FRIEND_AREA_CRYPTIC_CAVE')
PYID
...
"$PYTHON" tools/run_pmdred_eu_native_fixture.py \
  --fixture "$FIX" --ground h13p01 --expected-screenshots 17 \
...
print('H13P01_NATIVE_TERMINATION_PASS rc=0 phase=Unload')
PYTERM
test "$(wc -l < "$FIX/events.jsonl")" -eq 76
test "$(find "$FIX/appdata/SCREENSHOT" -maxdepth 1 -type f -name '*.png' | wc -l)" -eq 17
...
assert r['grounds']==['h13p01'] and r['sample_count']==17 and len(primary)==16
assert r['exact_sample_count']==17 and r['fully_opaque_sample_count']==17
assert len({s['actual_rgba_sha256'] for s in primary})==4
...
assert validations[('h13p01','primary')]['movement_probe']=='PASS'
assert validations[('h13p01','primary')]['blocked_probe']=='PASS'
assert validations[('h13p01','reload')]['load']=='LOAD_PASS'
assert all(s['mismatched_pixels']==0 and s['maximum_channel_delta']==0 for s in r['samples'])
print('H13P01_EXHAUSTIVE_PASS samples=17 unique_rgba=4')
PY
...
test "$(sha256sum Data/Ground/h13p01.rsground | cut -d' ' -f1)" = 26aa27ed7b672be74a2295b4232a343ae9010df2eb795c9dd6811f60232bd717
test "$(sha256sum Content/Tile/h13p01_Base.tile | cut -d' ' -f1)" = b04de3ec4470fd865d4344bc714ca9171f3ecbd8189fdbd911ee3ff461328ef1
test ! -e "$POST_CANDIDATE" && test ! -e "$POST_FIX"
mkdir -p "$POST_CANDIDATE/grounds" "$POST_CANDIDATE/tiles"
cp Data/Ground/h13p01.rsground "$POST_CANDIDATE/grounds/"
cp Content/Tile/h13p01_Base.tile "$POST_CANDIDATE/tiles/"
cp "$CANONICAL/conversion_report.json" "$POST_CANDIDATE/"
"$PYTHON" tools/build_pmdred_eu_runtime_fixture.py \
  --conversion-set remaining --candidate-root "$POST_CANDIDATE" --plan "$PLAN" \
  --ids h13p01 --output "$POST_FIX"
env "${PMDO_ENV[@]}" timeout --signal=TERM --kill-after=5 300 "$BUNDLE/PMDO" \
...
# Add --montage-all to the comparator invocation when all 17 comparative PNGs,
