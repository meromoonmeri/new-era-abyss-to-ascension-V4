#!/usr/bin/env python3
"""Extract redacted Reminiscencia environmental timelines, assets and animations."""
...
FPS=60;FRAME_MS=1000/30;COMMIT="6963878956ec4c53833c03f26aa6a9aafd17800f"
INCLUDED_PICTURES={
...
def timing(payload):
 payload["timing"]={"source_unit":"Graphics frames","nominal_frame_rate":60,"nominal_frame_duration_ms":1000/60,"authority":"Data/export/0000 Settings.rb: Graphics.frame_rate = 60","branching_note":"wait durations are exact per command; do not sum branches"}
 for seq in payload.get("sequences",[]):
  for c in seq["timeline"]:
   if c["category"]=="wait":
    n=c["parameters"]["frames"];c["parameters"]={"frames":n,"nominal_ms_at_source_60fps":n*1000/60}
...
def render_animation(aid,a,source,dest,usage):
 ...
 meta={"schema_version":"1.0.0","animation_id":f"reminiscencia_rgss_animation_{aid:03d}","source_animation_id":aid,"source_name_sha256":sha256_bytes(decode_text(ivar(a,"name","")).encode()),"status":"SOURCE_DOCUMENTED","usage":usage,"position_mode":int(ivar(a,"position",1)),"frame_count":len(frames),"loop":False,"source_frame_duration_ms":FRAME_MS,"timing_authority":{"frame_rate":60,"update_interval_graphics_frames":2,"script":"Data/export/0020 RPG__Sprite.rb","script_sha256":sha256_file(source/"Data/export/0020 RPG__Sprite.rb")},"canvas":{"dimensions_px":[w,h],"origin_px":[-minx,-miny],"pmdo_scale":[1,2]},"source_sheet":{"identity_sha256":sha256_bytes(name.encode()),"source_sha256":sha256_file(path),"dimensions_px":[sheet.width,sheet.height],"hue_degrees":int(ivar(a,"animation_hue",0)),"pixels_exported_only_as_composed_frames":True},"blend_modes":{"0":"NORMAL_ALPHA","1":"ADDITIVE"},"additive_cell_render_count":adds,"frames":frames,"timings":timings,"contact_sheet":contact(ims,dest/"contact_sheet.png"),"audio_exported":False,"casting_sprites_exported":False};write_json(dest/"metadata.json",meta);return {"source_animation_id":aid,"file":"metadata.json","metadata_sha256":sha256_file(dest/"metadata.json"),"frame_count":len(frames),"status":"SOURCE_DOCUMENTED"}
def entity_sprites(source,game,maps):
 ...
 for name,contexts in sorted(grouped.items()):
  ...
  im=load_png(path);req(im.width%4==0 and im.height%4==0,"entity matrix");fw=im.width//4;fh=im.height//4;hsh=sha256_file(path);dest=game/"vfx/entity_sprites"/f"reminiscencia_environment_entity_{hsh[:12]}";frames=[];ims=[]
...
def build(source,game):
 ...
 for (kind,directory,name),contexts in sorted(group.items()):
  ...
  aid=f"reminiscencia_{slug(kind)}_{slug(name)}_{h[:12]}"
 ...
 for aid,usage in used.items():
  dest=anims/f"reminiscencia_rgss_animation_{aid:03d}";row=render_animation(aid,adata[aid],source,dest,usage);row["file"]=(dest/"metadata.json").relative_to(game).as_posix();arows.append(row)
 aman={"schema_version":"1.0.0","result":"REMINISCENCIA_USED_ANIMATIONS_EXTRACTED","animation_count":len(arows),"total_frame_count":sum(x["frame_count"] for x in arows),"frame_duration_ms":FRAME_MS,"animations":arows};write_json(anims/"manifest.json",aman)
 manifest={"schema_version":"1.0.0","result":"REMINISCENCIA_ENVIRONMENTAL_VFX_AUDIT_PASS","source_commit":COMMIT,"map_timeline_count":len(timeline_rows),"common_event_timeline_count":cp["event_count"],"visual_command_counts":dict(sorted(counts.items())),"static_script_visual_audit_required_count":review,"environment_asset_count":len(assets),"animated_environment_asset_count":animated,"included_environmental_picture_count":sum(x["kind"]=="ENVIRONMENT_OVERLAY" for x in assets),"excluded_cast_or_ui_picture_count":len(excluded),"unresolved_environment_count":len(unresolved),"used_rgss_animation_count":len(arows),"used_rgss_animation_frame_count":aman["total_frame_count"],"environment_entity_sprite_sheet_count":len(entity_rows),"environment_entity_sprite_context_count":entity_contexts,"excluded_character_sheet_count":excluded_sheets,"environment_entity_sprites":entity_rows,"dialogue_contents_exported":False,"script_bodies_exported":False,"casting_pixels_exported":False,"timing_authorities":{"graphics_fps":60,"settings_sha256":sha256_file(source/"Data/export/0000 Settings.rb"),"animation_script_sha256":sha256_file(source/"Data/export/0020 RPG__Sprite.rb")},"timelines":timeline_rows,"common_events":{"file":cpath.relative_to(game).as_posix(),"sha256":sha256_file(cpath)},"environment_assets":assets,"environment_contact_sheet":envcontact,"excluded_assets":excluded,"unresolved_assets":unresolved,"animations":{"file":(anims/"manifest.json").relative_to(game).as_posix(),"sha256":sha256_file(anims/"manifest.json")}}
 req(review==0 and not unresolved,"unresolved visual audit");req(len(assets)+len(excluded)==len(group),"classification mismatch");write_json(vfx/"manifest.json",manifest);(vfx/"README.md").write_text("# VFX environnementaux Reminiscencia\n\nTimelines, fogs, panoramas, Pictures et animations expurgés.\n",encoding="utf-8");return manifest
def main():
