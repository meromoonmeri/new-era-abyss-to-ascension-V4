...
function magma_cavern.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  DEBUG.EnableDbgCoro()
  PrintInfo("=>> ExitSegment_magma_cavern result "..tostring(result).." segment "..tostring(segmentID))
  ...
  if segmentID == 0 and result == RogueEssence.Data.GameProgress.ResultType.Cleared then
    ...
    GAME:EnterGroundMap('arene_trone_magma', 'Main_Entrance_Marker')
    return
  end
  if segmentID == 1 then
    ...
    if result == RogueEssence.Data.GameProgress.ResultType.Cleared then
      ...
      LegendArc.Victory('molten_throne')
