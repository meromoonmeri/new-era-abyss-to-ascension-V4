...
--[[ Tour Joyeuse (joyous_tower) — chapitre 27.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[joyous_tower] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function joyous_tower.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'joyous_tower'
end

function joyous_tower.EnterSegment(zone, rescuing, segmentID, mapID)
...
function joyous_tower.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function joyous_tower.ExitSegment(zone, result, rescue, segmentID, mapID)
