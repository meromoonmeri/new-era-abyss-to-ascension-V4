...
--[[ Mer Lointaine (far_off_sea) — chapitre 27.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[far_off_sea] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function far_off_sea.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'far_off_sea'
end

function far_off_sea.EnterSegment(zone, rescuing, segmentID, mapID)
...
function far_off_sea.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function far_off_sea.ExitSegment(zone, result, rescue, segmentID, mapID)
