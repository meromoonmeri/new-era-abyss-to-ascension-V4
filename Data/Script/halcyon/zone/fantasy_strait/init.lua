...
--[[ Détroit Fantastique (fantasy_strait) — chapitre 24.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[fantasy_strait] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function fantasy_strait.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'fantasy_strait'
end

function fantasy_strait.EnterSegment(zone, rescuing, segmentID, mapID)
...
function fantasy_strait.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function fantasy_strait.ExitSegment(zone, result, rescue, segmentID, mapID)
