--[[ Tour du Ciel Fracturé — zone add-on (Réseau des Anciens Chemins, id origine 17).
     Progression fidèle pret/pmd-red : 26 étages + relais + 10 étages (cime).
     Aucun impact histoire : entrée/sortie via master_zone carte 1 (Metano). ]]
require 'origin.common'
require 'halcyon.GeneralFunctions'
require 'halcyon.VeilleurArc'

local new_era_zone_17 = {}

function new_era_zone_17.Init(zone)
  DEBUG.EnableDbgCoro()
  PrintInfo("=>> Init_new_era_zone_17")
end

function new_era_zone_17.EnterSegment(zone, rescuing, segmentID, mapID)
...
function new_era_zone_17.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function new_era_zone_17.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  if segmentID == 0 and result == RogueEssence.Data.GameProgress.ResultType.Cleared then
    GAME:EnterGroundMap('new_era_zone_17_relais', 'Main_Entrance_Marker')
    return
  end
  ...
  if segmentID == 2 then
    if result == RogueEssence.Data.GameProgress.ResultType.Cleared then
      ...
      VeilleurArc.Victory('new_era_zone_17')
      GAME:WaitFrames(20)
