...
--[[ Colline aux Drakes (wyvern_hill) — chapitre 18.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[wyvern_hill] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function wyvern_hill.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'wyvern_hill'
end

function wyvern_hill.EnterSegment(zone, rescuing, segmentID, mapID)
...
function wyvern_hill.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function wyvern_hill.ExitSegment(zone, result, rescue, segmentID, mapID)
