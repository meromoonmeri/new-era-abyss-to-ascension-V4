...
--[[ Forêt du Tumulte (uproar_forest) — chapitre 22.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[uproar_forest] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function uproar_forest.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'uproar_forest'
end

function uproar_forest.EnterSegment(zone, rescuing, segmentID, mapID)
...
function uproar_forest.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function uproar_forest.ExitSegment(zone, result, rescue, segmentID, mapID)
