...
--[[ mt_blaze_peak — chapitre PMD Red.
     Zone reconstruite par tools/dungeon_builder : 2 segment(s).
       ...
       * segment 1 : étage boss fixe canonique (LoadGen +
         MappedRoomStep('mt_blaze_peak_boss')) — contrepartie pixel-exacte du
         Ground canonique d09p03.
       * Après clear seg 1, transition vers le Ground canonique d09p03
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[mt_blaze_peak] Ground absent de la zone : ' .. tostring(name))
  return 0
end
...
local RETURN_GROUND = 'd09p03'

function mt_blaze_peak.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'mt_blaze_peak'
end

function mt_blaze_peak.EnterSegment(zone, rescuing, segmentID, mapID)
...
function mt_blaze_peak.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function mt_blaze_peak.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  elseif segmentID == 1 then
    ...
    SV.CanonicalDungeons.Pending = 'mt_blaze_peak_seg1'
    GAME:EnterZone(zone.ID, -1, ZONE_GROUND_IDX(zone, 'd09p03'), 0)
  else
    GeneralFunctions.EndDungeonRun(result, 'master_zone', -1, GROUND_IDX(RETURN_GROUND), 0, true, true)
  end
end

return mt_blaze_peak
