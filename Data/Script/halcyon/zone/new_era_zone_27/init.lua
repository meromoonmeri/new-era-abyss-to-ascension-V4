--[[ Relique Ensevelie — zone add-on (Réseau des Anciens Chemins, id origine 27).
     Progression fidèle pret/pmd-red : 100 étages.
     Aucun impact histoire : entrée/sortie via master_zone carte 1 (Metano). ]]
require 'origin.common'
require 'halcyon.GeneralFunctions'

local new_era_zone_27 = {}

function new_era_zone_27.Init(zone)
  DEBUG.EnableDbgCoro()
  PrintInfo("=>> Init_new_era_zone_27")
end

function new_era_zone_27.EnterSegment(zone, rescuing, segmentID, mapID)
...
function new_era_zone_27.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function new_era_zone_27.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  -- Veilleur du Réseau : segment 1 = arène (golurk).
  if segmentID == 0 and result == RogueEssence.Data.GameProgress.ResultType.Cleared then
    GAME:ContinueDungeon("new_era_zone_27", 1, 0, 0, RogueEssence.Data.GameProgress.DungeonStakes.Risk, true, false)
    return
  end
  if segmentID == 1 then
