...
--[[ Relique Enfouie (buried_relic) — chapitre 15.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[buried_relic] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function buried_relic.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'buried_relic'
end

function buried_relic.EnterSegment(zone, rescuing, segmentID, mapID)
...
function buried_relic.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function buried_relic.ExitSegment(zone, result, rescue, segmentID, mapID)
