...
--[[ Grotte Solaire (solar_cave) — chapitre 19.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[solar_cave] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function solar_cave.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'solar_cave'
end

function solar_cave.EnterSegment(zone, rescuing, segmentID, mapID)
...
function solar_cave.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function solar_cave.ExitSegment(zone, result, rescue, segmentID, mapID)
