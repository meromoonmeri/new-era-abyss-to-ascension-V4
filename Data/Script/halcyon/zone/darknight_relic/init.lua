...
--[[ Relique de la Nuit (darknight_relic) — chapitre 20.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[darknight_relic] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function darknight_relic.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'darknight_relic'
end

function darknight_relic.EnterSegment(zone, rescuing, segmentID, mapID)
...
function darknight_relic.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function darknight_relic.ExitSegment(zone, result, rescue, segmentID, mapID)
