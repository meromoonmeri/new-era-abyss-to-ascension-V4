...
--[[ Région Désertique (desert_region) — chapitre 17.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[desert_region] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function desert_region.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'desert_region'
end

function desert_region.EnterSegment(zone, rescuing, segmentID, mapID)
...
function desert_region.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function desert_region.ExitSegment(zone, result, rescue, segmentID, mapID)
