...
--[[ Grotte Statique (thunderwave_cave) — chapitre 7.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[thunderwave_cave] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'grotte_statique_seuil'

function thunderwave_cave.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'thunderwave_cave'
end
...
function thunderwave_cave.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  if segmentID == 0 then
    ...
    GAME:EnterGroundMap('d02p02', 'Main_Entrance_Marker')
  else
    GeneralFunctions.EndDungeonRun(result, 'master_zone', -1, GROUND_IDX(RETURN_GROUND), 0, true, true)
  end
end
