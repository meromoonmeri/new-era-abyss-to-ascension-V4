...
--[[ Caverne Étrange (oddity_cave) — chapitre 22.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[oddity_cave] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function oddity_cave.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'oddity_cave'
end

function oddity_cave.EnterSegment(zone, rescuing, segmentID, mapID)
...
function oddity_cave.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function oddity_cave.ExitSegment(zone, result, rescue, segmentID, mapID)
