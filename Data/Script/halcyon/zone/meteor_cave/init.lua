...
--[[ Grotte de la Météorite (meteor_cave) — chapitre 14.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[meteor_cave] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function meteor_cave.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'meteor_cave'
end

function meteor_cave.EnterSegment(zone, rescuing, segmentID, mapID)
...
function meteor_cave.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function meteor_cave.ExitSegment(zone, result, rescue, segmentID, mapID)
