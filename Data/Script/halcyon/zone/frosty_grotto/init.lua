...
--[[ Grotte Givrée (frosty_grotto) — chapitre 11.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[frosty_grotto] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'foret_givree_oree'

function frosty_grotto.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'frosty_grotto'
end

function frosty_grotto.EnterSegment(zone, rescuing, segmentID, mapID)
...
function frosty_grotto.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function frosty_grotto.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  if segmentID == 0 then
    ...
    SV.CanonicalDungeons.Pending = 'frosty_forest_summit'
