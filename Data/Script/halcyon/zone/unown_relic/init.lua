...
--[[ Relique des Zarbi (unown_relic) — chapitre 26.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[unown_relic] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function unown_relic.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'unown_relic'
end

function unown_relic.EnterSegment(zone, rescuing, segmentID, mapID)
...
function unown_relic.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function unown_relic.ExitSegment(zone, result, rescue, segmentID, mapID)
