...
--[[ Champ de Foudre (lightning_field) — chapitre 20.
...
local function ZONE_GROUND_IDX(zone, name)
  ...
  PrintInfo('[lightning_field] Ground absent de la zone : ' .. tostring(name))
  return 0
end

local RETURN_GROUND = 'metano_town'

function lightning_field.Init(zone)
  DEBUG.EnableDbgCoro()
  SV.TemporaryFlags.LastDungeonEntered = 'lightning_field'
end

function lightning_field.EnterSegment(zone, rescuing, segmentID, mapID)
...
function lightning_field.Rescued(zone, name, mail)
  COMMON.Rescued(zone, name, mail)
end

function lightning_field.ExitSegment(zone, result, rescue, segmentID, mapID)
  ...
  if segmentID == 0 then
    ...
    GAME:EnterDungeon('lightning_field', 1, 0, 0,
      RogueEssence.Data.GameProgress.DungeonStakes.Risk, true, false)
  elseif segmentID == 1 then
    -- Ground final canonique : cinématique, combat et fin au même endroit
    SV.CanonicalDungeons.Pending = 'lightning_field_seg1'
    -- champ_foudre ne porte aucun marqueur : la scène téléporte
    -- elle-même le joueur, on entre donc par index.
    GAME:EnterGroundMap(ZONE_GROUND_IDX(zone, 'champ_foudre'), 0)
  else
    GeneralFunctions.EndDungeonRun(result, 'master_zone', -1, GROUND_IDX(RETURN_GROUND), 0, true, true)
  end
end

return lightning_field
