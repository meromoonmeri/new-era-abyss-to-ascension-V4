...
-- Scène canonique PMD Sky EU : SCRIPT/D56P11A/n06a2401.ssb (ROM sha256 1fa39d35…).
...
return function(hero, partner)
  ...
  SkySceneKit.say({english=" Back where we started...", french=" De retour au point de départ...", german="Wieder dort, wo wir\nangefangen haben...", italian="Siamo di nuovo al punto\ndi partenza...", spanish=" Otra vez en el punto de partida..."})
  ...
  GAME:WaitFrames(2) -- join WaitExecuteLives
  pcall(function() UI:SetSpeaker(npc_npc_chaaremu) end)
  SkySceneKit.say({english=" ...What did you expect?", french=" ... A quoi vous attendiez-vous?", german=" ...Was habt ihr erwartet?", italian=" Che ti aspettavi?", spanish=" ¿Qué esperabas...?"})
  pcall(function() UI:SetSpeaker(npc_npc_chaaremu) end)
  SkySceneKit.say({english="We went in the same entrance\nas before!", french="Nous avons pris la même\nentrée que tout à l'heure!", german="Wir haben denselben Eingang\nwie zuvor gewählt!", italian="Abbiamo scelto la stessa entrata\ndi prima!", spanish="¡Si hemos ido por el mismo\ncamino que antes!"})
  -- message_Close
  pcall(function() GROUND:CharTurnToCharAnimated(npc_npc_chaaremu, hero, 4) end)
  GAME:WaitFrames(2) -- join WaitExecuteLives
  pcall(function() UI:SetSpeaker(npc_npc_chaaremu) end)
  SkySceneKit.say({english="[CS:Y]Lopunny[CR]![K]\nAren't you going to try the other entrance?", french="[CS:Y]Lockpin[CR]![K]\nTu ne veux pas essayer l'autre entrée?", german="[CS:Y]Schlapor[CR]![K] Willst du diesmal\nnicht den anderen Eingang ausprobieren?", italian="[CS:Y]Lopunny[CR]![K]\nNon dovremmo provare l'altra entrata?", spanish="¡[CS:Y]Lopunny[CR]![K]\n¿No piensas probar por la otra entrada?"})
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  ...
  pcall(function() UI:SetSpeaker(hero) end)
  SkySceneKit.say({english="I seem to be a\nbit confused. [M:H8]", french=" Je me sens un peu perdue. [M:H8]", german="Ich bin wohl ein wenig\nverwirrt. [M:H8]", italian="Sembra che mi sia confusa\nun po'. [M:H8]", spanish="Creo que estoy un poco\nconfusa. [M:H8]"})
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  pcall(function() SOUND:PlayBattleSE("EVT_Emote_Shock_2") end)
  -- ExecuteCommon(CORO_JUMP_ANGRY_FUNC_SERIES, 0) [gestion de station NDS: le chargement/la coroutine commune est assurée par le harnais journey PMDO]
  GAME:WaitFrames(2) -- join WaitExecuteLives
  pcall(function() UI:SetSpeaker(npc_npc_chaaremu) end)
  SkySceneKit.say({english=" This isn't interesting at all!", french=" Ça n'a rien d'intéressant!", german="Das ist überhaupt nicht\ninteressant!", italian=" Non è per nulla interessante!", spanish="¡A mí no me parece\nnada interesante!"})
  pcall(function() UI:SetSpeaker(npc_npc_chaaremu) end)
  SkySceneKit.say({english="This time, select a different\nentrance already! Gee whiz!", french="Prenons l'autre entrée! Allez,\nen piste!", german="Wähle dieses Mal endlich den\nanderen Eingang! Du Held!", italian="Questa volta scegli l'altra\nentrata! Forza!", spanish="¡Más te vale que esta vez elijas\notra entrada! ¡Jo, es que ya te vale!"})
  -- message_Close
