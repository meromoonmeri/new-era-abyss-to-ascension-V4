...
-- Scène canonique PMD Sky EU : SCRIPT/D32P32A/s04p1101.ssb (ROM sha256 1fa39d35…).
-- Dialogues 5 langues ROM embarqués ; conventions du pilote m01a0204.
local SkySceneKit = require 'halcyon.skyscenes.kit'
local SkyProg = require 'halcyon.skyscenes.progression'
return function(hero, partner)
  pcall(function() SOUND:StopBGM() end)
  -- back_SetGround(LEVEL_D32P32A) [neutre/état moteur]
  ...
  GAME:WaitFrames(2) -- join WaitExecuteObject(OBJECT_D32P44A4_87) [routine d'objet NDS non simulée - documenté]
  -- SetAnimation(2) [anim idle native]
  -- SetOutputAttribute(4) [neutre/état moteur]
  GAME:WaitFrames(60)
  -- MovePositionMark OBJECT_D32P44A4_87 [cible sans placement SSA zone: mouvement non joué]
  GAME:WaitFrames(2) -- join WaitExecuteObject(OBJECT_D32P44A4_87) [routine d'objet NDS non simulée - documenté]
  GAME:WaitFrames(30)
  ...
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN]A voice echoed from somewhere...", french="[CN]Une voix retentit de nulle part...", german="[CN]Eine Stimme schallt von irgendwoher...", italian="[CN]Si sente una voce echeggiare in lontananza...", spanish="[CN]Un eco proveniente de alguna parte..."})
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  ...
  GAME:WaitFrames(15)
  local npc_npc_rejirokku = SkySceneKit.spawn_npc("regirock", 184, 216, Direction.Down, "NPC_REJIROKKU")
  pcall(function() UI:SetSpeaker(npc_npc_rejirokku) end)
  SkySceneKit.say({english=" I AM [CS:N]REGIROCK[CR]...", french=" JE SUIS [CS:N]REGIROCK[CR]...", german=" ICH BIN [CS:N]REGIROCK[CR]...", italian=" IO SONO [CS:N]REGIROCK[CR]...", spanish=" YO SOY [CS:N]REGIROCK[CR]..."})
  pcall(function() UI:SetSpeaker(npc_npc_rejirokku) end)
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  -- switch(ProcessSpecial(PROCESS_SPECIAL_PREPARE_MENU_ACCEPT_TEAM_MEMBER, 0, 0)) [procédé/menu moteur NDS, corps vide: aucun embranchement canonique — équivalent moteur PMDO]
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  pcall(function() UI:SetSpeaker(npc_npc_rejirokku) end)
  SkySceneKit.say({english="[CN][player] turned down\n[CN][CS:N]Regirock[CR]'s offer to join.", french="[CN][player] refuse l'offre\n[CN]de [CS:N]Regirock[CR].", german="[CN][player] lehnt das Beitrittsangebot von\n[CN][CS:N]Regirock[CR] ab.", italian="[CN][player] rifiuta\n[CN]l'offerta di [CS:N]Regirock[CR].", spanish="[CN][player] declinó el ofrecimiento de\n[CN][CS:N]Regirock[CR]."})
  -- message_Close
  pcall(function() SOUND:FadeOutBGM(30) end)
  -- GAP: se_Play(7183) — id SE NDS sans portage PMDO identifié
  -- GAP: SetEffect EFFECT_WHITE_LIGHT_ENGULFING — VFX sans émote PMDO équivalente
