...
-- Scène canonique PMD Sky EU : SCRIPT/D12P41A/m99a0107.ssb (ROM sha256 1fa39d35…).
...
return function(hero, partner)
  ...
  GROUND:MoveToPosition(hero, 260, 172, false, 2)
  do local __slot = SkySceneKit.team_member(1); if __slot then GROUND:MoveToPosition(__slot, 284, 188, false, 2) end end
  do local __slot = SkySceneKit.team_member(2); if __slot then GROUND:MoveToPosition(__slot, 236, 188, false, 2) end end
  do local __slot = SkySceneKit.team_member(3); if __slot then GROUND:MoveToPosition(__slot, 260, 204, false, 2) end end
  ...
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN][player] cleared [CS:P]Amp Plains[CR]\n[CN]in admirable fashion!", french="[CN][player] a brillamment terminé\n[CN]les [CS:P]Plaines Elek[CR]!", german="[CN][player] meisterte die [CS:P]Ampere-Ebene[CR]\n[CN]auf beeindruckende Weise!", italian="[CN][player] ha completato le\n[CN][CS:P]Pianure Saetta[CR] in modo eccellente!", spanish="[CN]¡[player] logró atravesar\n[CN]la [CS:P]Pradera Destello[CR]!"})
  -- message_Close
  -- debug_Print('LABEL_D018_CLEAR3') [neutre/état moteur]
  else
  -- supervision_Acting(0) [neutre/état moteur]
  -- camera_SetMyself() [neutre/état moteur]
  GAME:FadeIn(30)
  -- GAP: BGM BGM_IN_THE_DEPTHS_OF_THE_PIT non mappé au roster (REQUIRES_MOD_ASSET ou canal ambiance)
  GROUND:MoveToPosition(partner, 244, 172, false, 2)
  GROUND:MoveToPosition(hero, 276, 172, false, 2)
  do local __slot = SkySceneKit.team_member(1); if __slot then GROUND:MoveToPosition(__slot, 260, 204, false, 2) end end
  do local __slot = SkySceneKit.team_member(2); if __slot then GROUND:MoveToPosition(__slot, 284, 196, false, 2) end end
  GAME:WaitFrames(2) -- join WaitExecuteLives
  GAME:WaitFrames(15)
  -- ExecuteCommon(CORO_LOOK_AROUND_FUNC_SERIES, 0) [gestion de station NDS: le chargement/la coroutine commune est assurée par le harnais journey PMDO]
  GAME:WaitFrames(12)
