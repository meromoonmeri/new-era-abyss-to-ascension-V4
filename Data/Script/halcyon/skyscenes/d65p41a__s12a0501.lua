...
-- Scène canonique PMD Sky EU : SCRIPT/D65P41A/s12a0501.ssb (ROM sha256 1fa39d35…).
...
return function(hero, partner)
  ...
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN]Oh?[K]\n[CN][CS:N]Heatran[CR] is saying something.", french="[CN]Oh![K]\n[CN][CS:N]Heatran[CR] vous parle.", german="[CN]Oh?[K]\n[CN][CS:N]Heatran[CR] will etwas sagen.", italian="[CN]Oh?[K]\n[CN][CS:N]Heatran[CR] sta dicendo qualcosa.", spanish="[CN]¿Qué?[K]\n[CN][CS:N]Heatran[CR] está diciendo algo."})
  pcall(function() UI:ResetSpeaker() end)
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  do local __sw = ((SV.SkyVars or {}).DUNGEON_EVENT_LOCAL or 0) -- switch($DUNGEON_EVENT_LOCAL) [GameVar DUNGEON_EVENT_LOCAL (SV.SkyVars)]
  if __sw == 1 then
  ...
  -- switch(ProcessSpecial(PROCESS_SPECIAL_PREPARE_MENU_ACCEPT_TEAM_MEMBER, 13, 0)) [procédé/menu moteur NDS, corps vide: aucun embranchement canonique — équivalent moteur PMDO]
  ...
  GAME:WaitFrames(10)
  local npc_npc_hiidoran = SkySceneKit.spawn_npc("heatran", 304, 200, Direction.Down, "NPC_HIIDORAN")
  ...
  GAME:WaitFrames(30)
  SV.SkyScenarioBitFlags = SV.SkyScenarioBitFlags or {}; SV.SkyScenarioBitFlags[34] = 0 -- $SCENARIO_MAIN_BIT_FLAG[34] = 0 (ROM)
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN][CS:N]Heatran[CR] was turned down\n[CN]as a team member.", french="[CN]Vous avez refusé l'offre\n[CN]de [CS:N]Heatran[CR].", german="[CN][CS:N]Heatran[CR] wird\n[CN]als Team-Mitglied abgelehnt.", italian="[CN]Non hai accettato [CS:N]Heatran[CR] come nuovo membro\n[CN]della squadra.", spanish="[CN]Has rechazado el ofrecimiento de [CS:N]Heatran[CR]\n[CN]de formar parte del equipo."})
  -- message_Close
  pcall(function() SOUND:FadeOutBGM(30) end)
  -- GAP: se_Play(7183) — id SE NDS sans portage PMDO identifié
  -- GAP: SetEffect EFFECT_WHITE_LIGHT_ENGULFING — VFX sans émote PMDO équivalente
