...
-- Scène canonique PMD Sky EU : SCRIPT/D66P41A/s12a0801.ssb (ROM sha256 1fa39d35…).
...
return function(hero, partner)
  ...
  SkySceneKit.say({english="[CN]The treasure box contained...[K]\n[CN]a [CS:I]Terra Cymbal[CR]!", french="[CN]Le Coffre au Trésor contient...[K]\n[CN]une [CS:I]Terracymbale[CR]!", german="[CN]Die Schatzbox enthält[K]\n[CN]ein [CS:I]Terrabecken[CR]!", italian="[CN]Il forziere contiene...[K]\n[CN]il [CS:I]Cembaloterra[CR]!", spanish="[CN]El cofre del tesoro contenía...[K]\n[CN]¡Un [CS:I]Terracímbalo[CR]!"})
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  -- item_Set(0, 61, 0) [table d'objets du script NDS: inventaire géré par le moteur PMDO]
  ...
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN]Oh?[K]\n[CN][CS:N]Groudon[CR] is saying something.", french="[CN]Oh![K]\n[CN][CS:N]Groudon[CR] vous parle.", german="[CN]Oh?[K]\n[CN][CS:N]Groudon[CR] will etwas sagen.", italian="[CN]Oh?[K]\n[CN][CS:N]Groudon[CR] sta dicendo qualcosa.", spanish="[CN]¿Qué?[K]\n[CN][CS:N]Groudon[CR] está diciendo algo."})
  pcall(function() UI:ResetSpeaker() end)
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  -- switch(ProcessSpecial(PROCESS_SPECIAL_PREPARE_MENU_ACCEPT_TEAM_MEMBER, 3, 0)) [procédé/menu moteur NDS, corps vide: aucun embranchement canonique — équivalent moteur PMDO]
  ...
  GAME:WaitFrames(10)
  local npc_npc_guraadon = SkySceneKit.spawn_npc("groudon", 304, 200, Direction.Down, "NPC_GURAADON")
  ...
  GAME:WaitFrames(30)
  SV.SkyScenarioBitFlags = SV.SkyScenarioBitFlags or {}; SV.SkyScenarioBitFlags[35] = 0 -- $SCENARIO_MAIN_BIT_FLAG[35] = 0 (ROM)
  ...
  -- CallCommon CORO_MESSAGE_CLOSE_WAIT_FUNC (fermeture/attente message: géré par say())
  pcall(function() UI:ResetSpeaker() end)
  SkySceneKit.say({english="[CN][CS:N]Groudon[CR] was turned down\n[CN]as a team member.", french="[CN]Vous avez refusé l'offre\n[CN]de [CS:N]Groudon[CR].", german="[CN][CS:N]Groudon[CR] wird\n[CN]als Team-Mitglied abgelehnt.", italian="[CN]Non hai accettato [CS:N]Groudon[CR] come nuovo membro\n[CN]della squadra.", spanish="[CN]Has rechazado el ofrecimiento de [CS:N]Groudon[CR]\n[CN]de formar parte del equipo."})
  -- message_Close
  pcall(function() SOUND:FadeOutBGM(30) end)
  -- GAP: se_Play(7183) — id SE NDS sans portage PMDO identifié
  -- GAP: SetEffect EFFECT_WHITE_LIGHT_ENGULFING — VFX sans émote PMDO équivalente
