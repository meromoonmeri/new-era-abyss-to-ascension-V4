...
-- Scène canonique PMD Sky EU : SCRIPT/S01P03A/enter00.ssb (ROM sha256 1fa39d35…).
-- Dialogues 5 langues ROM embarqués ; conventions du pilote m01a0204.
local SkySceneKit = require 'halcyon.skyscenes.kit'
return function(hero, partner)
  -- debug_Print('ENTER_S01P03A') [neutre/état moteur]
  do local __sw = 1 -- switch(sector()) [sector(): acting principal (kit PMDO)]
  if __sw == 1 then
  -- @label_0 [étiquette de flux ExplorerScript]
  -- debug_Print('RESCUE_RESUME') [neutre/état moteur]
  ...
  elseif __sw == 2 then
  -- debug_Print('RESCUE_ACTING') [neutre/état moteur]
  -- supervision_StationCommon(0) [gestion de station NDS: le chargement/la coroutine commune est assurée par le harnais journey PMDO]
  -- supervision_StationCommon(1) [gestion de station NDS: le chargement/la coroutine commune est assurée par le harnais journey PMDO]
  -- back2_SetMode(-1) [mode d'affichage sub NDS: géré par SubScreen]
  -- screen2_FadeIn(1, 30) [sub révélé en mode système NDS (aucun décor chargé): pas de contenu narratif - documenté]
  ...
  if ((SV.SkyScenarioBitFlags or {})[32] == 1) then -- if ROM: $SCENARIO_MAIN_BIT_FLAG[32]
  ...
  SkySceneKit.play_scene("s01p03a__dead00", "s01p03a__dead", hero, partner) -- supervision_ExecuteActing(LEVEL_S01P03A, 'DEAD', 0) [chaînage scène ROM]
  GAME:WaitFrames(1) -- hold
  elseif __sw == 9 then
  -- debug_Print('RESCUE_RETURN') [neutre/état moteur]
  -- screen2_FadeOut [sub déjà caché]
  -- back2_SetMode(0) [mode d'affichage sub NDS: géré par SubScreen]
  SkySceneKit.play_scene("s01p03a__return00", "s01p03a__return", hero, partner) -- supervision_ExecuteActing(LEVEL_S01P03A, 'RETURN', 0) [chaînage scène ROM]
  GAME:WaitFrames(1) -- hold
  elseif __sw == 8 then
  -- debug_Print('RESCUE_CONQUEST') [neutre/état moteur]
  -- screen2_FadeOut [sub déjà caché]
  -- back2_SetMode(0) [mode d'affichage sub NDS: géré par SubScreen]
  SkySceneKit.play_scene("s01p03a__conquest00", "s01p03a__conquest", hero, partner) -- supervision_ExecuteActing(LEVEL_S01P03A, 'CONQUEST', 0) [chaînage scène ROM]
  GAME:WaitFrames(1) -- hold
  elseif true then -- default
  ...
  SV.SkyScenarioBitFlags = SV.SkyScenarioBitFlags or {}; SV.SkyScenarioBitFlags[32] = 1 -- $SCENARIO_MAIN_BIT_FLAG[32] = 1 (ROM)
  -- debug_Print('RESCUE_1ST') [neutre/état moteur]
  -- screen2_FadeOut [sub déjà caché]
  -- back2_SetMode(0) [mode d'affichage sub NDS: géré par SubScreen]
  SkySceneKit.play_scene("s01p03a__first00", "s01p03a__first", hero, partner) -- supervision_ExecuteActing(LEVEL_S01P03A, 'FIRST', 0) [chaînage scène ROM]
  GAME:WaitFrames(1) -- hold
  end
  end
  end
