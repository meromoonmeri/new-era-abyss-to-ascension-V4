...
function vast_steppe_midpoint.Init(map)
  DEBUG.EnableDbgCoro()
  print('=>> Init_vast_steppe_midpoint <<= [build 2026-08-03-N]')
...
function vast_steppe_midpoint.North_Exit_Touch(obj, activator)
  ...
  if UI:ChoiceResult() then
    ...
    -- donjon doit avoir qu'un boss final »).
    --
    -- Le relais envoyait vers l'arene de Stantler + Mudbray (segment 1) tant
    -- que SV.Chapter5.SteppeMiniBossCleared etait faux. Ce detour est
    -- supprime : depuis le relais, la route mene DIRECTEMENT aux Profondeurs
    -- (segment 2), puis au Gardien de la Steppe, seul boss du donjon.
    --
    -- On garde la distinction session active / nouvelle session : c'etait le
    -- correctif de la cause racine du NRE IsGameOver (meme fix que le Mont
    -- Venteux), il n'a rien a voir avec le mini-boss et doit survivre.
